...
public class ContainerBootstrap implements SecureRunnable
{
	...
	public boolean run(Properties runProperties) throws Throwable
	{
		...
		ContainerConfiguration cc = new ContainerConfiguration(ConfigurationManager.getCurrentConfiguration());
		...
		try {
			...
			try {
				...
				tool.addArgument(String.format("http%s://localhost:%d/axis/services/VCGRContainerPortType", cc.isSSL() ? "s"
					: "", cc.getListenPort()));
				tool.addArgument(String.format("%s/%s", bProperties.getAttachPath(), hostname));
	...
	private void login(BootstrapProperties cProperties, OwnerInfo ownerInfo) throws Throwable
	{
		...
		try {
			...
			tool.addArgument("local:"
				+ Installation.getDeployment(new DeploymentName()).security()
					.getSecurityFile(cProperties.getCertGeneratorOutputStoreName()).getAbsolutePath());
			...
			tool = new LoginTool();
			tool.setNo_gui();
			tool.addArgument(String.format("rns:%s", ownerInfo.getUserPath()));
