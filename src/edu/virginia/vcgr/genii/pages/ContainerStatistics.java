...
public class ContainerStatistics extends GenesisIIStyledPage {
	...
	static private void printClassTable(PrintStream ps,
			Map<TimeInterval, MethodStatisticsReport> map, String className) {
		String classShortName = className;
		int index = classShortName.lastIndexOf('.');
		classShortName = classShortName.substring(index + 1);
		ps.format("<H3>Method Statistics for Class %s</H3>", String.format(
				"<A HREF=\"class-method-stats.html?className=%s\">%s</A>",
		...
		for (TimeInterval ti : map.keySet()) {
			...
			MethodStatisticsReportPoint point = mReport.classTotals().get(
					className);
			ps.format(
					"<TR><TD>%s</TD><TD>%d</TD><TD>%d</TD><TD>%d</TD><TD>%d</TD><TD>%.2f %%</TD><TD>%d</TD></TR>\n",
					ti.longDescription(), point.totalCallsStarted(),
					point.totalCompleted(), point.totalSucceeded(),
					point.totalFailed(), point.failureRate() * 100.0,
	...
	protected void generateContent(PrintStream ps) throws IOException {
		...
		ps.format("<H2>Database Statistics (last rejuvenated:  %tc)</H2><BR>",
				dbStats.databaseStartTime());
		ps.println("<TABLE BORDER=\"1\">");
		...
		for (TimeInterval ti : reports.keySet()) {
			...
			ps.format("<tr><td>%s</td><td>%d</td><td>%d</td><td>%d</td></tr>",
					ti.longDescription(), report.numOpened(),
		...
		Set<String> classes = mStats.report().get(TimeInterval.THIRTY_SECONDS)
				.classTotals().keySet();
