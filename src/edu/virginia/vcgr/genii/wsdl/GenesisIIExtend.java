...
public class GenesisIIExtend extends AbstractXMLComponent implements
		IXMLComponent
{
	private QName _extendedPortTypeName;
	private WsdlPortType _extendedPortType = null;
	
	public GenesisIIExtend(WsdlSourcePath sourcePath, IXMLComponent parent, 
		Node wsdlExtendNode) throws WsdlException
	{
		super(sourcePath, parent, wsdlExtendNode);
		
		_extendedPortTypeName = WsdlUtils.getQNameFromString(wsdlExtendNode,
			WsdlUtils.getAttribute(wsdlExtendNode.getAttributes(), 
				WsdlConstants.PORT_TYPE_ATTR, true));
	}
