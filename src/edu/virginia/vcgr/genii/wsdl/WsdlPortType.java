...
public class WsdlPortType extends AbstractXMLComponent implements IXMLComponent
{
	...
	private HashMap<QName, GenesisIIExtend> _extensions = 
		new HashMap<QName, GenesisIIExtend>();
	private HashMap<String, WsdlOperation> _operations =
		new HashMap<String, WsdlOperation>();
	
	public WsdlPortType(WsdlSourcePath sourcePath, IXMLComponent parent,
		Node portTypeNode) throws WsdlException
	{
		super(sourcePath, parent, portTypeNode);
		
		_name = WsdlUtils.getAttribute(portTypeNode.getAttributes(), 
		...
		for (int lcv = 0; lcv < children.getLength(); lcv++)
		{
			...
			if (child.getNodeType() == Node.ELEMENT_NODE)
			{
				...
				{
					GenesisIIExtend extend = new GenesisIIExtend(
				...
				{
					WsdlOperation oper = new WsdlOperation(
						_sourcePath, this, child);
	...
	public void normalize() throws WsdlException
	{
		for (GenesisIIExtend extension : _extensions.values())
		{
			...
			for (WsdlOperation oper : extendedPortType.getOperations())
			{
				...
				Node newNode = _representedNode.getOwnerDocument().importNode(
					oper.getRepresentedNode(), true);
				_representedNode.appendChild(newNode);
				WsdlOperation newOperation;
				
				try
				{
					AlternativeNamespaceResolution.setAlternativeResolver(
						oper.getRepresentedNode().getOwnerDocument());
					newOperation = new WsdlOperation(_sourcePath, this,
						newNode);
				}
				finally
				{
