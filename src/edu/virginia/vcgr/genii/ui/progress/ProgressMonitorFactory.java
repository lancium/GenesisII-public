...
public class ProgressMonitorFactory
{
		...
		TaskCompletionListener<ResultType> completionListener)
	{
		ProgressMonitorWorker<ResultType> worker = 
			new ProgressMonitorWorker<ResultType>(
		...
		implements Runnable
	{
		...
		private ProgressMonitorWorker(Window owner,
			String title, String subTitle, long delay,
			Task<ResultType> task,
			TaskCompletionListener<ResultType> completionListener)
		{
			_owner = owner;
		...
		{	
			...
			try
			{
				Future<ResultType> future = _executor.submit(
					new TaskExecutorWorker<ResultType>(
						_task, pipe));
				
				if (!initialWait(future, _task.showProgressDialog()))
				{
					dialog = new ProgressMonitorDialog(
						_owner, _title, _subTitle);
	...
	private class TaskExecutorWorker<ResultType>
