...
class JobStateColumn 
	extends AbstractRowTableColumnDefinition<JobInformation, Pair<QueueStates, String>>
{
	static private class StateRenderer extends DefaultTableCellRenderer
	{
		...
		{
			Pair<QueueStates, String> pair =
				(Pair<QueueStates, String>)value;
			
			String host = pair.second();
			
			super.setValue(host == null ?
				pair.first() : host);
	...
	{
		super("Job State", 
			(Class<Pair<QueueStates, String>>)new Pair<QueueStates, String>(null, null).getClass(),
			45);
	}
	...
	{
		return new Pair<QueueStates, String>(
			row.getJobState(), row.getScheduledOn());
	}
}