...
class HistoryEventTreeNode
{
		...
		Map<SequenceNumber, HistoryEventTreeNode> nodeMap, SequenceNumber childNumber)
	{
		...
		if (parentNode == null) {
			HistoryEvent parentEvent = new HistoryEvent(parentNumber, Calendar.getInstance(),
				new SimpleStringHistoryEventSource("Faux Source", null), HistoryEventLevel.Trace, HistoryEventCategory.Default,
				new HashMap<String, String>(), new HistoryEventData("Faux Event"));
