...
public class EPRDisplayPlugin extends AbstractUITabPlugin
	implements UITabPlugin
{
	...
	public JComponent getComponent(UIPluginContext context)
	{
		...
		if (set.preferText())
		{
			...
			{
				widget.appendHeader(String.format(
					"EPR for %s:\n\n", path.pwd()));
				try
				{
		...
		} else
		{
			...
			for (RNSPath path : paths)
			{
				widget.add(context.uiContext(), path.pwd(),
					new EPRTreeSource(path));
			}
			...
			return new JSplitPane(JSplitPane.VERTICAL_SPLIT,
				new JScrollPane(widget), new JScrollPane(textWidget));
		}
	}
