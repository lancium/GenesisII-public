...
class CommandParameterDialog extends JDialog
{
	...
	private CommandParameterDialog(Component ownerComponent,
		JavaCommandFunction function)
	{
		...
		content.add(new JLabel(String.format(
			"Parameters for Command Function \"%s\":", function)), 
		...
		for (JavaCommandParameter parameter : function.parameters())
		{
			...
			content.add(label, new GridBagConstraints(0, count, 1, 1, 0.0, 0.0,
				GridBagConstraints.EAST, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5),5 , 5));
			
			content.add(_answerComponents[count - 1], new GridBagConstraints(
				1, count, 1, 1, 1.0, 0.0, GridBagConstraints.CENTER,
		...
		content.add(ButtonPanel.createHorizontalButtonPanel(
			new ExecuteAction(), new CancelAction()), new GridBagConstraints(
				0, count + 1, 2, 1, 1.0, 1.0, GridBagConstraints.SOUTH,
				GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 5, 5));
	...
	static String[] fillInParameters(Component ownerComponent,
		JavaCommandFunction function)
	{
		CommandParameterDialog dialog = new CommandParameterDialog(
			ownerComponent, function);
