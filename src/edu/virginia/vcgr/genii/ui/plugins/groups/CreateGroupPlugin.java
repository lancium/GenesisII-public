...
{
	static final private String SERVICE_PATH =
	...
	static private class RemoveTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				tool.run(
					new EmptyWriter(), new EmptyWriter(), new EmptyReader());
			}
	...
	static private class GroupCreationListener
	...
	static private class GroupCreatorTask 
		extends AbstractTask<Boolean>
	{
		...
		private GroupCreatorTask(UIPluginContext context, 
			RNSPath parent, String newName)
		{
			_context = context;
			_parent = parent;
			_newName = newName;
			...
			throws Exception
		{
			...
			{
				if (sourcePath != null)
					_context.uiContext().executor().equals(
						new RemoveTask(sourcePath));
			}
		}	
	}
			...
			throws UIPluginException
	{
		...
		context.uiContext().progressMonitorFactory().createMonitor(
			context.ownerComponent(), "Creating Group",
			"Create Group Object", 1000L, new GroupCreatorTask(
				context, path, answer), new GroupCreationListener(context,
					retriever)).start();
	}
