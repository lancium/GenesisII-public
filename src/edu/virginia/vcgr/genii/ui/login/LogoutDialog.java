...
class LogoutDialog extends JDialog
{
	...
	private class LogoutAction extends AbstractAction implements ListSelectionListener
	{
		...
		{
			int answer = JOptionPane.showConfirmDialog((Component) e.getSource(), "Log out from selected credentials?",
			...
			if (answer == JOptionPane.YES_OPTION) {
				...
				for (int lcv = 0; lcv < values.length; lcv++)
					logoutItems[lcv] = ((CredentialWrapper) values[lcv])._credential;
	...
	LogoutDialog(Window owner, CredentialManagementContext context)
	{
		...
		_credentialList = new JList(createCredentialList(context.loginItems()));
