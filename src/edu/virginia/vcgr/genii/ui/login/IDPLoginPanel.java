...
final class IDPLoginPanel extends LoginPanel
{
	...
	IDPLoginPanel()
	{
		...
		add(_password, new GridBagConstraints(
			1, 1, 1, 1, 1.0, 0.0, GridBagConstraints.NORTH,
			GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 5, 5));
		add(new JLabel("Grid Path"), new GridBagConstraints(
			0, 2, 1, 1, 0.0, 1.0, GridBagConstraints.NORTHWEST,
			GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		add(_rnsPath, new GridBagConstraints(
			1, 2, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH,
			GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 5, 5));
	}
	
	@Override
	final public Collection<GIICredential> doLogin(UIContext uiContext)
		throws Throwable
	{
		...
		try
		{
			...
			RNSPath path = uiContext.callingContext().getCurrentPath().lookup(
				_rnsPath.getText(), RNSPathQueryFlags.DONT_CARE);
			if (!path.exists())
			{
				JOptionPane.showMessageDialog(this, 
					"Grid path doesn't exist!", "Bad Grid Path", 
			...
			transientCredentials._credentials.add(upt);
			...
			} finally 
			{
				if (upt != null) 
				{
					...
					transientCredentials._credentials.remove(upt);
				}
			}
		}
		finally
