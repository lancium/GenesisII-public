...
class TrashTransferHandler extends TransferHandler
{
	...
	protected Transferable createTransferable(JComponent c)
	{
		if (c instanceof JList) {
			...
			TrashCanEntryWrapper[] wrappers = new TrashCanEntryWrapper[values.length];
	...
	public boolean importData(TransferSupport support)
	{
		...
		if (support.getComponent() instanceof JList) {
			...
			if (support.getDropAction() == MOVE) {
				try {
					TrashCanEntryWrapper[] wrappers = (TrashCanEntryWrapper[]) support.getTransferable().getTransferData(
					...
					throw new ConfigurationException("Unable to drop data.", e);
				} catch (UnsupportedFlavorException e) {
					throw new ConfigurationException("Unable to drop data.", e);
