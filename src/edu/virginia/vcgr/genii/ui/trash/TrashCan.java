...
public class TrashCan
{
	...
	private Collection<TrashCanStateListener> _listeners = 
		new LinkedList<TrashCanStateListener>();
	...
	{
		_pDirectory = Persistence.persistence().directory(
			PERSISTENCE_DIRECTORY_NAME);
		
		for (PersistenceKey key : _pDirectory.keys())
		{
			_entries.put(key, new Pair<String, PersistenceKey>(
				TrashCanEntry.readPath(key), key));
		}
	}
	...
	{
		int size;
		PersistenceKey key = _pDirectory.addEntry(new TrashCanEntry(
			sourceContext.callingContext(), entry));
