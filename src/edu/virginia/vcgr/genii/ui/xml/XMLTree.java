...
public class XMLTree extends JTree
{
	...
	{
		remove(treeName);
		_root.add(new XMLTreeNodeDocumentRoot(context, treeName, new DefaultXMLTreeSource(reader)));
	}

	public void add(UIContext context, String treeName, MessageElement me)
	{
		remove(treeName);
		_root.add(new XMLTreeNodeDocumentRoot(context, treeName, new DefaultXMLTreeSource(me)));
	}

	public void add(UIContext context, String treeName, Object serializableObject)
	{
		remove(treeName);
		_root.add(new XMLTreeNodeDocumentRoot(context, treeName, new DefaultXMLTreeSource(serializableObject)));
	}

	public void remove(String name)
	{
		for (int lcv = 0; lcv < _root.getChildCount(); lcv++) {
			String nodeName = (String) ((DefaultMutableTreeNode) _root.getChildAt(lcv)).getUserObject();
