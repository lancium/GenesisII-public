...
public class RNSListTransferable extends ListTransferable
{
	...
	static {
		try {
			...
			RNS_PATH_LIST_FLAVOR = new DataFlavor(String.format(FLAVOR_PATTERN, DataFlavor.javaJVMLocalObjectMimeType,
	...
	public Object getTransferData(DataFlavor flavor) throws UnsupportedFlavorException, IOException
	{
		if (flavor == null)
			return null;
		if (_logger.isDebugEnabled())
			_logger.debug("got into rns list transferable, get data...");
		if (flavor.equals(RNS_PATH_LIST_FLAVOR)) {
			if (_logger.isDebugEnabled())
				_logger.debug("seeing rns path list flavor.");
