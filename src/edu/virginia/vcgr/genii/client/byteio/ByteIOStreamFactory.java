...
public class ByteIOStreamFactory
{
	 ...
	 *            The desired transfer protocol (if any) to use. THis value CAN be null.
	 ...
	 */
	static public InputStream createInputStream(EndpointReferenceType target, URI desiredTransferType, boolean createBuffered)
		throws FileNotFoundException, IOException, RemoteException
	{
		TypeInformation tInfo = new TypeInformation(target);
		if (tInfo.isRByteIO()) {
			RandomByteIOInputStream stream = new RandomByteIOInputStream(target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
		} else if (tInfo.isSByteIO() || tInfo.isSByteIOFactory()) {
			StreamableByteIOInputStream stream = new StreamableByteIOInputStream(target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
		} else {
			throw new FileNotFoundException("Endpoint specified is not a file.");
		}
	}
	...
	static public InputStream createInputStream(RNSPath target, URI desiredTransferType, boolean createBuffered)
		throws FileNotFoundException, RemoteException, RNSException, IOException
	{
		if (!target.exists())
			throw new FileNotFoundException("Couldn't find file \"" + target.pwd() + "\".");

		return createInputStream(target.getEndpoint(), desiredTransferType, createBuffered);
	}
	 ...
	 *            Whether or not the stream should be buffered.
	 * 
	 * @return A newly created Java OutputStream to use for writing bytes to the target ByteIO.
	 * 
	 * @throws FileNotFoundException
	 * @throws RemoteException
	 * @throws ConfigurationException
		...
		throws FileNotFoundException, RemoteException, IOException
	{
		TypeInformation tInfo = new TypeInformation(target);
		if (tInfo.isRByteIO()) {
			RandomByteIOOutputStream stream = new RandomByteIOOutputStream(target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
		} else if (tInfo.isSByteIO() || tInfo.isSByteIOFactory()) {
			StreamableByteIOOutputStream stream = new StreamableByteIOOutputStream(target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
		} else {
	 ...
	 * @throws RNSException
	 */
	static public OutputStream createOutputStream(RNSPath target, URI desiredTransferType, boolean createBuffered)
		throws FileNotFoundException, RemoteException, RNSException, IOException
	{
		if (!target.exists())
			target.createNewFile();

		return createOutputStream(target.getEndpoint(), desiredTransferType, createBuffered);
	}
