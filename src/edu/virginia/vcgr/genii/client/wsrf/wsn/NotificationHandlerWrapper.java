...
{
	static private Log _logger = LogFactory.getLog(NotificationHandlerWrapper.class);

	private TopicQueryExpression _filter;
	private NotificationHandler<ContentsType> _handler;

	NotificationHandlerWrapper(TopicQueryExpression filter, NotificationHandler<ContentsType> handler)
	{
		_filter = filter;
		_handler = handler;
	}
