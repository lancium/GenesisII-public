...
{
	static final QName TOPIC_QNAME = new QName(WSRFConstants.WSN_TOPIC_NS, "Topic");
		...
		if (any != null) {
			for (MessageElement value : any) {
				...
				if (name.equals(TOPIC_QNAME)) {
					TopicQueryExpression query = TopicQueryDialects.createFromElement(value);
	...
	final public <Type extends NotificationMessageContents> Type contents(Class<Type> contentsType)
	{
		return contentsType.cast(_contents);
	}

	final public NotificationMessageHolderType toAxisType() throws JAXBException, SOAPException
	{
