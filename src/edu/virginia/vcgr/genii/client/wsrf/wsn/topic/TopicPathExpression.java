...
public class TopicPathExpression
{
	...
	TopicPathExpression(NamespaceContext nsContext, 
		Collection<TopicNode> nodes)
	{
	...
	static public TopicPathExpression fromString(NamespaceContext nsContext, 
		String expression)
	{
	...
	static public TopicPathExpression fromString(
		Map<String, String> prefix2nsMap, String expression)
	{
		return fromString(new NamespaceMapResolver(prefix2nsMap), expression);
	}
	
	static public TopicPathExpression fromString(NamespaceResolver resolver,
		String expression)
	{
		String lastNamespaceURI = null;
		...
		for (String element : expression.split("/"))
		{
			if (element == null || element.length() == 0)
				throw new IllegalArgumentException(String.format(
					"\"%s\" is not a valid Topic expression.", expression));
			...
			if (index < 0)
			{
				if (lastNamespaceURI == null)
					throw new IllegalArgumentException(String.format(
						"Can't tell what namespace to start with."));
				
				nodes.addLast(new TopicNode(new QName(lastNamespaceURI,
					element, lastPrefix)));
			} else if (index > 0)
			{
				...
				if (lastNamespaceURI == null)
					throw new IllegalArgumentException(String.format(
						"Can't find a namespace for prefix %s.", lastPrefix));
				
				nodes.addLast(new TopicNode(new QName(lastNamespaceURI,
					element, lastPrefix)));
			} else
				throw new IllegalArgumentException(String.format(
					"%s is not a valid topic node.", element));
		}
