...
public class TypeInformation
{
	...
	public String getTypeDescription()
	{
		if (isByteIO())
			...
			PortType highestRank = PortType.getHighestRankedPortType(_implementedPortTypes);
	...
	public long getByteIOSize()
	{
		try {
			...
			if (isRByteIO()) {
				RandomByteIORP rp = (RandomByteIORP) ResourcePropertyManager.createRPInterface(_epr, OGSARP.class,
					RandomByteIORP.class);
				value = rp.getSize();
			} else if (isSByteIO()) {
				StreamableByteIORP rp = (StreamableByteIORP) ResourcePropertyManager.createRPInterface(_epr, OGSARP.class,
	...
	{
		QName attrQName = new QName((isRByteIO() ? ByteIOConstants.RANDOM_BYTEIO_NS : ByteIOConstants.STREAMABLE_BYTEIO_NS),
			attrName);
		...
		Calendar time = ObjectDeserializer.toObject(any[0], Calendar.class);
		return time.getTime();
	}
