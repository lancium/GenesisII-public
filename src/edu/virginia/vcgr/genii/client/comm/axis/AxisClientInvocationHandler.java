...
public class AxisClientInvocationHandler implements InvocationHandler, IFinalInvoker
{
	...
	static private synchronized MessageLevelSecurityRequirements getMinClientMessageSec() throws GeneralSecurityException
	{
		...
		String minMessageSecurity = Installation.getDeployment(new DeploymentName()).security()
			.getProperty(SecurityConstants.Client.MESSAGE_MIN_CONFIG_PROP);
		...
		ResourceException
	{
		...
		try {
			...
			synchronized (validatedCerts) {
				if (!validatedCerts.containsKey(_resourceCert)) {
					...
					boolean okay = CertificateValidatorFactory.getValidator().validateCertPath(chain, true);
					if (!okay)
						throw new GeneralSecurityException("failed to validate cert chain: " + chain[0].getIssuerDN());
		...
		try {
			...
			ArrayList<ISecuritySendHandler> sendHandlers = getHandler((SimpleChain) _providerConfig.getGlobalRequest(),
			...
			ArrayList<ISecurityRecvHandler> recvHandlers = getHandler((SimpleChain) _providerConfig.getGlobalResponse(),
			...
			try {
				...
				if ((throwable instanceof TryAgainFaultType) && (!tryAgain)) {
					...
					if (!"destroy".equalsIgnoreCase(methodName) && !"createIndirectSubscriptions".equalsIgnoreCase(methodName)
						...
						// method
						// only then cache has been refreshed. Destroy method is ignored because
						// otherwise
						// there is a chance of cycle formation as the cache management system
						// itself use
						// WS-resources that are destroyed with a cache refresh and invocation of
						// destroy
						// on those resources can fail too. Meanwhile, notification management
						// methods are
	...
	protected Object doInvoke(Method calledMethod, Object[] arguments, int timeout) throws Throwable
	{
		...
		if (_logger.isTraceEnabled())
			_logger.trace(String.format("Finished an outcall for %s on thread [%x]%s (duration was %d ms).",
