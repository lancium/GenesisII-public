		...
		ConfigurationUnloadedListener
{
	...
	protected static SSLSocketFactory createDelegateFactory()
	{
		...
		try
		{
			...
			String trustStoreLoc =
					sslProps
							.getProperty(GenesisIIConstants.TRUST_STORE_LOCATION_PROPERTY);
			String trustStoreType =
					sslProps.getProperty(
							GenesisIIConstants.TRUST_STORE_TYPE_PROPERTY,
							GenesisIIConstants.TRUST_STORE_TYPE_DEFAULT);
			String trustStorePass =
					sslProps
							.getProperty(GenesisIIConstants.TRUST_STORE_PASSWORD_PROPERTY);
			...
			if (trustStoreLoc != null)
			{
				KeyStore ks =
						CertTool.openStoreDirectPath(
								new DeploymentRelativeFile(trustStoreLoc),
	...
	static private Properties getSSLProperties() throws ConfigurationException
	{
		XMLConfiguration conf =
				ConfigurationManager.getCurrentConfiguration()
						.getClientConfiguration();
		return (Properties) conf
				.retrieveSection(GenesisIIConstants.SSL_PROPERTIES_SECTION_NAME);
	}
}
