...
public class VcgrSslSocketFactory extends SSLSocketFactory implements ConfigurationUnloadedListener
{
	...
	static private LRUCache<KeyAndCertMaterialCacheKey, SSLSocketFactory> _sslSessionCache = null;
	...
	static {
		...
		try {
			File sslCachePropertiesFile = Installation.getDeployment(new DeploymentName()).getConfigurationDirectory()
		...
		_sslSessionCache = new LRUCache<KeyAndCertMaterialCacheKey, SSLSocketFactory>(maxCacheElements);
	}
	...
	public VcgrSslSocketFactory()
	{
		...
		_clientSocketConfigurer = Installation.getDeployment(new DeploymentName()).clientSocketConfigurer();
	...
	public synchronized void notifyUnloaded()
	{
		try {
			...
			String trustStoreLoc = sslProps.getProperty(SecurityConstants.Client.SSL_TRUST_STORE_LOCATION_PROP);
			String trustStoreType = sslProps.getProperty(SecurityConstants.Client.SSL_TRUST_STORE_TYPE_PROP,
				SecurityConstants.TRUST_STORE_TYPE_DEFAULT);
			String trustStorePass = sslProps.getProperty(SecurityConstants.Client.SSL_TRUST_STORE_PASSWORD_PROP);
			...
			if (trustStoreLoc != null) {
				try {
					trustStore = CertTool.openStoreDirectPath(Installation.getDeployment(new DeploymentName()).security()
						.getSecurityFile(trustStoreLoc), trustStoreType, trustStorePassChars);
				} catch (Throwable cause) {
					_logger.info("Trust store failed to load from file " + trustStoreLoc
						+ "; will attempt to load trusted certificates from directory.", cause);
				}
			}

			String trustedCertificatesDirectory = sslProps
				.getProperty(SecurityConstants.Client.SSL_TRUSTED_CERTIFICATES_LOCATION_PROP);
			if (trustedCertificatesDirectory == null) {
				...
				try {
					...
					if (certificateList != null && !certificateList.isEmpty()) {
						if (trustStore == null) {
							trustStore = createTrustStoreFromCertificates(trustStoreType, trustStorePass, certificateList);
						} else {
	...
	private KeyStore createTrustStoreFromCertificates(String proposedTrustStoreType, String password,
