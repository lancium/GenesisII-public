...
public class ClientUtils
{
	...
	static public synchronized int getClientRsaKeyLength() throws GeneralSecurityException
	{
		...
		String rsaKeyLength = Installation.getDeployment(new DeploymentName()).security()
			.getProperty(SecurityConstants.Client.CLIENT_RSA_KEY_LENGTH_PROP);
		...
		SecurityUpdateResults results) throws GeneralSecurityException
	{
		...
		try {
			...
			if (!ConfigurationManager.getCurrentConfiguration().isClientRole()) {
				...
				retval = generateKeyAndCertMaterial(
					Math.max(edu.virginia.vcgr.genii.security.SecurityConstants.CredentialExpirationMillis, // valid
																											// 24
																											// hours
		...
		while (itr.hasNext()) {
			...
			try {
				...
				// in-flight).
				cred.checkValidity(0, new Date(System.currentTimeMillis() + (10 * 1000)));
	...
	static public <IFace> IFace createProxy(Class<IFace> iface, EndpointReferenceType epr, ICallingContext callContext)
		throws ResourceException, GenesisIISecurityException
	{
		return createProxy(Thread.currentThread().getContextClassLoader(), iface, epr, callContext);
	}
