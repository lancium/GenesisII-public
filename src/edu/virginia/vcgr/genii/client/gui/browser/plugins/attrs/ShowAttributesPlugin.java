...
/**
 * The ShowAttributesPlugin is a simple plugin which, in a tab, displays
 * the attributes document for a selected resource.
 * 
 * @author mmm2a
 */
public class ShowAttributesPlugin implements ITabPlugin
{
	...
	{
		/* All we do to show the attributes is create a new panel that has the
	...
	public PluginStatus getStatus(RNSPath[] selectedResources)
			throws PluginException
	{
		/* If the currently selected paths exists and contains exactly one
		 * path endpoint, then we can show the attributes.
		 */
			...
			&& selectedResources[0].exists())
		{
			try
			{
				...
				/* In order to show attributes, we also require that the
				 * endpoint supports atleast one known interface.  This is
				 * essentially equivalent to saying that the endpoint is a
				 * Genesis II endpoint (i.e., supports attributes).
				 */
			...
			{
				/* This really shouldn't happen as we already asked if it
				 * exists.
				 */
