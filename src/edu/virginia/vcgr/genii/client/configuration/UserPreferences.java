...
public class UserPreferences
{
	...
	static final private String PREFER_GUI_PREFERENCE_KEY = "prefer-gui";
	static final private String EXCEPTION_HANDLER_PREFERENCE_KEY = "exception-handler";
	static final private String SHELL_PROMPT_TEMPLATE_KEY = "shell-prompt-template";
	...
	static private MacroResolver createShellPromptMacroResolver()
	{
		...
		Map<String, Object> dynamicGridResolution = new HashMap<String, Object>();
		...
		dynamicGridResolution.put("net.name", new Object()
		{
			...
			public String toString()
			{
				String property = System.getProperty("edu.virginia.vcgr.genii.net-name");
				if (property != null)
					return property;
		...
		prefixResolver.addResolver("grid", new MapMacroResolver(dynamicGridResolution));
		prefixResolver.addResolver("env", new MapMacroResolver(System.getenv()));
		prefixResolver.addResolver("java", new PropertiesMacroResolver(System.getProperties()));
		prefixResolver.addResolver("gridenv", new GridEnvironmentMacroResolver());
	...
	static private class ShellPromptImpl implements ShellPrompt
	{
		...
		private ShellPromptImpl(String template)
		{
	...
	private UserPreferences()
	{
		...
		_preferGUI = prefs.getBoolean(PREFER_GUI_PREFERENCE_KEY, DEFAULT_PREFER_GUI);
		exceptionHandlerClassName = prefs.get(EXCEPTION_HANDLER_PREFERENCE_KEY, DEFAULT_EXCEPTION_HANDLER.getClass().getName());
		_shellPrompt = new ShellPromptImpl(prefs.get(SHELL_PROMPT_TEMPLATE_KEY, DEFAULT_SHELL_PROMPT_TEMPLATE));
	...
	static private IExceptionHandler createExceptionHandler(String className)
	{
		try {
			...
			Class<? extends IExceptionHandler> cl = (Class<? extends IExceptionHandler>) loader.loadClass(className);
			return cl.getConstructor().newInstance();
		} catch (Throwable cause) {
			_logger.warn("Unable to load exception handler -- using default.", cause);
	...
	public void setExceptionHandler(IExceptionHandler handler) throws BackingStoreException
	{
		_exceptionHandler = handler;

		Preferences prefs = Preferences.userNodeForPackage(UserPreferences.class);

		prefs.put(EXCEPTION_HANDLER_PREFERENCE_KEY, _exceptionHandler.getClass().getName());
		prefs.flush();
	}
	...
	public void preferGUI(boolean preferGUI) throws BackingStoreException
	{
