...
public class ApplicationDescriptionUtils
{
	...
	static {
		try {
			DEPLOYMENT_TYPE_ZIPJAR = new URI("http://vcgr.cs.virginia.edu/genii/" + "application-description/zip-jar");
			DEPLOYMENT_TYPE_BINARY = new URI("http://vcgr.cs.virginia.edu/genii/" + "application-description/bin");
		} catch (Throwable t) {
			throw new RuntimeException(t.getLocalizedMessage(), t);
		}
	}

	static public QName ZIPJAR_DEPLOYMENT_ELEMENT_QNAME = new QName(DEPLOYMENT_TYPE_ZIPJAR.toString(), "zip-jar");
	static public QName BINARY_DEPLOYMENT_ELEMENT_QNAME = new QName(DEPLOYMENT_TYPE_BINARY.toString(), "binary");

	static public URI determineDeploymentType(DeploymentDocumentType deployment) throws DeploymentException
	{
	...
	static public DeploymentDocumentType uploadLocalSources(File deployDirectory, DeploymentDocumentType deploymentDocument,
	...
	static private ZipJarDeploymentType uploadLocalSources(File deployDirectory, ZipJarDeploymentType deploymentInfo,
		IUploadProgressListener listener) throws IOException, RNSException
	{
		ZipJarSourceType source = deploymentInfo.getSource();

		source.set_any(uploadLocalSource(deployDirectory, source.get_any(), listener));
		deploymentInfo.setSource(source);

		return deploymentInfo;
	}

	static private BinDeploymentType uploadLocalSources(File deployDirectory, BinDeploymentType deploymentInfo,
		IUploadProgressListener listener) throws IOException, RNSException
	{
		NamedSourceType[] sources = deploymentInfo.getBinary();
		if (sources != null) {
			for (int lcv = 0; lcv < sources.length; lcv++)
				sources[lcv].set_any(uploadLocalSource(deployDirectory, sources[lcv].get_any(), listener));
		...
		if (otherSources != null) {
			for (int lcv = 0; lcv < otherSources.length; lcv++)
				otherSources[lcv].set_any(uploadLocalSource(deployDirectory, otherSources[lcv].get_any(), listener));
		...
		IUploadProgressListener listener) throws IOException, RNSException
	{
		...
		for (int lcv = 0; lcv < any.length; lcv++) {
			...
			if (name.equals(LOCAL_FILE_SOURCE_NAME)) {
				...
				any[lcv] = new MessageElement(REMOTE_ENDPOINT_NAME, uploadLocalFile(deployDirectory, localPath, listener));
			}
		}
		...
		IUploadProgressListener listener) throws IOException
	{
		...
		try {
			...
			newFile = ResourceCreator.createNewResource("RandomByteIOPortType", null, null);
			...
			throw new IOException("Unable to create ByteIO Resource:  " + ce.getLocalizedMessage());
		} finally {
