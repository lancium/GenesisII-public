...
public class DirectoryManager {
	...
	public static void addNewDirEntry(RNSPath path, GenesisIIFilesystem fs, boolean entryCreated) {
		...
		UnixDirectory directory = (UnixDirectory) 
			CacheManager.getItemFromCache(parentPath, UnixDirectory.class);
		if (directory != null) {
			try {
				...
				FilesystemStatStructure structure = 
					fs.stat(UnixFilesystemPathRepresentation.INSTANCE.parse(null, localPath));
				...
				throw new FSRuntimeException(
						FSExceptions.translate("Unable to stat entry.", e));
			}
		}
	...
	public static String getPathForDirectoryEntry(String parentPath, String entryName) {
		if (parentPath.equals("/")) return parentPath + entryName;
		return parentPath + "/" + entryName;
	}
	
	/*
	 * Updating the resource configuration because of addition, removal, or movement of 
	 * directory entries is not a required operation. This is done just to reflect the 
	 * local updates in the resource configuration as soon as possible, which will subsequently
	 * help to reduce possible redundant lookup calls.
	 * */
	...
	/*
	 * This method is used to judge the effectiveness of using the local-cache for a directory lookup operation.
	 * As we prefetch attributes with lookup call, when attributes are absent in the cache but the directory entry
	 * is not, it is better to let the RPC to pass instead of returning result from the local-cache since at the 
	 * end it will reduce the overall RPCs. 
	 * */
	private static boolean isCacheContainNecessaryDirectoryInfo(String directoryPath, UnixDirectory directory) {
		
		WSResourceConfig directoryConfig = (WSResourceConfig) CacheManager.getItemFromCache(
				directoryPath, WSResourceConfig.class);
		
		// We adhere to a pessimistic approach here. Additionally, since we track last time usage of per-container
		// resources by tracking down when an EPR or a ResourceConfiguration has last been accessed, it is safer to
		// be pessimistic to avoid collecting invalid statistics.
		if (directoryConfig == null) return false;
		String directoryContainerId = directoryConfig.getContainerId();
		if (directoryContainerId == null) return false;
		
		// If the number of elements in the directory does not match with the element-count property of the RNS resource
		// then the cached directory is certainly stale. On the other hand, when the element-count property is not in
		// the cache it is safer to assume that the cached directory is invalid. In both cases, an RPC will be issued to
		// replenish the missing contents in the directory.
		MessageElement elementCountElement = (MessageElement) CacheManager.getItemFromCache(
				directoryConfig.getWsIdentifier(), RNSConstants.ELEMENT_COUNT_QNAME, MessageElement.class);
		if (elementCountElement == null) return false;
		int elementCount = Integer.parseInt(elementCountElement.getValue());
		if (elementCount != directory.getEntries().size()) return false;
		...
		for (FuseDirEnt entry : directory.getEntries()) {
			...
			WSResourceConfig entryConfig = 
				(WSResourceConfig) CacheManager.getItemFromCache(entryPath, WSResourceConfig.class);
			...
			if (directoryContainerId.equals(entryContainerId)) {
				// Make a probabilistic judgment about the effectiveness of returning result from cache
				// based on the availability of permissions-string attribute. This attribute is chosen
	...
	public static String getEntryName(String entryPath) {
		...
		if (rootPath.equals(entryPath)) return "";
