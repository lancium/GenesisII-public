...
public class MetadataManager {
	...
	static private SingleResourcePropertyTranslator permissionTranslater = 
		new PermissionsStringTranslator();
	static private SingleResourcePropertyTranslator translator = 
	...
	public static void updateAttributesAfterWrite(EndpointReferenceType EPR) {
		...
		if (nameSpaceForAttributes != null) {
			CacheManager.removeItemFromCache(EPR, 
					new QName(nameSpaceForAttributes, ByteIOConstants.SIZE_ATTR_NAME), 
			...
			CacheManager.putItemInCache(EPR,
					new QName(nameSpaceForAttributes, ByteIOConstants.ACCESSTIME_ATTR_NAME), 
					modTimeElement);				
		}
	}
	...
	public static int getMode(FilesystemStatStructure statstruct) {
		...
		if (p.isSet(PermissionBits.OWNER_READ))			mode |= FuseStat.OWNER_READ;
		if (p.isSet(PermissionBits.OWNER_WRITE)) 		mode |= FuseStat.OWNER_WRITE;
		if (p.isSet(PermissionBits.OWNER_EXECUTE)) 		mode |= FuseStat.OWNER_EXECUTE;
		
		if (p.isSet(PermissionBits.GROUP_READ)) 		mode |= FuseStat.GROUP_READ;
		if (p.isSet(PermissionBits.GROUP_WRITE)) 		mode |= FuseStat.GROUP_WRITE;
		if (p.isSet(PermissionBits.GROUP_EXECUTE)) 		mode |= FuseStat.GROUP_EXECUTE;
		
		if (p.isSet(PermissionBits.EVERYONE_READ)) 		mode |= FuseStat.OTHER_READ;
		if (p.isSet(PermissionBits.EVERYONE_WRITE)) 	mode |= FuseStat.OTHER_WRITE;
		if (p.isSet(PermissionBits.EVERYONE_EXECUTE)) 	mode |= FuseStat.OTHER_EXECUTE;
		
		FilesystemEntryType entryType = statstruct.getEntryType();
		if (entryType == FilesystemEntryType.DIRECTORY) mode |= FuseStat.TYPE_DIR;
		else											mode |= FuseStat.TYPE_FILE;
		
		return mode;
	}
	
	// TODO: We have to modify the implementation to support caching of resource 
	// configurations of EndPointReferences that do not have any EndPointIdentifier.
	public static int generateInodeNumber(EndpointReferenceType target) {
		...
		if (name.isValidWSName()) {
			...
			/* Updating or storing the resource configuration object in the cache.
			 * It is important to update the cache here as we do not expect to regenerate
			 * the INode number again as long as our concerned resource is already 
			 * in the cache.
			 * */
			WSResourceConfig config = 
				(WSResourceConfig) CacheManager.getItemFromCache(
						endpointIdentifier, WSResourceConfig.class);
			if (config == null) {
	...
	/*
	 * This is the bridge between FUSE and RP-level cache. Here, we construct a file
	 * system INode structure from information stored in the resource configuration 
	 * and the RP-attribute caches. The method returns null if any required attribute
	 * is found missing; in that case an RPC is initiated by the caller to retrieve 
	 * the missing piece information, and INode is constructed in a normal fashion 
	 * without the supervision of MetadataManager.
	 * */
	public static FilesystemStatStructure retrieveStat(String rnsPathString) {
		WSResourceConfig config = 
			(WSResourceConfig) CacheManager.getItemFromCache(rnsPathString, WSResourceConfig.class);
		if (config == null) return null;
		Integer inodeNumber = config.getInodeNumber();
		if (inodeNumber == null) return null; 
		...
		if (permissionElement == null) return null;

		FilesystemEntryType type = (config.isDirectory()) ? 
		...
		try {
			...
			if (config.isDirectory()) {
				...
				size = (Long) getDeserializedAttributeFromCache(Long.class, 
						wsIdentifier, ByteIOConstants.rsize, ByteIOConstants.ssize);
			...
			return new FilesystemStatStructure(inodeNumber, name, 
	...
	private static QName getSizeAttributeQName(EndpointReferenceType EPR) {
		...
		return (namespace == null) ? null 
				: new QName(namespace, ByteIOConstants.SIZE_ATTR_NAME);
	}
	
	private static MessageElement getAttributeFromCache(URI wsIdentifier, QName...names) {
		for (QName name : names) {
			MessageElement element = 
				(MessageElement) CacheManager.getItemFromCache(
						wsIdentifier, name, MessageElement.class);
			if (element != null) return element;
		}
		return null;
	}
	
	private static Object getDeserializedAttributeFromCache(Class<?> attributeType, 
			URI wsIdentifier, QName...names) throws Exception {
	...
	/*
	 * This method is adequate for Unix, but not for Windows. If in future we managed to 
	 * make FUSE works for Windows, we will have to change the implementation.
	 * */
	private static String getNameFromPath(String path) {
		if (path == null) return "/";
		String trimmedPath = path.trim();
		if (trimmedPath.length() == 1) return "/";
