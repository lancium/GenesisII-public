...
public class AuthZConfigCache extends CommonAttributeCache {
	...
	public void putItem(Object cacheKey, Object target, Object value) throws Exception {
		...
		if (qName.equals(GenesisIIBaseRP.PERMISSIONS_STRING_QNAME)) {
			...
			// Since an authZConfig holds reference to a set of MessageElements, there are references from 
			// it to the SOAPMessage that has returned this authZConfig. We have to cleanup all such references
	...
	public boolean cacheKeyMatches(Object cacheKey) {
		if (cacheKey instanceof QName) {
			return (cacheKey.equals(GenesisIIBaseRP.AUTHZ_CONFIG_QNAME) 
					|| cacheKey.equals(GenesisIIBaseRP.PERMISSIONS_STRING_QNAME));
		}
		return false;
	}
	...
	private Permissions getPermissions(AuthZConfig authZConfig) {
		if (authZConfig == null) return null;
		try {
			...
			return GenesisIIACLManager.getPermissions(Acl.decodeAcl(authZConfig), callerIdentities);
		} catch (Exception ex) {
