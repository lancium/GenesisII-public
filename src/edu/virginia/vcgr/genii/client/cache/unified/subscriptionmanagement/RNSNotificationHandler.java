...
public class RNSNotificationHandler {
	...
	public static void handleContentChangeNotification(
			RNSContentChangeNotification notification, EndpointReferenceType publisher) {
	...
	private static void updateElementCount(RNSContentChangeNotification notification,
	...
	private static void handleEntryAddition(RNSContentChangeNotification notification,
		 ...
		 * is done to avoid the pitfall of having stuck with an empty directory when there exists some 
		 * elements within it. As an example, if the client has a subscription in '/home/a' and someone
		 * extracts an archive 'b' there then the initial notification the client will receive is an 
		 * empty directory 'b' has been added in '/home/a'. It will not receive any notification for 
		 * update under '/home/a/b' done by the other client as 'b' is not within the subscribed list
		 * of resources. Therefore, if the current client goes to '/home/a/b' before the initially retrieved
		 * elementCount property has been expired, directory listing will show incorrect result. 
		 * Furthermore, to make things worse, a subscription will be created for 'b' and the wrong element
		 * count will become permanent in the cache.
		 * 
		 * Note that, if 'b' was an already subscribed existing archive moved from one directory to another
		...
		if (additionalAttributes != null) {
			for (MessageElement attribute : additionalAttributes) {
				if (RNSConstants.ELEMENT_COUNT_QNAME.equals(attribute.getQName())) continue;
	...
	private static void handleNotificationBlocking(EndpointReferenceType publisher, NotificationMessageContents message) {
		...
		// removing element count from the cache so that subsequent lookup calls should make an outcall. 
		...
		// set blockade flag in resource configuration so that element count and directory cache cannot be updated 
		// for the time being.
