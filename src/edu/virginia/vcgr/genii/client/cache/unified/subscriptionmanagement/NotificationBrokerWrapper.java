...
public class NotificationBrokerWrapper {
	...
	private static final long NOTIFICATION_POLLING_SCHEDULE_TESTING_INTERVAL = 100L; // hundred milliseconds
	...
	public NotificationBrokerWrapper(EnhancedNotificationBrokerPortType brokerPortType, 
			String containerId,
			long brokerResourceLifeTime,
			boolean mode,
	...
	public void testBrokerMode() {
		if (lastModeTestTrialTimeInMillis != null) {
			...
			if (currentTimeInMillis - lastModeTestTrialTimeInMillis > MODE_TEST_TRIAL_INTERVAL) return; 
		}
		try {
	...
	public void destroyBroker() {
		...
		// applied.
		if (brokerDestroyed || brokerExpired()) return;
		...
		public void run() {
			...
			while (!brokerInActiveMode && !brokerDestroyed && !brokerExpired()) {
				try {
					...
					} else Thread.sleep(NOTIFICATION_POLLING_SCHEDULE_TESTING_INTERVAL); 
