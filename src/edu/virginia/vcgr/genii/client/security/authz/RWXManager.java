...
public class RWXManager
{
		...
		Class<?> service)
	{
		...
		{
			HashMap<String, RWXCategory> classMap = _cachedValues.get(
				service.getName());
			if (classMap == null)
			{
	...
	static private HashMap<String, RWXCategory> createClassMap(
		Class<?> service)
	{
		HashMap<String, RWXCategory> classMap = 
	...
	static private RWXCategory lookup(Class<?> service, String methodName)
	{
		...
		if (category == null)
			throw new RWXMappingException(
				"Couldn't find RWX mapping for method \""
				+ methodName + "\" in class \"" + service + "\".");
