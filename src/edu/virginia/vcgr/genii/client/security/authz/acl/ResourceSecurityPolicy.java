...
public final class ResourceSecurityPolicy
{
	...
	public ResourceSecurityPolicy(EndpointReferenceType epr)
		throws ResourcePropertyException, AuthZSecurityException, GeneralSecurityException
	{
		_propertyManager =  (GenesisIIBaseRP) ResourcePropertyManager.createRPInterface(
				epr, GenesisIIBaseRP.class);
		AuthZConfig config = _propertyManager.getAuthZConfig();
		_acl = (config == null ? new Acl() : Acl.decodeAcl(config));
		X509Certificate[] certChain = EPRUtils.extractCertChain(epr);
		_identity = new X509Identity(certChain);
	}

	public void addResource(ResourceSecurityPolicy resource)
	...
	public void addResource(ResourceSecurityPolicy resource, RWXCategory category)
		throws AuthZSecurityException
	{
	...
	public void copyFrom(ResourceSecurityPolicy resource)
		throws AuthZSecurityException
	{
	...
	{
		_propertyManager.setAuthZConfig(Acl.encodeAcl(_acl));
	}
}
