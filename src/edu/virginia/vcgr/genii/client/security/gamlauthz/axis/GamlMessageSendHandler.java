...
public class GamlMessageSendHandler
{
	...
	static private Log _logger =
			LogFactory.getLog(GamlMessageSendHandler.class);
	...
	public static void messageSendPrepareHandler(
			ICallingContext callingContext, Method method,
			...
			throws GenesisIISecurityException
	{
		...
		try
		{
				...
				else if (cred instanceof SignedAssertion)
				{
					...
					if ((clientKeyMaterial != null)
							&& (signedAssertion.getAuthorizedIdentity()[0]
									.equals(clientKeyMaterial._clientCertChain[0])))
					{
						...
						{
							// Because we know an identity for the remote
							// resource, delegate the
							// current credential assertion to it
							DelegatedAttribute delegatedAttribute =
									new DelegatedAttribute(null,
											signedAssertion, resourceCertChain);

							signedAssertion =
									SignedCredentialCache
			...
			if (clientKeyMaterial._clientCertChain.length > 1)
			{
				...
				IdentityAttribute myIdentityAttr =
						new IdentityAttribute(new BasicConstraints(System
								.currentTimeMillis()
								- (1000L * 60 * 15), // 15 minutes ago
								GenesisIIConstants.CredentialExpirationMillis, // valid
																				// 24
																				// hours
								10), // 10 delegations
								new X509Identity(
				...
				// unlike incoming credentials that we have no control over, we
				// don't cache
				// these (yet) because we can give ourselves a full expiration
				// window
				SignedAssertion signedAssertion =
						new SignedAttributeAssertion(myIdentityAttr,
								clientKeyMaterial._clientPrivateKey);

				if (resourceCertChain != null)
				{
					// Because we know an identity for the remote resource,
					// delegate the
					// current credential assertion to it
					DelegatedAttribute delegatedAttribute =
							new DelegatedAttribute(null, signedAssertion,
									resourceCertChain);
					signedAssertion =
