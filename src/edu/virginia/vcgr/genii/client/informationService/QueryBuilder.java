...
public class QueryBuilder {
	...
	public String BuildQueryFromGUI(GUIInternalStruct GUIStructure) {
		...
		/*
		 * Determining the conditions and forming the partial queries
		 * Dependion on what values the user has entered in the GUI a corresponding XQuery query is
		 * formed.
		 */
		if (!GUIStructure.getOSTypeValue().equals("")){
			...
			else resultingQuery = resultingQuery.concat(" and "+OSTypeQuery);
		}
		...
		if (!GUIStructure.getOSVersionValue().equals("")){
			...
			else resultingQuery = resultingQuery.concat(" and "+OSVersionQuery);
		}
		...
		if (!GUIStructure.getPhysicalMemoryValue().equals("")){
			...
			else resultingQuery = resultingQuery.concat(" and "+physicalMemoryQuery);
		}
		...
		if (!GUIStructure.getVirtualMemoryValue().equals("")){
			...
			else resultingQuery = resultingQuery.concat(" and "+virtualMemoryQuery);
		}
		...
		if (!GUIStructure.getCommonNameValue().equals("")){
			...
			"[ns2:CommonName='"+ GUIStructure.getCommonNameValue()+"']";
			if (resultingQuery.equals(""))
				resultingQuery = resultingQuery.concat(commonNameQuery);
			else resultingQuery = resultingQuery.concat(" and "+commonNameQuery);
		}
		...
		if (!GUIStructure.getTotalNumberOfActivitiesValue().equals("")){
			...
			else resultingQuery = resultingQuery.concat(" and "+totalNumberOfActivitiesQuery);
		}
		...
		if (!GUIStructure.getLocalResourcemanagerValue().equals("")){
			...
			else resultingQuery = resultingQuery.concat(" and "+localResourceManagerQuery);
		}
		...
		if (!GUIStructure.getNamingProfilevalue().equals("")){
			...
			"[ns2:NamingProfile='"+ GUIStructure.getNamingProfilevalue()+"']";
				if (resultingQuery.equals(""))
					resultingQuery = resultingQuery.concat(namingProfileQuery);
				else resultingQuery = resultingQuery.concat(" and "+namingProfileQuery);
		}
		...
		//if (!GUIStructure.getIsAcceptingNewActivitiesValue().equals(""))
			...
			else resultingQuery = resultingQuery.concat(" and "+isAcceptingNewActivitiesQuery);
