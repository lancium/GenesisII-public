...
public class GenesisIICachedFilesystem implements FSFilesystem
{
	...
	private FileHandleTable<GeniiOpenFileHandle> _resourceHandleTable = new FileHandleTable<GeniiOpenFileHandle>(1024);
	...
	public void link(String[] sourcePath, String[] targetPath) throws FSException
	{
		...
		GeniiCachedDir cachedParent = (GeniiCachedDir) _cacheManager.getCacheItem(getParent(targetPath), false);
	...
	public void mkdir(String[] path, Permissions initialPermissions) throws FSException
	{
		...
		if (cachedDir != null) {
			throw new FSEntryAlreadyExistsException(String.format("Directory %s already exists.", fullPath));
		} else {
			cachedDir = new GeniiCachedDir(path, initialPermissions, true);
			_cacheManager.putCacheItem(fullPath, cachedDir);

			// Make sure parent isn't cached. If it is, add to it
			GeniiCachedDir cachedParent = (GeniiCachedDir) _cacheManager.getCacheItem(getParent(path), true);
			if (cachedParent != null) {
		...
		try {
			...
			if (cachedFile != null) {
				...
				if (flags.isExclusive())
					throw new FSEntryAlreadyExistsException(String.format("Path %s already exists.", fullPath));
			} else {
				cachedFile = new GeniiCachedFile(path, flags, mode, initialPermissions);
			}
	...
	public void rename(String[] fromPath, String[] toPath) throws FSException
	{
		...
		// Make sure parent isn't cached. If it is, add to it
		GeniiCachedDir fromCachedParent = (GeniiCachedDir) _cacheManager.getCacheItem(getParent(fromPath), false);
		...
		// Make sure parent isn't cached. If it is, add to it
		GeniiCachedDir toCachedParent = (GeniiCachedDir) _cacheManager.getCacheItem(getParent(toPath), false);
	...
	public FilesystemStatStructure stat(String[] path) throws FSException
	{
		...
		if (cachedResource == null) {
			throw new FSEntryNotFoundException(String.format("Couldn't find path %s.", fullPath));
		} else {
	...
	public void unlink(String[] path) throws FSException
	{
		...
		GeniiCachedDir cachedParent = (GeniiCachedDir) _cacheManager.getCacheItem(getParent(path), false);
		if (cachedParent != null) {
			cachedParent.refresh();
		}
	}
