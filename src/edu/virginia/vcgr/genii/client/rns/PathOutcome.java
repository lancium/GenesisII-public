...
/*
 * These values are standard returns for recursePath.  There can also be other
 * values returned by an ActionAlert that are specific to higher-level types. 
 */
public class PathOutcome {
    ...
    public boolean same(int outcome) { return this.getOutcome() == outcome; }
    public boolean same(PathOutcome outcome)
    { return (outcome != null) && (this.getOutcome() == outcome.getOutcome()); }
    public boolean differs(int outcome) { return this.getOutcome() != outcome; }
    ...
    public static String outcomeText(PathOutcome toText) {
        if (toText.same(OUTCOME_SUCCESS)) return "completely successful";
        if (toText.same(OUTCOME_ERROR)) return "an error occurred during the operation";
        if (toText.same(OUTCOME_BOUNCED)) return "operation halted due to inappropriate or cyclic path";
        if (toText.same(OUTCOME_NONEXISTENT)) return "the path does not exist";
        if (toText.same(OUTCOME_NO_ACCESS)) return "there were insufficient permissions";
        if (toText.same(OUTCOME_NON_EMPTY)) return "the directory object was non-empty";
        if (toText.same(OUTCOME_EMPTY)) return "the directory object was empty";
        if (toText.same(OUTCOME_WRONG_TYPE)) return "the filesystem object was the wrong type";
        if (toText.same(OUTCOME_NOTHING)) return "the operation seemed to have no effect";
        if (toText.same(OUTCOME_EXISTENT)) return "there was an existing object in that location";
        if (toText.same(OUTCOME_TOO_DEEP)) return "tree traversal went too deep, possibly due to cyclic path";
        if (toText.same(OUTCOME_CONTINUABLE)) return "tree traversal saw a bounce condition but can continue";
        if (toText.same(OUTCOME_UNSUPPORTED)) return "cannot traverse RNS space when lacking EPIs for path";
        return "Unknown Error";
    }
    
    public PathOutcome(int outcome) { this._outcome = outcome; }
    public int getOutcome() { return _outcome; }
}
