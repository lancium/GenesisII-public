...
public class WindowsFirewall
{
	...
	static private class OpenPortTask extends AbstractExecutionTask
	{
		...
		{
			return new String[] { "netsh", "firewall", "add", "portopening", String.format("protocol=%s", _portType),
				String.format("name=%s", _name), String.format("port=%d", _port) };
		}

		@Override
		public String toString()
		{
			return String.format("Open Firewall Port %s(%s)", _portType, _port);
		}
	}
	...
	static private class ClosePortTask extends AbstractExecutionTask
	{
		...
		public String[] getCommandLine()
		{
			return new String[] { "netsh", "firewall", "delete", "portopening", String.format("protocol=%s", _portType),
		...
		public String toString()
		{
			return String.format("Close Firewall Port %s(%s)", _portType, _port);
		}
	}

	static public ExecutionTask createOpenPortTask(String name, int port, FirewallPortTypes portType)
	{
		return new OpenPortTask(name, port, portType);
	}

	static public ExecutionTask createClosePortTask(int port, FirewallPortTypes portType)
	{
