...
public class ExecutionEngine
{
	...
	static private void logCommandStart(ExecutionTask task,
		String []cLine)
	{
		...
		_logger.info(String.format("Task(%s) -- Executing command:  %s",
			task, builder));
	}
	
	static public void execute(InputStream stdin,
		OutputStream stdout, OutputStream stderr,
		ExecutionTask task) throws ExecutionException, IOException
	{
		ByteArrayOutputStream outStorage = new ByteArrayOutputStream();
		ByteArrayOutputStream errStorage = new ByteArrayOutputStream();
		
		stdout = (stdout == null) ? 
			outStorage : new TeeOutputStream(stdout, outStorage);
		stderr = (stderr == null) ?
			errStorage : new TeeOutputStream(stderr, errStorage);
		...
		if (data != null && data.length > 0)
			_logger.info(String.format(
				"Task(%s) -- Command produced output:  %s", 
				task, new String(data)));
		
		data = errStorage.toByteArray();
		if (data != null & data.length > 0)
			_logger.warn(String.format(
				"Task(%s) -- Command produced error:  %s",
				task, new String(data)));
