...
public class SecurityPolicy
{
		...
		Collection<MessageElement> policyReferences)
	{
		...
		metaPolicy.set_any(policyReferences.toArray(
			new MessageElement[policyReferences.size()]));

		// construct AppliesTo
		org.w3.www.ns.ws_policy.URI appliesToUri = 
			new org.w3.www.ns.ws_policy.URI("urn:wsaaction:*");
	...
	static public MessageElement usernamePasswordPolicy(boolean isOptional)
	{
		try
		{
			URI usernameTokenUri =
				new URI(SecurityConstants.USERNAME_TOKEN_URI);
			...
			MessageElement usernameTokenMel =
				new MessageElement(
					PolicyReference.getTypeDesc().getXmlType().getNamespaceURI(),
					"PolicyReference", usernameTokenReference);
			
			if (isOptional)
				usernameTokenMel.setAttribute(
					Policy.getTypeDesc().getXmlType().getNamespaceURI(), 
					"Optional", "true");
	...
	{
		Collection<MessageElement> policyComponents = 
