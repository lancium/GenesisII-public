...
public class FilesystemManager implements Serializable
{
	...
	private Map<String, JSDLFileSystem> _filesystems = new HashMap<String, JSDLFileSystem>();
	...
	public File lookup(FilesystemRelativePath path) throws JSDLException
	{
		if (path == null)
			return null;
		...
		if (fsName == null) {
			...
			if (fs == null)
				throw new JSDLException(String.format("Couldn't locate file system \"%s\".", fsName));

			try {
				return fs.relativeTo(relativePath);
			} catch (IOException ioe) {
				throw new JSDLException("Unable to look up path relative to file system.", ioe);
			}
		}
	}
