...
public class DurationTranslator implements SingleResourcePropertyTranslator
{
	@Override
	public <Type> Type deserialize(Class<Type> clazz, MessageElement element) throws ResourcePropertyException
	{
		...
		try {
			...
			edu.virginia.vcgr.genii.client.utils.units.Duration gDur = edu.virginia.vcgr.genii.client.utils.units.Duration
				.fromApacheDuration(aDur);
			return clazz.cast(gDur);
		} catch (Exception e) {
			throw new ResourcePropertyException("Unable to extract duration from property.", e);
		}
	}
