...
public class MinimumVersionException extends RemoteException
{
	...
	static private final String ERROR_MESSAGE_FORMAT =
		"Client (%s) does not meet the minumum version requirements (%s).";
	...
	{
		super(String.format(ERROR_MESSAGE_FORMAT,
			clientVersion, minimumVersion));
	...
	static public MinimumVersionException reformException(String message)
	{
		...
		if (matcher.matches())
			return new MinimumVersionException(
				new Version(matcher.group(1)),
				new Version(matcher.group(2)));
