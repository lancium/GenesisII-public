	...
	public int handleException(Throwable cause, Writer eStream)
	{
		...
		while (cause != null) {
			if (cause instanceof MinimumVersionException) {
				...
				builder.append(tab
					+ String.format("Your client at version \"%s\" doesn't appear to meet\n"
						+ "the minimum version requirements for the target\n"
						+ "operation (%s).  Please exit the grid shell and\n"
						+ "run the grid-update tool to upgrade your client.", me.getClientVersion(), me.getMinimumVersion()));
				...
				if ((operation != null) && (failedAsset != null)) {
					...
					MinimumVersionException mve = MinimumVersionException.reformException(message);
					if (mve != null) {
