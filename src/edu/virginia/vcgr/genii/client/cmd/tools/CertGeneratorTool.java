...
{
	static private final String _DESCRIPTION = "edu/virginia/vcgr/genii/client/cmd/tools/description/dcert-generator";
	static private final String _USAGE_RESOURCE = "edu/virginia/vcgr/genii/client/cmd/tools/usage/ucert-generator";
	...
	{
		super(new FileResource(_DESCRIPTION), new FileResource(_USAGE_RESOURCE), false, ToolCategory.SECURITY);
	}
		...
		InvalidToolUsageException
	{
		...
		issuerPrivateKey = (PrivateKey) keyStore.getKey(issuerCertAlias, entryPassChars);
		...
		MessageElement[] createProps = CertGeneratorUtils.createCreationProperties(issuerCertChain, issuerPrivateKey,
			defaultValidity);

		if (!url) {
			RNSPath path = RNSPath.getCurrent();
			path = path.lookup(service, RNSPathQueryFlags.MUST_EXIST);
			epr = CreateResourceTool.createInstance(path.getEndpoint(), (optTargetName == null) ? null : new GeniiPath(
				optTargetName), createProps);
		} else {
			epr = CreateResourceTool.createInstance(EPRUtils.makeEPR(service), (optTargetName == null) ? null : new GeniiPath(
				optTargetName), createProps);
		}
		return epr;
	}
	...
	static public X509Name generateX509Name(String cn, String c, String st, String l, String o, String ou, String email)
