...
public class CreateJndiStsTool extends BaseGridTool
{
	...
	{
		super(new FileResource("edu/virginia/vcgr/genii/client/cmd/tools/description/dcreate-jndi-sts"), new FileResource(
			"edu/virginia/vcgr/genii/client/cmd/tools/usage/ucreate-jndi-sts-usage"), false, ToolCategory.ANTIQUATED);
	}
	...
	protected void verify() throws ToolException
	{
		...
		if (_jndiType != null) {
			if ((_jndiType == null) || (!_jndiType.equalsIgnoreCase("NIS") && !_jndiType.equalsIgnoreCase("LDAP")))
	...
	protected int runCommand() throws Throwable
	{
		...
		if (jndiAuthnPortTypePath == null) {
			...
			jndiAuthnPortTypeRNS = RNSUtilities.findService("/containers/BootstrapContainer", "JNDIAuthnPortType",
				new PortType[] { WellKnownPortTypes.JNDI_AUTHN_SERVICE_PORT_TYPE }, jndiAuthnPortTypePath);
		...
		// type
		constructionParms.add(new MessageElement(SecurityConstants.NEW_JNDI_STS_TYPE_QNAME, _jndiType));
		// new name
		constructionParms.add(new MessageElement(SecurityConstants.NEW_JNDI_STS_NAME_QNAME, stsName));
		// host
		constructionParms.add(new MessageElement(SecurityConstants.NEW_JNDI_STS_HOST_QNAME, _host));
		// ldap searchbases
		for (String searchBase : _searchBases) {
			constructionParms.add(new MessageElement(SecurityConstants.NEW_JNDI_STS_SEARCHBASE_QNAME, searchBase));

		}
		// nis domain
		if (_domain != null) {
			constructionParms.add(new MessageElement(SecurityConstants.NEW_JNDI_NISDOMAIN_QNAME, _domain));
		}

		// create the new idp resource and link it into context space
		CreateResourceTool.createInstance(jndiAuthnPortTypeRNS.getEndpoint(), null, // no link
																					// needed
	 ...
	 * @throws IOException
	 */
	protected RNSPath getJndiAuthnPortTypePathFromUser(DialogProvider wp) throws DialogException, UserCancelException
	{
		...
		while (true) {
			try {
				...
				RNSPath idpService = RNSUtilities.findService("/containers/BootstrapContainer", "X509AuthnPortType",
	 ...
	 * @throws DialogException
	 */
	protected String getStsNameFromUser(DialogProvider wp, RNSPath idpServicePath) throws DialogException, UserCancelException
	{
		InputDialog dialog = wp.createInputDialog("New Instance IDP Name", "New IDP instance name?");
		dialog.setHelp(new TextContent("The IDP name is the new name inside the IDP service by",
	...
	static private class IDPInstanceInputValidator implements InputValidator
	{
		...
		public String validateInput(String input)
		{
			...
			if (input.contains("/"))
				return "Invalid IDP instance name.  " + "Names cannot contain '/' characters.";
