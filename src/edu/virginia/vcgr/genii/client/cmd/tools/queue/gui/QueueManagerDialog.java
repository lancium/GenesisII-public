...
public class QueueManagerDialog extends JFrame
{
	...
	static private class TimestampRenderer extends DefaultTableCellRenderer
	{
		...
		protected void setValue(Object value)
		{
			super.setValue(String.format("%1$tH:%1$tM %1$tZ %1$td %1$tb %1$tY", value));
		}
	}
	...
	private class QueueManipulatorWorker implements Runnable
	{
		...
		public QueueManipulatorWorker(Collection<JobTicket> tickets, boolean isComplete)
		{
			_tickets = tickets;
		...
		public void run()
		{
			...
			try {
				...
				JOptionPane.showMessageDialog(QueueManagerDialog.this, "Unable to complete/kill jobs.", "Queue Error",
	...
	QueueManagerDialog(EndpointReferenceType queue) throws RemoteException
	{
		...
		_table.getColumnModel().getColumn(4).setCellRenderer(new StateRenderer());
		_table.getColumnModel().getColumn(1).setCellRenderer(new TimestampRenderer());
		prepareColumns(_table);
		JScrollPane scroller = new JScrollPane(_table);
		scroller.setPreferredSize(new Dimension(1000, 500));
		content.add(scroller, new GridBagConstraints(0, 0, 5, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
			new Insets(5, 5, 5, 5), 5, 5));
		content.add(new JButton(new ShowLogAction(_table)), new GridBagConstraints(0, 1, 1, 1, 1.0, 0.0,
			GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		content.add(new JButton(new ShowErrorsAction(_table)), new GridBagConstraints(1, 1, 1, 1, 1.0, 0.0,
			GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		content.add(new JButton(new CompleteAction(_table)), new GridBagConstraints(2, 1, 1, 1, 1.0, 0.0,
			GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		content.add(new JButton(new DeleteAction(_table)), new GridBagConstraints(3, 1, 1, 1, 1.0, 0.0,
			GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		content.add(new JButton(new RefreshAction()), new GridBagConstraints(4, 1, 1, 1, 1.0, 0.0, GridBagConstraints.CENTER,
			GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));

		content.add(status, new GridBagConstraints(0, 2, 5, 1, 1.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
			new Insets(5, 5, 5, 5), 5, 5));
	}
	...
	private class CompleteAction extends AbstractAction implements ListSelectionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			for (int index : indices) {
				jobs.add(new JobTicket(_table.getValueAt(index, 0).toString()));
			}
	...
	private class ShowErrorsAction extends AbstractAction implements ListSelectionListener
	{
		static final long serialVersionUID = 0L;
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			ErrorDisplay display = new ErrorDisplay(QueueManagerDialog.this, _queueEPR, new JobTicket(_table.getValueAt(index,
				0).toString()));
			display.pack();
	...
	private class ShowLogAction extends AbstractAction implements ListSelectionListener
	{
		static final long serialVersionUID = 0L;
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			LogDisplay display = new LogDisplay(QueueManagerDialog.this, _queueEPR, new JobTicket(_table.getValueAt(index, 0)
				.toString()));
			display.pack();
			GuiUtils.centerComponent(display);
			display.setModalityType(ModalityType.DOCUMENT_MODAL);
