...
public class JNDIResource extends RNSDBResource implements IJNDIResource {
	...
	public void initialize(GenesisHashMap constructionParams)
		...
		} else if (isServiceResource == null
				|| !isServiceResource.booleanValue()) {
	...
	/*
	 * What does this do? public Collection<SubscriptionInformation>
	 * matchSubscriptions( String topicExpression) throws ResourceException {
	 * 
	 ...
	 * revisit this issue again. public ReferenceParametersType
	 ...
	 * 
	 * // add the end-certificate as a new reference parameter X509Certificate[]
	 * certChain = (X509Certificate[])
	 * getProperty(IResource.CERTIFICATE_CHAIN_PROPERTY_NAME); if (certChain !=
	 * null && (refParams != null)) {
	 * 
	 * try { MessageElement certRef = new MessageElement(_RESOURCE_CERT_QNAME);
	 * certRef.addChild(WSSecurityUtils .makePkiPathSecTokenRef(certChain));
	 * mels.add(certRef); } catch (SOAPException e) { throw new
	 * ResourceException(e.getMessage(), e); } catch (GeneralSecurityException
	 * e) { throw new ResourceException(e.getMessage(), e); } }
	 * 
	 * // add all of our creation params try { ResourceKey stsKey =
	 * ResourceManager.getCurrentResource(); IResource stsResource =
	 * stsKey.dereference();
	 * 
	 * MessageElement stsParm = null;
	 * 
	 * stsParm = new MessageElement( SecurityConstants.NEW_JNDI_STS_HOST_QNAME,
	 * stsResource .getProperty(SecurityConstants.NEW_JNDI_STS_HOST_QNAME
	 * .getLocalPart())); mels.add(stsParm); stsParm = new MessageElement(
	 * SecurityConstants.NEW_JNDI_NISDOMAIN_QNAME, stsResource
	 * .getProperty(SecurityConstants.NEW_JNDI_NISDOMAIN_QNAME
	 * .getLocalPart())); mels.add(stsParm);
	 * 
	 * } catch (ResourceUnknownFaultType e) { throw new
	 * ResourceException(e.getMessage(), e); }
	 * 
