...
public class ApplicationDescriptionAttributeHandler extends AbstractAttributeHandler
{
	public ApplicationDescriptionAttributeHandler(AttributePackage pkg) throws NoSuchMethodException
	{
		super(pkg);
	}

	private String getApplicationDescription() throws ResourceException, ResourceUnknownFaultType
	{
		IResource resource;
		ResourceKey rKey = ResourceManager.getCurrentResource();
		resource = rKey.dereference();

		return (String) resource.getProperty(ApplicationDescriptionServiceImpl.APPLICATION_DESCRIPTION_PROPERTY_NAME);
	}

	private ApplicationVersion getApplicationVersion() throws ResourceException, ResourceUnknownFaultType
	{
		IResource resource;
		ResourceKey rKey = ResourceManager.getCurrentResource();
		resource = rKey.dereference();

		return (ApplicationVersion) resource.getProperty(ApplicationDescriptionServiceImpl.APPLICATION_VERSION_PROPERTY_NAME);
	}
	...
	public MessageElement getApplicationVersionAttr() throws ResourceUnknownFaultType, ResourceException
	{
		...
		return new MessageElement(ApplicationDescriptionConstants.APPLICATION_VERSION_ATTR_QNAME, versionS);
	}

	public Collection<MessageElement> getSupportDocumentAttributes() throws ResourceException, ResourceUnknownFaultType
	{
		ArrayList<MessageElement> supportDocuments = new ArrayList<MessageElement>();
		...
		for (InternalEntry entry : entries) {
			...
			for (MessageElement element : attributes) {
				...
				if (name.equals(ApplicationDescriptionConstants.SUPPORT_DOCUMENT_ATTR_QNAME)) {
			...
			if (supportDocument == null)
				throw new ResourceException("Corrupt application state: No support document attribute.");
	...
	{
		addHandler(ApplicationDescriptionConstants.APPLICATION_DESCRIPTION_ATTR_QNAME, "getApplicationDescriptionAttr");
		addHandler(ApplicationDescriptionConstants.APPLICATION_VERSION_ATTR_QNAME, "getApplicationVersionAttr");
		addHandler(ApplicationDescriptionConstants.SUPPORT_DOCUMENT_ATTR_QNAME, "getSupportDocumentAttributes");
	}
}