...
public class NotificationForwarder implements Runnable {
	...
	@Override
	public void run() {
		if (listOfActiveBrokers == null || listOfActiveBrokers.isEmpty()) return;
		...
		try {
			...
			for (NotificationBrokerDBResource resource : listOfActiveBrokers) {
				...
				// We pass the message index of the notification broker with each notification message sent so that the 
				...
				// A message-attributes-separator element is needed in the attribute list as the common notification helper
				// expects such a marker with each notification message having additional attributes so that it can decide
				// which attribute belongs to which notification message, when it is given with a set of messages at once.
				// Finally, ordering is important here to ensure that attribute separator has been placed at the beginning.
				...
				brokerSpecificMessageElements.add(new MessageElement(
						NotificationBrokerConstants.MESSAGE_INDEX_QNAME, messageIndexOfCurrentBroker));
				
				notify.set_any(brokerSpecificMessageElements.toArray(
						new MessageElement[brokerSpecificMessageElements.size()]));
