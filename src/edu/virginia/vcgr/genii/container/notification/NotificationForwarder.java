...
public class NotificationForwarder implements Runnable {
	...
	public NotificationForwarder(NotificationMessageOutcallContent message,
			List<NotificationBrokerDBResource> listOfActiveBrokers,
	...
	public void run() {
		...
		MessageElement[] additionalAttributes = message.contents()
		...
		try {
			notify = new Notify(
					new NotificationMessageHolderType[] { holder.toAxisType() },
					null);
			for (NotificationBrokerDBResource resource : listOfActiveBrokers) {

				// We pass the message index of the notification broker with
				// each notification
				// message sent so that the
				// client can determine whether or not it has missed any
				// notification message.
				int messageIndexOfCurrentBroker = resource.getMessageIndex();
				List<MessageElement> brokerSpecificMessageElements = new ArrayList<MessageElement>(
						messageElements.size() + 1);

				// A message-attributes-separator element is needed in the
				// attribute list as the
				// common notification helper
				// expects such a marker with each notification message having
				// additional attributes
				// so that it can decide
				// which attribute belongs to which notification message, when
				// it is given with a
				// set of messages at once.
				// Finally, ordering is important here to ensure that attribute
				// separator has been
				...
				notify.set_any(brokerSpecificMessageElements
						.toArray(new MessageElement[brokerSpecificMessageElements
								.size()]));

				EndpointReferenceType forwardingPort = resource
						.getForwardingPort();
				try {
					GeniiCommon common = ClientUtils.createProxy(
							GeniiCommon.class, forwardingPort, callingContext);
			...
			_logger.warn("an unexpected error while forwarding notifications",
					e);
		}
	}
}
