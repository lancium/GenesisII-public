...
public class NotificationBrokerDBResource extends BasicDBResource {
	...
	private static Log _logger = LogFactory
			.getLog(NotificationBrokerDBResource.class);

	private EndpointReferenceType forwardingPort;
	private boolean mode;
	private int messageIndex;

	public NotificationBrokerDBResource(ResourceKey parentKey,
	...
	public void createNotificationBroker(String resourceId, boolean mode,
			int messageIndex, String clientId,
			EndpointReferenceType forwardingPort) throws ResourceException {

		NotificationBrokerDatabase.createNotificationBroker(resourceId, mode,
				messageIndex, clientId, forwardingPort, _connection);
	}

	@Override
	public void destroy() throws ResourceException {
		NotificationBrokerDatabase.deleteNotificationBroker(_resourceKey,
				_connection);
		super.destroy();
	...
	public void storeSubscriptionTracesInDB(List<String> subscriptionEPIs,
			EndpointReferenceType publisher, long subscriptionTerminationTime)
			throws ResourceException {

		AddressingParameters addressingParameters = new AddressingParameters(
				publisher.getReferenceParameters());
		String publisherKey = addressingParameters.getResourceKey();

		NotificationBrokerDatabase.storeSubscriptionTraces(_resourceKey,
				subscriptionEPIs, publisherKey, subscriptionTerminationTime,
				_connection);
	}

	public void initializeResourceFromDB() throws ResourceException {
		NotificationBrokerDatabase.loadNotificationBroker(_resourceKey, this,
				_connection);
	}

	public void loadMessageIndexFromDB() throws ResourceException {
		this.messageIndex = NotificationBrokerDatabase.getMessageIndexOfBroker(
				_resourceKey, _connection);
	}

	public void updateModeInDB(boolean newMode) throws ResourceException {
		NotificationBrokerDatabase.updateMode(_resourceKey, newMode,
				_connection);
	}
}
