...
public class NotificationBrokerConstructionParams extends ConstructionParameters {
	...
	@XmlElement(namespace = GenesisIIConstants.GENESISII_NS,
			name = "notification-broker-mode", nillable = false, required = false)
	private Boolean mode;
	
	@XmlElement(namespace = GenesisIIConstants.GENESISII_NS,
			name = "broker-notification-forwarding-port", nillable = false, required = false)
	@XmlJavaTypeAdapter(EPRAdapter.class)
	private EndpointReferenceType forwardingPort;
	
	@XmlAttribute(namespace = GenesisIIConstants.GENESISII_NS,
			name = "scheduled-termination-time", required = true)
