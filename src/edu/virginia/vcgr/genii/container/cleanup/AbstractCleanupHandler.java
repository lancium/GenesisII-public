...
public abstract class AbstractCleanupHandler implements CleanupHandler
{
	...
	{
		for (Map.Entry<String, Collection<CleanupReason>> entry : 
			resources.entrySet())
		{
			try
			{
				_logger.info(String.format("Enacting clean-up on resource %s:  %s",
	...
	public abstract void enactCleanup(Connection connection, 
	...
	final public void doCleanup(Connection connection, boolean enactCleanup)
	{
		...
		try
		{
			...
			} else
			{
				connection.rollback();
				for (Map.Entry<String, Collection<CleanupReason>> entry :
					context.resourcesToClean().entrySet())
				{
