...
public class BasicResourceCleanupHandler extends AbstractCleanupHandler
{
	...
	static private void cleanupSubscriptions(Connection connection, String publisherKey)
		throws Throwable
	{
		ResultSet rs = null;
		...
		try
		{
			...
			while (rs.next())
			{
				WSNSubscriptionCleanupHandler handler = 
					new WSNSubscriptionCleanupHandler();
	...
	static protected void removeRowsFromTable(Connection connection,
		Triple<String, String, String>...tableResourceTriples) throws SQLException
	{
		PreparedStatement stmt = null;
		
		try
		{
			for (Triple<String, String, String> triple : tableResourceTriples)
			{
				stmt = connection.prepareStatement(String.format(
					"DELETE FROM %s WHERE %s = ?",
					triple.first(), triple.second()));
				stmt.setString(1, triple.third());
				stmt.executeUpdate();
			}
		}
		finally
	...
	static protected void validateEPRFromBlob(
		CleanupContext context, String resourceID, Blob blob, 
		boolean mustExist)
	{
		if (blob == null && mustExist)
		{
			context.addResource(resourceID,
				"Blob for EPR was null.");
			return;
		}
		...
		throws SQLException
	{
		...
		catch (IOException ioe)
		{
			throw new SQLException(
				"IO Exception thrown during BLOB read.", ioe);
		}
		catch (Throwable cause)
		{
			context.addResource(resourceID, "Couldn't derserialize %s from blob:  %s",
				type, cause);
			return null;
		}
	}
	...
	static protected <Type> Type validateBlobToJavaSer(CleanupContext context,
		String resourceID, Blob blob, Class<Type> type, boolean mustExist) throws SQLException
	{
		if (blob == null && mustExist)
		{
			context.addResource(resourceID,
		...
		try
		{
			...
			{
				context.addResource(resourceID,
					"Tried to deserialize to %s, but got %s.",
					type, obj.getClass());
			}
		...
		CleanupContext context, String resourceID) throws SQLException
	{
		...
		{
			stmt = connection.prepareStatement(
				"SELECT propvalue FROM properties WHERE resourceid = ?");
			...
			while (rs.next())
			{
				...
				validateBlobToJavaSer(context, resourceID, blob,
					Object.class, false);
			}
		}
		finally
	...
	static protected void evaluateResourcesTable(
		...
		boolean mustExist) throws SQLException
	{
		...
		{
			stmt = connection.prepareStatement(
				"SELECT resourceid FROM resources WHERE resourceid = ?");
			stmt.setString(1, resourceID);
	...
	static protected void evaluateResources2Table(
		Connection connection, CleanupContext context, String resourceID, 
		boolean mustExist) throws SQLException
	{
		PreparedStatement stmt = null;
