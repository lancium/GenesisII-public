...
public class RExportResolverUtils {
	...
	static public QName REFERENCE_RESOLVER_EPI_QNAME = new QName(
			REXPORT_RESOLVER_QNAME, REXPORT_RESOLVER_EPI_LNAME);
	...
	/**
	 * Given information in resolver entry, return epr to replica. Currently
	 * resolves to current single replica in entry.
	 * 
	 * @param thisEntry
	 ...
	 */
	static public EndpointReferenceType resolveEPR(
			RExportResolverEntry thisEntry) {
		// do magic to choose best replica for resolution
		return thisEntry.getReplicaEPR();
	}

	/**
	 * Generates augmented EPR for primary that contains the original EPR plus
	 * an EPR for the resolver added to end of metadata element.
	 * 
	 * @param primaryEPR
	 *            : epr of primary
	 * @param resolverEPR
	 ...
	 */
	static public EndpointReferenceType createResolutionEPR(
			EndpointReferenceType primaryEPR, EndpointReferenceType resolverEPR) {
		// dissect components of primaryEPR
		org.ws.addressing.AttributedURIType origAddress = primaryEPR
				.getAddress();
		org.ws.addressing.ReferenceParametersType origRefParams = primaryEPR
		...
		MessageElement newResolverElement = new MessageElement(
		...
		// setup new metadata elements with old elements and new resolver
		// addition
		...
		EndpointReferenceType newEPR = new EndpointReferenceType(origAddress,
				origRefParams, newMetadata, origMessageElements);

		return newEPR;
	}

	/**
	 * Subscribe resolver to primary's termination event. EPI of resolver in
	 * passed as user data.
	 * 
	 * @param entry
	...
	static public void createTerminationSubscription(RExportResolverEntry entry)
		...
		try {
			...
			SubscriptionFactory factory = new DefaultSubscriptionFactory(
					resolverEPR);
			RExportResolverTerminateUserData userData = new RExportResolverTerminateUserData(
					URI.create(entry.getCommonEPI().toString()));
			factory.subscribe(entry.getPrimaryEPR(),
					GenesisIIBaseTopics.RESOURCE_TERMINATION_TOPIC
							.asConcreteQueryExpression(), null, userData);
	 ...
	 * @param exportDirEPI
	 *            : EPI of directory containing new export entry Used to acquire
	 *            epr to containing dir's replica
	 * @param replicaName
	 *            : name of file/dir (last part of primaryLocalPath) for use in
	 *            dir entry
	 * 
	 * @return resolvedEPR: oldEPR augmented with epr of newly created resolver
	 *         on containerName
	 * 
	 */
	static public EndpointReferenceType setupRExport(
			EndpointReferenceType oldEPR, String replicaType,
			String primaryLocalPath, String exportDirEPI, String replicaName)
		...
		try {
			resolverFactoryProxy = ClientUtils.createProxy(
					...
					EPRUtils.makeEPR(Container
							.getServiceURL("RExportResolverFactoryPortType")));
		} catch (Exception e) {
			throw new CreationException(
					"Could not create proxy to resolver factory", e);
		}

		// acquire EPR to resolver for exportDir containing this new export
		// entry
		...
		// non-root resolvers do not create replica resource with resolver
		// creation
		resolverCreationProperties[1] = new MessageElement(
				RExportResolverServiceImpl.REXPORT_RESOLVER_TYPE, "non-root");
		...
		try {
			resolverCreationResponse = resolverFactoryProxy
					.createResolver(new CreateResolverRequestType(oldEPR,
		...
		// get resolution epr (oldEPR augmented with resolverEPR) from creation
		// response
		EndpointReferenceType resolvedEPR = resolverCreationResponse
				.getResolution_EPR();

		// get resolver EPR from creation response
		EndpointReferenceType resolverEPR = resolverCreationResponse
				.getResolver_EPR();
		...
		try {
			...
			// setup proxy to export resolver (to container where resolver was
			// created)
			...
			replicaEPR = resolverService.createReplica(
					new CreateReplicaRequest(resolvedEPR, primaryDataStream,
							replicaType, replicaName)).getReplica_EPR();
		} catch (Exception e) {
			throw new ResourceException(
					"Unable to connect to rexport resolver to create replica.",
					e);
		} finally {
		...
		try {
			...
			recordReplicaUnderDir(dirResolverEPR, replicaEPR, replicaName,
					replicaType);
		} catch (Exception e) {
			throw new ResourceException(
					"Unable to store replica info in rexportdir db resource.",
					e);
		}
		return resolvedEPR;
	}
	...
	/**
	 * Packages data from local harddrive of exported entry into a stream and
	 * returns EPR to it
	 * 
	 * @param primaryLocalPath
	 ...
	 */
	static public EndpointReferenceType getExportFileData(
			String primaryLocalPath, EndpointReferenceType exportFileServiceEPR)
			throws ResourceException {
		if (exportFileServiceEPR == null)
			exportFileServiceEPR = EPRUtils.makeEPR(Container
		...
		try {
			...
			primaryDataEPR = (exportedFileService.openStream(primaryLocalPath)
					.getEndpoint());
	...
	/**
	 * Determines if constructionParams contain "non-default" resolver type
	 * param
	 * 
	...
	static public boolean isRootResolver(MessageElement[] constructionParams)
		...
		for (MessageElement elem : constructionParams) {
			...
			if (elemName
					.equals(RExportResolverServiceImpl.REXPORT_RESOLVER_TYPE)) {
	 ...
	 * 
	 * looks up resolver on replicationURL associated with exportDirEPI connects
	 * to resolver and resolves to get replicaDirEPR connects to replica and
	 * adds new replica entry for replicaEPR with replicaname
	 * 
	 ...
	 *            :
	 */
	static protected void recordReplicaUnderDir(
			EndpointReferenceType dirResolverEPR,
			EndpointReferenceType replicaEPR, String replicaName,
			String replicaType) throws ResourceException, RemoteException {
		...
		// acquire EPR from resolver of RExportDir containing created replica
		// for new export entry
		...
		try {
			...
			throw new ResourceException(
					"Unable to connect/update/resolve rexportdir's resolver.",
					e);
		}
	...
	/**
	 * Returns EPR of RExportResolver associated with specified resource from
	 * resolver-resource mapping table Location sensitive - either (resolver)
	 * location passed in or current is used as default
	 * 
	 * @param resourcesEPI
	 *            : EPI of resource whose resolver is to be looked up
	 * @param resolverURL
	 *            : URL of machine where resolver resides if null, use current
	 *            container
	 * 
	 * @return resolverEPR: EPR of RExportResolver for specified resource
	 * 
	 */
	static public EndpointReferenceType lookupResolverByEPI(
			String resourcesEPI, String resolverContainerURL)
			throws ResourceException {
		...
		try {
			...
			throw new ResourceException("Could not create proxy to resolver.",
					e);
		}
	...
	/**
	 * Destroy resolver associated with export entry being destroyed also
	 * destroy resolver-resource mapping
	 * 
	 * called by ExportedDirDB on export destroy (quit)
	 * 
	 * location sensitive (resolver) location passed in else assumed same as
	 * current container
	 */
	static public void destroyResolverByEPI(String resourcesEPI,
			String resolverURL) throws ResourceException {
		EndpointReferenceType resolverEPR = lookupResolverByEPI(resourcesEPI,
				resolverURL);
	...
	/**
	 * Destroy resolver-resource mapping in table lookup entry by EPI from
	 * resolverEPR
	 * 
	 */
	static protected void destroyResolverResourceInfo(String resourcesEPI)
		...
		try {
			...
			// at current (primary) container where new export entry was created
			EndpointReferenceType rexportResolverServiceEPR = EPRUtils
					.makeEPR(Container.getServiceURL("RExportResolverPortType"));

			ResourceKey rKey = ResourceManager
					.getTargetResource(rexportResolverServiceEPR);
	...
	static public void destroyResolverByEPR(EndpointReferenceType resourceEPR)
		...
		if (!primaryWSName.isValidWSName())
			throw new ResourceException(
					"Could not extract EPI from resource EPR.");
	 ...
	 * 
	 * location independent potentially pass in name of resolver if more than
	 * one resolver possible
	 */
	...
	static public EndpointReferenceType extractResolverServiceEPR(
		...
		for (MessageElement elem : factoryPassedResolverCreationParams) {
			QName elemName = elem.getQName();
			if (elemName
					.equals(RExportResolverServiceImpl.REXPORT_RESOLVER_SERVICE_EPR_NAME)) {
				try {
					resolverServiceEPR = (EndpointReferenceType) elem
		...
		throw new ResourceException("Cannot create resolver with NULL EPR "
				+ "for resolver service in construction params.");
	}

	/*
	 * create creation params for creating a new RExportResolver adds new EPI
	 * and factoryPassedResolverCreationParams with to passed in params
	 * 
	...
	static public MessageElement[] createResolverCreationParams(
		...
		resolverCreationProperties[0].setValue(WSName.generateNewEPI()
				.toString());
		resolverCreationProperties[1] = new MessageElement(
	...
	static public void updateResolverResourceInfo(
			EndpointReferenceType resolverEPR, EndpointReferenceType primaryEPR)
			throws InvalidWSNameFaultType, ResourceUnknownFaultType,
			ResourceException {
		// get commonEPI
		String primaryEPI = null;
		...
		resource.updateResolverResourceInfo(primaryEPI, resolverEPI,
				resolverEPR, false);
	}
