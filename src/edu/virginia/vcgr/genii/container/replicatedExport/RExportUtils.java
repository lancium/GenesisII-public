...
public class RExportUtils {
	...
	/**
	 * Creates EPR for replica with EPI specified by primary. Creates RExport if
	 * file or RExportDir if dir
	 * 
	 * @param primaryEPR
	...
	static public EndpointReferenceType createReplica(
			EndpointReferenceType primaryEPR, String commonEPI,
			EndpointReferenceType resolverEPR, String primaryLocalPath,
			String replicaName, String entryType,
			EndpointReferenceType dataStreamEPR) throws RemoteException,
		...
		EndpointReferenceType replicaServiceEPR = EPRUtils.makeEPR(Container
				.getServiceURL(RExportService));

		// EPR for new replica
		EndpointReferenceType replicaEPR = null;

		/* create instance of correct (dir or file) replication service */
		GeniiCommon replicaCommon = ClientUtils.createProxy(GeniiCommon.class,
				replicaServiceEPR);
		VcgrCreateResponse replicaResp = replicaCommon
				.vcgrCreate(new VcgrCreate(replicaCreationProperties));
		...
		createReplicaSubscriptions(primaryEPR, resolverEPR, replicaEPR,
				primaryLocalPath, replicaName, entryType);
	...
	static protected void updateReplicaData(EndpointReferenceType replicaEPR,
			String primaryLocalPath) throws ResourceException {
		...
		try {
			...
			throw new ResourceException(
					"Exception on streaming data to rexport replica.", e);
		} finally {
	...
	static public void unpackDataStream(EndpointReferenceType replicaEPR,
			EndpointReferenceType primaryDataStream) throws ResourceException {

		InputStream primaryDataStreamIN = null;
		OutputStream replicaDataOUT = null;

		try {
			primaryDataStreamIN = ByteIOStreamFactory
					.createInputStream(primaryDataStream);
	...
	/**
	 * Subscribe replica to be notified of resolver termination and (if file
	 * replica) of RandomByteIO operations performed on primary
	 */
	static protected void createReplicaSubscriptions(
			EndpointReferenceType primaryEPR,
			EndpointReferenceType resolverEPR,
			EndpointReferenceType replicaEPR, String primaryLocalPath,
			String replicaName, String replicaType) throws ResourceException {
		PrimaryExportLocalpathUserData userData = new PrimaryExportLocalpathUserData(
				primaryLocalPath);
		...
		try {
			...
			_logger.error(
					"Could not create replica subscription to rexport resolver termination.",
					e1);
			throw new ResourceException(
					"Could not create replica subscription to rexport resolver termination.",
					e1);
		...
		// subscribe resolver of file replica to RandomeByteIO notifications
		// from export
		if (replicaType.equals(RExportResolverUtils._FILE_TYPE)) {
			...
			try {
				...
				throw new ResourceException(
						"Could not subscribe resolver to export randomByteIO ops.",
						e);
			}
		}
	}
