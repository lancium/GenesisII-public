...
public class ResourceManager {
	...
	static public ResourceKey getTargetResource(String serviceName,
			String resourceKey) throws ResourceException,
			ResourceUnknownFaultType {
		try {
			WorkingContext ctxt = WorkingContext.getCurrentWorkingContext();
			ResourceKey rKey = new ResourceKey(serviceName,
					new AddressingParameters(resourceKey, null, null));
		...
		if (ctxt != null) {
			...
			if (_logger.isTraceEnabled()) {
				if (key != null)
					...
					_logger.trace("no key found in context for field: "
							+ WorkingContext.CURRENT_RESOURCE_KEY);
			}
		}
		if (key == null) {
			...
			if (epr == null)
				throw new ResourceException(
						"Couldn't locate target EPR in current working context for key="
								+ key);

			String serviceName = (String) ctxt
					.getProperty(WorkingContext.TARGETED_SERVICE_NAME);
			if (serviceName == null)
				throw new ResourceException(
						"Couldn't locate target service name in current working context for key="
								+ key);

			if (_logger.isTraceEnabled())
				_logger.trace("about to make resource key for svc="
						+ serviceName + " epr=" + epr.getAddress());
			key = new ResourceKey(serviceName, new AddressingParameters(
	...
	static public ResourceKey getServiceResource(String serviceName)
		...
		WorkingContext ctxt = WorkingContext.getCurrentWorkingContext();
		ResourceKey rKey = new ResourceKey(serviceName,
				new AddressingParameters(null));
		ctxt.setProperty(new GUID().toString(), rKey);
		return rKey;
	}

	static public ResourceKey createServiceResource(String serviceName,
			GenesisHashMap constructionParameters) throws ResourceException {
		WorkingContext ctxt = WorkingContext.getCurrentWorkingContext();
	...
	static public EndpointReferenceType createEPR(ResourceKey resource,
		...
		if (resource != null) {
			AddressingParameters addrParams = resource
					.getAddressingParameters();
			if (addrParams != null)
				refParams = addrParams.toReferenceParameters();
		}

		return new EndpointReferenceType(address, refParams, createMetadata(
				implementedPortTypes, resource, masterType), null);
	}

	/**
	 * Added 2014-01-09 by ASG to allow us to selectively not put X.509
	 * SecurityTokens in EPR's. It would be great if we could use
	 * "if ((this instanceof GeniiNoOutCalls))" instead of the kludge with class
	 * names, but we don't have a reference to the class For now we must
	 * explicitly list every type we want to avoid. The list should come out of
	 * a configuration file.
	 */
	static private void MetaDataSecurityToken(
			ArrayList<MessageElement> metaDataAny, IResource resource)
			throws ResourceException {
		String serviceName = ((ResourceKey) resource.getParentResourceKey())
				.getServiceName();
		// ASG 2014-01-11
		// This is where the list of classes that do not get their own X.509
		// goes
		...
		if (p == null) {
			...
			InputStream in = HelpLinkConfiguration.class.getClassLoader()
					.getResourceAsStream("config/EPRConstruction.properties");
			try {
				p.load(in);
			} catch (IOException e) {
		...
		// Then get the list of classes that should NOT have an X.509 in their
		// EPR
		String r = p.getProperty(NO_X509_CLASS_LIST);
		if (r == null) {
			throw new RuntimeException(
					"Could not find config/EPRConstructon.properties "
							+ NO_X509_CLASS_LIST);
		}
		// ASG: 2014-01-21 Now check the serviceName against the list of classes
		// in which we do put
		...
		if (!matches) { // this EPR should get an X.509 in it
			try {
				...
				X509Certificate[] certChain = (X509Certificate[]) resource
						.getProperty(IResource.CERTIFICATE_CHAIN_PROPERTY_NAME);

				if (certChain != null) {
					// ASG: This is where the X.509 embedded security token gets
					// added
					MessageElement wseTokenRef = WSSecurityUtils
	...
	static private void addSecureAddressingElements(
		...
		try {
			...
			IAuthZProvider handler = AuthZProviders
			...
			boolean includeServerTls = Container.getContainerConfiguration()
					.isSSL();
			...
			if (!minMsgSec.isNone() || includeServerTls) {
				...
				if (minMsgSec.isSign()) {
					...
					MessageElement[] samlSubEls = { new MessageElement(
							ClaimsType.getTypeDesc().getXmlType()
									.getNamespaceURI(), "Claims", claims) };
					...
					MessageElement signedSupportingTokensMel = new MessageElement(
							new QName(NestedPolicyType.getTypeDesc()
				...
				org.apache.axis.types.URI usernameTokenUri = new org.apache.axis.types.URI(
				...
				MessageElement[] appliesToAny = { new MessageElement(URI
						.getTypeDesc().getXmlType(), appliesToUri) };
				...
				MessageElement policyAttachmentMel = new MessageElement(
						PolicyAttachment.getTypeDesc().getXmlType(),
						policyAttachment);
		...
		try {
			...
			IAuthZProvider handler = AuthZProviders
					.getProvider(((ResourceKey) resource.getParentResourceKey())
							.getServiceName());
			if (handler != null) {
				MessageLevelSecurityRequirements minMsgSec = handler
						.getMinIncomingMsgLevelSecurity(resource);

				RequiredMessageSecurityTypeMin minAttr = RequiredMessageSecurityTypeMin
						.fromString(minMsgSec.toString());
				RequiredMessageSecurityType min = new RequiredMessageSecurityType(
						handler.getClass().getName(), minAttr);
	...
	static public MetadataType createMetadata(PortType[] portTypes,
			ResourceKey resourceKey, String masterType)
			throws ResourceException {
		if (portTypes == null)
		...
		if (resourceKey != null) {
			...
			if (portTypes.length == 0) {
				any.add(new MessageElement(new QName(
						WSAddressingConstants.WSA_NS, "PortType"), new QName(
		...
		any.add(new MessageElement(
				ContainerConstants.CONTAINER_ID_METADATA_ELEMENT, Container
	...
	/**
	 * returns a textual name for the resource specified. this is very limited
	 * in capabilities, and currently only returns the key name for the
	 * resource. there seems to be no general and simple way to map a resource
	 * to its RNS path.
	 */
