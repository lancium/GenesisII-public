...
/*
 * This attribute handler has been added to make Kerberos IDP specific attributes retrievable within STS replication feature. Note that the
 * IDP name is the sensitive attribute for a Kerberos IDP resource, which is taken care of in common-STS-attribute-handler class, not the
 * realm and KDB attributes. Hence, although this handler is added for only replication purpose. We added no additional security checking that
 * might restrict access to its methods beyond what is already employed by GenesisIIBase class's default access checking code.
 */
