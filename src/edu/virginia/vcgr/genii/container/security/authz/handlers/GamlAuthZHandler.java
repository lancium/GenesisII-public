		...
		if (callingContext != null) {
			...
			for (GamlCredential cred : transientCredentials._credentials) {
				...
				if (cred instanceof Identity) {
					...
					defaultOwners.add(((IdentityAttribute) ((SignedAssertion) cred).getAttribute()).getIdentity());
				}
			}
		}
...
*/
	...
	protected boolean checkAclAccess(
			Identity identity,
			Method operation, 
		        ...
		        try {
		        	...
		        	for (Identity trustedIdentity : trustList) {
		        		if (trustedIdentity instanceof X509Identity) {
		        			X509Certificate trustedCert = 
		        				((X509Identity) trustedIdentity).getAssertingIdentityCertChain()[0];
					...
					try {
						trustManager.checkClientTrusted(
								identityCertChain, 
								identityCertChain[0].getPublicKey().getAlgorithm());
	...
	public boolean checkAccess(
		...
		try {
			...
			// get ACL
			GamlAcl acl = (GamlAcl) resource.getProperty(GAML_ACL_PROPERTY_NAME);
			...
			for (GamlCredential cred : callerCredentials) {
				...
				if (cred instanceof Identity) {
					...
					if ((targetCertChain != null) && 
						...
						signedAssertion = ((DelegatedAssertion) signedAssertion).unwrap();
					}

					// verify that the request message signer is the same as the
					// authorized user of the assertion
					if (!signedAssertion.getAuthorizedIdentity()[0].equals(callerCert)) {
						throw new AuthZSecurityException("GAML credential \"" + signedAssertion + "\" does not match the incoming message sender");
					}

					// if its an identity assertion, check it against our ACLs
					if (signedAssertion.getAttribute() instanceof IdentityAttribute) {
						IdentityAttribute identityAttr = (IdentityAttribute) signedAssertion.getAttribute();

						if (checkAclAccess(identityAttr.getIdentity(), operation, acl)) {
							allowed = true;
						}
					}
				}
	...
	public MessageLevelSecurity getMinIncomingMsgLevelSecurity(
		...
		try {
			// get ACL
			GamlAcl acl = (GamlAcl) resource
					.getProperty(GAML_ACL_PROPERTY_NAME);
