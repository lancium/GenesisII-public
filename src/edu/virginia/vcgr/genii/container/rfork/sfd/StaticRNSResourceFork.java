 ...
 * entries.  In fact, they can change any time the resource is constructed,
 * but that behavior is incidental and should not be relied upon.
 * 
 * @author mmm2a
 */
public abstract class StaticRNSResourceFork extends ReadOnlyRNSResourceFork
{
	...
	protected StaticRNSResourceFork(ResourceForkService service, 
		String forkPath)
	{
	...
	/**
	 * This is a convenience method which allows a sub-class to
	 * easily add an entry which uses the default resource fork
	 * information structure.
	 * 
	 * @param entryName The new entry's name.
	 * @param forkClass The resource fork's class.
	 */
	protected void addDefaultEntry(String entryName,
		Class<? extends ResourceFork> forkClass)
	{
		_entries.put(entryName,
			new DefaultResourceForkInformation(forkClass,
				getForkPath() + "/" + entryName));
	}
	
	/**
	 * This abstract method is called automatically to add the entries into
	 * this static resource fork.
	 * 
	 * @param entries The map of entries the resource fork contains. 
	 */
	protected abstract void addEntries(
		Map<String, ResourceForkInformation> entries);
