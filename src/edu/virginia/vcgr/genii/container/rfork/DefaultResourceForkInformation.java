...
public class DefaultResourceForkInformation implements ResourceForkInformation {
	...
	public DefaultResourceForkInformation(
			Class<? extends ResourceFork> resourceForkClass, String forkPath) {
		...
		try {
			...
			synchronized (_resourceForkClass) {
				if (_constructor == null) {
					_constructor = _resourceForkClass.getConstructor(
			...
			throw new ResourceException(
					"Unable to find appropriate constructor for resource fork.",
					nsme);
		} catch (IllegalArgumentException e) {
			throw new ResourceException("Unable to instantiate resource fork.",
					e);
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		_constructor = null;
		_resourceForkClass = (Class<? extends ResourceFork>) Class.forName(in
				.readUTF());
