...
public class ReadRequest extends DefaultRequest
{
		...
		throws UnknownHostException, IOException
	{
		...
		w_offset = Commons.formPrefix(request, Constants.FILE_OPN_BYTE, Constants.FILE_READ_CMD_BYTE, path, nonce);
	...
	public static ReadResponse recv(Socket clientSocket, int resp_size) throws IOException
	{
		...
		if (read_buf_size > resp_size) {
			...
			return new ReadResponse(-1, "Server is sending a larger response " + "than it should");
		}
		...
		while (true) {
			if (readOffset == read_buf_size) {
				if (successResponse) {
					...
					return new ReadResponse(errorCode, new String(read_buff));
				}
			}
			...
			if (readOffset + bytes_read > read_buf_size) {
				return new ReadResponse(-1, Constants.LARGE_RESPONSE_ERROR);
			}
