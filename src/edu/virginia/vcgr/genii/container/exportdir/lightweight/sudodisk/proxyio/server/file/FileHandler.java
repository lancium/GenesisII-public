...
public class FileHandler
{
	...
	public static byte delete(String path, Socket socket)
	{
		...
		if (!file.exists()) {
			DefaultResponse.send(socket, ErrorCode.FNF_CODE, ErrorCode.getErrorMsgFromErrorCode(ErrorCode.FNF_CODE));
	...
	public static int write(String path, byte[] buf, long offset, Socket socket)
	{
		...
		try {
			...
			DefaultResponse.send(socket, ErrorCode.FNF_CODE, e.toString());
			return ErrorCode.FNF_CODE;
		}
		...
		try {
			...
			DefaultResponse.send(socket, ErrorCode.IO_ERROR_CODE, e.toString());
	...
	public static int truncAppend(String path, byte[] buf, long offset, Socket socket)
	{
		if (path == null) {
			DefaultResponse.send(socket, ErrorCode.BAD_PATH_CODE, ErrorCode.getErrorMsgFromErrorCode(ErrorCode.BAD_PATH_CODE));
			return ErrorCode.BAD_PATH_CODE;
	...
	public static int stat(String path, Socket socket)
	{
		...
		StatAttributes attributes =
			new StatAttributes(file.getFileName().toString(), attr.lastAccessTime(), attr.lastModifiedTime(), attr.size(), type);
	...
	public static int read(String path, long offset, long numBytes, Socket socket)
	{
		if (path == null) {
			DefaultResponse.send(socket, ErrorCode.BAD_PATH_CODE, ErrorCode.getErrorMsgFromErrorCode(ErrorCode.BAD_PATH_CODE));
			return ErrorCode.BAD_PATH_CODE;
		...
		try {
			...
			// Couldn't seek/read!
			DefaultResponse.send(socket, ErrorCode.IO_ERROR_CODE, ioe.toString());
			return ErrorCode.IO_ERROR_CODE;
		} finally {
			try {
				raf.close();
	...
	public static int createNewFile(String path, Socket socket)
	{
		...
		if (f.exists()) {
			DefaultResponse.send(socket, ErrorCode.FILE_ALREADY_EXISTS_CODE,
				ErrorCode.getErrorMsgFromErrorCode(ErrorCode.FILE_ALREADY_EXISTS_CODE));
