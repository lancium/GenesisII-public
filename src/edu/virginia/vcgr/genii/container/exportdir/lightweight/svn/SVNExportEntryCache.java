...
class SVNExportEntryCache 
	extends LRUCache<SVNPathIdentifier, SVNExportEntry>
{
	...
	public SVNExportEntry lookup(Object key) throws SVNException
	{
		...
		if (entry == null)
		{
			entry = new SVNExportEntry(
				id.repository(), id.repositoryURL(),
				id.revision(),
				id.repository().info(id.relativePath(), id.revision()),
				id.relativePath(), this);
			put(id, entry);
		}
