...
/**
 * This class contains information on fields required to communicate with an already started
 * fileserver process for a given user.
 * 
 * It contains the nonce: that goes in every request the port: where the fileserver is running the
 * process object itself which can be used to check if the fileserver is actually running (or has it
 * been killed)
 * 
 */
