...
public class DirHandler
{
	...
	public static int mkdir(String path, Socket socket)
	{
		...
		if (newDir.exists()) {
			DefaultResponse.send(socket, ErrorCode.DIR_ALREADY_EXISTS_CODE,
				ErrorCode.getErrorMsgFromErrorCode(ErrorCode.DIR_ALREADY_EXISTS_CODE));
		...
		DefaultResponse.send(socket, ErrorCode.MKDIR_FAIL_CODE, ErrorCode.getErrorMsgFromErrorCode(ErrorCode.MKDIR_FAIL_CODE));
	...
	public static int listlong(String path, Socket socket)
	{
		...
		ArrayList<StatAttributes> stat_list = new ArrayList<StatAttributes>();

		for (File file : files) {
			try {
				StatAttributes sa = getFileAttributes(file.getAbsolutePath());
				if (sa != null) {
	...
	private static StatAttributes getFileAttributes(String path) throws IOException
	{
	...
	public static int isDir(String path, Socket socket)
	{
		...
		if (f.isDirectory()) {
			...
			DefaultResponse.send(socket, ErrorCode.NOT_DIR_CODE, null);
