...
public class LightWeightExportUtils {
	...
	static VExportRoot getRoot(ResourceKey myKey) throws IOException {
		...
		if (rKey != null) {
			...
			String rootDirString = (String) resource
					.getProperty(LightWeightExportConstants.ROOT_DIRECTORY_PROPERTY_NAME);
			if (rootDirString != null) {
				...
				/*
				 * if (rootDirString.matches("^.+:.*$")) { // Handle SVN Long
				 * revision = (Long)resource.getProperty(
				 ...
				 * revision); if (revision == null) return vroot; } else
				 */
				{
	...
	static public VExportDir getDirectory(String forkPath, ResourceKey rKey)
			throws IOException {
		if (rKey == null)
	...
	static VExportFile getFile(String forkPath) throws IOException {
		VExportEntry ret = getEntry(forkPath);
		if (!ret.isFile())
			throw new IOException(String.format("Entry \"%s\" is not a file.",
					forkPath));

		return (VExportFile) ret;
	}

	static public VExportFile getFile(String forkPath, ResourceKey rKey)
			throws IOException {
		if (rKey == null)
