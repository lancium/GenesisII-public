...
// , GeniiNoOutCalls
{
	static private Log _logger = LogFactory
			.getLog(ExportedDirServiceImpl.class);
	...
	protected ExportedDirServiceImpl(String serviceName) throws RemoteException {
		...
		addImplementedPortType(WellKnownPortTypes
				.EXPORTED_DIR_SERVICE_PORT_TYPE());
	...
	protected ResourceKey createResource(GenesisHashMap constructionParameters)
		...
		initInfo = ExportedDirUtils
				.extractCreationProperties(constructionParameters);

		constructionParameters.put(
				IExportedDirResource.PATH_CONSTRUCTION_PARAM,
				initInfo.getPath());
		constructionParameters.put(
				IExportedDirResource.PARENT_IDS_CONSTRUCTION_PARAM,
				initInfo.getParentIds());
		constructionParameters.put(IExportedDirResource.REPLICATION_INDICATOR,
				initInfo.getReplicationState());
		constructionParameters.put(IExportedDirResource.LAST_MODIFIED_TIME,
				initInfo.getLastModifiedTime());

		return super.createResource(constructionParameters);
	}

	/*
	 * Looks like this is dead code -- mmm2a protected void fillIn(ResourceKey
	 * rKey, EndpointReferenceType newEPR, ConstructionParameters cParams,
	 * GenesisHashMap creationParameters, Collection<MessageElement>
	 * resolverCreationParams) throws ResourceException, BaseFaultType,
	 * RemoteException { super.postCreate(rKey, newEPR, cParams,
	 ...
	 * resource.setCreateTime(c); resource.setModTime(c);
			...
			try {
				if (!(new File(fullPath).createNewFile())) {
					throw FaultManipulator
							.fillInFault(new RNSEntryExistsFaultType(null,
									null, null, null, null, null, filename));
				}
			} catch (IOException ioe) {
				throw new RemoteException(String.format(
						"Unable to create new file on disk (%s).", fullPath),
						ioe);
			}

			try {
				WorkingContext
						.temporarilyAssumeNewIdentity(EPRUtils.makeEPR(
								Container.getServiceURL("ExportedFilePortType"),
								false));

				entryReference = new ExportedFileServiceImpl().vcgrCreate(
						new VcgrCreate(ExportedFileUtils
								.createCreationProperties(fullPath, parentIds,
										_resource.getReplicationState())))
						.getEndpoint();
				...
				_logger.warn(
						"Cleaning up state from a failed export file create.",
						t);
				...
				if (entryReference != null) {
					try {
						...
						_logger.error("Unable to clean up bogus file object.",
								tt);
					}
				}
				...
				if (t instanceof RemoteException)
					...
					throw new RemoteException(
							""
									+ "Exception occurred while creating exported file.",
							t);
			} finally {
	...
	protected RNSEntryResponseType add(RNSEntryType addRequest)
		...
		try {
			...
			newRef = vcgrCreate(
					new VcgrCreate(ExportedDirUtils.createCreationProperties(
							null, fullPath, null, null, null, parentIds,
							isReplicated))).getEndpoint();

			String newEntryId = (new GUID()).toString();
			ExportedDirEntry newEntry = new ExportedDirEntry(_resource.getId(),
					name, newRef, newEntryId, ExportedDirEntry._DIR_TYPE, attrs);
		...
		IExportedDirResource newResource = (IExportedDirResource) newRefKey
				.dereference();
	...
	public LookupResponseType lookup(String[] lookupRequest)
		...
		for (ExportedDirEntry exportDirEntry : entries) {
			RNSEntryResponseType entry = new RNSEntryResponseType(
					...
					RNSUtilities.createMetadata(
							exportDirEntry.getEntryReference(),
							exportDirEntry.getAttributes()), null,
		...
		try {
			...
			return RNSContainerUtilities.translate(entryCollection,
