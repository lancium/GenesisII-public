...
{
	static private Log _logger = LogFactory.getLog(InformationContainerService.class);
	...
	protected void startService() throws Throwable
	{
		...
		String numThreads = _configurationProperties.getProperty(NUM_THREADS_PROPERTY);
		if (numThreads == null)
			throw new RuntimeException("Information Manager not configured.");

		_logger.info(String.format("Starting %s with %s threads in the pool.", SERVICE_NAME, numThreads));
	...
	{
		if (persister == null)
			throw new IllegalArgumentException("The \"persister\" parameter cannot be null.");

		if (resolver == null)
			throw new IllegalArgumentException("The \"resolver\" parameter cannot be null.");
		...
		return new InformationPortal<InfoType>(_executor, persister, resolver, defaultTimeout, defaultCacheWindow);
	}
}