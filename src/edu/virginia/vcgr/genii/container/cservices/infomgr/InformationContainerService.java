...
/**
 * This is the main container service that is started to hand out information
 * portals. Technically speaking, this probably didn't have to be a "service"
 * per se, but it works and give a convenient factory mechanism for creating the
 * executor that we are going to use.
 * 
 * @author mmm2a
 */
public class InformationContainerService extends AbstractContainerService {
	static private Log _logger = LogFactory
			.getLog(InformationContainerService.class);

	static private final String NUM_THREADS_PROPERTY = "edu.virginia.vcgr.genii.cservices.infomgr.num-threads";
	 ...
	 *            A properties object representing configuration properties
	 *            given when the container service was configured.
	 */
	...
	protected void startService() throws Throwable {
		...
		String numThreads = _configurationProperties
				.getProperty(NUM_THREADS_PROPERTY);
		if (numThreads == null)
			throw new RuntimeException("Information Manager not configured.");
	...
	/**
	 * Portals are created by parts of the Genesis II system that need to be
	 * able to access information stored and cached about things. In general, a
	 * single portal should be created once for each type of information cache
	 * (for example, one is created by the Queue class and shared amongst all
	 * queues in the system for all time).
	 * 
	 * @param <InfoType>
	 *            The data type of the type of information that is stored. This
	 *            need not correspond to a SOAP method data type but can be some
	 *            other type that SOAP messages are translated into.
	 * @param persister
	 *            A persister instance that is used to store the data that is
	 *            obtained for caching purposes.
	 * @param resolver
	 *            A resolver instance that is used to obtain the information
	 *            when the cache is not valid.
	 * @param timeout
	 *            The amount of time to wait before a request for data is timed
	 *            out.
	 * @param timeoutUnits
	 *            The units for the timeout value.
	 * @param cacheWindow
	 *            The amount of time a piece of data is valid for before it is
	 *            considered stale.
	...
	public <InfoType> InformationPortal<InfoType> createNewPortal(
			...
			InformationResolver<InfoType> resolver, Duration defaultTimeout,
			Duration defaultCacheWindow) {
		if (persister == null)
