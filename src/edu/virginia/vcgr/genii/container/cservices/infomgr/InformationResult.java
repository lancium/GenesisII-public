...
/**
 * A bundle class that wraps together information about an endpoint, Any
 * exceptions that occurred the last time the endpoint was communicated with,
 * and the timestamp of the last update attempt.
 * 
 * @author mmm2a
 * 
 * @param <InfoType>
...
public class InformationResult<InfoType> {
	 ...
	 * @param lastUpdated
	 *            The time that the last update was attempted.
	 * @param exception
	 *            Any exceptions that occurred (or null) trying to update the
	 *            information.
	 */
	...
	public String toString() {
		...
		pw.format(
				"%sLast Updated:  %tc\n",
				(_exception == null) ? "" : String.format("[(%s) %s] ",
	...
	/**
	 * Construct a new result with the given parameters. This is equivalent to
	 * constructing a result with a null exception parameter.
	 * 
	 * @param information
	 *            The information (if available) for the result.
	 * @param lastUpdated
	...
	final public boolean timedOut() {
		return ((_exception != null) && (_exception instanceof TimeoutException));
	}
	...
	/**
	 * Retrieve any information stored with this result. This information is not
	 * guaranteed to be up to date unless "wasResponsive" returns true. If an
	 * attempt to update results fails, and old information exists, then the old
	...
	/**
	 * The exception (if any) that occurred the last time this information was
	 * updated (or attempted).
	 * 
