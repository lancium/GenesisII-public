...
public class PersistentOutcallContainerService extends AbstractContainerService {
	...
	static private Log _logger = LogFactory
			.getLog(PersistentOutcallContainerService.class);
	...
	final public boolean schedule(OutcallActor actor,
			AttemptScheduler scheduler, EndpointReferenceType target,
			ICallingContext callingContext, GeniiAttachment attachment) {
	...
	final protected void loadService() throws Throwable {
		...
		try {
			...
			if (_logger.isDebugEnabled())
				_logger.debug(String.format("%s loaded %d entries",
						SERVICE_NAME, _entries.size()));
		} finally {
			getConnectionPool().release(connection);
		}
	}
	...
	final protected void startService() throws Throwable {
		...
		Thread th = new Thread(new OutcallWorker(), "Persisent Outcall Worker");
		th.setDaemon(true);
	...
	final private void reAdd(Connection connection, PersistentOutcallEntry entry) {
		try {
			...
			if (_logger.isDebugEnabled())
				_logger.debug("entry " + entry.entryID() + " now at "
						+ entry.numAttempts() + " attempts.");
			Calendar nextAttempt = entry.scheduler().nextAttempt(
	...
	final private void doOutcall(PersistentOutcallEntry entry) {
		...
		try {
			connection = getConnectionPool().acquire(false);
			CommunicationInformation commInfo = PersistentOutcallDatabase
					.getCommunicationInformation(connection, entry);
			connection.commit();
			if (commInfo.outcallActor.enactOutcall(commInfo.callingContext,
					commInfo.targetEPR, commInfo.attachment)) {
				if (_logger.isDebugEnabled())
			...
			_logger.warn(
					"Persistent outcall service tried to make outcall, but "
							+ "got an exception -- putting it back in the list.",
					cause);
			try {
				reAdd(connection, entry);
			} catch (Throwable inner_cause) {
				_logger.error(
						"Unexpected exception--unable to re-add outcall to list.",
						inner_cause);
			}
			try {
				connection.commit();
				...
				_logger.error(
						"Unable to commit connection for persistent outcall db.",
						sqe);
			}
		} finally {
			getConnectionPool().release(connection);
				...
				try {
					...
					synchronized (_entries) {
						...
						if (next == null)
							...
							_entries.wait(next.getTimeInMillis()
									- now.getTimeInMillis());
						else
					...
					_logger.warn(
							"Persistent outcall service saw an exception.",
							cause);
				}
			}
		}
	...
	static public boolean schedulePersistentOutcall(OutcallActor actor,
			AttemptScheduler scheduler, EndpointReferenceType target,
