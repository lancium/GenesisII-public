...
public class PersistentOutcallContainerService extends AbstractContainerService
{
	...
	static private Log _logger = LogFactory.getLog(PersistentOutcallContainerService.class);
	...
	final public boolean schedule(OutcallActor actor, AttemptScheduler scheduler, EndpointReferenceType target,
		ICallingContext callingContext, GeniiAttachment attachment)
	{
		...
		try {
			...
			_logger.warn("Unable to add persistent outcall to container service.", e);
			return false;
		} finally {
	...
	final protected void loadService() throws Throwable
	{
		...
		try {
			...
			if (_logger.isDebugEnabled())
				_logger.debug(String.format("%s loaded %d entries", SERVICE_NAME, _entries.size()));
		} finally {
			getConnectionPool().release(connection);
		}
	}
	...
	final private void reAdd(Connection connection, PersistentOutcallEntry entry)
	{
		try {
			...
			_logger.warn("Unable to re-add/remove persistent outcall entry into/from database.", cause);
		}
	}
	...
	final private void doOutcall(PersistentOutcallEntry entry)
	{
		...
		try {
			connection = getConnectionPool().acquire(false);
			CommunicationInformation commInfo = PersistentOutcallDatabase.getCommunicationInformation(connection, entry);
			connection.commit();
			if (commInfo.outcallActor.enactOutcall(commInfo.callingContext, commInfo.targetEPR, commInfo.attachment)) {
				if (_logger.isDebugEnabled())
			...
			_logger.warn("Persistent outcall service tried to make outcall, but " + "got an exception -- putting it back in the list.");
					// ,cause); Commented out by ASG 2021-01-03 - got tired of dealing with huge stack traces
			try {
				reAdd(connection, entry);
			...
			try {
				...
				_logger.error("Unable to commit connection for persistent outcall db.", sqe);
			}
		} finally {
			getConnectionPool().release(connection);
		}
