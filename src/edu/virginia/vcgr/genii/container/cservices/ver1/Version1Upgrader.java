...
public class Version1Upgrader
{
		...
		ClassNotFoundException
	{
		...
		File target = new File(targetDirectory, className + ".xml");
		JAXBContext v2Context = JAXBContext.newInstance(ContainerServiceConfiguration.class);
		v2Context.createMarshaller().marshal(conf, target);
	}

	static public void upgrade(File version1File, File targetDirectory) throws IOException
	{
		...
		if (!targetDirectory.exists())
			throw new FileNotFoundException(String.format("Unable to create target directory \"%s\".", targetDirectory));

		try {
			JAXBContext v1Context = JAXBContext.newInstance(Version1Configuration.class);
			Version1Configuration v1Conf = (Version1Configuration) v1Context.createUnmarshaller().unmarshal(version1File);
			...
			throw new IOException("Failed to upgrade old container services configuration.", e);
		}
	}
}