...
public class NotificationRateController {
	...
	static private Log _logger = LogFactory
			.getLog(NotificationRateController.class);
	...
	static {
		...
		CONSTRICTED_NOTIFICATION_TYPES
				.add(ByteIOTopics.BYTEIO_ATTRIBUTES_UPDATE_TOPIC);
	}

	private static final long BLOCKAGE_PERIOD_TIME_SPAN = 2 * 60 * 1000L; // two
																			// minutes
	private static final long BURST_PERIOD_TIME_SPAN = 30 * 1000L; // thirty
																	// seconds
	...
	public NotificationRateController() {
		publisherToRecentNotificationTimeMappings = Collections
				.synchronizedMap(new HashMap<String, List<Long>>());
		blockedPublisherToBlockageExpiryTimeMappings = Collections
	...
	public boolean isPublisherBlocked(String publisherKey) {
		return blockedPublisherToBlockageExpiryTimeMappings
				.containsKey(publisherKey);
	}

	public Long getBlockadeCreationTime(String publisherKey) {
		Long blockExpiryTime = blockedPublisherToBlockageExpiryTimeMappings
				.get(publisherKey);
	...
	private boolean isBlockedPublisher(String publisherKey) {
		if (blockedPublisherToBlockageExpiryTimeMappings
				.containsKey(publisherKey)) {
			Long blockageExpiryTime = blockedPublisherToBlockageExpiryTimeMappings
					.get(publisherKey);
	...
	private void submitNotificationForRateControl(String publisherKey,
			NotificationMessageContents message) {
		List<Long> publishersRecentNotificationTimes = publisherToRecentNotificationTimeMappings
				.get(publisherKey);
		if (publishersRecentNotificationTimes == null) {
			publishersRecentNotificationTimes = Collections
					.synchronizedList(new ArrayList<Long>(
							ALLOWED_BURSTINESS_THRESHOLD));
			publisherToRecentNotificationTimeMappings.put(publisherKey,
	...
	private void blockPublisherAndUpdateMessage(String publisherKey,
			NotificationMessageContents message) {
		message.setPublisherBlockedFromFurtherNotifications(true);
		message.setBlockageTime(BLOCKAGE_PERIOD_TIME_SPAN);
		long blockageExpiryTime = System.currentTimeMillis()
				+ BLOCKAGE_PERIOD_TIME_SPAN;
		blockedPublisherToBlockageExpiryTimeMappings.put(publisherKey,
				blockageExpiryTime);
		_logger.info("Publisher "
				+ publisherKey
		...
		public void run() {
			while (true) {
				try {
					...
					long tooOldNotificationThreshold = System
							.currentTimeMillis() - BURST_PERIOD_TIME_SPAN;
					List<String> publishersWithEmptyLists = new ArrayList<String>();
					for (Map.Entry<String, List<Long>> entry : publisherToRecentNotificationTimeMappings
							.entrySet()) {
						String publisherKey = entry.getKey();
						List<Long> recentNotificationTimes = entry.getValue();
						synchronized (recentNotificationTimes) {
							Iterator<Long> iterator = recentNotificationTimes
									.iterator();
					...
					for (String publisherKey : publishersWithEmptyLists) {
						publisherToRecentNotificationTimeMappings
								.remove(publisherKey);
					}
				} catch (Exception ex) {
					_logger.info(
							"notification cleaner thread faced some problem",
							ex);
				}
			}
		}
		...
		public void run() {
			while (true) {
				try {
					...
					for (Map.Entry<String, Long> entry : blockedPublisherToBlockageExpiryTimeMappings
							.entrySet()) {
						String publisherKey = entry.getKey();
					...
					for (String publisherKey : releasedPublishers) {
						blockedPublisherToBlockageExpiryTimeMappings
								.remove(publisherKey);
					}
				} catch (Exception ex) {
					_logger.info("blockade lifter thread faced some problem",
							ex);
				}
			}
		}
