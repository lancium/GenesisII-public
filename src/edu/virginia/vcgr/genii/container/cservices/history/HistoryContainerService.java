...
public class HistoryContainerService extends AbstractContainerService
{
	...
	private Map<String, SequenceNumber> _nextSequenceMap =
		new HashMap<String, SequenceNumber>();
	...
	private HistoryEvent getEvent(long historyRecordID)
	{
		...
		catch (SQLException sqe)
		{
			_logger.warn(String.format(
				"Error trying to get history event for history record id %d.", historyRecordID),
				sqe);
			return null;
		}
		finally
	...
	protected void startService() throws Throwable
	{
		...
		InMemoryAlarmManager.MANAGER.addAlarm(
			new CleanupAlarmHandler(), CLEANUP_INTERVAL);
	}
	...
	final public void deleteRecords(String resourceID)
	{
		...
		{
			_logger.warn(String.format(
				"Error trying to delete history records for resource %s.", resourceID),
				sqe);
		}
		finally
		{
	...
	public Collection<HistoryEvent> getEvents(String resourceID)
	{
		...
		{
			_logger.warn(String.format(
				"Error trying to get history events for resource %s.", resourceID),
				sqe);
