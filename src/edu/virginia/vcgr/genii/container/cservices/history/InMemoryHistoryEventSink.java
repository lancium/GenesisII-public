...
public class InMemoryHistoryEventSink {
	static private Log _logger = LogFactory
			.getLog(InMemoryHistoryEventSink.class);
	...
	static final private QName SERIALIZATION_QNAME = new QName(
			SERIALIZATION_NS, SERIALIZATION_NAME);
	...
	static private List<HistoryEvent> extractHistoryEvents(
			SequenceNumber parentNumber, String stringName,
			HistoryEventSource knownTo, MessageElement[] any) {
		List<HistoryEvent> ret = new ArrayList<HistoryEvent>(any == null ? 0
				: any.length);
		...
		if (any != null) {
			for (MessageElement e : any) {
				...
				if (name.equals(SERIALIZATION_QNAME)) {
					try {
						HistoryEventBundleType bundle = (HistoryEventBundleType) e
								.getObjectValue(HistoryEventBundleType.class);
						HistoryEvent event = (HistoryEvent) DBSerializer
								.deserialize(bundle.getData());
						ret.add(new HistoryEvent(event.eventNumber().wrapWith(
								parentNumber), event.eventTimestamp(),
								new SimpleStringHistoryEventSource(stringName,
										knownTo, event.eventSource()), event
										.eventLevel(), event.eventCategory(),
								event.eventProperties(), event.eventData()));
					} catch (Throwable cause) {
						_logger.warn("Error trying to retrieve sub events.",
								cause);
					}
				}
			}
	...
	static public HistoryEventToken wrapEvents(SequenceNumber parentNumber,
			String responsibleName, HistoryEventSource knownTo,
			String resourceKey, MessageElement[] elements) {
		List<HistoryEvent> besEvents = extractHistoryEvents(parentNumber,
				responsibleName, knownTo, elements);
		...
		for (HistoryEvent event : besEvents) {
			service.addRecord(resourceKey, event.eventNumber(),
					event.eventCategory(), event.eventLevel(),
					event.eventProperties(), event.eventSource(),
	...
	final public MessageElement[] eventMessages() {
		try {
			...
			for (HistoryEvent event : events) {
				ret[lcv] = new MessageElement(SERIALIZATION_QNAME,
						new HistoryEventBundleType(DBSerializer.serialize(
								event, -1L)));
				lcv++;
			}
			...
			_logger.warn(
					"Unable to create message elements for history events.",
					cause);
			return null;
		}
	}

	final HistoryEventToken add(HistoryEventCategory category,
			HistoryEventLevel level, Map<String, String> properties,
			HistoryEventSource eventSource, HistoryEventData eventData) {
		synchronized (_events) {
			_events.add(new HistoryEvent(_nextNumber, Calendar.getInstance(),
					eventSource, level, category, properties, eventData));
