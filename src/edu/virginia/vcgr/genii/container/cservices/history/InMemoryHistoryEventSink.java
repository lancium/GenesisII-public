...
public class InMemoryHistoryEventSink
{
	static private Log _logger = LogFactory.getLog(
		InMemoryHistoryEventSink.class);
	
	static final private String SERIALIZATION_NS = 
	...
	static private List<HistoryEvent> extractHistoryEvents(
		...
		String stringName, HistoryEventSource knownTo, 
		MessageElement []any)
	{
		List<HistoryEvent> ret = new ArrayList<HistoryEvent>(
			any == null ? 0 : any.length);
		...
		if (any != null)
		{
			for (MessageElement e : any)
			{
				...
				if (name.equals(SERIALIZATION_QNAME))
				{
					try
					{
						HistoryEventBundleType bundle = 
							(HistoryEventBundleType)e.getObjectValue(
								HistoryEventBundleType.class);
						HistoryEvent event =
							(HistoryEvent)DBSerializer.deserialize(
								bundle.getData());
						ret.add(new HistoryEvent(
							event.eventNumber().wrapWith(parentNumber),
							event.eventTimestamp(),
							new SimpleStringHistoryEventSource(stringName, knownTo,
								event.eventSource()),
							event.eventLevel(),
							event.eventCategory(),
	...
	static public HistoryEventToken wrapEvents(
		SequenceNumber parentNumber, String responsibleName, 
		HistoryEventSource knownTo, String resourceKey, 
		MessageElement []elements)
	{
		List<HistoryEvent> besEvents = extractHistoryEvents(
			parentNumber, responsibleName, knownTo, elements);
		...
		HistoryEventSource eventSource, HistoryEventData eventData)
	{
		synchronized(_events)
		{
			_events.add(new HistoryEvent(
				_nextNumber, Calendar.getInstance(), eventSource,
				level, category, properties, eventData));
			HistoryEventToken token = new InMemoryHistoryEventToken(
				_nextNumber);
