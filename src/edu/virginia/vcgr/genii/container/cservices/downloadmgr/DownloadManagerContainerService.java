...
public class DownloadManagerContainerService extends AbstractContainerService {
	static private Log _logger = LogFactory
			.getLog(DownloadManagerContainerService.class);
	...
	private void selectDownloadDirectory(String propertyValue) {
		...
		if (propertyValue == null) {
			if (_configuredDownloadTmpDir == null) {
				dirPath = String.format("%s/download-tmp", ConfigurationManager
		...
		try {
			...
			throw new ConfigurationException(
					"Unable to find download manager service directory.", ioe);
		}
	...
	public void setProperties(Properties properties) {
		...
		_configuredDownloadTmpDir = properties
				.getProperty(DOWNLOAD_TMP_DIR_PROPERTY);
	}
	...
	public DataTransferStatistics download(URI source, File target,
		...
		synchronized (_inProgressLocks) {
			...
			if (lock == null) {
				...
				_logger.info(String
						.format("(Download Manager) -- %s needs to be copied from source",
								target));
		...
		if (!iAmResponsible) {
			_logger.info(String
					.format("(Download Manager) -- Another thread is downloading %s so I'll wait.",
							target));
			try {
				lock.waitForSignal();
			...
			_logger.info(String
					.format("(Download Manager) -- The other thread signaled me that %s is done.",
							target));
			lock.checkException();
			return ret.finishTransfer();
		}
		...
		try {
			...
			_logger.warn(String.format(
					"(Download Manager) -- Unable to copy %s from source.",
					target), ioe);
			exception = ioe;
		} catch (Throwable e) {
			_logger.warn(String.format(
		...
		synchronized (_inProgressLocks) {
			...
			synchronized (lock) {
				...
				_logger.info(String
						.format("(Download Manager) -- Signalling waiting threads that %s is done.",
								target));
				lock.signal();
			}
		}
	...
	private long doDownload(URI source, File realTarget,
		...
		synchronized (_lockObject) {
			tmpTarget = File
					.createTempFile("dload", ".tmp", _downloadDirectory);
		}
