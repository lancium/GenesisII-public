...
public class FilesystemUnderstanding {
	...
	public JSDLFileSystem createScratchFilesystem(String jobAnnotation)
		...
		if (!isScratchFileSystem())
			throw new JSDLException(String.format(
					"Don't know how to handle file system \"%s\".", _fsName));
		...
		try {
			...
			ScratchFSManagerContainerService service = ContainerServices
					.findService(ScratchFSManagerContainerService.class);
	...
	public JSDLFileSystem createGridFilesystem() throws JSDLException {
		...
		try {
			FuseFilesystemService ffs = ContainerServices
					.findService(FuseFilesystemService.class);
