...
public class CommonPosixLikeSPMDApplicationFacet extends DefaultSPMDApplicationFacet
{
	@Override
		...
		throws JSDLException
	{
		if (useActualCPUCount)
			throw new UnsupportedJSDLElement(new QName(SPMDConstants.JSDL_SPMD_NS, "useActualCPUCount"));

		((PosixLikeApplicationUnderstanding) currentUnderstanding).setNumProcesses(numberOfProcesses.intValue());
	}
	...
	public void consumeProcessesPerHost(Object currentUnderstanding, Integer processesPerHost) throws JSDLException
	{
		...
		((PosixLikeApplicationUnderstanding) currentUnderstanding).setNumProcessesPerHost(processesPerHost.intValue());
	}
	...
	public void consumeArgument(Object currentUnderstanding, String filesystemName, String argument) throws JSDLException
	{
		...
		if (filesystemName == null)
			...
			arg = new StringOrPath(new FilesystemRelativePath(filesystemName, argument));

		((PosixLikeApplicationUnderstanding) currentUnderstanding).addArgument(arg);
	}
		...
		throws JSDLException
	{
		...
		if (filesystemName == null)
			...
			env = new StringOrPath(new FilesystemRelativePath(filesystemName, environment));

		((PosixLikeApplicationUnderstanding) currentUnderstanding).addEnvironment(name, env);
	}
