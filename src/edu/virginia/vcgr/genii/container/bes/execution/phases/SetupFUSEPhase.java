...
public class SetupFUSEPhase extends AbstractFUSEPhases {
	...
	public void execute(ExecutionContext context) throws Throwable {
		...
		history.createTraceWriter("FUSE Mounting Grid")
				.format("FUSE mounting grid to %s", mountPoint).close();

		if (!mountPoint.exists()) {
			if (!mountPoint.mkdirs()) {
				history.createErrorWriter("FUSE Mount Failed")
						.format("Unable to create mount point").close();
			...
			if (!mountPoint.isDirectory()) {
				history.createErrorWriter("FUSE Mount Failed")
						.format("Mount point not a directory").close();
		...
		ProcessBuilder builder = new ProcessBuilder(Installation
				.getGridCommand().getAbsolutePath(), "fuse", "--mount",
				String.format("--sandbox=%s", sandbox), getMountPoint(context)
						.getAbsolutePath());
		builder.redirectErrorStream(true);

		StreamRedirectionSink sink = new FileRedirectionSink(new File(
				"fuse-output.log"));
		StreamRedirectionDescription desc = new StreamRedirectionDescription(
				null, sink, null);

		Process proc = builder.start();
		desc.enact(context, proc.getOutputStream(), proc.getInputStream(),
				proc.getErrorStream());
