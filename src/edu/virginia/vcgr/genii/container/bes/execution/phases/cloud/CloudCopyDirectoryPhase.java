...
public class CloudCopyDirectoryPhase extends AbstractCloudExecutionPhase
	...
	static private Log _logger = LogFactory
			.getLog(CloudCopyDirectoryPhase.class);
	...
	public void execute(ExecutionContext context) throws Throwable {
		...
		HistoryContext history = HistoryContextFactory
				.createContext(HistoryEventCategory.CloudSetup);

		history.createInfoWriter("Setting up working directory").close();

		tryExecuteCommand(resourceID, "mkdir " + _workingDir, System.out,
				System.err, tManage);

		copyDirectory(tManage, new File(_workingDir), lDir, resourceID,
	...
	private void copyDirectory(CloudManager tManage, File root, File dir,
			String resourceID, OutputStream out, OutputStream err)
			throws Exception {
		...
		for (int i = 0; i < fileList.length; i++) {
			if (fileList[i].isDirectory()) {
				...
				tryExecuteCommand(resourceID, "mkdir " + _workingDir
						+ fileList[i].getName(), out, err, tManage);
				copyDirectory(tManage, new File(root.getAbsolutePath() + "/"
						+ fileList[i].getName()), fileList[i], resourceID, out,
						err);
			} else
