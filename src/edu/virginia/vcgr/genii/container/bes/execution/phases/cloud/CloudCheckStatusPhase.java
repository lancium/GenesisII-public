...
public class CloudCheckStatusPhase extends AbstractCloudExecutionPhase
	...
	public void execute(ExecutionContext context) throws Throwable {
		...
		HistoryContext history = HistoryContextFactory
				.createContext(HistoryEventCategory.Checking);

		history.createInfoWriter(
				"Polling for completion of " + _checkPhase + " Phase").close();
		...
		while (true) {
			...
			try {
				...
				// Poll
				if (tManage.checkFile(resourceID, _workingDir
						+ _completeFileName)) {
					break;
				} else {
					_logger.info("CloudBES: Activity " + _activityID
							+ " waiting on " + _checkPhase + " phase");
					Thread.sleep(20000);
				}
		...
		_logger.info("CloudBES: Activity " + _activityID + " " + _checkPhase
				+ " phase complete");
