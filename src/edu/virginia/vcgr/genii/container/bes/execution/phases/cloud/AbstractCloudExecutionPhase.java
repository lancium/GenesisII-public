			...
			try {
				...
				if (_failed){
					long sleep = (long) ((_backoff * 1000) *
							Math.exp(.5 * _tries));
					getLog().info("Failed to Execute " + _activityID + 
		...
		if (tManage != null){
			try {
				...
				tryExecuteCommand(resourceID,
						"killall -9 -g runScript.sh", System.out, System.err, tManage);
				tryExecuteCommand(resourceID,
						"killall -9 -g grid", System.out, System.err, tManage);
