...
public class StreamRedirectionDescription implements Serializable {
	...
	static private Log _logger = LogFactory
			.getLog(StreamRedirectionDescription.class);
	...
	static private void redirect(ExecutionContext context, OutputStream sink,
			StreamRedirectionSource source) {
		if (source != null) {
			try {
				Thread thread = new Thread(new StreamRedirector(
						source.openSource(context), sink),
	...
	static private void redirect(ExecutionContext context,
		...
		try {
			Thread thread = new Thread(new StreamRedirector(source,
