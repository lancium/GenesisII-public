...
public class SetupOGRSHPhase extends AbstractExecutionPhase
{
	...
	public void execute(ExecutionContext context) throws Throwable
	{
		...
		try {
			...
			writeMount(config, "/dev", "local-fs-provider", "local-fs-session", "/dev");
			writeMount(config, "/usr", "local-fs-provider", "local-fs-session", "/usr");
			writeMount(config, "/tmp", "local-fs-provider", "local-fs-session", "/tmp");
			writeMount(config, "/etc", "local-fs-provider", "local-fs-session", "/etc");
			writeMount(config, "/var", "local-fs-provider", "local-fs-session", "/var");
			writeMount(config, "/uva-genii", "genesisII-fs-provider", "uva-geniinet-session", "/");
	...
	static private void writeGridProvider(PrintStream config, File storedContextFile) throws MalformedURLException
	{
