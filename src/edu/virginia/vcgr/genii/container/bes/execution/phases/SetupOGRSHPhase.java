...
public class SetupOGRSHPhase extends AbstractExecutionPhase {
	...
	public void execute(ExecutionContext context) throws Throwable {
		File configFile = new File(context.getCurrentWorkingDirectory()
				.getWorkingDirectory(), _configFilename);
		File contextFile = new File(context.getCurrentWorkingDirectory()
				.getWorkingDirectory(), _storedContextFilename);
		...
		try {
			...
			writeMount(config, "/home/bes-job", "local-fs-provider",
					"local-fs-session", context.getCurrentWorkingDirectory()
							.getWorkingDirectory().getAbsolutePath());
			writeMount(config, "/bin", "local-fs-provider", "local-fs-session",
					"/bin");
			...
			writeMount(config, "/dev", "local-fs-provider", "local-fs-session",
					"/dev");
			writeMount(config, "/usr", "local-fs-provider", "local-fs-session",
					"/usr");
			writeMount(config, "/tmp", "local-fs-provider", "local-fs-session",
					"/tmp");
			writeMount(config, "/etc", "local-fs-provider", "local-fs-session",
					"/etc");
			writeMount(config, "/var", "local-fs-provider", "local-fs-session",
					"/var");
	...
	static private void writeGridProvider(PrintStream config,
			File storedContextFile) throws MalformedURLException {
	...
	static private void writeMount(PrintStream config, String mountLocation,
			String mountProvider, String mountSession, String rootDirectory) {
