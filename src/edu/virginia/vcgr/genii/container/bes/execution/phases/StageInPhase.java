...
public class StageInPhase extends AbstractExecutionPhase implements
		Serializable {
	...
	public StageInPhase(URI source, File target,
		...
		if (target == null)
			throw new IllegalArgumentException(
					"Parameter \"targetName\" cannot be null.");

		_source = source;
		_target = target;
		_creationFlag = (creationFlag == null) ? CreationFlagEnumeration.overwrite
				: creationFlag;
	}
	...
	public void execute(ExecutionContext context) throws Throwable {
		...
		history.createInfoWriter("Staging in to %s", _target.getName())
				.format("Staging in from %s to %s.", _source, _target).close();
		DataTransferStatistics stats;

		try {
			File target = _target;
			if (_creationFlag.equals(CreationFlagEnumeration.dontOverwrite)) {
				DownloadManagerContainerService service = ContainerServices
			...
			history.createTraceWriter("%s: %d Bytes Transferred",
					_target.getName(), stats.bytesTransferred())
			...
			history.createErrorWriter(cause, "Error staging in to %s",
					_target.getName())
					.format("Error staging in from %s to %s.", _source, _target)
					.close();
			throw cause;
		}
	}
