...
{
	static private Log _logger = LogFactory.getLog(ActivityListRNSResourceFork.class);

	public ActivityListRNSResourceFork(ResourceForkService service, String forkPath)
	{
	...
	{
		Collection<InternalEntry> response = new LinkedList<InternalEntry>();

		IBESResource resource = (IBESResource) ResourceManager.getCurrentResource().dereference();
	...
	{
		throw new IOException("Not allowed to make new directory using this resource fork.");
	}
		...
		try {
			...
			for (BESActivity activity : resource.getContainedActivities()) {
				...
				if (query == null || query.equals(name)) {
					...
					try {
						tat = GeniiBESServiceImpl.terminateActivity(activity.getActivityEPR());
						if (tat.getFault() != null) {
							_logger.error("Unable to remove activity \"" + name + "\":  " + tat.getFault());
							failed = true;
						}
					} catch (NoSuchActivityFault nsaf) {
						if (_logger.isDebugEnabled())
