...
public class ActivityListRNSResourceFork extends AbstractRNSResourceFork {
	static private Log _logger = LogFactory
			.getLog(ActivityListRNSResourceFork.class);
	...
	public Iterable<InternalEntry> list(EndpointReferenceType exemplarEPR,
		...
		IBESResource resource = (IBESResource) ResourceManager
		...
		try {
			...
			for (BESActivity activity : resource.getContainedActivities()) {
				...
				if (query == null || query.equals(name)) {
					try {
						response.add(new InternalEntry(name, activity
								.getActivityEPR()));
					} catch (NoSuchActivityFault nsaf) {
						if (_logger.isDebugEnabled())
							_logger.debug(
									"We lost an activity between the "
		...
		try {
			...
			for (BESActivity activity : resource.getContainedActivities()) {
				...
				if (query == null || query.equals(name)) {
					...
					try {
						tat = GeniiBESServiceImpl.terminateActivity(activity
								.getActivityEPR());
						if (tat.getFault() != null) {
							_logger.error("Unable to remove activity \"" + name
									+ "\":  " + tat.getFault());
							failed = true;
						}
					} catch (NoSuchActivityFault nsaf) {
						if (_logger.isDebugEnabled())
							_logger.debug(
									"We lost an activity between the time "
