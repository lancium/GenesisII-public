...
class Version1Upgrader
{
		...
		Properties props, String propName)
	{
		...
		return (value == null) ? null :
			OperatingSystemNames.valueOf(value);
	}
		...
		Properties props, String name)
	{
		...
		return (value == null) ? null :
			ProcessorArchitecture.valueOf(value);
	}
	
	static private Integer getInteger(Properties props,
		String name)
	{
		String value = props.getProperty(name);
	...
	static private ClockSpeed getClockSpeed(Properties props,
		String name)
	{
		String value = props.getProperty(name);
		if (value == null)
	...
	static private Size getSize(Properties props,
		String name)
	{
		String value = props.getProperty(name);
		if (value == null)
	...
	static private Collection<UnixSignals> getSignals(
		Properties props, String name) throws IOException
	{
		String value = props.getProperty(name);
		if (value == null)
			return null;
		...
		String provName)
	{
		...
		conf.qsub(getAppConf(
			props, APPPATH_FMT, APPARG_FMT, provName, "qsub"));
		conf.qstat(getAppConf(
			props, APPPATH_FMT, APPARG_FMT, provName, "qstat"));
		conf.qdel(getAppConf(
			props, APPPATH_FMT, APPARG_FMT, provName, "qdel"));
		...
		String appName)
	{
		...
		while (true)
		{
			String arg = props.getProperty(String.format(
				argFmt, provName, appName, lcv++));
			if (arg == null)
				break;
			args.add(arg);
		}
		
		return new ExecutableApplicationConfiguration(
			path, args);
	}
