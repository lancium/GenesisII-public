...
public class AccountingDirFileFork extends AbstractRandomByteIOResourceFork
{
	private File getTargetFile() throws IOException
	{
		File ret;
		String relativePath = getForkPath();
		if (!relativePath.startsWith(AccountingDirectoryFork.FORK_BASE_PATH))
			throw new FileNotFoundException(String.format("Invalid fork path specified (%s).", relativePath));
		relativePath = relativePath.substring(AccountingDirectoryFork.FORK_BASE_PATH.length());
	...
	public AccountingDirFileFork(ResourceForkService service, String forkPath)
	{
