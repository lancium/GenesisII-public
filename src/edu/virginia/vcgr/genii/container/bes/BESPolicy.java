...
public class BESPolicy
{
	...
	public BESPolicy(BESPolicyActions userLoggedInAction, BESPolicyActions screenSaverInactiveAction)
	{
	...
	public BESPolicyActions getCurrentAction(Boolean userLoggedIn, Boolean screenSaverActive)
	{
		boolean uli = (userLoggedIn == null) ? false : userLoggedIn.booleanValue();
		boolean ssia = (screenSaverActive == null) ? false : !screenSaverActive.booleanValue();
	...
	public MessageElement toMessageElement(QName elementName)
	{
		...
		MessageElement loggedIn = new MessageElement(new QName(BESConstants.GENII_BES_NS, "user-logged-in-action"),
			getUserLoggedInAction().toString());
		MessageElement screenSaverInactive = new MessageElement(new QName(BESConstants.GENII_BES_NS,
			"screensaver-inactive-action"), getScreenSaverInactiveAction().toString());
	...
	static public BESPolicy fromMessageElement(MessageElement element)
	{
		...
		while (iter.hasNext()) {
			...
			if (childName.equals(new QName(BESConstants.GENII_BES_NS, "user-logged-in-action"))) {
				userLoggedInAction = BESPolicyActions.valueOf(child.getValue());
			} else if (childName.equals(new QName(BESConstants.GENII_BES_NS, "screensaver-inactive-action"))) {
				screenSaverInactiveAction = BESPolicyActions.valueOf(child.getValue());
			}
		}
