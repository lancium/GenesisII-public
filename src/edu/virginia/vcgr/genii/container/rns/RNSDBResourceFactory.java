...
public class RNSDBResourceFactory extends BasicDBResourceFactory implements
		IResourceFactory {
	static private final String _CREATE_ENTRY_TABLE_STMT = "CREATE TABLE entries ("
	...
	// the endpoint_id was not in the original RNS entries table. We have added
	// this to facilitate
	// subscription based client-side metadata caching. To support online
	// migration this column has
	// been added separately in the table -- in case it is not present there
	// already; the column
	// will be there beforehand when the container itself is created after the
	// caching related
	// update.
	static private final String _ADD_ENDPOINT_ID_COLUMN_STMT = "ALTER TABLE entries ADD COLUMN endpoint_id VARCHAR(128)";

	public RNSDBResourceFactory(ServerDatabaseConnectionPool pool)
			throws SQLException {
		super(pool);
	}
	...
	protected void createTables() throws SQLException {
		...
		try {
			...
			DatabaseTableUtils.addColumns(conn, false,
					_ADD_ENDPOINT_ID_COLUMN_STMT);
			DatabaseTableUtils.createTables(conn, false,
