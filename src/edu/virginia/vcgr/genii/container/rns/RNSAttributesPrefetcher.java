...
public class RNSAttributesPrefetcher extends
		DefaultGenesisIIAttributesPreFetcher<IRNSResource> {

	static private Log _logger = LogFactory
			.getLog(RNSAttributesPrefetcher.class);

	public RNSAttributesPrefetcher(EndpointReferenceType target)
			throws ResourceException, ResourceUnknownFaultType {
		this((IRNSResource) ResourceManager.getTargetResource(target)
				.dereference());
	}
	...
	protected void fillInAttributes(Collection<MessageElement> attributes) {
		...
		try {
			Integer elementCount = (Integer) getResource().getProperty(
					IRNSResource.ELEMENT_COUNT_PROPERTY);
			if (elementCount != null) {
				attributes.add(new MessageElement(
						RNSConstants.ELEMENT_COUNT_QNAME, elementCount));
			}
