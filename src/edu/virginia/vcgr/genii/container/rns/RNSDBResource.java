...
public class RNSDBResource extends BasicDBResource implements IRNSResource {
	...
	public RNSDBResource(ResourceKey parentKey,
	...
	public void addEntry(InternalEntry entry) throws ResourceException,
		...
		String entryId = null;
		try {
			entryId = new AddressingParameters(entry.getEntryReference()
					.getReferenceParameters()).getResourceKey();
		} catch (Exception ex) {
	...
	public Collection<String> listEntries(String name) throws ResourceException {
		...
		try {
			if (name == null)
				...
				stmt = _connection
						.prepareStatement(_SELECT_SINGLETON_ENTRY_STMT);
		...
		try {
			...
			if (isBatch) {
				while (rs1.next()) {
					...
					EndpointReferenceType entryEPR = EPRUtils.fromBlob(rs1
							.getBlob(2));
					MessageElement[] entryAttributes = ObjectDeserializer
							.anyFromBytes(rs1.getBytes(4));
					String entryResourceKey = rs1.getString("endpoint_id");
					InternalEntry entry = new InternalEntry(entryNameFromDB,
							entryEPR, entryAttributes);
					ret.add(entry);
					if (entryResourceKey == null) {
						entriesWithMissingResourceKeys.put(entryNameFromDB,
								entryEPR);
					}
				}
			} else {
				if (rs1.next()) {
					...
					InternalEntry entry = new InternalEntry(entryName, null,
							null, false); // signifying
											// that
											// the
											// entry
											// name
			...
			if (!entriesWithMissingResourceKeys.isEmpty()) {
				...
				if (!entryNameToResourceKeyMappings.isEmpty()) {
					...
					for (Map.Entry<String, String> entry : entryNameToResourceKeyMappings
							.entrySet()) {
	...
	public Collection<InMemoryIteratorEntry> retrieveIdOfEntry(String request)
		...
		try {
			if (request == null) {
				stmt = _connection
						.prepareStatement(_RETRIEVE_PART_ENTRY_ALL_STMT);
				isBatch = true;
			}

			else {
				stmt = _connection
						.prepareStatement(_RETRIEVE_PART_ENTRY_ONE_STMT);
			...
			if (isBatch) {
				while (rs.next()) {
					InMemoryIteratorEntry imie = new InMemoryIteratorEntry(
							rs.getString(1), rs.getString(2), true,
							FileOrDir.UNKNOWN); // entry
												// exists
					ret.add(imie);
				}
			}
				...
				if (rs.next()) {
					...
					InMemoryIteratorEntry imie = new InMemoryIteratorEntry(
							rs.getString(1), new String(""), false,
							FileOrDir.UNKNOWN); // entry
												// does
												// not
												// exist
	...
	public static InternalEntry retrieveByIndex(Connection connection, String id)
			throws ResourceException {
		// returns null if the id doesn't exist
		...
		try {
			...
			if (rs.next()) {
				...
				final EndpointReferenceType entryEPR = EPRUtils.fromBlob(rs
						.getBlob(2));
				final String entryName = rs.getString(1);
				ie = new InternalEntry(entryName, entryEPR,
				...
				if (endpointId == null) {
					try {
						endpointId = new AddressingParameters(
								entryEPR.getReferenceParameters())
								.getResourceKey();
					} catch (Exception e) {
						// entry reference-parameters section did not match
						// GenesisII convention.
					}
				}
				if (endpointId != null) {
	...
	private Map<String, String> getEntryNameToResourceKeyMappings(
			Map<String, EndpointReferenceType> nameToEPRMappings) {
		Map<String, String> nameToResourceIdMappings = new HashMap<String, String>();
		for (Map.Entry<String, EndpointReferenceType> entry : nameToEPRMappings
				.entrySet()) {
			EndpointReferenceType entryEPR = entry.getValue();
			try {
				String resourceKey = new AddressingParameters(
						entryEPR.getReferenceParameters()).getResourceKey();
				if (resourceKey != null) {
					String entryName = entry.getKey();
					nameToResourceIdMappings.put(entryName, resourceKey);
				}
			} catch (Exception e) {
				// entry reference-parameters section did not match GenesisII
				// convention.
			}
		}
