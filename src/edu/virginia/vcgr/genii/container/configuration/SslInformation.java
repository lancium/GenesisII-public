...
public class SslInformation
{
	...
	{
		_keystoreFilename = properties.getProperty(SecurityConstants.Container.SSL_KEY_STORE_PROP);
		_keystoreType = properties.getProperty(SecurityConstants.Container.SSL_KEY_STORE_TYPE_PROP);
		_keystorePassword = properties.getProperty(SecurityConstants.Container.SSL_KEY_STORE_PASSWORD_PROP);
		_keyPassword = properties.getProperty(SecurityConstants.Container.SSL_KEY_PASSWORD_PROP);

		if (_keystoreFilename == null)
			throw new ConfigurationException("Required ssl property \"" + SecurityConstants.Container.SSL_KEY_STORE_PROP
				+ "\" not found.");
		if (_keystoreType == null)
			throw new ConfigurationException("Required ssl property \"" + SecurityConstants.Container.SSL_KEY_STORE_TYPE_PROP
				+ "\" not found.");
	}
