...
class AnnotationSuppliedGenesisIIServiceConfiguration implements
	...
	static private <Type> Type instantiate(Class<Type> cl) {
		...
		try {
			...
			throw new ConfigurationException(String.format(
					"Unable to instantiate %s for service configuration.", cl),
					e);
	...
	AnnotationSuppliedGenesisIIServiceConfiguration(
		...
		if (serviceConfiguration != null) {
			_resourceProvider = instantiate(serviceConfiguration
					.resourceProvider());
			_defaultAuthZProvider = instantiate(serviceConfiguration
					.defaultAuthZProvider());
			_defaultResolverFactoryProxy = instantiate(serviceConfiguration
					.defaultResolverFactoryProxy());

			_defaultServiceCertificateLifetime = getPositiveLong(serviceConfiguration
					.defaultServiceCertificateLifetime());
			_defaultResourceCertificateLifetime = getPositiveLong(serviceConfiguration
					.defaultResourceCertificateLifetime());

			_jaxbServiceConfigurationClass = getNonInterfaceClass(serviceConfiguration
