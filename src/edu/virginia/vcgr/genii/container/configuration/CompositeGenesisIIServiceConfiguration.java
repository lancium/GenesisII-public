...
class CompositeGenesisIIServiceConfiguration
	implements GenesisIIServiceConfiguration
{
	private LinkedList<GenesisIIServiceConfiguration> _values =
		new LinkedList<GenesisIIServiceConfiguration>();
	...
	CompositeGenesisIIServiceConfiguration(Class<?> serviceClass)
	{
		...
		for (Class<?> newServiceClass = serviceClass;
			newServiceClass != Object.class;
			newServiceClass = newServiceClass.getSuperclass())
		{
			GeniiServiceConfiguration serviceConf = 
				newServiceClass.getAnnotation(GeniiServiceConfiguration.class);
			
			if (serviceConf != null)
			{
				Class<?> jaxbOverride =
					serviceConf.jaxbServiceConfigurationClass();
				
				if (!jaxbOverride.isInterface())
				{
					if ((lastJAXBClassOverride != null) &&
						!jaxbOverride.isAssignableFrom(
				...
				_values.addLast(
					new AnnotationSuppliedGenesisIIServiceConfiguration(
						serviceConf));
			}
		}
