...
{
	static private Log _logger = LogFactory.getLog(DatabaseConnectionManager.class);
	...
	static private class ConnectionDescription
	{
		...
		public ConnectionDescription(DatabaseConnectionPool connectionPool, Connection databaseConnection)
		{
	...
	static private class ConnectionStorage extends ThreadLocal<Collection<ConnectionDescription>>
	{
	...
	static private ConnectionStorage _storage = new ConnectionStorage();

	static private HashMap<String, DatabaseConnectionPool> _pools = new HashMap<String, DatabaseConnectionPool>();

	static public Connection acquireConnection(String connectionPoolName, boolean useAutoCommit) throws SQLException
	{
		DatabaseConnectionPool pool;
		synchronized (_pools) {
			pool = _pools.get(connectionPoolName);
			if (pool == null)
				_pools.put(connectionPoolName, (pool = findConnectionPool(connectionPoolName)));
		}
	...
	static public void releaseAllConnections(boolean success)
	{
		for (ConnectionDescription desc : _storage.get()) {
			...
			try {
				...
				_logger.error("Problem committing or rolling back the database.", t);
			} finally {
				pool.release(conn);
			}
		}
