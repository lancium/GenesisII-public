...
public class DeployDatabase implements Closeable
{
	...
	static private final String _FIND_STALE_INSTANCES_TEXT =
		"SELECT INSTANCE_ID, DIRECTORY_NAME FROM DEPLOYMENTS WHERE " + "(((USE_COUNT = 0) AND ((STATE = ?) OR "
	...
	static private final String _FIND_KNOWN_DEPLOYMENTS_TEXT =
		"SELECT INSTANCE_ID, COMPONENT_ID, MODIFICATION_TIME FROM " + "DEPLOYMENT_STAMPS";
	...
	static {
		try {
			...
			throw new RuntimeException("Can't create deployment database.", e);
		}
	}
