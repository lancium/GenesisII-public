...
public class PersistentContainerProperties
{
		...
		for (int lcv = 0; lcv < 5; lcv++)
		{
			try
			{
				...
				stmt = connection.prepareStatement(
					"SELECT propertyvalue FROM containerproperties " +
					"WHERE propertyname = ?");
		...
		throws SQLException, IOException
	{
		...
		try
		{
			connection = _pool.acquire(false);
			stmt = connection.prepareStatement(
				"DELETE FROM containerproperties WHERE propertyname = ?");
			...
			Blob b = DBSerializer.toBlob(value,
				"containerproperties", "propertyvalue");
			stmt.setBlob(2, b);
			if (stmt.executeUpdate() != 1)
				throw new ResourceException(
					"Unable to update container property \"" +
					name + "\".");
			connection.commit();
		}
		finally
