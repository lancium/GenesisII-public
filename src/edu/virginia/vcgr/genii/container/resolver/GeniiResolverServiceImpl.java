...
public class GeniiResolverServiceImpl extends GenesisIIBase implements
		GeniiResolverPortType, ResolverTopics, AclTopics {
	static private Log _logger = LogFactory
			.getLog(GeniiResolverServiceImpl.class);

	// MessageElement keys for VcgrCreate()
	static public final QName TARGET_EPR_PARAMETER = new QName(
			GenesisIIConstants.GENESISII_NS, "genii-resolver-target-epr");

	// Use these keys to store data in the addressing parameters additional user
	// data.
	...
	static public final QName TARGET_CONTAINER_PARAMETER = new QName(
			GENII_RESOLVER_NS, "container-id");
	static public final QName TARGET_ID_PARAMETER = new QName(
			GENII_RESOLVER_NS, "target-id");
	...
	protected GeniiResolverServiceImpl(String serviceName)
	...
	/**
	 * Create a new simple resolver resource. Write the target EPR to the
	 * database.
	 */
	@Override
	public void postCreate(ResourceKey rKey, EndpointReferenceType myEPR,
		...
		IGeniiResolverResource resource = (IGeniiResolverResource) rKey
				.dereference();
		...
		EndpointReferenceType targetEPR = (EndpointReferenceType) constructionParameters
				.get(TARGET_EPR_PARAMETER);
		if (primaryEPR != null) {
			...
			if (EPRUtils.isUnboundEPR(targetEPR))
				throw new ResourceException(
						"Missing EPR Address for new GeniiResolver resource");
			WSName wsname = new WSName(targetEPR);
			URI targetEPI = wsname.getEndpointIdentifier();
			if (targetEPI == null)
				throw new ResourceException(
						"Invalid EPR for GeniiResolver - not a WSName");
			int targetID = 0;
			resource.addTargetEPR(targetEPI, targetID, targetEPR);
			GeniiResolverUtils.createTerminateSubscription(targetID, targetEPR,
					myEPR, resource);
			resource.commit();
		}
	}
	...
	private EndpointReferenceType doResolve(URI targetEPI)
		...
		int targetID = -1;
		try {
			AddressingParameters addParams = new AddressingParameters(
					myEPR.getReferenceParameters());
			Map<String, Serializable> infoMap = addParams
					.getAdditionalUserInformation();
			if (infoMap != null) {
				Integer targetValue = (Integer) infoMap
						.get(GENII_RESOLVER_TARGET_ID_KEY);
				if (targetValue != null) {
		...
		return GeniiResolverUtils.createResolutionEPR(_resource, resolutionEPR,
				myEPR, resolutionID);
	}
	...
	public UpdateResponseType update(UpdateRequestType updateRequest)
		...
		try {
			...
			TopicSet space = TopicSet
					.forPublisher(GeniiResolverServiceImpl.class);
			PublisherTopic publisherTopic = space
					.createPublisherTopic(RESOLVER_UPDATE_TOPIC);
			publisherTopic.publish(new ResolverUpdateContents(targetID, newEPR,
					vvr));
		} finally {
			_resourceLock.unlock();
		}
		EndpointReferenceType myEPR = (EndpointReferenceType) WorkingContext
				.getCurrentWorkingContext().getProperty(
						WorkingContext.EPR_PROPERTY_NAME);
		newEPR = GeniiResolverUtils.createResolutionEPR(_resource, newEPR,
				myEPR, targetID);
	 ...
	 */
	private int addEPR(IGeniiResolverResource resource,
			EndpointReferenceType targetEPR) throws RemoteException,
			InvalidWSNameFaultType {
		if (_logger.isDebugEnabled())
		...
		targetIDList = GeniiResolverUtils.initializeNextTargetID(resource,
				targetEPI, targetIDList);
		int targetID = 0;
		if (targetIDList.length > 0) {
		...
		if (targetID == 0) {
			// With a new resource (targetID == 0), the resolver initiates the
			// conversation.
			// The resolver tells the resource,
			// "tell me when you are terminated."
			//
			// With a replica (targetID > 0), the replica initiates the
			// conversation.
	...
	public EndpointReferenceType extResolveEPI(ExtResolveRequestType request)
		...
		if (containerID != null) {
			try {
				_resourceLock.lock();
				resolutionEPR = resolveContainer(_resource, targetEPI,
						containerID);
			} finally {
				_resourceLock.unlock();
			}
	...
	private static EndpointReferenceType resolveContainer(
			IGeniiResolverResource resource, URI targetEPI, String containerID)
			throws ResourceException {
		int[] targetIDList = resource.getTargetIDList(targetEPI);
		for (int targetID : targetIDList) {
			EndpointReferenceType targetEPR = resource.getTargetEPR(targetEPI,
					targetID);
			...
			if (guid.toString().equals(containerID)) {
				EndpointReferenceType myEPR = (EndpointReferenceType) WorkingContext
						.getCurrentWorkingContext().getProperty(
								WorkingContext.EPR_PROPERTY_NAME);
				return GeniiResolverUtils.createResolutionEPR(resource,
						targetEPR, myEPR, targetID);
			}
		}
		return null;
	}
	...
	/* RNS port type */
	@RWXMapping(RWXCategory.WRITE)
	public RNSEntryResponseType[] add(RNSEntryType[] request)
			throws RemoteException {
	...
	public LookupResponseType lookup(String[] lookupRequest)
		...
		if (_resource.getKey() == null) {
			...
			if (resolvers != null && resolvers.size() > 0) {
				ResourceKey rKey = (ResourceKey) _resource
						.getParentResourceKey();
				...
				while (iterator.hasNext()) {
					...
					if (requestContainsFilename(lookupRequest,
							targetEPI.toString())) {
						String entryResourceKey = mapEntry.getValue();
						ResourceKey entryKey = ResourceManager
								.getTargetResource(serviceName,
										entryResourceKey);
						EndpointReferenceType resolverEPR = ResourceManager
								.createEPR(entryKey, currentURL,
										implementedPortTypes, null);
						resolverEPR = GeniiResolverUtils.createUserInfoEPR(
								resolverEPR, GENII_RESOLVER_TARGET_EPI_KEY,
								targetEPI, true);
						result.add(new RNSEntryResponseType(resolverEPR, null,
								null, targetEPI.toString()));
					}
				}
			}
		} else {
			// Is the user asking to list the instances of a specific resolved
			// resource?
			URI targetEPI = null;
			AddressingParameters addParams = new AddressingParameters(
					myEPR.getReferenceParameters());
			...
			if (targetEPI == null) {
				...
				for (int idx = 0; idx < targetEPIList.length; idx++) {
					...
					if (requestContainsFilename(lookupRequest,
							targetEPI.toString())) {
						EndpointReferenceType resolverEPR = GeniiResolverUtils
								.createUserInfoEPR(myEPR,
										GENII_RESOLVER_TARGET_EPI_KEY,
										targetEPI, true);
						result.add(new RNSEntryResponseType(resolverEPR, null,
								null, targetEPI.toString()));
					}
				...
				for (int idx = 0; idx < targetIDList.length; idx++) {
					...
					if (requestContainsFilename(lookupRequest, name)) {
						EndpointReferenceType targetEPR = _resource
								.getTargetEPR(targetEPI, targetID);
						if (targetEPR != null)
							result.add(new RNSEntryResponseType(targetEPR,
									null, null, name));
					}
				}
			}
	...
	private static boolean requestContainsFilename(String[] lookupRequest,
			String name) {
	...
	private class LegacyResourceTerminationNotificationHandler extends
		...
		public String handleNotification(TopicPath topic,
			...
			SimpleResolverTerminateUserData notifyData = contents
					.additionalUserData(SimpleResolverTerminateUserData.class);
			...
			try {
				...
				PublisherTopic publisherTopic = space
						.createPublisherTopic(RESOLVER_UPDATE_TOPIC);
				publisherTopic.publish(new ResolverUpdateContents(targetEPI,
						targetID, vvr));
			} finally {
				_resourceLock.unlock();
			}
	...
	private class ResolverUpdateNotificationHandler extends
			AbstractNotificationHandler<ResolverUpdateContents> {
		...
		public String handleNotification(TopicPath topicPath,
			...
			if ((targetID < 0) || (targetID > 999)) {
				_logger.warn("SimpleResolverServiceImpl.notify: invalid targetID "
						+ targetID);
			...
			try {
				...
				if (targetEPR == null) {
					...
					resource.addTargetEPR(targetEPI, targetID, targetEPR);
					GeniiResolverUtils.updateNextTargetID(resource, targetEPI,
							targetID);
				}
	...
	/**
	 * Write the entire contents of the resolverentries table to an output
	 * stream so another resolver resource can become a replica of this
	 * resource.
	 */
	@RWXMapping(RWXCategory.READ)
	public OpenStreamResponse openStream(Object request)
		...
		try {
			_resourceLock.lock();
			File superDir = Container.getConfigurationManager()
					.getUserDirectory();
			...
			params[0] = new MessageElement(
					ByteIOConstants.SBYTEIO_DESTROY_ON_CLOSE_FLAG, "true");
			params[1] = new MessageElement(SByteIOResource.FILE_PATH_PROPERTY,
					filename);
			params[2] = new MessageElement(
					SByteIOResource.MUST_DESTROY_PROPERTY, "true");
			StreamableByteIOServiceImpl service = new StreamableByteIOServiceImpl();
			String serviceURL = Container
					.getServiceURL("StreamableByteIOPortType");
			EndpointReferenceType entryReference = service.CreateEPR(params,
					serviceURL);
