...
public class GeniiResolverDBResource extends BasicDBResource implements
		IGeniiResolverResource {
	...
	static private final String _GET_TARGETEPI_LIST_STMT = "SELECT DISTINCT epi FROM resolverentries "
			+ "WHERE resourceid = ?";
	...
	static private final String _GET_ENTRY_COUNT_STMT = "SELECT COUNT(targetid) FROM resolverentries "
			+ "WHERE resourceid = ?";
	...
	public GeniiResolverDBResource(ResourceKey parentKey,
			ServerDatabaseConnectionPool connectionPool) throws SQLException {
		super(parentKey, connectionPool);
	}

	public void addTargetEPR(URI targetEPI, int targetID,
			EndpointReferenceType targetEPR) throws ResourceException {
		...
		try {
			...
			stmt.setBlob(4,
					EPRUtils.toBlob(targetEPR, "resolverentries", "endpoint"));
	...
	public URI[] getTargetEPIList() throws ResourceException {
		...
		try {
			...
			throw new ResourceException(exception.getLocalizedMessage(),
					exception);
		} finally {
	...
	/**
	 * Return the total number of entries in the resolverEntries table for this
	 * resource.
	 */
	...
	public void writeAllEntries(ObjectOutputStream ostream)
		...
		try {
			...
			while (result.next()) {
				...
				EndpointReferenceType targetEPR = EPRUtils.fromBlob(result
						.getBlob(3));
