...
public class GeniiResolverUtils
{
	...
	static public final String NEXT_TARGET_ID_PROP_NAME =
		"edu.virginia.vcgr.genii.resolver.next-id";

	/**
	 * Generates a new EPR that is the given original EPR plus a Resolver element
	 * added to the the metadata element.  
	 */
	static public EndpointReferenceType createResolutionEPR(IGeniiResolverResource resource,
			EndpointReferenceType targetEPR, EndpointReferenceType resolverEPR, int targetID)
	...
	/**
	 * Subscribe to the "terminate" topic of the new EPR,
	 * so that the resolver will be notified if the new resource is destroyed.
	 */
	static public EndpointReferenceType createTerminateSubscription(int targetID,
			EndpointReferenceType targetEPR, EndpointReferenceType resolverEPR,
			IResource resource)
		throws ResourceException
	{
		// _logger.debug("resolver: createTerminateSubscription()");
		EndpointReferenceType subscriptionEPR = null;
		try
		{
			if (resolverEPR == null)
			{
				resolverEPR = (EndpointReferenceType) WorkingContext.getCurrentWorkingContext().
	...
	/**
	 * Return a new EPR with the contents of the original EPR plus the given additionalUserInfo.
	 * The original EPR is unmodified.  However, it shares data structures with the new EPR,
	 * so the new EPR should be treated as read-only.
	 */
	public static EndpointReferenceType createUserInfoEPR(EndpointReferenceType epr,
			String key, Serializable data, boolean doClear)
	{
	...
	/**
	 * If this resolver is adding a resource that is a replica of itself,
	 * then it sets a property that indicates that when it returns its own EPR,
	 * it should include this resource as a resolver.
	 */
	...
	static public void initializeNextTargetIDinReplica(IGeniiResolverResource resource)
		throws ResourceException
	{
