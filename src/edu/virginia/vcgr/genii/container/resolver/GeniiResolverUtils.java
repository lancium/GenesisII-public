...
public class GeniiResolverUtils {
	...
	/**
	 * Generates a new EPR that is the given original EPR plus a Resolver
	 * element added to the the metadata element.
	 */
	static public EndpointReferenceType createResolutionEPR(
			IGeniiResolverResource resource, EndpointReferenceType targetEPR,
			EndpointReferenceType resolverEPR, int targetID)
		...
		// The target EPR contains the resolver EPR (in case the target address
		// is down),
		// and the resolver EPR contains the target EPR's ID (so that the
		// resolver can select
		// another replica when the target EPR is down).
		resolverEPR = createUserInfoEPR(resolverEPR,
				GeniiResolverServiceImpl.GENII_RESOLVER_TARGET_ID_KEY,
				new Integer(targetID), true);
		resolvers.add(new ResolverDescription(targetEPI, resolverEPR,
				ResolverType.EPI_RESOLVER));

		Integer nextID = (Integer) resource
				.getProperty(NEXT_TARGET_ID_PROP_NAME);
		if (nextID != null) {
	...
	/**
	 * Subscribe to the "terminate" topic of the new EPR, so that the resolver
	 * will be notified if the new resource is destroyed.
	 */
	static public EndpointReferenceType createTerminateSubscription(
			int targetID, EndpointReferenceType targetEPR,
			EndpointReferenceType resolverEPR, IResource resource)
			throws ResourceException {
		EndpointReferenceType subscriptionEPR = null;
		try {
			if (resolverEPR == null) {
				resolverEPR = (EndpointReferenceType) WorkingContext
			...
			SimpleResolverTerminateUserData userData = new SimpleResolverTerminateUserData(
					targetEPI, targetID);
			...
			if (_logger.isDebugEnabled())
				_logger.debug(
						"Could not create subscription to resource termination.",
						exception);
		}
	...
	/**
	 * Return a new EPR with the contents of the original EPR plus the given
	 * additionalUserInfo. The original EPR is unmodified. However, it shares
	 * data structures with the new EPR, so the new EPR should be treated as
	 * read-only.
	 */
	public static EndpointReferenceType createUserInfoEPR(
			EndpointReferenceType epr, String key, Serializable data,
			boolean doClear) {
		...
		try {
			...
			if (_logger.isDebugEnabled())
				_logger.debug("Could not create AddressingParameters.",
						exception);
		}
		return new EndpointReferenceType(epr.getAddress(), refParams,
				epr.getMetadata(), epr.get_any());
	}

	/**
	 * If this resolver is adding a resource that is a replica of itself, then
	 * it sets a property that indicates that when it returns its own EPR, it
	 * should include this resource as a resolver.
	 */
	static public int[] initializeNextTargetID(IGeniiResolverResource resource,
			URI targetEPI, int[] targetIDList) throws ResourceException {
	...
	static public void initializeNextTargetIDinReplica(
			IGeniiResolverResource resource) throws ResourceException {
		resource.setProperty(NEXT_TARGET_ID_PROP_NAME, new Integer(0));
	}

	static public void updateNextTargetID(IGeniiResolverResource resource,
			URI targetEPI, int targetID) throws ResourceException {
		// Not this EPI? Do nothing.
		EndpointReferenceType myEPR = (EndpointReferenceType) WorkingContext
				.getCurrentWorkingContext().getProperty(
						WorkingContext.EPR_PROPERTY_NAME);
