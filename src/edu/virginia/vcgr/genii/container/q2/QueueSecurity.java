...
public class QueueSecurity {
	...
	static public Collection<Identity> getCallerIdentities(
		...
		try {
			...
			if (filterOutGroups)
				return SecurityUtilities.filterCredentials(
						KeystoreManager.getCallerIdentities(callingContext),
	 ...
	 * supplied job owners. We have a match if any of the identities in the
	 * current calling context matches any of the identities in the listed job
	 * owners.
	 * 
	 * @param jobOwners
	 *            The list of job owners that we are checking against.
	 * 
	 * @return True if the calling context represents an owner of the indicated
	 *         job owners, false otherwise.
	 * 
	 * @throws AuthZSecurityException
	 */
	...
	static public boolean isOwner(Collection<Identity> jobOwners,
			Collection<Identity> callers) throws AuthZSecurityException {
	...
	/**
	 * Check to see if any of the caller's identities match the identity of the
	 * job given.
	 * 
	 * @param callers
	 ...
	 * 
	 * @return True if the caller has an identity that matches the indicated job
	...
	/**
	 * Converts a list of Identities into an array of byte arrays. We do this so
	 * that they identities can easily be serialized into a SOAP message.
	 * 
	 * @param identities
	...
	public static boolean isQueueAdmin() throws AuthZSecurityException {
		...
		try {
			...
			IAuthZProvider authZHandler = AuthZProviders.getProvider(rKey
					.getServiceName());
			...
			throw new AuthZSecurityException(
					"Unable to load queue Administrators", e);
