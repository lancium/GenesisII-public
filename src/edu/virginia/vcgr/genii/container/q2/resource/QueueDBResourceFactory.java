...
public class QueueDBResourceFactory extends BasicDBResourceFactory {
	...
	static private Log _logger = LogFactory
			.getLog(QueueDBResourceFactory.class);
	...
	static private final String[] _CREATE_STMTS = new String[] {
			...
			"CREATE TABLE q2eprs (" + "queueid VARCHAR(256) PRIMARY KEY, "
					+ "queueepr BLOB(2G) NOT NULL)",
			"CREATE TABLE q2errors ("
					+ "errorid BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, "
					+ "queueid VARCHAR(256) NOT NULL, "
					+ "jobid BIGINT NOT NULL, " + "attempt SMALLINT NOT NULL, "
					+ "errors BLOB(2G))",
			"CREATE TABLE q2logs (" + "jobid BIGINT PRIMARY KEY, "
					+ "queueid VARCHAR(256) NOT NULL, "
					+ "logtarget BLOB(2G) NOT NULL, "
					+ "logepr BLOB(2G) NOT NULL)",
			"CREATE TABLE q2joblogtargets("
			...
			"CREATE TABLE q2jobpings (" + "jobid BIGINT PRIMARY KEY,"
					+ "failedcommattempts INTEGER NOT NULL)",
			"create table security_headers(jobid BIGINT PRIMARY KEY,"
					+ "certificate BLOB(2G))" };

	// a command that will add newer rpcid field to the queue jobs table
	// structure.
	static private final String _ADD_RPC_ID_COLUMN = "ALTER TABLE q2jobs ADD COLUMN rpcid VARCHAR(256)";

	public QueueDBResourceFactory(ServerDatabaseConnectionPool pool)
			throws SQLException {
		super(pool);
	}
