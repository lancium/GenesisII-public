...
public class QueueDBResource extends BasicDBResource implements IQueueResource {
	...
	public void destroy() throws ResourceException {
		...
		try {
			stmt = _connection
					.prepareStatement("DELETE FROM q2resources WHERE queueid = ?");
			...
			stmt = null;
			stmt = _connection
					.prepareStatement("DELETE FROM q2jobs WHERE queueid = ?");
			stmt.setString(1, _resourceKey);
			stmt.executeUpdate();
			...
			stmt = null;
			stmt = _connection
					.prepareStatement("DELETE FROM q2eprs WHERE queueid = ?");
			stmt.setString(1, _resourceKey);
			stmt.executeUpdate();
			...
			stmt = null;
			stmt = _connection
					.prepareStatement("DELETE FROM q2errors WHERE queueid = ?");
			stmt.setString(1, _resourceKey);
			stmt.executeUpdate();
			...
			stmt = null;
			stmt = _connection
					.prepareStatement("DELETE FROM q2logs WHERE queueid = ?");
			stmt.setString(1, _resourceKey);
			stmt.executeUpdate();
			...
			stmt = null;
			stmt = _connection
					.prepareStatement("DELETE FROM q2joblogtargets WHERE queueid = ?");
			stmt.setString(1, _resourceKey);
			stmt.executeUpdate();
			...
			stmt = null;
			stmt = _connection
					.prepareStatement("DELETE FROM q2jobhistorytokens WHERE queueid = ?");
			stmt.setString(1, _resourceKey);
			stmt.executeUpdate();

			HistoryContainerService service = ContainerServices
					.findService(HistoryContainerService.class);
			service.deleteRecordsLike(_connection,
					String.format("q2:%s:%%", _resourceKey));
	...
	public void setEPR(EndpointReferenceType epr) throws ResourceException {
		...
		try {
			stmt = _connection
					.prepareStatement("INSERT INTO q2eprs (queueid, queueepr) VALUES (?, ?)");
			stmt.setString(1, _resourceKey);
