...
public class JobUpdateWorker implements OutcallHandler {
	...
	public void run() {
		...
		try {
			...
			try {
				jobEndpoint = _jobEndpointResolver.getJobEndpoint(connection,
						_jobInfo.getJobID());
			} catch (Throwable cause) {
				String message = String
						.format("Failure to get job endpoint on job %s with connection=%s jobId=%s",
								_data, connection, _jobInfo.getJobID());
			...
			if (jobEndpoint == null) {
				history.createDebugWriter("Job No Longer Running")
						.format("Job is no longer marked as running "
								+ "(anticipated asynchronous behavior).")
						.close();

				// Another thread has removed this from the database.
				if (_logger.isDebugEnabled())
					_logger.debug(String
							.format("Asked to check status on job %s which is "
			...
			if (oldAction != null) {
				...
				_logger.error(String.format(
						"Attempting to check job status for %s, found that "
								+ "we are in the process of doing action:  "
								+ oldAction, _data));
				return;
			}
			...
			try {
				...
				activityStatuses = new GetActivityStatusResponseType[] { new GetActivityStatusResponseType(
						jobEndpoint, new ActivityStatusType(null,
			...
			if (activityStatuses == null || activityStatuses.length != 1) {
				history.createWarnWriter("Job Status Check Failed")
						.format("BES didn't return the expected information for the job.")
						.close();
				...
				if (_logger.isDebugEnabled())
					_logger.debug("do we need this oldState object due to side effects?  contains: "
							+ oldState);

				_data.setBESActivityStatus(activityStatuses[0]
						.getActivityStatus());
				try {
					if (activityStatuses[0] != null) {
						...
						if (fault != null) {
							...
							PrintWriter faultWriter = history
									.createErrorWriter("Job Faulted").format(
											"Job threw fault:\n");
							try {
								ObjectSerializer
										.serialize(faultWriter, fault,
					...
					_logger.error(String.format(
							"Error trying to get fault detail for job %s.",
							_data), cause);
				}

				/*
				 * if (faults == null) faults = new Vector<String>(1);
				 * faults.add(0, String.format("Job ran on BES resource %s.",
				 * besName == null ? "<unknown>" : besName));
				 */
				if (faults != null && faults.size() > 0) {
					_jobManager
							.addJobErrorInformation(connection,
									_jobInfo.getJobID(),
									_data.getRunAttempts(), faults);
				}

				if (wasSecurityException) {
					history.createDebugWriter("Certificate Expired")
							.format("It looks like the certificate for the job might have expired.");

					Collection<String> messages = new Vector<String>();
					messages.add("The certificates for this job have expired.");
					_jobManager.addJobErrorInformation(connection,
							_jobInfo.getJobID(), _data.getRunAttempts(),
							messages);
				}
			...
			_logger.warn("Unable to update job status for job " + _data, cause);
			try {
				...
				_logger.warn(String.format(
						"Unable to update database for failed job %s.", _data),
						cause2);
			}
		} finally {
