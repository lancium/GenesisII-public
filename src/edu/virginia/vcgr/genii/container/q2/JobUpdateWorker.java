		...
		try {
			...
			if (activityStatuses == null || activityStatuses.length != 1) {
				...
				try {
					...
					_logger.error(String.format("Error trying to get fault detail for job %s.", _data), cause);
				}
			...
			try {
				...
				_logger.warn(String.format("Unable to update database for failed job %s.", _data), cause2);
			}
		} finally {
