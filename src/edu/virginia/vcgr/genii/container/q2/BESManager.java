...
public class BESManager implements Closeable {
	 ...
	 * been non-responsive, in which case the update cycle is 5 minutes *
	 * 2^(MIN(numberOfMissedUpdates, 10)).
	 */
	...
	/**
	 * The scheduling event is an object which allows code to raise and wait on
	 * events indicating that a good opportunity exists to schedule a job.
	...
	/**
	 * A map of container ID to update information. Right now this is basically
	 * information about when it was updated and what the result was, but
	 * eventually it could help us determine what the "characteristics" of that
	 * container are.
	 */
	...
	public BESManager(QueueDatabase database, SchedulingEvent schedulingEvent,
		...
		/*
		 * Create the updater worker. This object will continually update the
		 * bes resources on a regular basis. We know what the update frequency
		 * is from a constant, but we'll actually have the "updater" object wake
		 * up 10 times as often to check. This is essentially me being lazy but
		 * its a lot easier to have the guy wake up frequently and check to see
		 * if anyone needs an update.
		 */
		_updater = new BESResourceUpdater(connectionPool, this,
				_BES_UPDATE_CYCLE / 10);
	}
	...
	public void addNewBES(Connection connection, String name,
		...
		Collection<BESUpdateInformation> toUpdate = new ArrayList<BESUpdateInformation>(
				1);

		/*
		 * Add the new information into the database and get back the new DB key
		 * for it.
		 */
		...
		synchronized (this) {
			/*
			 * Go ahead and put the "in-memory" information into the correct
			 * lists.
			 */
			BESData data = new BESData(id, name, 1);
			_containersByID.put(new Long(id), data);
			_containersByName.put(name, data);
			_updateInformation.put(new Long(id),
					updateInfo = new BESUpdateInformation(id,
							_BES_UPDATE_CYCLE, _MISS_CAP));
		}

		/*
		 * Finally, go ahead and kick off an update of this information. By
		 * doing this early, we could potentially get a near simultaneous update
		 * from our update thread, but it's OK...two updates won't hurt us.
		 */
	...
	private class BESAttributePrefetcher implements AttributePreFetcher {
		...
		public Collection<MessageElement> preFetch() {
			...
			ret.add(new MessageElement(
					GenesisIIBaseRP.PERMISSIONS_STRING_QNAME, "r--r--"));
			return ret;
		}
	}

	/**
	 * List all BES's contained in the queue which match a given regular
	 * expression.
	 * 
	 * @param connection
	 *            The database connection to use.
	 ...
	 * @return The list of bes entries contained in the queue with the given
	 *         pattern.
	 * 
	 * @throws SQLException
	 * @throws ResourceException
	...
	synchronized public Collection<LegacyEntryType> listBESs(
			Connection connection, String entryName) throws SQLException,
			ResourceException {
		HashMap<Long, LegacyEntryType> ret = new HashMap<Long, LegacyEntryType>();

		/*
		 * Go through all bes containers in the queue, looking for the right
		 * ones.
		 */
		for (BESData data : _containersByID.values()) {
			...
			if (entryName == null || entryName.equals(data.getName())) {
				/*
				 * If so, add it's entry information (but leave the EPR blank,
				 * we'll back-fill that in a second.
				 */
				ret.put(new Long(data.getID()),
						new LegacyEntryType(data.getName(),
								new BESAttributePrefetcher().preFetch()
										.toArray(new MessageElement[0]), null));
			}
		}

		/*
		 * Now that we have all of the entries we want to return, back-fill the
		 * EPRs in those entries by going to the database.
		 */
	...
	synchronized public Collection<String> removeBESs(Connection connection,
		...
		for (BESData data : _containersByID.values()) {
			if (entryName == null || entryName.equals(data.getName())) {
				/*
				 * We found a match. Add the name to a return list. We'll also
				 * keep a list of matching data structures for containers that
				 * we have to delete from the database in a second.
				 */
		...
		/*
		 * Now that we have all of the containers that matched, go ahead and
		...
		/*
		 * It's been committed. Everything after this can be restored from DB if
		 * the JVM crashes or the container goes down.
		 */

		/*
		 * Now we have to go through all of our in-memory lists and remove the
		 * records
		 */
		for (BESData data : toRemove) {
			...
			if (_logger.isDebugEnabled())
				_logger.debug("Removed bes container " + data.getID()
						+ " from queue.");
		}
	...
	synchronized public void configureBES(Connection connection, String name,
		...
		/*
		 * The update information has been committed to the DB. We can now
		 * restore from load if necessary.
		 */

		/*
		 * set the in memory representation of the slots (and keep track of the
		 * old one)
		 */
		...
		if (_logger.isDebugEnabled())
			_logger.debug("BES resource " + data.getID()
					+ " configured to have " + newSlots + " slots.");

		/*
		 * If we increased the number of slots, we have a scheduling opportunity
		 * here
		 */
	...
	/**
	 * Make out-calls to the indicated resources to get their latest update
	 * information.
	 * 
	 * @param connection
	 *            The database connection.
	...
	private void updateResources(Connection connection,
			...
			throws SQLException, ResourceException, GenesisIISecurityException {
		/*
		 * The queue uses the same calling context for ALL BES updates (it's
		 * own). Load that context from the database.
		 */
		ICallingContext queueCallingContext = _database
				.getQueueCallingContext(connection);
		connection.commit();
		...
		for (BESUpdateInformation info : resourcesToUpdate) {
			 ...
			 * the updater worker to "resolve" the bes id into it's EPR. This
			 * allows us to delay loading the EPR into memory (a potentially
			 * memory-expensive prospect) until the last possible minute. This
			 * way, we guarantee that the maximum number of EPRs loaded at any
			 * time is equal to the maximum number of out call threads in use.
			 */
			...
			_informationPortal.getInformation(
					new BESEndpoint(_database.getQueueID(), info.getBESID(),
							besName, resolver),
					new InformationUpdateListener(), force);
		}
	}

	private class InformationUpdateListener implements
			InformationListener<BESInformation> {
		@Override
		public void informationUpdated(InformationEndpoint endpoint,
			...
			if (information.wasResponsive()) {
				...
				markBESAsMissed(besEndpoint.getBESID(), String.format(
						"Exception during communication %s(%s)", information
								.exception().getClass().getName(), information
	...
	/**
	 * Update ALL resources (that need the update) currently linked into the
	 * queue.
	 * 
	 * @param connection
	 *            The database connection.
	 ...
	 * @throws GenesisIISecurityException
	 */
	public void updateResources(Connection connection) throws SQLException,
			ResourceException, GenesisIISecurityException {
		/*
		 * We need to collect a list of BES resources that are ready for an
		 * update.
		 */
		...
		synchronized (this) {
			/*
			 * Loop through all containers (their update information structures
			 * actually)
			 */
	...
	synchronized public void markBESAsAvailable(long besID) {
		/*
		 * Get it's update information from memory and note that it's now
		 * responsive and available.
		 */
		BESUpdateInformation updateInfo = _updateInformation
				.get(new Long(besID));
		updateInfo.update(true);

		/*
		 * See if that resource is currently listed in the "available" resources
		 * list.
		 */
		if (!_scheduleableContainers.containsKey(new Long(besID))) {
			/*
			 * If it isn't, we need to add it to that list. Get's it's in memory
			 * data structure and put the data structure into the available
			 * list.
			 */
			...
			/*
			 * Finally, if the thing had more then 0 slots allocated to it, the
			 * fact that it is now available (and wasn't before) means that we
			 * have a new scheduling opportunity.
			 */
	...
	/**
	 * Similar to markBESAsAvailable, we are marking a resource as unavailable.
	 * Unavailable doesn't mean that the container isn't responding. It simply
	 * means that the container won't take jobs right now for some reason.
	 * 
	 * @param besID
	...
	synchronized public void markBESAsUnavailable(long besID, String reason) {
		...
		/*
		 * Remove the structure from the available list (if it isn't there, this
	...
	/**
	 * Similar to markBESAsAvailable, we are marking a resource as missed.
	 * Missed implies that we couldn't talk to the container at all (it isn't
	 * responsive).
	 * 
	 * @param besID
	...
	synchronized public void markBESAsMissed(long besID, String reason) {
		...
		if (data != null)
			_logger.info(String.format(
					"Marking BES container \"%s\" as unresponsive.  %s.",
					data.getName(), reason));
	}

	/**
	 * Simple getter method to return the list of all currently available
	 * resources.
	 * 
	...
	/**
	 * An internal class that we use to "late-bind" BES keys to the BES
	 * container's EPR. This allows us to avoid ever loading an excessive number
	 * of EPRs into memory at any given time.
	 * 
	 * @author mmm2a
	 */
	private class BESPortTypeResolver implements IBESPortTypeResolver {
		/**
		 * The calling context that we will use to make out calls. This needs to
		 * get used during the client-stub's creation.
		 */
		...
		public GeniiBESPortType createClientStub(Connection connection,
			 ...
			 * already has a method to do this by filling in EntryType instances
			 * (used by the list method). We'll just re-use that operation by
			 * creating a faux entry type.
			 */
	...
	synchronized public void summarize(PrintStream out) throws IOException,
			SQLException {
