...
/**
 * This is the main data structure for keeping all information about a job that
 * is small enough to store in memory.
 * 
 * @author mmm2a
 */
public class JobData {
	 ...
	 * "active" state of a job. Is it in the process of being created or having
	 * it's status checked? If the value is null, then no action is going on.
	 */
	...
	/**
	 * An internal flag used to indicate that the job has been killed and needs
	 * to be cleaned up.
	 */
	...
	/**
	 * The Job's priority. Lower numbers imply a higher priority (will run
	 * first).
	 */
	...
	/**
	 * The time at which the job was submitted to the queue. This information is
	 * kept so that we can ORDER jobs by submit time (after taking priority into
	 * account).
	 */
	private Date _submitTime;

	/**
	 * The BES ID associated with this job. Notice that we use the wrapper class
	 * for longs here which allows this field to be null. If it is null, then
	 * the job hasn't been matched to a resource yet. If the besID is non-null,
	 * then it is in fact the bes key of the bes resource that we are running on
	 * (or starting on).
	 */
	private Long _besID;
	...
	public JobData(long jobID, String jobName, String jobTicket,
			short priority, QueueStates jobState, Date submitTime,
	...
	public JobData(long jobID, String jobName, String jobTicket,
		...
		this(jobID, jobName, jobTicket, priority, jobState, submitTime,
				runAttempts, null, history, loggingContext);
	}
	...
	public void setBESActivityStatus(ActivityStatusType ast) {
		...
		try {
			...
			_besActivityStatus = (ActivityStatusType) ObjectDeserializer
