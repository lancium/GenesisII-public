...
public abstract class MatchingParameter {
	...
	static private MatchingParameter createDefaultMatchingParameter(
			MessageElement e, Boolean fromJSDL) {
		return new DefaultMatchingParameter(
				e.getAttribute(GeniiPropertyFacet.PROPERTY_NAME_ATTRIBUTE),
				e.getAttribute(GeniiPropertyFacet.PROPERTY_VALUE_ATTRIBUTE),
				fromJSDL);
	}
	...
	static private MatchingParameter createOrMatchingParameter(MessageElement e) {
		...
		while (iter.hasNext()) {
			...
			DefaultMatchingParameter child = new DefaultMatchingParameter(
					...
					element.getAttribute(GeniiPropertyFacet.PROPERTY_VALUE_ATTRIBUTE),
					true);
			if (child != null)
	...
	static Collection<MatchingParameter> matchingParameters(
			MessageElement[] any, Boolean fromJSDL) {
