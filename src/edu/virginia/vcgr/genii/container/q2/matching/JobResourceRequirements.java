...
public class JobResourceRequirements
{
	...
	public boolean equals(JobResourceRequirements other)
	{
		...
		if (!equalsWithNulls(_gpuarch, other._gpuarch))
			return false;
	...
	public boolean matches(BESInformation besInfo)
	{
		...
		if (_osVersion != null) {
			...
			if (bOsVersion == null) {
				_logger.warn("BES does not have an operating system version.");
				return false;
			}
		...
		if (_memoryRequirement != null) {
			...
			if (bMemory == null) {
				_logger.warn("BES does not have a physical memory attribute.");
				return false;
			}
	...
	public String toString()
	{
		...
		appendParameter(parameters, "GPU Arch", _gpuarch);
