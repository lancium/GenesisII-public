...
/**
 * This is the asynchronous worker class that periodically checks the status of
 * running jobs on a regular interval.
 * 
 * @author mmm2a
 */
public class JobStatusChecker {
	...
	private long _updateFrequency; // provided by caller for the slower status
									// checking interval.
	private int STATUS_THREAD_PERIOD = 5 * 1000; // notified status update
													// checking interval, in
													// milliseconds.
	// tracks when the next long running status check should occur.
	private volatile Calendar _nextSlowCheck;

	public JobStatusChecker(ServerDatabaseConnectionPool connectionPool,
	...
	private class UpdaterWorker implements Runnable {
		...
		private void performSlowRunningCheck() {
			...
			try {
				/*
				 * Acquire a connection from the connection pool and ask the
				 * manager to check the job statuses.
				 */
		...
		public void run() {
			...
			while (!_closed) {
				// we do the checks on notifications every time through the
				// loop.
				performNotifiedStatusChecks();
				// we only do the periodic status check every so often, as
				// controlled by the update
				// frequency.
				if (_nextSlowCheck.before(Calendar.getInstance())) {
					performSlowRunningCheck();
					_nextSlowCheck = Calendar.getInstance();
					_nextSlowCheck.add(Calendar.MILLISECOND,
