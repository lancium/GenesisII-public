 ...
 * opportunities. An opportunity is not a guarantee that a job can be schedule,
 * but rather a strong suspician that one can. It's up to the scheduler to
 * determine if this is true and to actually do the matching.
 * 
 * @author mmm2a
 */
public class SchedulingEvent {
	/**
	 * A simple boolean which keeps track of wether or not there is an
	 * opportunity.
	 */
	private boolean _schedulingEvent = false;

	/**
	 * A Java Object instances whose sole purpose in life is to give us an
	 * object to lock, wait on, and signal on.
	 */
	...
	/**
	 * This operation is identical to the one above, except that it never times
	 * out.
	 * 
		...
		synchronized (_lock) {
			while (true) {
				...
				if (_nextScheduledEvent != null)
					timeout = (_nextScheduledEvent.getTime() - System
