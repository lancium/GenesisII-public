...
public class DBSubscriptionResource extends BasicDBResource {
	...
	void createSubscription(EndpointReferenceType subscriptionReference,
			SubscriptionConstructionParameters cons) throws ResourceException {
		try {
			SubscriptionsDatabase.createSubscription(_connection, _resourceKey,
					cons.publisherResourceKey(), subscriptionReference,
					cons.consumerReference(), cons.topicQuery(),
					cons.policies(), cons.additionalUserData());
	...
	void toggleSubscriptionPause(boolean markPaused) throws ResourceException {
		try {
			SubscriptionsDatabase.toggleSubscriptionPause(_connection,
					_resourceKey, markPaused);
		} catch (SQLException e) {
			throw new ResourceException(
