...
void GenesisInitializeFCB(PGENESIS_FCB fcb){ 	
	...
	fcb->GenesisFileID = GENII_FILE_INVALID;

	KeInitializeSemaphore(&(fcb->FcbPhore), 1, 1);
}

void GenesisInitializeCCB(PGENESIS_CCB ccb){
	char buffer[256];
	RtlFillMemory(buffer, 256, ' ');
	buffer[255] = '\0';

	ccb->FileIndex = 0;
	ccb->CurrentByteOffset.QuadPart = 0;
	RtlInitString(&(ccb->Target), buffer);

	ccb->IsTargetSet = FALSE;
}
...
--*/
{
    ...
    if( NetRoot->Type == NET_ROOT_DISK && NT_SUCCESS(Status) ) {
		...
		if(NewFileObject!= NULL){
			DbgPrint("NulMrxCreate:  Attempt to open FILE OBJECT %wZ\n", NewFileObject->FileName.Buffer);
		}
		...
		if(PtrIoStackLocation->Parameters.Create.EaLength > 0){
			...
			DbgPrint("NulMrxCreate:  Failed! EaLength > 0\n");
			try_return(Status);
		}
...
--*/
{
	...
	GenesisInitializeCCB((PGENESIS_CCB)RxContext->pFobx->Context2);
