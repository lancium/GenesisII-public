...
testCreatingFileOnMount()
{
  ...
  cp $TEST_TEMP/local-file.txt "$TESTING_DIR/testing-file-in-grid.txt"
  assertEquals "Testing copy of local file to mounted grid folder." 0 $?

  diff "$TESTING_DIR/testing-file-in-grid.txt" "$TEST_TEMP/local-file.txt"
...
testCopyingFromMountToLocal()
{
  ...
  cp "$TESTING_DIR/testing-file-in-grid.txt" "$TEST_TEMP/local-file1.txt"
...
{
  if ! fuse_supported; then return 0; fi
  diff "$TEST_TEMP/local-file.txt" "$TESTING_DIR/testing-file-in-grid.txt"
  assertEquals "Diff the local-file with the grid-file" 0 $?

  diff "$TESTING_DIR/testing-file-in-grid.txt" "$TEST_TEMP/local-file1.txt"
