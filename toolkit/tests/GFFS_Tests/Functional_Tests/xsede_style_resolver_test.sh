...
function testCreatingResolvers()
{
  ...
  bash $GFFS_TOOLKIT_ROOT/tools/xsede_admin/top_level_replication_setup.sh localhost $BACKUP_PORT_NUMBER
...
function createUserAndReplicate()
{
  ...
  bash $GFFS_TOOLKIT_ROOT/library/create-user-and-group.sh \
...
function testReplicationSoFar()
{
  ...
  bash "$GFFS_TOOLKIT_ROOT/library/zap_genesis_javas.sh" "$DEPLOYMENT_NAME"
