...
testGoodUsersProvided()
{
  ...
  run_any_command expect "$GFFS_TOOLKIT_ROOT/library/ssh_expecter.tcl" $Flimsy $FlimsyPassword localhost "echo hello venus"
  reval=$?
...
{
  run_any_command bash "$GFFS_TOOLKIT_ROOT/library/set_acls.sh" "$Flimsy" "$LOCAL_PATH" "000"
  assertEquals "Revoking all rights for Flimsy account $Flimsy should work" 0 $?
  run_any_command bash "$GFFS_TOOLKIT_ROOT/library/set_acls.sh" "$Grantee" "$LOCAL_PATH" "rwx"
  assertEquals "Adding all rights for Flimsy account $Flimsy should work" 0 $?
}

testCheckFlimsyCannotAccess()
{
  run_any_command expect "$GFFS_TOOLKIT_ROOT/library/ssh_expecter.tcl" $Flimsy $FlimsyPassword localhost "echo hello venus >$LOCAL_PATH/gerkins.txt" "mkdir $LOCAL_PATH/grommet"
  reval=$?
  # make sure we didn't experience a failure on the other side.
  grep YO_FAILURE $GRID_OUTPUT_FILE &>/dev/null
  if [ $? -eq 0 ]; then
...
testLicenseeCanAccess()
{
  ...
  grid cp local:"$GFFS_TOOLKIT_CONFIG_FILE" grid:$EXPORTED_GRID_PATH
