...
#hmmm: hide output from complaining failures where resolver already exists?
  ...
  "$GENII_INSTALL_DIR/bin/grid" resolver $resolver_parm_line $item /etc/resolvers/rootResolver 
  # if it already had a resolver or we couldn't create one, we don't want
  # to replicate it.
  if [ $? -eq 0 ]; then
    "$GENII_INSTALL_DIR/bin/grid" replicate $replicate_parm_line $item /resources/xsede.org/containers/gffs-2.xsede.org
  fi
}
...
function replicate_resource_hierarchy()
{
  ...
  "$GENII_INSTALL_DIR/bin/grid" ls $service_provider_path \
  ...
  while read top_level_item; do
    if [ ! -z "$top_level_item" ]; then
      ...
      echo "  [$outer_item]"
      "$GENII_INSTALL_DIR/bin/grid" ls $outer_item \
...
function chmod_for_all()
{
  ...
  "$GENII_INSTALL_DIR/bin/grid" <<eof &>"$errchk"
...
function create_resources_structure()
{
  ...
  "$GENII_INSTALL_DIR/bin/grid" mkdir -p $service_provider_path
  if [ $? -ne 0 ]; then
...
function link_sp_container()
{
  ...
  "$GENII_INSTALL_DIR/bin/grid" ln --service-url=$g2_url $service_provider_path/containers/$container_name
  "$GENII_INSTALL_DIR/bin/grid" ls $service_provider_path/containers/$container_name | grep -q "Services"
  if [ $? -ne 0 ]; then
  ...
  "$GENII_INSTALL_DIR/bin/grid" script local:./configure_container_permissions.xml $service_provider_path/containers/$container_name $grid_identity
}
...
function create_unicore_bes()
{
  ...
  "$GENII_INSTALL_DIR/bin/grid" mint-epr --link=$service_provider_path/bes-containers/$bes_name --certificate-chain=local:$certificate_path $u6_url
}

function create_queue_for_bes()
{
  echo "Creating $service_provider_path/queues/$bes_name-queue queue resource" 
  "$GENII_INSTALL_DIR/bin/grid" create-resource $service_provider_path/containers/$container_name/Services/QueuePortType $service_provider_path/queues/$bes_name-queue
  chmod_for_all $service_provider_path/queues/$bes_name-queue

  echo "Giving queue permissions on BES resource"
  "$GENII_INSTALL_DIR/bin/grid" chmod $service_provider_path/bes-containers/$bes_name +rx $service_provider_path/queues/$bes_name-queue &>/dev/null
  ...
  "$GENII_INSTALL_DIR/bin/grid" ln $service_provider_path/bes-containers/$bes_name $service_provider_path/queues/$bes_name-queue/resources/$bes_name
  ...
  "$GENII_INSTALL_DIR/bin/grid" qconfigure $service_provider_path/queues/$bes_name-queue $bes_name $num_slots
}
...
echo "----------------------"
"$GENII_INSTALL_DIR/bin/grid" whoami
echo "----------------------"
