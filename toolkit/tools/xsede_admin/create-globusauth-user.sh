...
if [ ! -z "$USE_FASTGRID" ]; then
  GRIDTOOL=$GENII_INSTALL_DIR/bin/fastgrid
else
  GRIDTOOL=$GENII_INSTALL_DIR/bin/grid
fi
...
$GRIDTOOL ping --eatfaults "/users/globus-auth/$username" &>/dev/null
if [ $? -eq 0 ]; then
...
# xsede.org identity.
$GRIDTOOL ping --eatfaults "/users/xsede.org/$username" &>/dev/null
if [ $? -ne 0 ]; then
...
$GRIDTOOL ping --eatfaults "$containerpath/Services/KerbAuthnPortType/$username" &>/dev/null
if [ $? -ne 0 ]; then
...
$GRIDTOOL ping --eatfaults "/users/globus-auth" &>/dev/null
if [ $? -ne 0 ]; then
  $GRIDTOOL mkdir "/users/globus-auth"
  check_if_failed "creation of /users/globus-auth hierarchy"
  $GRIDTOOL chmod "/users/globus-auth" +r --everyone
  check_if_failed "giving everyone permission to read globus auth dir"
fi

# then create the idp using idptool.
$GRIDTOOL idp --type=USER --validDuration=10years "$containerpath/Services/GlobusAuthnPortType" "$username"
...
$GRIDTOOL ln "$containerpath/Services/GlobusAuthnPortType/$username" "/users/globus-auth/$username"
...
$GRIDTOOL ln "/groups/xsede.org/gffs-users" "/users/globus-auth/$username/gffs-users"
...
$GRIDTOOL chmod "/users/xsede.org/$username" +rx "/users/globus-auth/$username"
...
$GRIDTOOL ln "/users/xsede.org/$username" "/users/globus-auth/$username/kerb-${username}" 
