...
present=$("$GENII_INSTALL_DIR/bin/grid" ls /etc/resolvers | grep rootResolver)
if [ ! -z "$present" ]; then
  echo Setting permissions on rootResolver.
  "$GENII_INSTALL_DIR/bin/grid" chmod /etc/resolvers/rootResolver +w /groups/xsede.org/gffs-admins
  "$GENII_INSTALL_DIR/bin/grid" chmod /etc/resolvers/rootResolver +w /groups/xsede.org/gffs-amie
  "$GENII_INSTALL_DIR/bin/grid" chmod /etc/resolvers/rootResolver +rx --everyone
else
...
present=$("$GENII_INSTALL_DIR/bin/grid" ls /etc/resolvers | grep stsResolver)
if [ ! -z "$present" ]; then
  echo Setting permissions on stsResolver.
  "$GENII_INSTALL_DIR/bin/grid" chmod /etc/resolvers/stsResolver +w /groups/xsede.org/gffs-admins
  "$GENII_INSTALL_DIR/bin/grid" chmod /etc/resolvers/stsResolver +w /groups/xsede.org/gffs-amie
