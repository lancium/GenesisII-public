...
"$GENII_INSTALL_DIR/bin/grid" ls $MIRRORPATH >"$MIRROR_CHK_FILE"
...
"$GENII_INSTALL_DIR/bin/grid" chmod $MIRRORPATH/Services/GeniiResolverPortType \
  +rx --everyone
...
"$GENII_INSTALL_DIR/bin/grid" resolver / /etc/resolvers/rootResolver
...
"$GENII_INSTALL_DIR/bin/grid" resolver -q / >"$RESOLVER_TMP_FILE"
...
"$GENII_INSTALL_DIR/bin/grid" <<eof
...
"$GENII_INSTALL_DIR/bin/grid" ls /groups/xsede.org \
  | tail -n +2 >$HOME/group_list
while read line; do \
  if [ ! -z "$line" ]; then \
    "$GENII_INSTALL_DIR/bin/grid" resolver /groups/xsede.org/$line \
      /etc/resolvers/rootResolver; \
    check_if_failed "Adding a resolver for group $line"
    "$GENII_INSTALL_DIR/bin/grid" replicate /groups/xsede.org/$line \
...
"$GENII_INSTALL_DIR/bin/grid" logout --all
