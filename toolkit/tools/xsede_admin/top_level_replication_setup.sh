...
"$GENII_BINARY_DIR/grid" ls $MIRRORPATH >"$MIRROR_CHK_FILE"
...
"$GENII_BINARY_DIR/grid" chmod $MIRRORPATH/Services/GeniiResolverPortType \
  +rx --everyone
...
"$GENII_BINARY_DIR/grid" resolver / /etc/resolvers/rootResolver
...
"$GENII_BINARY_DIR/grid" resolver -q / >"$RESOLVER_TMP_FILE"
...
"$GENII_BINARY_DIR/grid" ls /groups/xsede.org \
  | tail -n +2 >$HOME/group_list
while read line; do \
  if [ ! -z "$line" ]; then \
    "$GENII_BINARY_DIR/grid" resolver /groups/xsede.org/$line \
      /etc/resolvers/rootResolver; \
    check_if_failed "Adding a resolver for group $line"
    "$GENII_BINARY_DIR/grid" replicate /groups/xsede.org/$line \
...
"$GENII_BINARY_DIR/grid" logout --all
