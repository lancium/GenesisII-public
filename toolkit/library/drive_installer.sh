...
if [ -z "$GENII_INSTALL_DIR" -o -z "$TEST_TEMP" -o -z "$GFFS_TOOLKIT_SENTINEL" ]; then
...
expect "$GFFS_TOOLKIT_ROOT/library/installation_expecter.tcl" "$playground/$(basename $installer_file)" "$GENII_INSTALL_DIR" "$normal_account" "$normal_password" "$port_num" "$grid_choice" "$(hostname --fqdn)" "$keypair_path" "$keypair_password"
if [ $? -ne 0 ]; then
