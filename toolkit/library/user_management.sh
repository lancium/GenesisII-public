...
function create_user() {
  ...
  silent_grid script "local:'$GFFS_TOOLKIT_ROOT/library/create_one_user.xml'" "$STS_LOC" "$user" "$full_user" "$passwd" "$grp" "$full_group" "$HOMES_LOC" "$(dirname "$full_user")"
}

# creates a group in the grid.  this group is not expected to have any
# special permissions.  this also checks if the group already exists, and
# skips creating it if so.
function create_group()
{
  ...
  local grp="$(basename $full_group)"
  silent_grid ping --eatfaults "$STS_LOC/Services/X509AuthnPortType/$grp" &>/dev/null
  if [ $? -ne 0 ]; then
    multi_grid <<eof
