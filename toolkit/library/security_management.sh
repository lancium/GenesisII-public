...
function add_in_bootstrapped_certs()
{
  local SECURITY_DIR="$ACTUAL_DEPLOYMENT_FOLDER/security"
#echo securit dir is $SECURITY_DIR
  local OWNER_CER="$SECURITY_DIR/owner.cer"
  local ADMIN_CER="$SECURITY_DIR/admin.cer"

  # the admin cert should be created by populate deployment.
...
#echo owner cer is $OWNER_CER
  ...
  # create the root signing certificate.
...
#hmmm: get the password magic below into some functional method.
  ...
  grid_chk keystoreLogin "--password='$adminpass'" "local:'$ACTUAL_DEPLOYMENT_FOLDER/security/admin.pfx'"
}
