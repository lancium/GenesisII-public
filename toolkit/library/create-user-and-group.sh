...
silent_grid script "local:'$GFFS_TOOLKIT_ROOT/library/create_one_user.xml'" "$CONTAINERPATH" "$(basename $new_username)" "$new_username" "$new_password" "$(basename "$new_group")" "$new_group" "$home_path" "$(dirname "$new_username")"
