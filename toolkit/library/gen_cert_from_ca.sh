...
if [ $# -lt 7 ]; then
  ...
  echo "3: the alias of the key to use within the signing PFX (maybe 'Container')."
  ...
  echo
  echo 'bash $GFFS_TOOLKIT_ROOT/library/gen_cert_from_ca.sh ~/GenesisII/deployments/current_grid/security/signing-cert.pfx signer signing-cert $HOME/new_cert.pfx myPassword certalias "Fred Powers"'
  exit 1
fi
