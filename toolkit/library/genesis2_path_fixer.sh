...
echo "if [[ ! \"\$1\" =~ .*\\.jar$ && ! \"\$1\" =~ .*\\.zip$ ]]; then sed -i -e 's/$seeking_path/$replacement_path/g' \$1; if [ \$? -eq 0 ]; then echo \"fixing \$1\" ; fi ; fi" >$path_fixer

# fix the root of the build.
find "$current_path" -maxdepth 1 -type f -exec bash "$path_fixer" "{}" ';'
# fix the application watcher's directory.
find "$current_path/bin" -type f -exec bash "$path_fixer" "{}" ';'
# fix the application watcher's directory.
