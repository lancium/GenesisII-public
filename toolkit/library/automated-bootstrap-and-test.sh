...
INPUTFILE_FOR_JENKINS="$GRITTY_TESTING_TOP_LEVEL/examples/toolkit_config_files/gffs_toolkit.config-jenkins-xsede"
...
sed -e "s/BACKUP_USER_DIR=.*/BACKUP_USER_DIR=\$BACKUP_USER_DIR/" \
  ...
  > "$GRITTY_TESTING_TOP_LEVEL/gffs_toolkit.config"
bail_on_fail
...
#hmmm: this needs reexamination, since port should automatically get plugged in.
bash scripts/change-port.sh "$ACTUAL_DEPLOYMENT_FOLDER" 18080 $DIFFERENT_PORT
