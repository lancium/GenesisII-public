...
if [ $# -lt 7 ]; then
  ...
  echo
  echo 'bash $XSEDE_TEST_ROOT/library/gen_cert_from_ca.sh ~/GenesisII/deployments/current_grid/security/signing-cert.pfx signer signing-cert $HOME/new_cert.pfx myPassword certalias "Fred Powers"'
  exit 1
fi
