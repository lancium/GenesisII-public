...
# check for whether we need to use the port types config that is missing JNDI auth.
#ACTIVITY_123_PORT_TYPES=
#if [ "$installer_name" == "act123_installer.install4j" ]; then
  #ACTIVITY_123_PORT_TYPES=true
#fi
...
if [ ! -z "$ACTIVITY_123_PORT_TYPES" ]; then
  replace_activity123_porttypes
fi
...
if [ ! -z "$ACTIVITY_123_PORT_TYPES" ]; then
#temporary cleanup step to make sure we don't accidentally run with the activity 123 port types.
  pushd $GENII_INSTALL_DIR
  ant clean
  popd
fi
