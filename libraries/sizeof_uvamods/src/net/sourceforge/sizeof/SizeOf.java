...
public class SizeOf
{
	static private Log _logger = LogFactory.getLog(SizeOf.class);
...
//	private static OutputStream out = System.out;
	...
	private static long deepSizeOf(Object o, Map<Object,Object> doneObj, int depth)
	{
		...
		{
			if (debug)
				print("%s{ yet computed }", indent(depth));
			return 0;
		}

		if (debug)
			print("%s{ %s", indent(depth), o.getClass().getName());
		...
		} else
		{
			...
			for (Field field : fields)
			{
				...
				{
					if (debug)
						print("%s %s = %s", indent(depth), field.getName(), obj.toString());
				}
			}
		}

		if (debug)
			print("%s} size = %s", indent(depth), humanReadable(size));

		if (_logger.isDebugEnabled()) {
			if (MIN_CLASS_SIZE_TO_LOG > 0 && size >= MIN_CLASS_SIZE_TO_LOG) {
				print("found big object: %s%s@%s size: %s", indent(depth), o.getClass().getName(), System.identityHashCode(o), humanReadable(size));
			}
		}
...
//		try
//		{
			_logger.debug(String.format(s, args));
