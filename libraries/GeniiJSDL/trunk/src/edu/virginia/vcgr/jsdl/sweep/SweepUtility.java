...
/**
 * The SweepUtility class encapsulates common methods for dealing with parameter sweeps. In particular, it has a public method for determining
 * the cardinality of a parameter sweep (that is, how many jobs a parameter sweep JSDL document represents). It also has methods for
 * performing the sweep (creating individual singleton jobs from a jsdl sweep definition).
 * 
...
{
	/**
	 * Retrieves the cardinality (number of singleton jobs) represented by a single JSDL Parameter Sweep document. If the JobDefinition given
	 * does not represent a parameter sweep, the cardinality is 1.
	 * 
	 * @param jobDef
	...
	/**
	 * Performs the parameter sweep described by the given Job Definition. This method iterates through all of the parameters represented by
	 * the given parameter sweep document and, for each single job that it produces, calls an "emit" method on a provided callback handler. If
	 * the job definition that is given does not represent a parameter sweep, then the emit method is called exactly once for the original job
	 * definition document.
	 * 
	 * @param context
	 *            The JAXBContext that will be used to marshall and unmarshall the XML jsdl document. If this parameter is null, then the
	 *            default JAXBContext for marshalling and unmarshalling JSDL will be used.
	 * @param jobDef
	 *            The job definition document to "sweep" over.
	 * @param listener
	 *            The callback listener that will receive singleton jobs produced by the parameter sweep.
	 * @return A sweep token that can be used to "block" for the sweep to finish.
	 * @throws SweepException
	 */
	static public SweepToken performSweep(JAXBContext context, JobDefinition jobDef, SweepListener listener) throws SweepException
	{
		if (context == null)
	...
	/**
	 * This method is identical to the more specific performSweep operation defined above except that it assumes that you will be using the
	 * default JAXBContext provided with this library.
	 * 
	 * @param jobDef
	 *            The job definition document to "sweep" over.
	 * @param listener
	...
	static private class SweepCreator implements Runnable
	{
		...
		private SweepCreator(SweepTokenImpl token, JobDefinition jobDefinition, List<Sweep> parameterSweeps, SweepListener listener,
			JAXBContext context)
		{
			_token = token;
