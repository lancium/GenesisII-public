...
public class WsdlUtils {
	...
	static protected String getAttribute(NamedNodeMap attrs, String attrName,
			boolean required) throws WsdlException {
		Node attrNode = attrs.getNamedItem(attrName);
		if (attrNode == null)
			if (required)
				throw new WsdlException("Unable to find \"" + attrName
						+ "\" attribute in wsdl:import node.");
			else
				return null;
	...
	static private Pattern _QNAME_PATTERN = Pattern
			.compile("^\\{([^\\}]+)\\}(.+)$");

	static public QName getQNameFromString(String str) throws WsdlException {
		Matcher m = _QNAME_PATTERN.matcher(str);
		if (!m.matches())
			throw new WsdlException("\"" + str
					+ "\" does not appear to be a qname.");
		return new QName(m.group(1), m.group(2));
	}

	static QName getQNameFromString(Node someNode, String str)
		...
		if (namespace == null) {
			Document alternativeResolver = AlternativeNamespaceResolution
					.getAlternativeResolver();
			if (alternativeResolver != null) {
				namespace = alternativeResolver.lookupNamespaceURI(str
						.substring(0, index));
			}

			if (namespace == null)
				throw new WsdlException("Couldn't find namespace for "
						+ str.substring(0, index) + " prefix.");
		}
	...
	static public String findHierarchicalAttribute(Node myNode, String attrName) {
		try {
			while (myNode != null) {
				String value = getAttribute(myNode.getAttributes(), attrName,
						false);
				if (value != null)
					return value;
