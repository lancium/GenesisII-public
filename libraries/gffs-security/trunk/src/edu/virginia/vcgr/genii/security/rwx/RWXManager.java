...
public class RWXManager {
	...
	static private HashMap<String, ClassMapEntry> retrieveClassMap(
		...
		synchronized (_cachedValues) {
			HashMap<String, ClassMapEntry> classMap = _cachedValues.get(service
					.getName());
			if (classMap == null) {
	...
	static private MappingResolver newResolver(
		...
		try {
			Constructor<? extends MappingResolver> cons = resolver
					.getConstructor();
	...
	static private void fillInClassMap(Class<?> service,
		...
		for (Method m : service.getDeclaredMethods()) {
			...
			RWXMappingResolver resolver = m
					.getAnnotation(RWXMappingResolver.class);
			if (resolver != null)
				classMap.put(methodName, new ClassMapEntry(m,
						newResolver(resolver.value())));
	...
	static private HashMap<String, ClassMapEntry> createClassMap(
			Class<?> service) {
