...
public class TestPKCS12
{
	 ...
	 */
	public static Certificate createIntermediateCert(PublicKey pubKey, PrivateKey caPrivKey, X509Certificate caCert, String cn)
		throws Exception
	{
		//
		// subject name table.
	...
	public void testPKCS()
	{
		...
		// intermediate keys.
		//
		RSAPublicKeySpec intPubKeySpec = new RSAPublicKeySpec(new BigInteger(
		...
		// ca keys
		//
		RSAPublicKeySpec caPubKeySpec = new RSAPublicKeySpec(new BigInteger(
		...
		try {
			...
			chain[2] = (X509Certificate) createIntermediateCert(caPubKey, caPrivKey, (X509Certificate) chain[3], "e");
			chain[1] = (X509Certificate) createIntermediateCert(caPubKey, caPrivKey, (X509Certificate) chain[2], "f");
			chain[0] = (X509Certificate) createIntermediateCert(caPubKey, caPrivKey, (X509Certificate) chain[1], "g");
		} catch (Exception e) {
		...
		//
		try {
			store.setKeyEntry("Eric's Key", privKey, "hello world".toCharArray(), chain);
		} catch (Exception e) {
			_logger.error("failed to create get instances of pkcs12.", e);
			exceptionsSeen++;
		}
		 ...
		 * KeyStore.TrustedCertificateEntry entry = new KeyStore.TrustedCertificateEntry(master); store2.setEntry("Bouncy Primary Certificate"
		 * , entry, null); // store2.setCertificateEntry("Bouncy Primary Certificate", chain[2]); // TrustManagerFactory tmf =
		 * TrustManagerFactory.getInstance("SunX509"); // tmf.init(store2); PKIXBuilderParameters pkixParams = new
		 * PKIXBuilderParameters(store2, new X509CertSelector()); pkixParams.setRevocationEnabled(false); ManagerFactoryParameters trustParams
		 * = new CertPathTrustManagerParameters(pkixParams); TrustManagerFactory tmf = TrustManagerFactory.getInstance("PKIX", "SunJSSE");
