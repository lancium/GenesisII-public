...
public class TestAttributeCertificate
{
	...
	public static X509Certificate createClientCert(PublicKey pubKey, PrivateKey caPrivKey, PublicKey caPubKey) throws Exception
	{
		...
		v3CertGen.addExtension(MiscObjectIdentifiers.netscapeCertType, false, new NetscapeCertType(NetscapeCertType.objectSigning
	...
	@Test
	public void testAttribCertificates()
	{
		...
		try {
			...
			_logger.error("failed to create objects for test.", e);
			exceptionsSeen++;
		}
		...
		try {
			...
			clientCert = createClientCert(pubKey, caPrivKey, caPubKey);
		} catch (Exception e) {
			_logger.error("failed to create certificates.", e);
			exceptionsSeen++;
		}
		Assert.assertEquals(0, exceptionsSeen);
		...
		try {
			...
			_logger.error("failed to set holder or issuer.", e);
			exceptionsSeen++;
		}
		Assert.assertEquals(0, exceptionsSeen);
		...
		X509V2AttributeCertificate att = null;
		try {
			// finally create the AC
			att = (X509V2AttributeCertificate) acGen.generate(caPrivKey, "BC");
		} catch (Exception e) {
			_logger.error("failed to generate AC.", e);
			exceptionsSeen++;
		}
		Assert.assertEquals(0, exceptionsSeen);
