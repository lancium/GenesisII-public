...
public class FSViewConnectionInformation implements Serializable
{
	...
	@XmlElements({
		...
		@XmlElement(namespace = FSViewXMLConstants.NS, name = "domain-username-password", nillable = true,
			type = DomainUsernamePassswordAuthenticationInformation.class) })
	...
	public FSViewConnectionInformation(URI connectionURI, FSViewAuthenticationInformation authInfo)
	{
	...
	public FSViewSession openSession() throws IOException
	{
		...
		return factory.openSession(_connectionURI, _authenticationInformation, _readOnly);
	}
	...
	{
		return String.format("%s[read-only: %s]\n%s", _connectionURI, _readOnly, _authenticationInformation);
	}
}