...
final class CifsFSViewDirectoryEntry extends AbstractFSViewDirectoryEntry<CifsFSViewSession>
{
	...
	final protected FSViewFileEntry createFileImpl(String name) throws IOException
	{
		...
		return (FSViewFileEntry) typedSession().wrapSmbFile(this, name, newFile);
	}
	...
	final protected FSViewDirectoryEntry createDirectoryImpl(String name) throws IOException
	{
		SmbFile newFile = new SmbFile(_entry, name);
		if (newFile.exists())
			throw new IOException(String.format("Path %s/%s already exists.", this, name));

		newFile.mkdir();
		return (FSViewDirectoryEntry) typedSession().wrapSmbFile(this, name, newFile);
	}
	...
	CifsFSViewDirectoryEntry(CifsFSViewSession session, FSViewDirectoryEntry parentEntry, String entryName, SmbFile entry)
	{
	...
	final public FSViewEntry[] listEntries() throws IOException
	{
		...
		for (int lcv = 0; lcv < entries.length; lcv++)
			ret[lcv] = typedSession().wrapSmbFile(this, entries[lcv].getName(), entries[lcv]);
	...
	final public FSViewEntry lookup(String name) throws IOException
	{
		...
		if (!file.exists())
			throw new FileNotFoundException(String.format("Unable to locate samba file %s/%s", this, name));
