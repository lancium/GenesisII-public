...
final public class AuthenticationInformationMultiModel extends
		AbstractFSViewInformationModel<FSViewAuthenticationInformation> {
	private Map<FSViewAuthenticationInformationTypes, AuthenticationInformationModel> _models = new EnumMap<FSViewAuthenticationInformationTypes, AuthenticationInformationModel>(
	...
	public AuthenticationInformationMultiModel(
			FSViewAuthenticationInformationTypes[] supportedTypes) {
		super("Multi");

		if (supportedTypes == null || supportedTypes.length == 0)
			supportedTypes = new FSViewAuthenticationInformationTypes[] { FSViewAuthenticationInformationTypes.Anonymous };

		_isSingletonAnonymous = (supportedTypes.length == 1)
				&& (supportedTypes[0] == FSViewAuthenticationInformationTypes.Anonymous);
		...
		for (FSViewAuthenticationInformationTypes type : supportedTypes) {
			AuthenticationInformationModel model = (AuthenticationInformationModel) type
					.createModel();
	...
	final AuthenticationInformationModel model(
			FSViewAuthenticationInformationTypes type) {
	...
	final public Component createGuiComponent() {
		...
		if (!_isSingletonAnonymous) {
			...
			if (_models.size() == 1) {
				panel.add(_models.get(_selectedType).createGuiComponent(),
				...
				FSViewAuthenticationInformationTypes[] types = _models
						.keySet()
						.toArray(
								new FSViewAuthenticationInformationTypes[_models
										.size()]);
				Arrays.sort(types);
				...
				for (int lcv = 0; lcv < types.length; lcv++) {
					JRadioButton button = new JRadioButton(new RadioAction(
							cardPanel, cardLayout, types[lcv]));
					group.add(button);

					panel.add(button, new GridBagConstraints(0, lcv, 1, 1, 0.0,
							1.0, GridBagConstraints.WEST,
							GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5,
							5));

					cardPanel.add(_models.get(types[lcv]).createGuiComponent(),
							types[lcv].name());
				}

				cardLayout.show(cardPanel, _selectedType.name());
				panel.add(cardPanel, new GridBagConstraints(1, 0, 1,
						types.length, 1.0, 1.0, GridBagConstraints.CENTER,
	...
	private class RadioAction extends AbstractAction {
		...
		private RadioAction(JPanel panel, CardLayout layout,
				FSViewAuthenticationInformationTypes buttonType) {
	...
	private class FSViewInformationListenerImpl implements
			FSViewInformationListener<FSViewAuthenticationInformation> {
		@Override
		public void contentsChanged(
				FSViewInformationModel<FSViewAuthenticationInformation> model) {
			fireContentsChanged();
		}
	}
}