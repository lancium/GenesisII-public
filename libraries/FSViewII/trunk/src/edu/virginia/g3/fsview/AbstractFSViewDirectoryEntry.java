	...
	implements FSViewDirectoryEntry
{
	...
	abstract protected FSViewFileEntry createFileImpl(String name) throws IOException;

	abstract protected FSViewDirectoryEntry createDirectoryImpl(String name) throws IOException;
	...
	public FSViewEntry lookup(String name) throws IOException
	{
		...
		throw new FileNotFoundException(String.format("Unable to locate entry %s in %s.", name, this));
	}
	...
	@Override
	final public FSViewDirectoryEntry createDirectory(String name) throws IOException
	{
		if (session().isReadOnly())
			throw new IOException(String.format("File system %s is read-only!", session()));
