...
public abstract class AbstractFSViewFactory implements FSViewFactory
{
		...
		FSViewAuthenticationInformationTypes... additionalAuthTypes)
	{
		...
		if (supportedURISchemes == null || supportedURISchemes.length == 0)
			throw new IllegalArgumentException("Must have at least 1 supported URI scheme.");

		if (description == null)
			throw new IllegalArgumentException("Description cannot be null.");

		if (authType == null)
			throw new IllegalArgumentException("Must have at least 1 supported authentication type.");
		...
		System.arraycopy(additionalAuthTypes, 0, _supportedAuthenticationTypes, 1, additionalAuthTypes.length);
	}
