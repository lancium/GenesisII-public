...
public class MName
{
	...
	private void initialize(String contextIdentifier, String name)
	{
		...
		if (name.contains(":"))
			throw new IllegalArgumentException(String.format("%s is an invalid MNaming name.", this));
	}
	...
	public MName(String name)
	{
		...
		if (index < 0)
			...
			throw new IllegalArgumentException(String.format("%s is an invalid MNaming name.", name));
		else
