...
final class InjectionPointDatabase
{
	...
	static private void validateInjectionInformation(Field field, MInject injectionInformation) throws InjectionException
	{
	...
	static final Collection<InjectionPoint> injectionPoints(Class<?> targetClass) throws InjectionException
	{
		...
		synchronized (_points) {
			...
			if (ret == null) {
				...
				for (Field field : targetClass.getDeclaredFields()) {
					MInject injectionInformation = field.getAnnotation(MInject.class);
					if (injectionInformation != null) {
						validateInjectionInformation(field, injectionInformation);
						if (injectionInformation.recursive())
							ret.add(new RecursiveInjectionPoint(field, injectionInformation));
						else
							ret.add(new FieldInjectionPoint(injectionInformation, field));
					}
				}

				for (Method method : targetClass.getDeclaredMethods()) {
					MInject injectionInformation = method.getAnnotation(MInject.class);
					if (injectionInformation != null) {
						validateInjectionInformation(method, injectionInformation);

						ret.add(new MethodInjectionPoint(injectionInformation, method));
					}
				}
