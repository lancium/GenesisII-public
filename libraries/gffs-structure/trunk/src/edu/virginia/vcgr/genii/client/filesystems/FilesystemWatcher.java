...
class FilesystemWatcher {
	...
	private void performCallbacks(FilesystemManager manager,
			FilesystemUsageInformation usageInformation, boolean matched) {
		for (FilesystemWatchCallback callback : _callbacks) {
			if (!_cancelled)
				callback.performCallback(manager, _filesystemName, _filesystem,
						usageInformation, matched);
			else
				return;
		}
	}

	FilesystemWatcher(long checkPeriod, String filesystemName,
			Filesystem filesystem, FilesystemWatchFilter filter,
			Collection<FilesystemWatchCallback> callbacks) {
		_checkPeriod = checkPeriod;
		if (_logger.isTraceEnabled())
			_logger.trace(String.format(
					"using fs check interval of %d seconds.",
					_checkPeriod / 1000));
	...
	final Calendar performCheck(FilesystemManager manager) {
		FilesystemUsageInformation usageInformation = _filesystem
				.currentUsage();
		...
		performCallbacks(manager, usageInformation,
				_filter.matches(usageInformation));
