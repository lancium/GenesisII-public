...
/**
 * This class is used to extract common behavior of all caches that saves
 * attributes in a single location.
 */
public abstract class CommonAttributeCache extends CommonCache {
	static protected Log _logger = LogFactory
			.getLog(CommonAttributeCache.class);

	public CommonAttributeCache(int priorityLevel, int capacity,
	...
	/*
	 * Since we only subscribe to RNS, not to byteIOs, byteIO attribute changes
	 * are propagated through subscription on container directories. As a
	 * result, the lifetime of a byteIO attribute depends on subscription on
	 * parent RNS directory. Meanwhile, the lifetime of an RNS directory
	 * attribute depends subscription on the RNS itself and on the parent
	 * directory as we propagate RNS attributes updates through parent
	 * directories too. This is done to avoid making RPCs for attributes of RNS
	 * directories that lie on some directory user has traversed, but the not
	...
	protected long getCacheLifeTime(URI endpointIdentifierURI) {
		...
		if (resourceConfig.isDirectory()) {
			attributeLifeTime = Math.max(attributeLifeTime,
		...
		WSResourceConfig parentConfig = (WSResourceConfig) CacheManager
				.getItemFromCache(parentPath, WSResourceConfig.class);

		// An RNS can propagate entry attribute changes only when both of the
		// resources are in the
		// same container.
		if (parentConfig == null
				|| !parentConfig.getContainerId().equals(
						resourceConfig.getContainerId())) {
			return attributeLifeTime;
		}
		return Math.max(attributeLifeTime,
				parentConfig.getMillisecondTimeLeftToCallbackExpiry());
	}

	protected Collection<String> getCacheKeysForLifetimeUpdateRequest(
			URI commonIdentifier) {

		WSResourceConfig commonConfig = (WSResourceConfig) CacheManager
				.getItemFromCache(commonIdentifier, WSResourceConfig.class);
		...
		for (String rnsPath : commonConfig.getRnsPaths()) {
			...
			Map<String, WSResourceConfig> matchingChildConfigs = CacheManager
					.getMatchingItemsWithKeys(childPaths,
							WSResourceConfig.class);

			if (matchingChildConfigs != null) {
				for (Map.Entry<String, WSResourceConfig> entry : matchingChildConfigs
						.entrySet()) {
					String uriString = entry.getKey();
					WSResourceConfig childConfig = entry.getValue();
					if (containerIdOfBaseResource.equals(childConfig
							.getContainerId())) {
		...
		if (_logger.isTraceEnabled())
			_logger.trace("number of entries selected for update: "
					+ cacheKeys.size());
		return cacheKeys;
	}
}
