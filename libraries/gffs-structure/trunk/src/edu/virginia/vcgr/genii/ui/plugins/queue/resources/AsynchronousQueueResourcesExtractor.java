...
class AsynchronousQueueResourcesExtractor implements
		Callable<EnhancedRNSPortType> {
	...
	final public EnhancedRNSPortType call() throws Exception {
		EndpointReferenceType queueEPR = _uiContext.endpointRetriever()
		...
		EnhancedRNSPortType rpt = ClientUtils.createProxy(
				EnhancedRNSPortType.class, queueEPR, _uiContext.uiContext()
						.callingContext());
		RNSLegacyProxy proxy = new RNSLegacyProxy(rpt, _uiContext.uiContext()
				.callingContext());
		EndpointReferenceType resourcesEPR = proxy.lookup("resources")[0]
				.getEndpoint();
		return ClientUtils.createProxy(EnhancedRNSPortType.class, resourcesEPR,
