...
public class PollingIntervalDirectory {
	...
	private static final Log _logger = LogFactory
			.getLog(PollingIntervalDirectory.class);

	public static final long DEFAULT_POLLING_INTERVAL = 15 * 1000L; // 15
																	// seconds
	public static final long MINIMUM_POLLING_INTERVAL = 5 * 1000L; // 5 seconds
	public static final long MAXIMUM_POLLING_INTERVAL = 4 * 60 * 1000L; // four
																		// minutes

	// The poller should test and update its polling interval every now and
	// then. Otherwise
	...
	public static void storePollingTimeSettings(
			PollingTimeSettings pollingTimeSettings) {
	...
	/*
	 * This method is used whenever a broker makes an polling outcall or the
	 * client receive information piggy-backed from a container indicating the
	 * client-cache is fresh, which is equivalent to making a polling outcall.
	 */
	public static void notifyAboutPolling(String containerId) {
		PollingTimeSettings pollingTimeSettings = POLLING_TIME_CONFIGURATIONS
				.get(containerId);

		// This indicates the client is directly receiving notifications from
		// the container.
	...
	/*
	 * This method is invoked to schedule an immediate polling once the client
	 * knows that there are some updates in the container that are not reflected
	 * in the cache.
	 */
	...
	public static boolean isPollingDue(String containerId,
			long pollingTimeCheckingInterval) {
		PollingTimeSettings pollingTimeSettings = POLLING_TIME_CONFIGURATIONS
				.get(containerId);
		if (pollingTimeSettings == null) {
			throw new RuntimeException(
					"Missing required polling time settings information.");
		}
		synchronized (pollingTimeSettings) {
			Date nextPollingTime = pollingTimeSettings.getNextPollingTime();
			long scheduledPollingTimeInMillis = nextPollingTime.getTime();
			long millisTillNextPolling = scheduledPollingTimeInMillis
					- System.currentTimeMillis();
			if (millisTillNextPolling >= 0) {
				return (millisTillNextPolling < pollingTimeCheckingInterval);
			} else {
				final long deviation = MAX_DEVIATION_FROM_MISSED_POLLING_DEADLINE
						- Math.abs(millisTillNextPolling);
				if (deviation < 0) {
	...
	public static void changePollingInterval(String containerId,
