...
public class ApplicationBase {
	...
	// the optional environment variable that points at the installation
	// location.
	...
	// environment variable used to point at a different deployment than the one
	// found in the
	// installation directory.
	static public final String DEPLOYMENT_DIR_ENVIRONMENT_VARIABLE = "GENII_DEPLOYMENT_DIR";

	// this property value can be used in certain property files as a
	// placeholder that is
	// translated into the value of the user state directory variable (see
	// above).
	static public final String USER_DIR_PROPERTY_VALUE = "env-GENII_USER_DIR";

	/**
	 * a simple search for the deployment directory in the environment
	 * variables. this does not have a default value implemented here, and must
	 * fall-back to a default elsewhere.
	 */
	...
	/*
	 * this provides a way for the osgi support to know we're running inside
	 * eclipse, which imposes a different structure on the locations of files.
	 */
	...
	static public void setupUserDir(File userdir) {
		if (!userdir.exists()) {
			try {
				File userDirFile = new GuaranteedDirectory(
						userdir.getAbsolutePath(), true);
	...
	static protected void prepareServerApplication()
		...
		CertificateValidatorFactory.setValidator(new SecurityUtilities(
				KeystoreManager.getResourceTrustStore()));

		GridEnvironment.loadGridEnvironment();

		ContainerStatistics.instance();

		ContainerProperties cProperties = ContainerProperties
	 ...
	 * inspected. If that is not present (or empty), the default configuration
	 * location located in a well-known spot from the installation directory (as
	 * per the installation system property) is used.
	 */
	...
	public enum GridStates {
		...
		// there was no connection, but we have established one. shell must
		// reload.
		CONNECTION_GOOD_NOW
	}

	static public GridStates establishGridConnection(Writer output,
			Writer error, Reader input) {
		...
		String connectCmd = ContainerProperties.getContainerProperties()
		...
		if (_logger.isDebugEnabled())
			_logger.debug("trying grid connection with parameters: "
					+ connectCmd);

		// split up our line which is expected to be two quoted strings. those
		// are our arguments.
		...
		parameters[0] = parameters[0].substring(1); // take off initial quote on
													// first parm.
		parameters[1] = parameters[1].substring(0, parameters[1].length() - 1); // take
																				// off
																				// last
																				// quote
																				// on
																				...
																				// parm.
		if (_logger.isDebugEnabled())
			_logger.debug("got arguments: [0]=" + parameters[0] + " [1]="
					+ parameters[1]);
		...
		try {
			...
			_logger.error("failure during grid connection: " + e.getMessage(),
					e);
		}
		return GridStates.CONNECTION_FAILED;
	}

	// loads the value for the genesis user state directory from the
	// environment.
	...
	static String getDefaultUserDir() {
		return String.format("%s/%s", System.getProperty("user.home"),
				GenesisIIConstants.GENESISII_STATE_DIR_NAME);
	}

	/**
	 * supports replacing a few keywords (or one really, currently) with
	 * environment variables.
	 * 
	 * (this must not use InstallationProperties.getUserDir, since that is based
	 * on this.)
	 */
	public static String replaceKeywords(String pathToFix) {
		// test for well-known singular replacements first.
		if ((pathToFix != null)
				&& pathToFix.equals(ApplicationBase.USER_DIR_PROPERTY_VALUE)) {
			// there's our sentinel for loading the state directory from the
			// environment variables.
			// let's try to load it.
			pathToFix = ApplicationBase.getUserDirFromEnvironment();
			if (pathToFix != null)
				return pathToFix;
			// nothing in environment, so fall back to default state directory,
			// since we know this.
			return ApplicationBase.getDefaultUserDir();
		}
		// test for generalized "env-NAME" patterns for other environment
		// variables.
		/*
