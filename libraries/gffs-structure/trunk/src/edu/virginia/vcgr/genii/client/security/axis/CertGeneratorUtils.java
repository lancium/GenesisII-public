...
public class CertGeneratorUtils
{
		...
		X509Certificate[] issuerChain, PrivateKey issuerPrivateKey) throws IOException
	{
		parameters.put(CERT_GENERATOR_DEFAULT_VALIDITY_CONSTRUCTION_PARAMETER,
			new MessageElement(CERT_GENERATOR_DEFAULT_VALIDITY_CONSTRUCTION_PARAMETER, defaultValidity.toString()));
		parameters.put(CERT_GENERATOR_ISSUER_CHAIN_CONSTRUCTION_PARAMETER,
			new MessageElement(CERT_GENERATOR_ISSUER_CHAIN_CONSTRUCTION_PARAMETER, DBSerializer.serialize(issuerChain, Long.MAX_VALUE)));
		...
		throws IOException
	{
		...
		any[1] = new MessageElement(CERT_GENERATOR_ISSUER_PRIVATE_KEY_CONSTRUCTION_PARAMETER,
