...
public class RNSTreeModel extends DefaultTreeModel {
	...
	public RNSTreeModel(ApplicationContext appContext, UIContext uiContext)
		...
		super(new RNSTreeNode(new RNSFilledInTreeObject(uiContext
				.callingContext().getCurrentPath().getRoot())), true);
	...
	TreePath translatePath(TreePath anotherPath) {
		...
		for (int lcv = 1; lcv < anotherPath.getPathCount(); lcv++) {
			...
			RNSTreeNode next = parent.lookup(anotherPath.getPathComponent(lcv)
					.toString());
			if (next == null)
				return null;
