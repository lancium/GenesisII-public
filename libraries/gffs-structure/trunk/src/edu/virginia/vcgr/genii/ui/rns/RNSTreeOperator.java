...
public abstract class RNSTreeOperator {
	...
	protected RNSTreeOperator(UIContext uiContext, RNSTree targetTree,
	...
	protected RNSPath getTargetPath(RNSPath parentPath, String originalName) {
		try {
			RNSPath ret = parentPath.lookup(originalName,
			...
			int lcv = 2;
			while (true) {
				ret = parentPath.lookup(
						String.format("Copy of %s (%d)", originalName, lcv++),
						RNSPathQueryFlags.DONT_CARE);
				if (!ret.exists())
					return ret;
			}
