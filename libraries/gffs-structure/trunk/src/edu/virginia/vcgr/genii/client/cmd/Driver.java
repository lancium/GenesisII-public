...
public class Driver extends ApplicationBase {
	...
	static public void loadClientState() {
		...
		try {
			...
			System.err.println("Security validation setup failure: "
					+ t.getMessage());
			System.exit(1);
		}

		// hmmm: never even knew there was an environment variable override for
		// the deployment name.
		// this should be unified with other ways of loading things; pretty sure
		// we are not always handling this uniformly.
		...
		if (deploymentName != null) {
			...
			deploymentName = System
					.getProperty(DeploymentName.DEPLOYMENT_NAME_PROPERTY);
	...
	static public void main(String[] args) {
		...
		if ((args.length > 1) && (args[0].equals("connect"))) {
			...
			ApplicationBase.GridStates gridOkay = establishGridConnection(
					new PrintWriter(System.out, true), new PrintWriter(
							System.err, true), new InputStreamReader(System.in));
			switch (gridOkay) {
			case CONNECTION_FAILED: {
				// this means we are not on the grid when we think we should
				// have been able to
				// get on.
				System.err.println("Failed to build a connection to the grid.");
				System.err
						.println("You can try to connect manually using the configured command, e.g.:");
				String connectCmd = ContainerProperties
						.getContainerProperties().getConnectionCommand();
			...
			default: // default should never be seen, but must be included as a
						// case.
			case CONNECTION_MEANS_UNKNOWN: {
				// we are not going to get connected at this point. we don't
				// know how. this had
	...
	static private int doShell(BufferedReader in) throws ReloadShellException {
		...
		while (true) {
			...
			try {
				...
				lastExit = runner.runCommand(passArgs, new PrintWriter(
						System.out, true), new PrintWriter(System.err, true),
						in);
				...
				int toReturn = ExceptionHandlerManager.getExceptionHandler()
						.handleException(cause,
								new OutputStreamWriter(System.err));
				if (toReturn != 0)
					return toReturn;
				lastExit = 1; // at least let them know we were displeased, if
								// this is the last
								// command in the loop.
			} finally {
	...
	static private void doNonShell(BufferedReader in, String[] args)
		...
		try {
			System.exit(runner.runCommand(args, new PrintWriter(System.out,
					true), new PrintWriter(System.err, true), in));
		} catch (ReloadShellException re) {
			throw re;
		} catch (Throwable cause) {
			ExceptionHandlerManager.getExceptionHandler().handleException(
					cause, new OutputStreamWriter(System.err));
			System.exit(1);
		}
	}
}
