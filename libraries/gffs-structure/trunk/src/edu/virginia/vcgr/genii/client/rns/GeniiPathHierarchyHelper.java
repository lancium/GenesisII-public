 ...
 */
public class GeniiPathHierarchyHelper implements
		TreeTraversalPathQuery<GeniiPath> {
	static private Log _logger = LogFactory
			.getLog(GeniiPathHierarchyHelper.class);
	...
	public GeniiPathHierarchyHelper(boolean bounceResourceForks) {
		_bounceResourceForks = bounceResourceForks;
	}

	@Override
	public PathOutcome checkPathSanity(GeniiPath path,
			TreeTraversalActionAlert<GeniiPath> bounce) {
		if (path == null)
		...
		// check that the path is okay for our purposes if it's actually a grid
		// path.
		RNSPath tempRNS = path.lookupRNS();
		if (tempRNS != null) {
			PathOutcome test = RNSPathHierarchyHelper.checkPathIsNormal(
					tempRNS, _bounceResourceForks);
	...
	public Collection<GeniiPath> getContents(GeniiPath path) {
		...
		try {
			if (path.pathType() == GeniiPathType.Grid) {
				...
				for (RNSPath file : curr.expand(path.path() + "/*")) {
					if (file.exists()) {
						if (_logger.isDebugEnabled())
							_logger.debug("getContents adding GeniiPath of "
									+ file.toString());
				...
				for (int i = 0; i < filesFound.length; i++) {
					arr.add(new GeniiPath("local:"
							+ filesFound[i].getAbsolutePath()
									.replace('\\', '/')));
				}
			}
		...
		if (_logger.isDebugEnabled()) {
			if (_logger.isDebugEnabled())
				_logger.debug("returning an array with " + arr.size()
						+ " elems:");
