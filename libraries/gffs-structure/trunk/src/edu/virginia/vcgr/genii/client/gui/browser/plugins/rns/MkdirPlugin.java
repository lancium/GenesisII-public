...
/**
 * This plugin is a relatively simple menu plugin which makes it easier for
 * users to create new RNS directories.
 * 
 * @author mmm2a
 */
public class MkdirPlugin implements IMenuPlugin {
	...
	public void performAction(RNSPath[] selectedResources, JFrame ownerDialog,
			IActionContext actionContext) throws PluginException {
		/*
		 * Prompt the user with an input dialog to get the name of the new
		 * directory.
		 */
		...
		/*
		 * Once we have the new directory's name, we have to create a
		 * long-running action to actually create the new path.
		 */
	...
	/**
	 * The DirectoryMaker is a long running action that takes care of actually
	 * calling out to the grid to make the requested directory.
	 * 
	 * @author mmm2a
	 */
	static private class DirectoryMaker implements ILongRunningAction {
		...
		/**
		 * Create a new directory maker task with a given RNSPath as a parent,
		 * and the given directory name as the new directory to create.
		 * 
		 * @param parent
		 *            The existing parent directory under which the new
		...
		public void run(IActionContext actionContext) throws Throwable {
			try {
				RNSPath newPath = _parent.lookup(_newName,
						RNSPathQueryFlags.MUST_NOT_EXIST);
				newPath.mkdir();

				/*
				 * We need to tell the rns tree in the main browser to update
				 * since we have changed its internal structure.
				 */
				actionContext.refreshSubTree(_parent);
			} catch (RNSPathAlreadyExistsException e) {
				/*
				 * We explicitly deal with this exception as we have a little
				 * more information about what went wrong here.
				 */
