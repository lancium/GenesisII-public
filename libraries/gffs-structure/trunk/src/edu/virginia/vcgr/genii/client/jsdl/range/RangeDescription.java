...
public class RangeDescription {
	protected Collection<RangeDescription> _descriptions = new LinkedList<RangeDescription>();
	...
	public void addDescription(RangeDescription desc) {
		...
		if (_lowerBound != null)
			...
			else if ((!desc.getLowerBound().equals(Double.NaN))
					&& (desc.getLowerBound() < _lowerBound))
				_lowerBound = desc.getLowerBound();
		...
		if (_upperBound != null)
			...
			// if new is not NaN, then compare
			else if ((desc.getUpperBound().equals(Double.NaN))
					&& (desc.getUpperBound() > _lowerBound))
				_upperBound = desc.getUpperBound();
	}
