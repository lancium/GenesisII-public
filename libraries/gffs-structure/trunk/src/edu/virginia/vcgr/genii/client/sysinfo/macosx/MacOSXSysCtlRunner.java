package edu.virginia.vcgr.genii.client.sysinfo.macosx;
...
class MacOSXSysCtlRunner {
	...
	static MacOSXSysCtlRunner run() throws IOException {
		List<String> results = ExecutionEngine.simpleMultilineExecute("sysctl",
				"-n", CPU_FREQUENCY_KEY, MEMSIZE_KEY);

		return new MacOSXSysCtlRunner(Long.parseLong(results.get(0)),
