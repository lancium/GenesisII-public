...
public class Subscriber {
	...
	/*
	 * This is producer-consumer type queue for managing subscription requests.
	 * The subscriber acts as a producer while the SubscriptionOutcallHandler
	 * acts as a consumer.
	 */
	...
	private Subscriber() {
		...
		/*
		 * We need to create an SSL server to get the notifications in a secure
		 * way. Current HTTPServer is a makeshift implementation as I don't know
		 * the standard way of accessing and creating SSL certificates.
		 */
		LightweightNotificationServer notificationServer = LightweightNotificationServer
		...
		NotificationBrokerDirectory
				.registerNotificationMulitplexer(notificationMultiplexer);
		...
		/*
		 * If we cannot start the notification server then none of the feature
		 * related to subscription will work. However, we are not going to
		 * propagate the problem to the caller method. Instead the subscription
		 * mechanism should fail silently. This is done to keep the rest of the
		 * code unaffected by the failure of subscription system. Note that if
		 * you did not completely isolate the logic of caching from the rest of
		 * the system this would not be a good idea.
		 */
	...
	private boolean isSubscribableTarget(EndpointReferenceType target) {
		...
		// The target does not represent an implied GenesisII resource;
		...
		if (!typeInfo.hasPortType(WellKnownPortTypes
				.NOTIFICATION_PRODUCER_PORT_TYPE()))
			return false;

		// The target is not a GenesisII resource. Even if it implements the
		// notification producer
		// port type, we may not be able to interpret the notification messages
		// issued by it. So to
		// remain at the safe side we don't subscribe to this target.
		if (EPRUtils.getGeniiContainerID(target) == null)
			return false;

		// We don't support notification from arbitrary path of a resource fork.
		// In addition we
		// should not cache information for long for many of the resource forks,
		// e.g. fork for job
