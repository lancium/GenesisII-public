...
public class SocketConfigurer {
	...
	static final public String KEEP_ALIVE_PROPERTY = PROPERTY_BASE
			+ "keep-alive";
	static final public String PERFORMANCE_PREFERENCES_PROPERTY = PROPERTY_BASE
			+ "performance-preferences";

	static final public String SEND_BUFFER_SIZE_PROPERTY = PROPERTY_BASE
			+ "send-buffer-size";
	static final public String RECEIVE_BUFFER_SIZE_PROPERTY = PROPERTY_BASE
			+ "receive-buffer-size";

	static final public String REUSE_ADDRESS_PROPERTY = PROPERTY_BASE
			+ "reuse-address";

	static final public String LINGER_PROPERTY = PROPERTY_BASE + "so-linger";
	static final public String LINGER_TIME_PROPERTY = PROPERTY_BASE
			+ "so-linger.time";

	static final public String TIMEOUT_PROPERTY = PROPERTY_BASE + "timeout";

	static final public String TCP_NO_DELAY_PROPERTY = PROPERTY_BASE
			+ "tcp-no-delay";

	static final public String TRAFFIC_CLASS_PROPERTY = PROPERTY_BASE
			+ "traffic-class";
	...
	static final private Pattern SIZE_PATTERN = Pattern
			.compile("^\\s*(\\d+)\\s*([kbKB]?)\\s*$");
	...
	final public void configureSocket(Socket socket) {
		...
		try {
			...
			if (_performancePreferences != null) {
				if (_logger.isDebugEnabled())
					_logger.debug(String
							.format("Setting performance preferences to (connection=%d, latency=%d, bandwidth=%d)",
			...
			if (_sendBufferSize != null) {
				...
				if (_logger.isDebugEnabled())
					_logger.debug(String.format(
							"Set send buffer size to %d (now it's %d).",
			...
			if (_receiveBufferSize != null) {
				...
				if (_logger.isDebugEnabled())
					_logger.debug(String.format(
							"Set receive buffer size to %d (now it's %d).",
							_receiveBufferSize, socket.getReceiveBufferSize()));
			} else {
				if (_logger.isDebugEnabled())
					_logger.debug(String.format(
							"Left receive buffer size at %d.",
							socket.getReceiveBufferSize()));
			}

			if (_reuseAddress != null) {
				if (_logger.isDebugEnabled())
					_logger.debug(String.format("Setting SO_REUSEADDR to %s.",
							_reuseAddress));
			...
			if (_linger != null && _lingerTime != null) {
				if (_logger.isDebugEnabled())
					_logger.debug(String.format("Setting SO_LINGER to %s(%d).",
							_linger, _lingerTime));
			...
			if (_timeout != null) {
				if (_logger.isDebugEnabled())
					_logger.debug(String.format("Setting SO_TIMEOUT to %d.",
							_timeout));
			...
			if (_noDelay != null) {
				if (_logger.isDebugEnabled())
					_logger.debug(String.format("Setting TCP_NO_DELAY to %s.",
							_noDelay));
			...
			if (_trafficClass != null) {
				if (_logger.isDebugEnabled())
					_logger.debug(String.format("Setting Traffic Class to %s.",
							_trafficClass));
