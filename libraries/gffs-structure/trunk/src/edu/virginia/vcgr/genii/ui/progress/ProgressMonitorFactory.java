...
public class ProgressMonitorFactory {
	...
	public <ResultType> ProgressMonitor createMonitor(
			Component parentComponent, String dialogTitle,
			String dialogSubTitle, long dialogDelay, Task<ResultType> task,
		...
		ProgressMonitorWorker<ResultType> worker = new ProgressMonitorWorker<ResultType>(
				SwingUtilities.getWindowAncestor(parentComponent), dialogTitle,
				dialogSubTitle, dialogDelay, task, completionListener);
		return worker;
	}

	private class ProgressMonitorWorker<ResultType> extends
			AbstractTaskProgressMonitor implements Runnable {
		...
		private boolean initialWait(Future<ResultType> future, boolean finite)
		...
		private ProgressMonitorWorker(Window owner, String title,
				String subTitle, long delay, Task<ResultType> task,
				TaskCompletionListener<ResultType> completionListener) {
			_owner = owner;
			_title = title;
			_subTitle = subTitle;
		...
		public void run() {
			...
			try {
				Future<ResultType> future = _executor
						.submit(new TaskExecutorWorker<ResultType>(_task, pipe));

				if (!initialWait(future, _task.showProgressDialog())) {
					dialog = new ProgressMonitorDialog(_owner, _title,
							_subTitle);
						...
						try {
							...
							if (_completionListener != null)
								new TaskCompletedWorker<ResultType>(this,
										_completionListener, _task, _result)
										.run();
							return;
	...
	private class TaskExecutorWorker<ResultType> implements
			Callable<ResultType> {
		private Task<ResultType> _task;
		private TaskProgressListener _progressListener;

		private TaskExecutorWorker(Task<ResultType> task,
	...
	private class TaskCancelledWorker<ResultType> implements Runnable {
		...
		private TaskCancelledWorker(AbstractTaskProgressMonitor monitor,
				TaskCompletionListener<ResultType> listener,
