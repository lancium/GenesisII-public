...
public class ContainerProperties extends Properties {
	...
	/**
	 * for all normal run-time classes, the container properties is accessed
	 * this way.
	 */
	...
	/**
	 * This is not to be used in general; normally the containerProperties
	...
	/**
	 * Intended only for use by ApplicationBase.getUserDir. This loads the user
	 * directory setting from container.properties if it exists or returns null.
	 */
	...
	/**
	 * returns the configured deployments directory, if one is defined.
	...
	public String getDeploymentsDirectory() {
		...
		// well, nothing worked, so use a default based on the installation
		// directory.
		if (toReturn == null)
			toReturn = new File(Installation.getInstallDirectory(),
					DEPLOYMENTS_DIRECTORY_NAME).getAbsolutePath();
		return toReturn;
	}

	public String getDeploymentName() {
		String toReturn = InstallationProperties.getInstallationProperties()
				.getProperty(GENII_DEPLOYMENT_NAME_PROPERTY_NAME);
		if (toReturn == null)
	...
	/**
	 * returns the command line for a grid connect command (minus 'grid' and
	 * 'connect'). if this property has not been set, then this returns null.
	 * this will also first consult our installation properties in case we can
	 * locate an override there.
	 */
	public String getConnectionCommand() {
		String toReturn = InstallationProperties.getInstallationProperties()
				.getProperty(GRID_CONNECTION_COMMAND_PROPERTY);
		if (toReturn == null)
