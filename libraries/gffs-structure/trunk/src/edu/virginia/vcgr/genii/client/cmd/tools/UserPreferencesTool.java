...
public class UserPreferencesTool extends BaseGridTool {
	...
	static private final String _DESCRIPTION = "config/tooldocs/description/duser-preferences";
	static private final String _USAGE = "config/tooldocs/usage/uuser-preferences";
	static final private String _MANPAGE = "config/tooldocs/man/user-preferences";
	...
	protected int runCommand() throws Throwable {
		...
		MenuItem viewCurrentUserPreferencesItem = new SimpleMenuItem("V",
				new ViewCurrentUserPreferencesHandler());
		MenuItem changeExceptionHandlerItem = new SimpleMenuItem("E",
				new ChangeExceptionHandler());
		MenuItem changeGUIPreferencesItem = new SimpleMenuItem("G",
				new ChangeGUIPreferencesHandler());
		MenuItem changeShellPromptItem = new SimpleMenuItem("S",
				new ChangeShellPromptHandler());
		MenuItem quitItem = new SimpleMenuItem("Q", "Quit Preferences Tool");

		ComboBoxDialog menu = provider.createSingleListSelectionDialog(
				"User Preferences", "What would you like to do?", quitItem,
				viewCurrentUserPreferencesItem, changeExceptionHandlerItem,
				changeGUIPreferencesItem, changeShellPromptItem, quitItem);
	...
	static private class ViewCurrentUserPreferencesHandler implements
			RunnableMenuItem {
		public void run(DialogProvider provider) throws DialogException,
				UserCancelException {
			String handlerString;
			IExceptionHandler handler = UserPreferences.preferences()
					.getExceptionHandler();
			...
			ShellPrompt shellPrompt = UserPreferences.preferences()
					.shellPrompt();

			InformationDialog dialog = provider.createInformationDialog(
					"Current User Preferences",
					new TextContent(String.format("Exception Handler:  %s",
							handlerString), String.format("Prefer GUI?  %s",
							preferGUI ? "Yes" : "No"), String.format(
							"Shell Prompt Template:  %s",
							shellPrompt.describe())));
			dialog.showDialog();
		}
	...
	static private class ChangeExceptionHandler implements RunnableMenuItem {
		public void run(DialogProvider provider) throws DialogException,
				UserCancelException {
			MenuItem simple = new SimpleMenuItem("S",
			...
			ComboBoxDialog dialog = provider.createSingleListSelectionDialog(
					...
					"Please select the new exception handler to use:", null,
					simple, debug);
			dialog.showDialog();
			...
			try {
				...
				throw new RuntimeException("Unable to store user preferences.",
						bse);
			}
		}
	...
	static private class ChangeGUIPreferencesHandler implements
			RunnableMenuItem {
		public void run(DialogProvider provider) throws DialogException,
				UserCancelException {
	...
	static private class ChangeShellPromptHandler implements RunnableMenuItem {
		public void run(DialogProvider provider) throws DialogException,
				UserCancelException {
			InputDialog dialog = provider.createInputDialog("Shell Prompt",
					"What would you like to use for your shell prompt?");
			dialog.setDefaultAnswer(UserPreferences.preferences().shellPrompt()
					.describe());
			dialog.showDialog();

			try {
				UserPreferences.preferences().shellPromptTemplate(
						dialog.getAnswer());
			} catch (BackingStoreException bse) {
				_logger.info("exception occurred in run", bse);
				throw new RuntimeException("Unable to store user preferences.",
