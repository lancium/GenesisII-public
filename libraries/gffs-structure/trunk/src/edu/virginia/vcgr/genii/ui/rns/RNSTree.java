...
public class RNSTree extends JTree implements EndpointRetriever {
	...
	private void setupInputMap(InputMap iMap) {
		...
		// TODO: should be converted to 'go up one level'. ==>
		// iMap.put(CommonKeyStrokes.BACKSPACE,
		// no! do not delete for backspace:
	...
	private void checkPathExpansion(RNSTree original, TreePath originalPath,
			TreePath newPath) {
		if (original.isExpanded(originalPath)) {
			setExpandedState(newPath, true);

			RNSTreeNode originalNode = (RNSTreeNode) originalPath
					.getLastPathComponent();
			RNSTreeNode newNode = (RNSTreeNode) newPath.getLastPathComponent();

			int size = originalNode.getChildCount();
			for (int lcv = 0; lcv < size; lcv++) {
				RNSTreeNode originalChild = (RNSTreeNode) originalNode
						.getChildAt(lcv);
	...
	public RNSTree(ApplicationContext appContext, UIContext uiContext)
	...
	public void deletePaths(TreePath[] paths) {
		...
		for (TreePath tp : paths) {
			...
			if (node != null) {
				RNSTreeObject obj = (RNSTreeObject) node.getUserObject();
				if (obj != null
						&& obj.objectType() == RNSTreeObjectType.ENDPOINT_OBJECT) {
		...
		model.uiContext()
				...
				.createMonitor(this, "Delete Entries", "Deleting entries",
						1000L, new DeleteTask(model.uiContext(), pathList),
						null).start();
	}
	...
	private class TearoffHandlerImpl implements TearoffHandler {
		...
		public Window tearoff(JComponent arg0) {
			...
			Window ret = new RNSBrowserTearoffWindow(_applicationContext,
					context, newTree);
			...
			if (parent instanceof JViewport) {
				...
				if (path != null) {
					TreePath newPath = ((RNSTreeModel) newTree.getModel())
							.translatePath(path);
					if (newPath != null)
	...
	private class TreeWillExpandListenerImpl implements TreeWillExpandListener {
		...
		public void treeWillCollapse(TreeExpansionEvent event)
			...
			RNSTreeObject obj = (RNSTreeObject) node.getUserObject();
			if (obj.objectType() == RNSTreeObjectType.ENDPOINT_OBJECT)
				_expandedNodes.remove(((RNSFilledInTreeObject) obj).path()
						.pwd());
			node.collapse();
		}
		...
		public void treeWillExpand(TreeExpansionEvent event)
			...
			RNSTreeObject obj = (RNSTreeObject) node.getUserObject();
			if (obj.objectType() == RNSTreeObjectType.ENDPOINT_OBJECT)
				_expandedNodes.put(((RNSFilledInTreeObject) obj).path().pwd(),
						node);
			node.noteExpansion();
	...
	private class DirectoryChangeListenerImpl implements
	...
	private class DeleteAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (paths != null && paths.length > 0) {
				...
				if (paths.length == 1)
					answer = JOptionPane.showConfirmDialog(RNSTree.this, String
							.format("Are you sure you wish to delete \"%s\"?",
		...
		public Integer execute(TaskProgressListener progressListener)
			...
			try {
				...
				for (Pair<RNSTreeNode, RNSPath> path : _paths) {
					_sourceContext.trashCan()
							.add(_sourceContext, path.second());
					path.second().unlink();
	...
	private class AncestorListenerImpl implements AncestorListener {
		...
		public void ancestorAdded(AncestorEvent event) {
			...
			model.uiContext().directoryChangeNexus()
					.addDirectoryChangeListener(_dChangeListener);
		}
		...
		public void ancestorRemoved(AncestorEvent event) {
			...
			model.uiContext().directoryChangeNexus()
					.removeDirectoryChangeListener(_dChangeListener);
		}
	}
