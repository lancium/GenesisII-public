...
public class BESFaultManager {
	...
	static final public QName FAULT_QNAME = new QName(FAULT_NS,
			"activity-fault");
	...
	static private Detail constructDetail(Throwable[] causes) {
		Collection<MessageElement> faultStrings = new ArrayList<MessageElement>(
				causes.length);
		...
		for (Throwable cause : causes) {
			...
			writer.flush();
			faultStrings.add(new MessageElement(FAULT_DETAIL_NAME, sWriter
					.toString()));
		}
	...
	static public Fault constructFault(Throwable... causes)
		...
		try {
			return new Fault(FAULT_QNAME, "Fault occurred in activity.",
					new URI("http://somefault-actor"), constructDetail(causes));
		} catch (Throwable cc) {
