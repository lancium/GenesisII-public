...
public class NotificationBrokerWrapper {
	...
	private static Log _logger = LogFactory
			.getLog(NotificationBrokerWrapper.class);

	private static final int MAX_NUMBER_OF_TRIES_TO_CHANGE_MODE = 3;
	private static final long MODE_TEST_TRIAL_INTERVAL = 30 * 1000L; // thirty
																		// seconds
	...
	// A flag to stop the notification poller thread safely when the broker is
	// in polling mode.
	private boolean brokerDestroyed = false;

	public NotificationBrokerWrapper(
			EnhancedNotificationBrokerPortType brokerPortType,
			String containerId, long brokerResourceLifeTime, boolean mode,
			NotificationMultiplexer multiplexer) {
		this.brokerInActiveMode = mode;
		this.containerId = containerId;
		this.brokerPortType = brokerPortType;
		brokerResourceTerminationTime = System.currentTimeMillis()
				+ brokerResourceLifeTime;
	...
	public boolean isBrokerModeVerified() {
		return brokerInActiveMode
				|| (numberOfTriesToMakeActive >= MAX_NUMBER_OF_TRIES_TO_CHANGE_MODE);
	}
	...
	public void updateReceivedMessageIndex(int newMessageIndex)
			throws Exception {
	...
	public void destroyBroker() {
		...
		// This line is important as the container automatically deletes the
		// broker if a timeout
		// occurs. In that case, calling this method will result in an error.
		// Rather this method
		// is supposed to be called when broker is extant and we want an early
		// termination. To
		// avoid any mishap that may arise from an unintended use, the following
		// conditional is
		// applied.
	...
	private class NotificationPoller extends Thread {
		...
		public void run() {
			...
			PollingIntervalDirectory
					.storePollingTimeSettings(new PollingTimeSettings(
							containerId));

			while (!brokerInActiveMode && !brokerDestroyed && !brokerExpired()) {
				try {
					if (PollingIntervalDirectory.isPollingDue(containerId,
							NOTIFICATION_POLLING_SCHEDULE_TESTING_INTERVAL)) {

						GetMessagesResponse response = brokerPortType
								.getMessages(new GetMessages());
						PollingIntervalDirectory
								.notifyAboutPolling(containerId);

						if (response != null) {
							NotificationMessageHolderType[] notificationMessage = response
									.getNotificationMessage();
							if (notificationMessage != null
									&& notificationMessage.length != 0) {
								Notify notification = new Notify(
										notificationMessage, response.get_any());
								NotificationBrokerDirectory
										.pushNotificationMessageToMultiplexerQueue(notification);
							} else {
					...
					if (_logger.isDebugEnabled())
						_logger.debug(
								"There is some problem with the broker. Conservatively resetting the cache.",
								e);
					CacheManager.resetCachingSystem();
				} catch (RemoteException e) {
					if (_logger.isDebugEnabled())
						_logger.debug(
								"Unexpected RPC error. Conservatively resetting the cache.",
								e);
					CacheManager.resetCachingSystem();
				} catch (Exception e) {
					if (_logger.isDebugEnabled())
						_logger.debug(
								"Unexpected error. Conservatively resetting the cache.",
								e);
					CacheManager.resetCachingSystem();
				}
			}
