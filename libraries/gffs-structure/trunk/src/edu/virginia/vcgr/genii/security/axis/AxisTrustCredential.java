...
public class AxisTrustCredential implements XMLCompatible {
	...
	/**
	 * This is the method for converting a trust delegation into a form that can
	 * be transmitted over the wire using Axis web-services facilities.
	 */
	public Element convertToMessageElement() throws GeneralSecurityException {
		...
		synchronized (_realCred.getDelegation()) {
			MessageElement template = new MessageElement(
					BinarySecurity.TOKEN_BST);
			nodeRepresentation = AxisCredentialWallet.convertToAxis(template,
					_realCred.getDelegation().getXML());
		}
		MessageElement securityToken = new MessageElement(
				BinarySecurity.TOKEN_BST);
		securityToken.appendChild(nodeRepresentation);
		securityToken.setAttributeNS(null, "ValueType",
				SAMLConstants.SAML_DELEGATION_TOKEN_TYPE);
		MessageElement wseTokenRef = null;
		try {
			MessageElement embedded = new MessageElement(
					SAMLConstants.EMBEDDED_TOKEN_QNAME);
