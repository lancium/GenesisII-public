...
public class CopyMachine {
	...
	final static int MAX_COPY_DEPTH = 16; // we do not try to copy hierarchies
											// deeper than this.

	/**
	 * Constructs a copy machine for making a copy of the source to the target.
	 * If the sink is non-null, we will use it to record updates as files and
	 * directories are seen.
	 */
	public CopyMachine(String sourceIn, String targetIn,
			TaskProgressListener updateSink, boolean forceIn,
		...
		if (_logger.isDebugEnabled())
			_logger.debug("built a CopyMachine: src=" + _source + " dest="
					+ _target + " force=" + _force);
	}
	...
	/**
	 * traverses a directory tree at the source (even if just a file) and
	 * replicates the source into the target path, which must be a directory.
	 */
	public PathOutcome copyTree() {
		...
		// we hit them.
		GeniiPathRecurser cloner = new GeniiPathRecurser(
				new EnterDirectoryGeniiPath(this), new ExitDirectoryGeniiPath(
						this, _logLocation), new CopyFileGeniiPath(this,
						_logLocation), null);
		...
		if (sourceCheck.isDirectory()) {
			...
			try {
				...
				dirs.add(_target);
				int worked = MkdirTool
						.makeDirectory(false, null, dirs, _stderr);
				if (worked != 0) {
					_logger.error("failed to make directory in target of "
							+ targetCheck);
				...
				if (_logger.isDebugEnabled())
					_logger.debug(
							"failed to make missing directory, got exception",
							cause);
				return PathOutcome.OUTCOME_NO_ACCESS;
			}
	...
	/**
	 * copies a single file from the source location to the target. if the
	 * target is a directory, the file will be created inside of it. if the
	 * target is a file, then it is overwritten.
	 */
	public static PathOutcome copyOneFile(String sourceIn, String targetIn,
		...
		if (!source.exists()) {
			_logger.error(String.format("Unable to find source file %s!",
					source));
			return PathOutcome.OUTCOME_NONEXISTENT;
		}
		if (!source.isFile()) {
			_logger.error(String
					.format("Source path %s is not a file!", source));
			return PathOutcome.OUTCOME_WRONG_TYPE;
		}
		sourceName = source.getName();
		GeniiPath target = new GeniiPath(targetIn);

		// put the file into the target directory, if it is a directory. if
		// target is a file, we
		...
		if (target.exists() && target.isDirectory()) {
			...
			_logger.error("trying to copy onto a non-file object for target: "
					+ target);
			return PathOutcome.OUTCOME_WRONG_TYPE;
		}
		if (logLocation != null) {
			...
			ConcurrentCopyMachine copier = new ConcurrentCopyMachine(sourceIn,
					targetIn, logLocation, semaphore, mutex);
	...
	public static PathOutcome copy(InputStream in, OutputStream out) {
		...
		try {
			...
			// we have to take the result from above as good enough (that there
			// was no exception
			// from running the copy), because even if there were zero bytes
			// copied, that might
			// be right, for a zero length file.
			if (_logger.isDebugEnabled())
				_logger.debug("Transferred " + stat.bytesTransferred()
						+ " bytes from input stream to output.");
			return PathOutcome.OUTCOME_SUCCESS;
		} catch (Throwable cause) {
			_logger.error(
					"failed to copy the input stream to the output stream",
					cause);
			return PathOutcome.OUTCOME_ERROR;
		}
	}

	private static class CopyFileGeniiPath implements
			TreeTraversalActionAlert<GeniiPath> {
		...
		public PathOutcome respond(GeniiPath path) {
			if (path == null)
				return PathOutcome.OUTCOME_NOTHING;
			if (_logger.isDebugEnabled())
				_logger.debug("CopyFileGeniiPath hit the action for "
						+ path.toString());
			GeniiPath targetFile = new GeniiPath(parent._currentTarget + "/"
					+ path.getName());
			...
			if (parent._updates != null) {
				...
				parent._updates.updateSubTitle(partialDir + "/"
						+ path.getName());
			}
	...
	private static class EnterDirectoryGeniiPath implements
			TreeTraversalActionAlert<GeniiPath> {
		private CopyMachine parent;
		...
		public PathOutcome respond(GeniiPath path) {
			if (path == null)
				return PathOutcome.OUTCOME_NOTHING;
			if (_logger.isDebugEnabled())
				_logger.debug("EnterDirectoryGeniiPath hit the action for "
						+ path.toString());
			parent._currentTarget = parent._currentTarget.concat("/"
					+ path.getName());
			if (_logger.isDebugEnabled())
				_logger.debug("added to current target, now: "
						+ parent._currentTarget);
			...
			try {
				int worked = MkdirTool.makeDirectory(false, null, files,
						parent._stderr);
				if (worked != 0) {
					_logger.error("failed to make directory in target of "
							+ parent._currentTarget);
				...
				_logger.error(
						"entering directory creation attempt raised exception for: "
	...
	private static class ExitDirectoryGeniiPath implements
			TreeTraversalActionAlert<GeniiPath> {
		private CopyMachine parent;
		...
		public PathOutcome respond(GeniiPath path) {
			if (path == null)
				return PathOutcome.OUTCOME_NOTHING;
			if (_logger.isDebugEnabled())
				_logger.debug("ExitDirectoryGeniiPath hit the action for "
						+ path.toString());
			...
			parent._currentTarget = tempPath.pathType().toString() + ":"
					+ just_dir;
			if (_logger.isDebugEnabled())
				_logger.debug("trimmed current target down to "
						+ parent._currentTarget);
			return PathOutcome.OUTCOME_SUCCESS;
		}
	}
