...
public class ApplicationDescriptionUtils {
	...
	static public URI determineDeploymentType(DeploymentDocumentType deployment)
	...
	static public DeploymentDocumentType uploadLocalSources(
			File deployDirectory, DeploymentDocumentType deploymentDocument,
		...
		if (name.equals(ZIPJAR_DEPLOYMENT_ELEMENT_QNAME)) {
			ZipJarDeploymentType deploymentInfo = ObjectDeserializer.toObject(
					any[0], ZipJarDeploymentType.class);

			deploymentInfo = uploadLocalSources(deployDirectory,
					deploymentInfo, listener);
			deploymentDocument
					.set_any(new MessageElement[] { new MessageElement(
							ZIPJAR_DEPLOYMENT_ELEMENT_QNAME, deploymentInfo) });
		} else if (name.equals(BINARY_DEPLOYMENT_ELEMENT_QNAME)) {
			BinDeploymentType deploymentInfo = ObjectDeserializer.toObject(
			...
			deploymentDocument
					.set_any(new MessageElement[] { new MessageElement(
							BINARY_DEPLOYMENT_ELEMENT_QNAME, deploymentInfo) });
		} else {
			throw new DeploymentException("Unknown deployment type.");
		}
	...
	static private ZipJarDeploymentType uploadLocalSources(
		...
		source.set_any(uploadLocalSource(deployDirectory, source.get_any(),
				listener));
	...
	static private BinDeploymentType uploadLocalSources(File deployDirectory,
			BinDeploymentType deploymentInfo, IUploadProgressListener listener)
			throws IOException, RNSException {
		NamedSourceType[] sources = deploymentInfo.getBinary();
		if (sources != null) {
			for (int lcv = 0; lcv < sources.length; lcv++)
				sources[lcv].set_any(uploadLocalSource(deployDirectory,
						sources[lcv].get_any(), listener));
	...
	static private MessageElement[] uploadLocalSource(File deployDirectory,
			MessageElement[] any, IUploadProgressListener listener)
			throws IOException, RNSException {
		if (any == null)
			return null;
		...
		for (int lcv = 0; lcv < any.length; lcv++) {
			...
			if (name.equals(LOCAL_FILE_SOURCE_NAME)) {
				...
				any[lcv] = new MessageElement(REMOTE_ENDPOINT_NAME,
	...
	static private EndpointReferenceType uploadLocalFile(File deployDirectory,
			String localPath, IUploadProgressListener listener)
			throws IOException {
		...
		try {
			...
			newFile = ResourceCreator.createNewResource("RandomByteIOPortType",
					null, null);
