 ...
 * @author mmm2a
 */
public class SimpleRByteIOTransferer extends
		AbstractByteIOTransferer<RandomByteIOPortType> implements
		RandomByteIOTransferer, SimpleByteIOTransferer {
	/**
	...
	public byte[] read(long startOffset, int bytesPerBlock, int numBlocks,
		...
		TransferInformationType holder = new TransferInformationType(null,
				ByteIOConstants.TRANSFER_TYPE_SIMPLE_URI);

		ReadResponse resp = _clientStub.read(new Read(startOffset,
				bytesPerBlock, numBlocks, stride, holder));
