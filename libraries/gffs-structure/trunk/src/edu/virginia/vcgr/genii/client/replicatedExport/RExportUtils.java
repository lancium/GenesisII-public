...
public class RExportUtils {
	...
	static public MessageElement[] createCreationProperties(String path,
			String parentIds, URI epi) {
		MessageElement[] creationProperties = new MessageElement[3];
		creationProperties[0] = ClientConstructionParameters
				.createEndpointIdentifierProperty(epi);
	...
	static public RExportInitInfo extractCreationProperties(
		...
		// Get common EPI
		org.apache.axis.message.MessageElement epiElement = properties
				.getAxisMessageElement(IResource.ENDPOINT_IDENTIFIER_CONSTRUCTION_PARAM);
		try {
			epi = ClientConstructionParameters
					.getEndpointIdentifierProperty(epiElement);
