...
public class ErrorHandler
{
	...
	static public void handleError(UIContext uiContext, JComponent responsibleComponent, ClassifiedError cError)
	{
		if ((uiContext == null) || (cError == null))
			return;
		if (cError.classification() == ErrorClassification.IGNORABLE) {
			...
			String msg = cError.reportTitle() + ": " + cError.reportMessage() + " -- " + cError.cause().getMessage();
			_logger.error(msg);
			LoggingTarget.logInfo(msg, cError.cause());
		}
	}

	static public void handleError(UIContext uiContext, JComponent responsibleComponent, Throwable cause)
	{
		if ((uiContext == null) || (cause == null))
			return;
