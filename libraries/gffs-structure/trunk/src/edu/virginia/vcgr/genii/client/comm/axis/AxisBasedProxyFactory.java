...
public class AxisBasedProxyFactory implements IProxyFactory {
	...
	static public QName LOCATOR_REGISTRY_QNAME = new QName(
			GenesisIIConstants.GENESISII_NS, "locator-registry");
	...
	public <IFace> IFace createProxy(ClassLoader loader, Class<IFace> iface,
		...
		synchronized (conf) {
			_locators = (HashMap<String, Class<?>>) conf
					.retrieveSection(LOCATOR_REGISTRY_QNAME);
		}
		...
		if (locators[0] == null) {
			throw new ConfigurationException("Unable to find a locator for \""
					+ iface.getName() + "\".");
		}
	...
	private AxisClientInvocationHandler getInvocationHandler(Object clientProxy)
