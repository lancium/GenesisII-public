...
public class NotificationMessageHolder {
	static final QName TOPIC_QNAME = new QName(WSRFConstants.WSN_TOPIC_NS,
			"Topic");
				...
				if (any.length > 0) {
					...
					if (contentsType != null) {
						JAXBContext context = JAXBContext
								.newInstance(contentsType);
						Unmarshaller u = context.createUnmarshaller();
						_contents = u.unmarshal(any[0], contentsType)
								.getValue();
					}
				}
			}
		...
		if (any != null) {
			for (MessageElement value : any) {
				...
				if (name.equals(TOPIC_QNAME)) {
					TopicQueryExpression query = TopicQueryDialects
							.createFromElement(value);
	...
	final public NotificationMessageHolderType toAxisType()
		...
		return new NotificationMessageHolderType(_subscriptionReference,
				_publisherReference, me == null ? null
						: new NotificationMessageHolderTypeMessage(
								new MessageElement[] { me }),
				_topic == null ? null : new MessageElement[] { _topic
						.asConcreteQueryExpression().toTopicExpressionElement(
								TOPIC_QNAME, "ts%d") });
	}
}