...
public class ExportControl
{
	...
	/**
	 * this is an enumeration of the different modes available on the path. if there is no mode listed at all, then the path is unusable.
	 * similarly if the mode is DISALLOW_MODE, then the path cannot be used for exports.
	 */
	...
	{
		return _allowedModes.size() == 0 || ((_allowedModes.size() == 1) && (_allowedModes.contains(ModeAllowance.DISALLOW_MODE)));
	}
	...
	/**
	 * returns whether a particular mode request should be allowed by this restriction object (assuming user and path are appropriate).
	 */
	public boolean actionAllowed(ModeAllowance mode)
	{
		if (mode.equals(ModeAllowance.DISALLOW_MODE)) {
			/*
			 * what does this actually mean? we are interpreting it to be asking if this restriction denies all access.
			 */
	...
	/**
	 * combined check returns true if this restriction is appropriate for a particular user and also allows a particular mode of access.
	 */
	 ...
	 * important, because it is the number of directory components that match between the string passed in and the directory that this
	 * restriction is about; a larger number indicates more of the path matched, so when two appropriate paths are considered, the larger
	 * number is more specific for the particular path being checked. if only the root directory matches, the count is 1. if the root and one
	 * lower level directory match, then the count is 2, and so forth. this is because a match on two root directories is considered a
	 * positive result (and will not return a zero from the function).
	 */
	public int isPathAppropriate(String pathToCheck)
	{
		...
		if (pathToCheck.startsWith("/") && _path.startsWith("/")) {
			...
			if (pathToCheck.substring(1, 2).equals(":") && _path.substring(1, 2).equals(":") && pathToCheck.substring(2, 3).equals("/")
				...
				/*
				 * a broken call; one path is not absolute or matching drive letter, so we can't say anything about their relevance.
				 */
				return 0;
			}
		}
		...
		for (String ourComponent : ourComponents) {
			...
			if (elementToTest >= toTest.length) {
				/*
				 * this is a failure, since we ran out of components in the string to check. this means the path we have is actually longer
				 * than the one passed in, so it cannot be a match.
				 */
				return 0;
			}
	...
	/**
	 * reads a line from the "reader" specified, respecting comments and line continuation ('\') characters. if a line is a comment, it will
	 * be ignored and the next line will be tried. if there is a continuation character on a non-ignored line, then the next line will also be
	 * gathered, for as long as continuation characters are found at end of each line. blank lines in the file are automatically skipped. if
	 * the return value has no length, then a line could not be read from the file. this function should never return null.
	 */
	public static String readNextLine(BufferedReader reader)
	{
		/*
		 * we will set this to true when we do not care about the comment character or trimming white space.
		 */
		...
		while (true) {
			...
			try {
				...
				/*
				 * smash all white spaces into a single white space. that's fine even if we aren't supposed to be cleaning, since this should
				 * not change the string significantly; it just replaces double spaces and tabs and such with a single space.
				 */
			...
			if (!skipLineCleaning) {
				/*
				 * chop all whitespace at front and end of line. this could incorrectly tell us a backslash was actually at end when there
				 * were spaces after it, but such a line is already illegal based on the expected syntax (there should be a user name or a set
				 * of option characters instead of a backslash). at worse we would still handle a backslash with a goofed up space after it
				 * for continuation.
				 */
			...
			if (currentLine.length() == 0) {
				if (accumulator.length() > 0) {
					/*
					 * they could have had a continuation and a blank line after; if so, we'll return the line now.
					 */
					break;
				}
			...
			if (currentLine.endsWith("\\")) {
				/*
				 * for continuation, put current line in buffer sans backslash, and read next line. next line after continuation cannot be a
				 * comment, so we should skip the comment check in that case.
				 */
	 ...
	 * PATH USER [OPTIONS]
	 * 
	 * OPTIONS can be chosen from: r (read), w (write), n (none) OPTIONS are jammed together without intervening characters. it does not make
	 * sense to have other options together with 'n' for none.
	 * 
	 * if a PATH has no option next to it, then there are no exports allowed on that path (or on paths found under that path).
	 * 
	...
	public static ExportControl parseLine(String line)
	{
		...
		if (parts.length < 2) {
			_logger.error("invalid export restriction line found; does not have enough fields (at least 2 are required): " + line);
		}
		if (parts.length > 3) {
