...
public class CreateGroupPlugin extends AbstractCombinedUIMenusPlugin {
	static private class RemoveTask implements Runnable {
		...
		public void run() {
			...
			try {
				...
				tool.run(new EmptyWriter(), new EmptyWriter(),
						new EmptyReader());
	...
	static private class GroupCreationListener implements
			TaskCompletionListener<Boolean> {
		private UIPluginContext _context;
		private EndpointRetriever _retriever;

		private GroupCreationListener(UIPluginContext context,
	...
	static private class GroupCreatorTask extends AbstractTask<Boolean> {
		...
		private GroupCreatorTask(UIPluginContext context, RNSPath parent,
				String newName) {
			_context = context;
			_parent = parent;
			_newName = newName;
		...
		final public Boolean execute(TaskProgressListener progressListener)
			...
			String SERVICE_PATH = nsd.getRootContainer()
					+ "/Services/X509AuthnPortType";

			try {
				IdpTool tool = new IdpTool();
				tool.addArgument(SERVICE_PATH);
				tool.addArgument(_newName);
				int result = tool.run(new EmptyWriter(), new EmptyWriter(),
						new EmptyReader());
				if (result != 0)
					return false;
				...
				RNSPath source = _parent.lookup(sourcePath,
						RNSPathQueryFlags.MUST_EXIST);

				if (wasCancelled())
					return false;

				RNSPath target = _parent.lookup(_newName,
				...
				if (sourcePath != null)
					_context.uiContext().executor()
							.equals(new RemoveTask(sourcePath));
			}
		}
	}
	...
	protected void performMenuAction(UIPluginContext context, MenuType menuType)
		...
		context.uiContext()
				...
				.createMonitor(context.ownerComponent(), "Creating Group",
						"Create Group Object", 1000L,
						new GroupCreatorTask(context, path, answer),
						new GroupCreationListener(context, retriever)).start();
	}
	...
	public boolean isEnabled(
		...
		return (tp.isRNS() && !(tp.isContainer() || tp.isBESContainer()
				|| tp.isQueue() || tp.isIDP() || tp.isExport() || tp
					.isFSProxy()));
	}
}