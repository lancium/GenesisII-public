...
/**
 * Class for holding and managing a set of "outgoing" credentials within the
 * current calling context. This is a combination of all authenticated
 * certificate chains, caller credentials and credentials for the target.
 * 
 * @author dmerrill
 */
	...
	public void add(NuCredential cred) {
		...
		if (!(cred instanceof TrustCredential)) {
			if (cred instanceof Identity) {
				...
				// this is probably okay; may be username/password identity or
				// x509 identity.
			} else {
				if (_logger.isDebugEnabled())
					_logger.debug("skipping unknown type of credential!: "
							+ cred.toString());
				return;
			}
		}
		if (_logger.isTraceEnabled())
			_logger.trace("storing cred into transient set: " + cred.toString()
					+ " via " + ProgramTools.showLastFewOnStack(6));
		_credentials.add(cred);
	}

	public void addAll(Collection<NuCredential> newCreds) {
		// we intentionally invoke our own function, rather than addAll on the
		// list, in order to
		// check on things.
