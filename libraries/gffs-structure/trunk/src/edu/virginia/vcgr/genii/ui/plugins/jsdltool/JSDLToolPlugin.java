...
public class JSDLToolPlugin extends AbstractCombinedUIMenusPlugin {
	...
	static private void submitToBES(EndpointReferenceType targetEPR,
		...
		try {
			GeniiBESPortType bes = ClientUtils.createProxy(
					GeniiBESPortType.class, targetEPR);
	...
	protected void performMenuAction(UIPluginContext context, MenuType menuType)
		...
		try {
			Collection<RNSPath> paths = context.endpointRetriever()
					.getTargetEndpoints();
			JobTool.launch(null, new JobDefinitionListenerImpl(context
					.uiContext().callingContext(), paths.iterator().next()),
					null);
	...
	static private class JobDefinitionListenerImpl implements
			JobDefinitionListener {
		...
		public void jobDefinitionGenerated(JobDefinition jobDefinition) {
			...
			try {
				assumedContextToken = ContextManager
