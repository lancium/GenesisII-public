...
public class QSlotManagerTool extends BaseGridTool {
	// Will need to write up man page and usage when we finish, not for
	// prototype
	final static private String _DESCRIPTION = "config/tooldocs/description/dqslotmgr";
	final static private String _USAGE = "config/tooldocs/usage/uqslotmgr";
	final static private String _MANPAGE = "config/tooldocs/man/qslotmgr";

	private static final String[] WAITING_JOB_STATUSES = { "Pending", "Queued",
			"Held", "Zombie" };
	...
	/*
	 * We could use the JobInformation data types from the stat call to track
	 * waiting jobs, but that timing information may not always be available, so
	 * we keep a list in memory for estimating the wait time when we can't get
	 * it exactly.
	 */
	...
	/*
	 * Each BES has some data associated with it that we need as we modify the
	 * slot counts throughout execution. This keeps that data in one place and
	 * easily accessible via the BES name.
	 */
	...
	protected int runCommand() throws Throwable {
		...
		// runtime is the amount of time in minutes the program has been running
		// so far
		int runtime = 0;
		...
		if (RUN_ONCE) {
			...
			while (runtime < TIMEOUT * 60) { // runtime (minutes) < TIMEOUT
												// (hours)
				stdout.println("Starting next loop...");
				recycleSlots(manipulator);
				Thread.sleep(POLLING_PERIOD * 60 * 1000); // POLLING_PERIOD
															// minutes
	...
	/*
	 * Based on the code Ashwin wrote to find BES Slot counts; there are two
	 * sets of slot counts: current and max. Current is whatever the queue has
	 * at the moment, max is the highest we're allowed to set current. The max
	 * counts should be stored in a centralized location on the GFFS so our tool
	 * can access it wherever it runs; by convention, we will store them in
	 * /etc/queue-slots/<queue-path> (e.g.
	 * "/etc/queue-slots/queues/grid-queue"). The user may specify a different
	 * file path on the command line.
	 */
	private void populateSlotTable(String queuePath) throws IOException,
			RNSException {
		...
		if (!gPath.exists() || gPath.pathType() != GeniiPathType.Grid)
			throw new ResourceException(
					"Path does not exist or does not reference a Queue.");

		// Convert to RNSPath for listing contents
		ICallingContext ctxt = ContextManager.getExistingContext();
		RNSPath targetResource = ctxt.getCurrentPath().expandSingleton(
				gPath.path());
		...
		if (entries.size() > 0) {
			for (RNSPath entry : entries) {
				...
				if (currentSlots != null) {
					statTable.put(entry.getName(), new BESStats(currentSlots,
							-1));
				}
			}
			populateMaxBESSlots();
	...
	/*
	 * This should read a file (which already exists on GFFS) to determine the
	 * max slot counts for the BESes on the target queue. This file will either
	 * be in a fixed location for each queue, or specified by the user at run
	 * time.
	 */
	private void populateMaxBESSlots() throws IOException, RNSException {
		GeniiPath qPath = new GeniiPath(_QUEUE_MAX_FILE);
		if (!qPath.exists()) {
			throw new ResourceException("No file for max slot counts at "
					+ _QUEUE_MAX_FILE);
		} else {
			...
			try {
				...
				while ((line = in.readLine()) != null) {
					...
					if (separator < 0)
						throw new IOException("Input file " + qPath
								+ ": Wrong format");

					// break up the line
					String BES = line.substring(0, separator);
					int slotCount = Integer.parseInt(line
	...
	/*
	 * Each iteration we refresh this data, so we need to reset it at the
	...
	/*
	 * This function is the primary reschedule logic. It goes through the jobs
	 * from the queue, determines the state of each job, and decides if it
	 * should be rescheduled. When a job is chosen for reschedule, the BES that
	 * is currently hosting that job loses slots, and the job is re-queued
	 * without penalty.
	 */
	private void rescheduleJobs(QueueManipulator manipulator)
		...
		while (stats.hasNext()) {
			...
			if (isWaitingJob(besStatus)) {
				...
				if (!watchList.containsKey(ticket)) {
					...
					timer = new JobTimer(getWaitingTime(stat), true,
							stat.getScheduledOn());
					...
					if (waitingTime == 0) {
						...
						timer.runtime = waitingTime / 60; // runtime is in
															// minutes
					}
				}
				...
				if (timer.runtime >= THRESHOLD_TIME) {
					...
					stdout.println("   Wait time on job " + ticket.toString()
							+ " exceeded threshold");
					needReschedule = true;
				} else {
					timer.seen = true;
	...
	/*
	 * This is the method YAN designed to read in a BES-level job status Returns
	 * true if the job is Waiting on a BES somewhere
	 */
	...
	private int getWaitingTime(JobInformation jobInformation) {
		...
		if (submitTimeInBES == null)
			return 0;
		long waitingTimeInMillis = System.currentTimeMillis()
				- submitTimeInBES.getTimeInMillis();
		long waitingTimeInSeconds = waitingTimeInMillis / 1000;
		return (int) waitingTimeInSeconds;
	}

	/*
	 * We should increase the slot count if: a) all the current slots are full,
	 * and b) all the current jobs are running (not waiting), and c) the current
	 * slot count is less than the maximum We can either increase by one at a
	 * time, or we can go straight to the max; currently it's one at a time.
	 */
	private void incrementSlots(QueueManipulator manipulator)
			throws RemoteException {
		for (String key : statTable.keySet()) {
			BESStats bes = statTable.get(key);
			stdout.println("   BES " + key + ": using " + bes.current + " of "
					+ bes.max);
			...
			} else if (bes.current < bes.max && bes.scheduled >= bes.current
					&& bes.waiting == 0) {
