...
public class CacheConfigurer
{
	...
	// Some default values are kept with minimal caching to ensure that caching is not by default disabled if the
	// property file is missing.
	public static long DEFAULT_CACHE_TIMOUT_TIME = 30 * 1000L; // 30 seconds
	// We keep old subscribed contents in the cache even if no polling is done for 4 minutes. This attribute
	// is used with subscription based caching and when polling is used to make cached contents temporarily unaccessible
	// if current polling interval is too long. The system changes from slow to rapid polling mode as soon as any activity 
	...
	public static void initializeCaches()
	{
		...
		if (cacheEnabled != null) CACHING_ENABLED = Boolean.parseBoolean(cacheEnabled);
		...
		if (subscriptionEnabled != null) SUBSCRIPTION_BASED_CACHING_ENABLED = Boolean.parseBoolean(subscriptionEnabled);
		...
		if (cacheTimeout != null) DEFAULT_CACHE_TIMOUT_TIME = Integer.parseInt(cacheTimeout) * 1000L;
		...
		if (validityPeriod != null) DEFAULT_VALIDITY_PERIOD_FOR_CACHED_CONTENT = Integer.parseInt(validityPeriod) * 1000L;
		...
		if (CACHING_ENABLED) {
			...
			if (timeoutStr != null) configCacheTimeout = Integer.parseInt(timeoutStr) * 60 * 1000L;
