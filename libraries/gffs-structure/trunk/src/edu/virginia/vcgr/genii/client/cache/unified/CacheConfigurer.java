...
public class CacheConfigurer {
	...
	public static final long DEFAULT_VALIDITY_PERIOD_FOR_CACHED_CONTENT = 240 * 1000L;
	...
	private static void initializeCaches() {
		...
		// Monitoring is enabled for only RNSLookup and ResourceConfig caches as
		// these two
		// are sufficient to collect per-container usage statistics.
		LIST_OF_CACHES.add(new RNSLookupCache(0, 500, 30 * 1000L, true));

		// Identifiers linker, we need to keep info in the cache as long as
		// possible and
		// as much as possible too. Furthermore, as this cache gets used the
		// most therefore
		// it may be better to always use memory-based cache implementation for
		// this cache
		// even if we change the implementation of other caches in future.
		LIST_OF_CACHES.add(new ResourceConfigCache(0, 2500, 30 * 60 * 1000L,
				true));
	...
	public static void resetCaches() {
		try {
			...
			_logger.info("Exception occurred while resetting the cache management system "
					+ ex.getMessage());
		}
	}
}
