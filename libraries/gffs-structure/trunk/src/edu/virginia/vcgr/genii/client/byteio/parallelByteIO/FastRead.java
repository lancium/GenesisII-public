	...
	public void run()
	{
		...
		try {
			...
			while (remainingLength > 0) {
				...
				try {
					...
					if (e.getMessage().contains("Stream closed")) {
						...
						if (failureCount < MAX_FAILURES) {
							_logger.debug("caught axis attachment problem; retrying read for posn " + currOffset + " at attempt #" + failureCount);
				...
				if (_logger.isDebugEnabled())
					_logger.debug("copying data from offset " + currOffset +" into buffer at index " + ((currOffset - startOffset) + bufferOffset));
