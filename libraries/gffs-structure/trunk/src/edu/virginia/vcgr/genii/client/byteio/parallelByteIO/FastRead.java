...
public class FastRead implements Runnable {
	...
	private RandomByteIOTransferer transferer; // Transferer associated with
												// this thread
	...
	// Each thread will fill parent stream's byte-buffer at index (threadId -
	// 1)*subBufferSize

	private FillerAndChecker fac = null;

	public FastRead(RandomByteIOTransferer transferer, long startOffset,
			int blkReadSize, FillerAndChecker fac, int threadId,
			int subBufferSize) {
	...
	public void run() {
		try {
			byte[] temp_buffer = transferer.read(startOffset, block_read_size,
					1, 0);
