...
/**
 * This is the default resource property translator used for single-valued
 * resource properties.
 * 
 * @author mmm2a
 */
public class DefaultSingleResourcePropertyTranslator implements
	...
	static private Log _logger = LogFactory
			.getLog(DefaultSingleResourcePropertyTranslator.class);
	...
	public <Type> Type deserialize(Class<Type> clazz, MessageElement element)
		...
		_logger.debug("deserializing type " + clazz.getCanonicalName()
				+ " from elem real type "
				+ element.getClass().getCanonicalName());
		try {
			Object ob = ObjectDeserializer.toObject(element, clazz);
			_logger.debug("object type deserialized: "
					+ ob.getClass().getCanonicalName());
			Type toReturn = clazz.cast(ob);
			_logger.debug("...now returning type "
					+ toReturn.getClass().getCanonicalName());
			return toReturn;
		} catch (ResourceException re) {
			String msg = "Unable to deserialize resource property "
					+ element.getQName() + " for type "
					+ clazz.getCanonicalName() + " from elem real type "
					+ element.getClass().getCanonicalName();
			_logger.error(msg);
