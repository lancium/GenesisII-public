...
public class IdpTool extends BaseLoginTool {
	...
	public IdpTool() {
		...
		if (_logger.isTraceEnabled())
			_logger.trace(String.format(
					"idptool--valid millis default: %.2f days / %.2f years.",
					TimeHelpers.millisToDays(getValidMillis()),
	...
	protected int runCommand() throws Throwable {
		...
		if (gPath.pathType() != GeniiPathType.Grid)
			throw new InvalidToolUsageException(
					"idpServicePath must be a grid path. ");
		...
		if ((_authnUri == null) && (_storeType == null)) {
			...
			constructionParms.add(new MessageElement(
					SecurityConstants.IDP_VALID_MILLIS_QNAME,
					_credentialValidMillis));

			if ((_username != null) && (_password != null)) {

				// actually create a new idp that delegates a username/password
				// token.
				UsernamePasswordIdentity ut = new UsernamePasswordIdentity(
						_username, _password);

				MessageElement delegatedIdentParm = new MessageElement(
						SecurityConstants.IDP_STORED_CREDENTIAL_QNAME);
				delegatedIdentParm.addChild((MessageElement) ut
						.convertToMessageElement());
			...
			if (_authnUri == null) {
				// Keystore login
				assertions = tool.doKeystoreLogin(null,
						ContextManager.getExistingContext(), idpCertChain);
			} else {
				if (tPath.pathType() == GeniiPathType.Local) {
					...
					try {
						assertions = tool.doKeystoreLogin(fis,
					...
					RNSPath authnPath = ContextManager
							.getExistingContext()
							.getCurrentPath()
							.lookup(authnSource.getSchemeSpecificPart(),
									RNSPathQueryFlags.MUST_EXIST);
					EndpointReferenceType epr = authnPath.getEndpoint();
					TypeInformation type = new TypeInformation(epr);
					if (type.isIDP()) {
						// IDP Login
						assertions = IDPLoginTool.doIdpLogin(epr,
								BaseGridTool._credentialValidMillis,
								idpCertChain);
					} else if (type.isByteIO()) {
						// log into keystore from rns path to keystore file
						InputStream in = ByteIOStreamFactory
								.createInputStream(epr);
						try {
							assertions = tool.doKeystoreLogin(in,
									ContextManager.getExistingContext(),
									idpCertChain);
						} finally {
							in.close();
			...
			XMLCompatible xup = XMLConverter.upscaleCredential(assertions
					.get(0));
			if (xup == null) {
			...
			delegatedIdentParm.addChild((MessageElement) xup
					.convertToMessageElement());
		...
		CreateResourceTool.createInstance(idpService.getEndpoint(), null, // no
																			// link
																			// needed
	...
	protected void verify() throws ToolException {
		...
		if (((_kerbRealm != null) || (_kerbKdc != null))
				&& ((_kerbRealm == null) || (_kerbKdc == null))) {
