...
public class ByteIORPCache extends CommonAttributeCache {
	...
	/*
	 * When a byteIO is blocked because of rapid write/append operations, we
	 * don't want to client to cache attributes. However, we are replacing each
	 * get-attribute call with an aggregate call for all attributes. So if you
	 * don't cache information at least momentarily, we will miss the
	 * opportunity to utilize the call aggregation and issue RPC for individual
	 * attributes of a blocked ByteIO. To circumvent this problem, we use a
	 * small non-zero cache-life-time interval for blocked byteIOs.
	 */
	...
	public ByteIORPCache(int priorityLevel, int capacity, long cacheLifeTime,
			boolean monitoingEnabled) {
		super(priorityLevel, capacity, cacheLifeTime, monitoingEnabled);

		sizeAttributeCache = new TimedOutLRUCache<String, Long>(capacity,
				cacheLifeTime);
		modTimeAttributeCache = new TimedOutLRUCache<String, Calendar>(
				capacity, cacheLifeTime);
		accessTimeAttributeCache = new TimedOutLRUCache<String, Calendar>(
				capacity, cacheLifeTime);

		// Create time cache has a very long life time for cached entries as
		// once cached
		// we never have to remove this information from the cache. The same is
		// true for
		// transfer mechanism attributes cache. We could not use Long.MAX_VALUE
		// to simulate
		// an infinite lifetime as in that case the cache implementation we are
		// using fails
		// to safe the attributes.
		long millisecondsInDay = 24 * 60 * 60 * 1000L;
		createTimeAttributeCache = new TimedOutLRUCache<String, Calendar>(
				capacity, millisecondsInDay);
		xferAttributeCache = new TimedOutLRUCache<String, Set<String>>(
				capacity, millisecondsInDay);
	...
	public Object getItem(Object cacheKey, Object target) {
		...
		if (targetProperty == null)
			return null;
		if (qName.equals(ByteIOConstants.rxferMechs)
				|| qName.equals(ByteIOConstants.sxferMechs)) {
			@SuppressWarnings("unchecked")
	...
	public void putItem(Object cacheKey, Object target, Object value)
		...
		if (qName.equals(ByteIOConstants.rsize)
				|| qName.equals(ByteIOConstants.ssize)) {
			Long fileSize = translator.deserialize(Long.class, element);
			sizeAttributeCache.put(EPI, fileSize, lifetime);
		} else if (qName.equals(ByteIOConstants.rcreatTime)
				|| qName.equals(ByteIOConstants.screatTime)) {
			Calendar createTime = translator.deserialize(Calendar.class,
					element);
			createTimeAttributeCache.put(EPI, createTime);
		} else if (qName.equals(ByteIOConstants.rmodTime)
				|| qName.equals(ByteIOConstants.smodTime)) {
			Calendar modificationTime = translator.deserialize(Calendar.class,
					element);
			modTimeAttributeCache.put(EPI, modificationTime, lifetime);
		} else if (qName.equals(ByteIOConstants.raccessTime)
				|| qName.equals(ByteIOConstants.saccessTime)) {
			Calendar accessTime = translator.deserialize(Calendar.class,
					element);
			accessTimeAttributeCache.put(EPI, accessTime, lifetime);
		} else if (qName.equals(ByteIOConstants.rxferMechs)
				|| qName.equals(ByteIOConstants.sxferMechs)) {
	...
	public boolean cacheKeyMatches(Object cacheKey) {
		if (cacheKey instanceof QName) {
			QName qName = (QName) cacheKey;
			return (qName.equals(ByteIOConstants.rxferMechs)
					|| qName.equals(ByteIOConstants.sxferMechs)
					|| qName.equals(ByteIOConstants.rsize)
					|| qName.equals(ByteIOConstants.ssize)
					|| qName.equals(ByteIOConstants.rcreatTime)
					|| qName.equals(ByteIOConstants.screatTime)
					|| qName.equals(ByteIOConstants.rmodTime)
					|| qName.equals(ByteIOConstants.smodTime)
					|| qName.equals(ByteIOConstants.raccessTime) || qName
	...
	public void updateCacheLifeTimeOfItems(Object commonIdentifierForItems,
		...
		for (String EPI : epiStrings) {
			...
			if (modificationTime != null) {
				modTimeAttributeCache.put(EPI, modificationTime,
						newCacheLifeTime);
			}
	...
	protected long getCacheLifeTime(URI endpointIdentifierURI) {
		WSResourceConfig resourceConfig = (WSResourceConfig) CacheManager
				.getItemFromCache(endpointIdentifierURI, WSResourceConfig.class);
	...
	private void addXferMechanismAttributeInSet(Set<String> existingMechanisms,
			URI newMechanism) {
		...
		} else if (newMechanism
				.equals(ByteIOConstants.TRANSFER_TYPE_SIMPLE_URI)) {
	...
	private Collection<MessageElement> getXferMechElements(
			QName xferAttributeName, Set<String> xferMechasims) {
		...
		if (xferMechasims.contains(ByteIOConstants.TRANSFER_TYPE_MTOM)) {
			elementList.add(new MessageElement(xferAttributeName,
