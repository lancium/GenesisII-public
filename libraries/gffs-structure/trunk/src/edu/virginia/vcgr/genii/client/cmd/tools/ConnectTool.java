...
public class ConnectTool extends BaseGridTool {
	...
	protected int runCommand() throws Throwable {
		...
		connect(connectURL, deploymentName == null ? null : new DeploymentName(
				deploymentName));
	...
	static public void connect(ICallingContext ctxt) throws ResourceException,
			IOException {
		ContextManager.storeCurrentContext(ctxt);
	}

	static public void connect(String connectURL) throws ResourceException,
	...
	static public void connect(ICallingContext ctxt,
			DeploymentName deploymentName) throws ResourceException,
			IOException {
		ContextManager.storeCurrentContext(ctxt);
		if (deploymentName != null) {
			...
			/*
			 * reload the configuration manager so that all config options are
			 * loaded from the specified deployment dir (instead of likely the
			 * "default" deployment).
			 */
