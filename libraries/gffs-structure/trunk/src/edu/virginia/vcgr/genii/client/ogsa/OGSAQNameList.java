 ...
 * properties. It involves putting them into a whitespace seperated list inside
 * of an XML element. To make it possible to parse/serialize these, we have to
 * implement a specialize class to "wrap" these lists. That's what this is.
 * 
 * @author mmm2a
 */
public class OGSAQNameList extends ArrayList<QName> {
	...
	/**
	 * Construct a new OGSAQNameList based off of an existing collection of
	 * QNames.
	 * 
	 * @param qnames
	 *            The qnames to seed the list with.
	 */
	...
	/**
	 * Create a new OGSAQNameList based off of an XML Element to deserialize.
	 * 
	 * @param qnameElement
	...
	public OGSAQNameList(MessageElement qnameElement)
		...
		StringTokenizer tokenizer = new StringTokenizer(
				qnameElement.getValue(), " \t\r\n");
		while (tokenizer.hasMoreTokens()) {
			String token = tokenizer.nextToken();

			/*
			 * See if the token has a colon. If so, it's a fully qualified qname
			 * that we have to split into a prefix and a local part. If not,
			 * then it's the whole name.
			 */
			int index = token.indexOf(':');
			if (index < 0) {
				// It didn't have a prefix, so let's put it in the default
				// namespace
	 ...
	 * 
	 * @param elementName
	 *            The name to give the new element when you serialize it.
