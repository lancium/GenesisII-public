...
public class OGSAQNameList extends ArrayList<QName>
{
	...
	/**
	 * Construct a new OGSAQNameList based off of an existing collection of QNames.
	 * 
	 * @param qnames
	 ...
	 */
	public OGSAQNameList(MessageElement qnameElement) throws ResourcePropertyException
	{
		Collection<QName> names = new ArrayList<QName>();

		/*
		 * The qnames come in whitespace seperated, so create a tokenizer to parse that.
		 */
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if (index < 0) {
				...
				if (ns == null)
					throw new ResourcePropertyException("Unable to parse returned qname \"" + token + "\".");
	 ...
	 */
	public MessageElement toMessageElement(QName elementName) throws SOAPException
	{
