...
public class CreateDirDialog extends JDialog {
	...
	public CreateDirDialog(UIPluginContext context, String ContainerPath,
			String TargetPath) throws FileLockException {
		Container container;
		...
		container.add(_keepWithParent = new JRadioButton(
				"Recommended: Store with parent directory"),
				new GridBagConstraints(0, 0, 2, 1, 1.0, 0.0,
						GridBagConstraints.WEST, GridBagConstraints.NONE,
						new Insets(5, 5, 5, 5), 0, 0));
		container.add(_selectNewContainer = new JRadioButton(
				"Choose a new container - for control over placement"),
		...
		container.add(_paths = new ResourcePathsWidget(false, true,
				_container_path, _target_path), new GridBagConstraints(0, 2, 2,
		...
		/*
		 * container.add(new JButton(new QuitButton()), new
		 * GridBagConstraints(0, 3, 1, 1, 0.0, 1.0, GridBagConstraints.EAST,
		 * GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		 * 
		 * container.add(new JButton(new GuiHelpAction(null,
		 * HelpLinkConfiguration
		 * .get_help_url(HelpLinkConfiguration.CREATE_DIRECTORY_HELP))), new
		 * GridBagConstraints(1, 3, 1, 1, 1.0, 0.0, GridBagConstraints.EAST,
		 * GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		 */

		container.add(createButtonPanel(action), new GridBagConstraints(0, 3,
	...
	private Component createButtonPanel(Action action) {
		...
		panel.add(new JButton(action), new GridBagConstraints(0, 0, 1, 1, 0.0,
				...
				new Insets(5, 5, 5, 5), 5, 5));
		panel.add(new JButton(new QuitButton()), new GridBagConstraints(1, 0,
		...
		panel.add(
				new JButton(
						new GuiHelpAction(
								null,
								HelpLinkConfiguration
										.get_help_url(HelpLinkConfiguration.CREATE_DIRECTORY_HELP))),
	...
	private boolean hasEnoughInformationToCreate() {
		...
		if (_selectNewContainer.isSelected()) {
			...
			try {
				...
				// lets keep what they already typed and let them edit it
				// instead
				// containerPath = null;
				return false;
	...
	private class CreateDirAction extends AbstractAction implements
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if ((_target_path.compareTo(rnsPath) != 0)
						&& ((_target_path + "/").compareTo(rnsPath) != 0)) {
					try {
						path = path.lookup(rnsPath,
				...
				if (!path.getParent().exists()) {
					JOptionPane.showMessageDialog(null, rnsPath,
							"Parent directory " + path.getParent().toString()
									+ " does not exist!",
							JOptionPane.ERROR_MESSAGE);
					rnsPath = null;
					return;
				...
				if (_selectNewContainer.isSelected()) {
					try {
						...
						// "Storage container path you selected does not exist or you do not have permission",
						// JOptionPane.ERROR_MESSAGE);
						// lets keep what they already typed and let them edit
						// it instead
						// containerPath = null;
						return;
					} catch (RNSPathAlreadyExistsException er) {
