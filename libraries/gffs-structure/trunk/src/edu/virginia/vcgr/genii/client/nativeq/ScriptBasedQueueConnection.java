...
public abstract class ScriptBasedQueueConnection<ProviderConfigType extends ScriptBasedQueueConfiguration>
	...
	static private Log _logger = LogFactory
			.getLog(ScriptBasedQueueConnection.class);
	...
	final protected Pair<File, List<String>> generateSubmitScript(
		...
		try {
			...
			finalCmdLine = generateApplicationBody(ps, workingDirectory,
					application);
			...
			throw new NativeQueueException("Unable to generate submit script.",
					ioe);
		} finally {
	...
	protected List<String> generateApplicationBody(PrintStream script,
		...
		try {
			...
			OperatingSystemNames desiredOperatingSystemType = overrides
					.operatingSystemName();
			OperatingSystemNames operatingSystemType = (desiredOperatingSystemType != null) ? desiredOperatingSystemType
					: OperatingSystemNames.getCurrentOperatingSystem();

			File stdoutRedirect = application
					.getStdoutRedirect(workingDirectory);
			if (stdoutRedirect == null
					&& operatingSystemType == OperatingSystemNames.LINUX) {
			...
			Vector<String> testCmdLine = wrapper.formCommandLine(
					...
					application.getResourceUsagePath(),
					execName,
					application.getArguments().toArray(
							new String[application.getArguments().size()]));
			if (_logger.isDebugEnabled())
				_logger.debug(String.format(
						"Pervious cmdLine format with pwrapper only:\n %s",
	...
	protected String execute(ProcessBuilder builder)
		...
		try {
			...
			throw new ScriptExecutionException(commandLine, result,
					stderrCopy.getResult());
	...
	static private void logProcessResult(int result, StreamCopier stdout,
			StreamCopier stderr) {
		if (_logger.isDebugEnabled())
	...
	static private class LazyEvaluatorResult {
		...
		public LazyEvaluatorResult(int result, StreamCopier stdout,
				StreamCopier stderr) {
			_result = result;
			_stdout = stdout;
			_stderr = stderr;
