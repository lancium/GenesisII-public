...
public class SubscriptionInfoProcessor {
	...
	/*
	 * This intercepter method first tries to satisfy a lookup request from the
	 * cache. If it fails then let the RPC to proceed. Then it search the
	 * returned RNS entries to retrieve the address of
	 * notification-broker-factory, which is used to create a broker subscriber
	 * with pull point to retrieve updates on the the returned RNS entries, if
	 * it deemed useful. Finally, its place the current target endPoint in the
	...
	public LookupResponseType lookup(InvocationContext ctxt, String[] names)
		...
		LookupResponseType resp = RNSCacheLookupHandler
				.getCachedLookupResponse(ctxt.getTarget(), names);
		if (resp == null) {
		...
		Subscriber.getInstance().requestForSubscription(
				new PendingRNSSubscription(target));
		return resp;
	}

	private void searchAndStoreNotificationBrokerFactoryAddress(
			RNSEntryResponseType member) {
		...
		for (MessageElement element : metadata.get_any()) {
			...
			if (GenesisIIConstants.NOTIFICATION_BROKER_FACTORY_ADDRESS
					.equals(qName)) {
