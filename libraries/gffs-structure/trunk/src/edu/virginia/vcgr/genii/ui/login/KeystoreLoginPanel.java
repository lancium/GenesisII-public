...
final class KeystoreLoginPanel extends LoginPanel {
	...
	KeystoreLoginPanel() {
		...
		add(new JLabel("Keystore Path"), new GridBagConstraints(0, 0, 1, 1,
				0.0, 0.0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		add(_keystoreFile, new GridBagConstraints(1, 0, 1, 1, 1.0, 0.0,
				GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 5, 5));
		add(new JButton(new BrowseAction()), new GridBagConstraints(2, 0, 1, 1,
				0.0, 0.0, GridBagConstraints.NORTH, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5), 5, 5));
		add(new JLabel("Keystore Password"), new GridBagConstraints(0, 1, 1, 1,
				0.0, 1.0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		add(_password, new GridBagConstraints(1, 1, 2, 1, 1.0, 1.0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 5, 5));
	}

	@Override
	public Collection<NuCredential> doLogin(UIContext uiContext)
			throws Throwable {
		if (_keystoreFile.getText().length() == 0)
			JOptionPane.showMessageDialog(this,
					"Keystore Path cannot be empty!", "Invalid Login",
			...
			try {
				...
				if (entry != null) {
					...
					TrustCredential tc = new TrustCredential(
							clientKeyMaterial._clientCertChain,
							IdentityType.CONNECTION, entry._certChain,
							IdentityType.USER, bc, RWXCategory.FULL_ACCESS);
					tc.signAssertion(entry._privateKey);

					Collection<NuCredential> ret = new ArrayList<NuCredential>(
							1);
					ret.add(tc);
					return ret;
				}
	...
	final private class BrowseAction extends AbstractAction {
		...
		final public void actionPerformed(ActionEvent e) {
			...
			if (result == JFileChooser.APPROVE_OPTION)
				_keystoreFile.setText(_fileChooser.getSelectedFile()
						.getAbsolutePath());
		}
	}
}