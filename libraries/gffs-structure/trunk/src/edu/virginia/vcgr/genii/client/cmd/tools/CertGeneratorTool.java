...
public class CertGeneratorTool extends BaseGridTool {
	...
	static private final String _USAGE_RESOURCE = "config/tooldocs/usage/ucert-generator";
	...
	private Long _default_validity = new Long(1000L * 60 * 60 * 24
			* (365 * 12 + 3));
	...
	public CertGeneratorTool() {
		super(new LoadFileResource(_DESCRIPTION), new LoadFileResource(
				_USAGE_RESOURCE), false, ToolCategory.SECURITY);
	}
	...
	protected int runCommand() throws Throwable {
		...
		if (_create_generator) {
			...
			createGenerator(_path_for_cert_generator_factory, _url,
					_path_for_cert_generator, _ks_path, _ks_password,
		...
		if (_gen_cert) {
			...
			X509Certificate[] certChain = createCert(newKeyPair,
					_path_for_cert_generator, _cn, _c, _st, _l, _o, _ou, _email);
			storeCert(newKeyPair, certChain, _ks_path, _ks_password, _ks_alias,
					_entry_password);
		}
	...
	protected void verify() throws ToolException {
		...
		if ((_gen_cert && _create_generator)
				|| (!_gen_cert && !_create_generator))
	...
	public EndpointReferenceType createGenerator(String service, boolean url,
		...
		if (!ksFile.exists()) {
			throw new CreationException("Key store file " + issuerCertKSPath
					+ " does not exist");
		}
		if (!ksFile.isFile()) {
			throw new CreationException("Key store path " + issuerCertKSPath
					+ " is not a file");
		}

		KeyStore keyStore = CertTool.openStoreDirectPath(ksFile, keyStoreType,
				keyStorePassChars);

		if (keyStore == null) {
			throw new CreationException("Could not open key store at path "
					+ issuerCertKSPath);
		}

		if (!keyStore.containsAlias(issuerCertAlias)) {
			throw new CreationException("Alias " + issuerCertAlias
					+ " not found in key store " + issuerCertKSPath);
		}
		...
		issuerPrivateKey = (PrivateKey) keyStore.getKey(issuerCertAlias,
				entryPassChars);

		Certificate[] chain = keyStore.getCertificateChain(issuerCertAlias);

		if (chain == null) {
			throw new CreationException("Could not retrieve cert "
					+ issuerCertAlias + " from key store at path "
					+ issuerCertKSPath);
		}
		...
		MessageElement[] createProps = CertGeneratorUtils
				.createCreationProperties(issuerCertChain, issuerPrivateKey,
						defaultValidity);
		...
		if (!url) {
			...
			epr = CreateResourceTool.createInstance(path.getEndpoint(),
					(optTargetName == null) ? null : new GeniiPath(
	...
	static public X509Certificate[] createCert(KeyPair newKeyPair,
			String generatorPath, String cn, String c, String st, String l,
			String o, String ou, String email) throws IOException,
			RNSException, CreationException, GeneralSecurityException {
		RNSPath path = RNSPath.getCurrent();
		path = path.lookup(generatorPath, RNSPathQueryFlags.MUST_EXIST);
		CertGeneratorPortType certGenerator = ClientUtils.createProxy(
				CertGeneratorPortType.class, path.getEndpoint());
		...
		PublicKeyType newPublicKeyType = new PublicKeyType(
				SecurityUtilities.serializePublicKey(newPublicKey));
		request.setPublicKey(newPublicKeyType);
		GenerateX509V3CertificateChainResponseType response = certGenerator
				.generateX509V3CertificateChain(request);
		CertificateChainType newCertChainType = response.getCertificateChain();
		X509Certificate[] newCertChain = KeystoreManager
				.decodeCertificateChain(newCertChainType);
	...
	static public void storeCert(KeyPair keyPair, X509Certificate[] certChain,
		...
		ks.setKeyEntry(keyAlias, keyPair.getPrivate(), entryPassChars,
				certChain);
	...
	static public X509Name generateX509Name(String cn, String c, String st,
			String l, String o, String ou, String email) throws SocketException {
		String nameString = "";
