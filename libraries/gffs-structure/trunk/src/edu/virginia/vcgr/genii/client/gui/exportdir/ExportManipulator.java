...
public class ExportManipulator {
	...
	static public RNSPath createExport(String containerPath, String localPath,
			String rnsPath, boolean isLightweight)
			throws FileNotFoundException, ExportException, RNSException,
		...
		try {
			...
			try {
				targetpath = RNSPath.getCurrent().lookup(rnsPath,
						RNSPathQueryFlags.MUST_NOT_EXIST);
			} catch (RNSPathAlreadyExistsException r) {
				throw new ResourceException("Path " + targetpath.toString()
						+ " already exists.");
			} catch (Exception r) {
			...
			try {
				...
				servicepath = cpath.lookup("Services/" + service,
						RNSPathQueryFlags.MUST_EXIST);
			} catch (RNSPathDoesNotExistException r) {
				throw new ResourceException("There is no Services/" + service
						+ " on the container " + cpath.toString());
			} catch (Exception r) {
				throw new ResourceException(
						"Problem with ensuring that there is a LightWeightExportPortType service.");
			}
		...
		try {
			EndpointReferenceType exEPR = ExportTool.createExportedRoot(
					targetpath.toString(), servicepath.getEndpoint(),
					localPath, "", "", 0L, targetpath.toString(), false);
			if (exEPR == null)
				_logger.debug("created null EPR with createExportedRoot");
		} catch (Exception r) {
			throw new ResourceException(
					"exception during export root creation.", r);
		}
		return RNSPath.getCurrent().lookup(rnsPath,
				RNSPathQueryFlags.MUST_EXIST);
	}

	/*
	 * static public RNSPath createExport(URL containerURL, File localPath,
	 * String rnsPath, boolean isLightweight) throws FileNotFoundException,
	 * ExportException, RNSException, CreationException,
	 * ResourceCreationFaultType, RemoteException, IOException,
	 * InvalidToolUsageException { validate(localPath);
	 * 
	 * RNSPath target = RNSPath.getCurrent().lookup(rnsPath,
	 * RNSPathQueryFlags.MUST_NOT_EXIST); validate(target);
	 * 
	 * ExportTool.createExportedRoot( rnsPath,
	...
	static public void validate(File localPath) throws FileNotFoundException,
			ExportException {
		if (!localPath.exists())
			throw new FileNotFoundException("Couldn't find source directory \""
					+ localPath.getAbsolutePath() + "\".");
		if (!localPath.isDirectory())
			throw new ExportException("Cannot export \""
					+ localPath.getAbsolutePath()
					+ "\" because it is not a directory.");
	}

	static public void validate(RNSPath rnsPath) throws ExportException,
			RNSPathDoesNotExistException {
		RNSPath parent = rnsPath.getParent();
		if (!parent.exists())
			throw new ExportException(
					"Cannot create export because target RNS path \""
							+ parent.pwd() + "\" does not exist.");
		if (!(new TypeInformation(parent.getEndpoint())).isRNS())
			throw new ExportException("RNS path \"" + parent.pwd()
					+ "\" is not an RNS capable endpoint.");
	}

	static public void quitExport(RNSPath exportRoot) throws RNSException,
			IOException {
