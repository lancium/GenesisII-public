...
public class ResourceAccessMonitor {
	...
	// Assigns unaccounted resource access right -- via both RPC and cache
	...
	synchronized public static void reportResourceUsage(Object resource) {
		...
		boolean hasUnaccountedAccess = ResourceAccessMonitor.hasUnaccountedAccess
				.get();
	...
	/*
	 * This determine whether a cached resource should be passed to the caller
	 * for use or the system should issue an RPC for the resource to get a fresh
	 * copy.
	 */
	public static boolean isCachedContentGuaranteedToBeFresh(
			Object targetOrCachedResource) {

		// Represent some cache-management related thread. For such a thread
		// cached-resource is
		// always accessible
		...
		// When no container ID can be derived from the cached resource we
		// assume the optimistic
		// approach that the attribute (which in this case most likely content
		// type) get just
		// fetched
		// by making RPC on EPR for which cache access was denied previously a
		// moment ago or
		// corresponding to a container for which direct notification works.
		if (containerId == null)
			return true;

		NotificationBrokerWrapper broker = NotificationBrokerDirectory
				.getExistingRepresentativeBroker(containerId);

		// If there is no broker running for the container, we can assume that
		// subscription-based
		// caching
		// is turned off or hasn't been started yet, consequently cached
		// resources are prefetched or
		// piggy-backed timeout based contents, therefore we can declare them as
		// fresh.
		if (broker == null)
			return true;

		// Container is directly notifying the client about update on any cached
		// resource. Hence,
		// resources
		...
		Date lastPollingTime = PollingIntervalDirectory
				.getLastPollingTime(containerId);

		// The broker should start polling immediately after its creation.
		// Otherwise, it may not be
		// working
		// properly, and we cannot say whether the cached content is fresh or
		// stale.
		if (lastPollingTime == null)
			return broker.isBrokerJustCreated();

		// Finally, check whether or not a polling request was issued within the
		// acceptable time
		// window.
		Long timePassedSinceLastPolling = System.currentTimeMillis()
				- lastPollingTime.getTime();
	...
	public static boolean isMonitoredObject(Object object) {
		if (object == null)
			return false;
		return (object instanceof EndpointReferenceType)
				|| (object instanceof WSResourceConfig);
	}
