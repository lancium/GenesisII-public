...
class DefaultDoubleClickListener implements RNSTreeListener
{
	...
	@Override
	final public void pathClicked(RNSTree tree, UIContext context, RNSFilledInTreeObject fObj)
	{
		// Do nothing
	}

	@Override
	final public void pathDoubleClicked(RNSTree tree, UIContext context, RNSFilledInTreeObject fObj)
	{
		TypeInformation typeInfo = fObj.typeInformation();
		if (typeInfo.isByteIO())
			EditPlugin.performEdit(tree, context, fObj.path(), false);
	}
}