...
public class LogDatabaseConnectionPool extends
		NotTheServerDatabaseConnectionPool {
	private static Log _logger = LogFactory
			.getLog(LogDatabaseConnectionPool.class);

	private static int _logPoolInstances = 0;

	public LogDatabaseConnectionPool(Properties connectionProperties)
		...
		synchronized (LogDatabaseConnectionPool.class) {
			...
			if (_logPoolInstances != 1) {
				_logger.error("We don't have exactly one log connection pool instance -- we have "
						+ _logPoolInstances);
				System.exit(1);
			}
