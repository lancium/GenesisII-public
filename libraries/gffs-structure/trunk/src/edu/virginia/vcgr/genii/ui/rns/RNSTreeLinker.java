...
public class RNSTreeLinker extends RNSTreeOperator {
	...
	static public RNSTreeOperator link(RNSTree sourceTree, RNSTree targetTree,
			TreePath targetPath, UIContext sourceContext,
			Collection<Pair<RNSTreeNode, RNSPath>> paths) {
		return new RNSTreeLinker(sourceTree, targetTree, targetPath,
				sourceContext, paths);
	}

	private RNSTreeLinker(RNSTree sourceTree, RNSTree targetTree,
			TreePath targetPath, UIContext sourceContext,
			Collection<Pair<RNSTreeNode, RNSPath>> paths) {
	...
	public boolean performOperation() {
		...
		_uiContext
				...
				.createMonitor(_targetTree, "Linking Endpoints",
						"Linking endpoints.", 1000L, new LinkerTask(), null)
				.start();
		return true;
	}
