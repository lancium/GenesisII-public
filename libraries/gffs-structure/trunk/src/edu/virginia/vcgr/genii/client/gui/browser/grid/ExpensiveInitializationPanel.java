 ...
 * easy for developers to write panels which get filled in with information that
 * may take a long time to fill in (for example, because outcalls to the grid
 * are involved). This component is meant to be used mostly by plugins that are
 * implementing the ITabPlugin interface.
 * 
 * @author mmm2a
 */
public abstract class ExpensiveInitializationPanel extends JPanel {
	...
	protected ExpensiveInitializationPanel() {
		...
		add(new JLabel("Initializating panel..."), new GridBagConstraints(0, 0,
	 ...
	 * This operation is called exactly once each time the plugin creates a new
	 * tab component, and then only after the user first tries to view the
	 * component. In the mean time, a message indicating that the panel is being
	 * initialized will be displayed.
	 * 
	 ...
	 * 
	 * @param newComponent
	 *            The new component that the user finally resolved and wants
	 *            displayed.
	 */
	private void switchComponent(Component newComponent) {
		 ...
		 */
		removeAll();
		add(newComponent, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0,
	 ...
	 * called, it de-registers itself from the listern queue as it will
	...
	private class LazyResolver extends ComponentAdapter {
		...
		public void componentShown(ComponentEvent evt) {
			...
			/*
			 * Start up a new thread to actually do whatever work it is that the
			 * user needs to do in order to create the real component display.
			 */
	...
	/**
	 * An internal method that is called when an exception occurred trying to
	 * create the user's display component. This will essentially get displayed
	 * instead of the user's component and will consist of a text area with the
	 * stack trace included in it.
	 * 
	 * @param cause
	...
	/**
	 * This runnable is used internal to call into the user's derived class code
	 * to resolve the component that he or she wants displayed.
	 * 
	 * @author mmm2a
	 */
	private class ComponentResolver implements Runnable {
		...
		public void run() {
			 ...
			 * only be done in the event dispatch thread. If we are already in
			 * that thread (which is probably never going to be the case), go
			 * ahead and call through to the "swizzler" code.
			 */
	...
	private class ComponentSwitcher implements Runnable {
		...
		/**
		 * Create a new component switcher with the new component to replace the
		 * old one with.
		 * 
		 * @param newComponent
		 */
		...
		public void run() {
			/*
			 * Just call through to the component "swizzler". We should now be
