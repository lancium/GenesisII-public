...
public class RNSOperation implements Serializable {
	...
	public Collection<NameMappingType> getOldNameNewNameMappingsForRenameOperation() {
		...
		List<NameMappingType> nameMappings = new ArrayList<NameMappingType>(
				affectedEntries.length);
		for (String mappingString : affectedEntries) {
			int indexOfSeperator = mappingString
					.indexOf(RENAME_NAME_MAPPING_SEPERATOR);
			if (indexOfSeperator == -1) {
				throw new RuntimeException(
						"Affected entry name is not in porper format");
			}
