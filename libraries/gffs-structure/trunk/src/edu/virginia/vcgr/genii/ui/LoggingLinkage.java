...
public class LoggingLinkage implements ILoggingRecipient {
	...
	/**
	 * extremely limited list model for our logging here. this does not support
	 * anything besides appends right now.
	 */
	...
	public static class LoggingListModel extends DefaultListModel {
		...
		public ArrayList<Throwable> getCauses() {
			return causes;
		}
		...
		public void mouseClicked(MouseEvent me) {
			...
			if (me.getClickCount() == 2) {
				...
				if (target.getModel() instanceof LoggingListModel) {
					LoggingListModel model = (LoggingListModel) target
							.getModel();
					if (target.getTopLevelAncestor() instanceof ClientApplication) {
						ClientApplication parent = (ClientApplication) target
								.getTopLevelAncestor();
						LoggingListModel lm = (LoggingListModel) _appLoggingTarget
								.getModel();
						ReportableErrorHandler.displayError(
								parent.getContext(), target,
								model.get(indies[0]).toString(), lm.getCauses()
										.get(indies[0]));
					}
				}
				me.consume();
	...
	public boolean consumeLogging(String toConsume, Throwable cause) {
		if (_appLoggingTarget == null)
			return false;
		// locate any extended noisy messages, and fix message to include just
		// the portion before
		// nested errors.
		...
		if (_appLoggingTarget.getModel() instanceof LoggingListModel) {
			LoggingListModel lm = (LoggingListModel) _appLoggingTarget
					.getModel();
			lm.addWithCause(lm.getSize(), (new Date()).toString() + ": "
					+ toConsume + "\n", cause);
			_appLoggingTarget.setSelectedIndex(lm.getSize() - 1);
			_appLoggingTarget.ensureIndexIsVisible(_appLoggingTarget
					.getSelectedIndex());
		}
		return true;
	}
