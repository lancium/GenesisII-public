...
class FilesystemImpl implements Filesystem {
	...
	private Set<FilesystemProperties> _properties = EnumSet
			.noneOf(FilesystemProperties.class);
	...
	FilesystemImpl(FilesystemManager manager, String filesystemName,
		...
		/*
		 * Mark Morgan -- We're not ready to do this yet. We need to fully go
		 * down this root first. if (!root.exists()) root.mkdirs();
		 * 
		 * if (!root.exists()) throw new FileNotFoundException(String.format(
		 * "Unable to find directory \"%s\".", root)); else if
		 * (!root.isDirectory()) throw new FileNotFoundException(String.format(
	...
	public void addSandbox(String sandboxName, String relativePath,
		...
		FilesystemSandbox sandbox = new FilesystemSandboxImpl(new File(
				_filesystemRoot, relativePath), doCreate);
	...
	public FilesystemWatchRegistration addWatch(Integer callLimit,
		...
		return _manager.addWatch(_filesystemName, this, callLimit, checkPeriod,
				checkPeriodUnits, filter, handler);
	}
