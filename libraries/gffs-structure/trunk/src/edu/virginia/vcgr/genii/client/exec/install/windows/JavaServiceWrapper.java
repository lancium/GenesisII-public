...
public class JavaServiceWrapper {
	static public abstract class JavaServiceWrapperTask extends
			AbstractExecutionTask {
		private File _wrapperExe;
		private File _configurationFile;

		protected Map<String, String> _variables;

		protected JavaServiceWrapperTask(File wrapperExe, File configurationFile)
				throws IOException {
		...
		protected void addVariables(Collection<String> cLine) {
			for (String key : _variables.keySet()) {
				cLine.add(String.format("set.%s=%s", key, _variables.get(key)));
			}
		}
	...
	static private class InstallServiceWrapper extends JavaServiceWrapperTask {
		...
		public InstallServiceWrapper(File wrapperExe, File configurationFile,
		...
		public String[] getCommandLine() {
			...
			if (_password == null)
				...
				cLine.add(String.format("wrapper.ntservice.password=%s",
						_password));
			}
	...
	static private class UninstallServiceWrapper extends JavaServiceWrapperTask {
		public UninstallServiceWrapper(File wrapperExe, File configurationFile)
				throws IOException {
			super(wrapperExe, configurationFile);
		}
	...
	static public JavaServiceWrapperTask installServiceWrapper(File wrapperExe,
			File configurationFile, String account, String password)
			throws IOException {
	...
	static public JavaServiceWrapperTask installServiceWrapper(File wrapperExe,
			File configurationFile, String account) throws IOException {
		return installServiceWrapper(wrapperExe, configurationFile, account,
				null);
	}
