...
/*
 * To avoid having a notifications from the container when we are not interested in the resource anymore, we should delete the subscriptions
 * from the database. Since we are doing all subscriptions through a notification broker, we can just hold onto the EPIs of the subscriptions
