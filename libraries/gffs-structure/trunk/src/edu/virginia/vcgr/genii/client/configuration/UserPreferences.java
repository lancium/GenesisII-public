...
public class UserPreferences {
	...
	static private MacroResolver createShellPromptMacroResolver() {
		PrefixMapMacroResolver prefixResolver = new PrefixMapMacroResolver(
				"grid");
		...
		dynamicGridResolution.put("net.name", new Object() {
			...
			public String toString() {
				String property = System
						.getProperty("edu.virginia.vcgr.genii.net-name");
				if (property != null)
					return property;
		...
		prefixResolver.addResolver("grid", new MapMacroResolver(
				dynamicGridResolution));
		prefixResolver
				.addResolver("env", new MapMacroResolver(System.getenv()));
	...
	private UserPreferences() {
		...
		_preferGUI = prefs.getBoolean(PREFER_GUI_PREFERENCE_KEY,
				DEFAULT_PREFER_GUI);
		exceptionHandlerClassName = prefs.get(EXCEPTION_HANDLER_PREFERENCE_KEY,
				DEFAULT_EXCEPTION_HANDLER.getClass().getName());
		_shellPrompt = new ShellPromptImpl(prefs.get(SHELL_PROMPT_TEMPLATE_KEY,
				DEFAULT_SHELL_PROMPT_TEMPLATE));
	...
	static private IExceptionHandler createExceptionHandler(String className) {
		try {
			...
			Class<? extends IExceptionHandler> cl = (Class<? extends IExceptionHandler>) loader
					.loadClass(className);
	...
	public void setExceptionHandler(IExceptionHandler handler)
		...
		prefs.put(EXCEPTION_HANDLER_PREFERENCE_KEY, _exceptionHandler
				.getClass().getName());
		prefs.flush();
	}
	...
	public void shellPromptTemplate(String template)
			throws BackingStoreException {
