...
public class CreateFSProxyPlugin extends AbstractCombinedUIMenusPlugin {
		...
		while (true) {
			...
			try {
				...
				FSViewCreatorDialog dialog = new FSViewCreatorDialog(
						SwingUtilities.getWindowAncestor(context
								.ownerComponent()));
				dialog.pack();
				dialog.setModalityType(ModalityType.DOCUMENT_MODAL);
				GUIUtils.centerWindow(dialog);
				dialog.setVisible(true);
				FSViewConnectionInformation connectInfo = dialog
				...
				if (connectInfo != null) {
					...
					path = path
							.lookup(answer, RNSPathQueryFlags.MUST_NOT_EXIST);

					RNSSelectionDialog rDialog = new RNSSelectionDialog(
							SwingUtilities.getWindowAncestor(context
									.ownerComponent()), context.uiContext(),
					...
					RNSPath proxyService = fObj.path().lookup(
							"Services/FSProxyPortType");
					CreateResourceTool.createInstance(
							proxyService.getEndpoint(),
							new GeniiPath(path.pwd()),
							new FSProxyConstructionParameters(connectInfo),
							connectInfo.shortName());
	...
	public boolean isEnabled(
		...
		if (selectedDescriptions == null || selectedDescriptions.size() != 1)
			return false;
		// ASG: 9-13-2013 This should be checking that it is a container ... not
		// that is an RNS.
