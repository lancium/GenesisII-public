...
/**
 * This Axis intercepter/handler class is used for retrieving the SAML
 * credentials wallet a client delegates to a resource when issuing an RPC. Note
 * that this class is populating the working context with the retrieved
 * credentials wallet. So this invoker must be invoked after we have already
 * processed the working context from the SOAP message context.
 * 
 * @author myanhaona
 * @author ckoeritz
 */
public class CredentialsWalletRetriever extends BasicHandler {
	...
	static private Log _logger = LogFactory
			.getLog(CredentialsWalletRetriever.class);
	...
	public void invoke(MessageContext messageContext) throws AxisFault {
		...
		while (iter.hasNext()) {
			...
			QName headerName = new QName(headerElement.getNamespaceURI(),
					headerElement.getLocalName());
			if (headerName
					.equals(GenesisIIConstants.DELEGATED_SAML_ASSERTIONS_QNAME)) {
		...
		Object oldcreds = workingContext
		...
		workingContext
				.setProperty(
						SAMLConstants.SAML_CREDENTIALS_WORKING_CONTEXT_CREDS_PROPERTY_NAME,
						wallet.getRealCreds());
	...
	/**
	 * A caller's SSL certificate is not a part of the delegated SAML
	 * credentials wallet. Nevertheless, we retrieve it and store it for the
	 * lifetime of an RPC because some of the access control rule may be
	 * directly applicable to the SSL certificate in use.
	 */
	private void retrieveSSLCertificate(MessageContext messageContext,
			WorkingContext workingContext) {
		Request request = (Request) messageContext
				.getProperty(HTTPConstants.MC_HTTP_SERVLETREQUEST);
		Object transport = request.getConnection().getEndPoint().getTransport();
		if (transport instanceof SSLSocket) {
			try {
				X509Certificate[] clientSSLCertificate = (X509Certificate[]) ((SSLSocket) transport)
