...
public enum SubscriptionPolicyTypes {
	UseRawPolicy(UseRawSubscriptionPolicy.class), BatchEvents(
			BatchEventsSubscriptionPolicy.class), CollapseEvents(
			CollapseEventsSubscriptionPolicy.class), IgnoreDuplicateEvents(
			IgnoreDuplicatesSubscriptionPolicy.class), PersistentNotification(
	...
	static public JAXBContext context() throws JAXBException {
		synchronized (SubscriptionPolicyTypes.class) {
			if (_context == null) {
				Class<?>[] classes = new Class<?>[SubscriptionPolicyTypes
						.values().length];
				int lcv = 0;
				for (SubscriptionPolicyTypes type : SubscriptionPolicyTypes
						.values())
