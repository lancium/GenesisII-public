...
public class TrashCanWidget extends JComponent {
	...
	public TrashCanWidget(ApplicationContext appContext, UIContext context) {
		...
		Dimension size = new Dimension(Math.max(Images.emptyTrashcan()
				.getWidth(), Images.fullTrashcan().getWidth()),
				Math.max(Images.emptyTrashcan().getHeight(), Images
						.fullTrashcan().getHeight()));
	...
	protected void paintComponent(Graphics _g) {
		...
		try {
			...
			BufferedImage image = _context.trashCan().isEmpty() ? Images
					.emptyTrashcan() : Images.fullTrashcan();
	...
	private class MouseClickListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2)
				TrashDialog.popupTrashDialog(TrashCanWidget.this, _appContext,
						_context);
		}
	}
