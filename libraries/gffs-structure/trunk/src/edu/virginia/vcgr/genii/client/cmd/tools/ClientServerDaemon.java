...
public class ClientServerDaemon
{
	...
	public ClientServerDaemon(IBackendFactory backendFactory, FTPConfiguration configuration)
	{
	...
	synchronized public void start() throws FTPException, IOException
	{
		...
		_serverThread.setName("ClientServer Daemon Server Thread");
				...
				while (true) {
					...
					if (!(addr instanceof InetSocketAddress) || !_configuration.connectionAllowed(addr)) {
						...
						ClientServerSession session =
							new ClientServerSession(_listenerManager, _configuration, nextSession++, _backendFactory.newBackendInstance(), socket);

						_vulture.addSession(session);

						Thread th = new Thread(session, "Client Server Session (" + (nextSession - 1) + ") Thread");
						th.setDaemon(true);
						th.start();
					}
				}
