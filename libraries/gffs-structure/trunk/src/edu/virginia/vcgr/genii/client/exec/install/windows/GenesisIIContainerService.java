...
public class GenesisIIContainerService {
	static public ExecutionTask installGenesisIIContainer(String account,
			String accountPassword) throws IOException {
		account = CommonFunctions.getAccount(account);

		File geniiInstallDir = CommonFunctions.getGeniiInstallDir();
		File wrapperExe = new File(geniiInstallDir,
				"ext\\JavaServiceWrapper\\bin\\wrapper.exe");
		File configurationFile = new File(geniiInstallDir,
				"ext\\JavaServiceWrapper\\conf\\runContainer.conf");
		File localJavaDir = new File(geniiInstallDir, "Java\\windows-i586\\jre");

		JavaServiceWrapper.JavaServiceWrapperTask task = JavaServiceWrapper
				.installServiceWrapper(wrapperExe, configurationFile, account,
						accountPassword);
	...
	static public ExecutionTask uninstallGenesisIIContainer()
			throws IOException {
		File geniiInstallDir = CommonFunctions.getGeniiInstallDir();
		File wrapperExe = new File(geniiInstallDir,
				"ext\\JavaServiceWrapper\\bin\\wrapper.exe");
		...
		JavaServiceWrapper.JavaServiceWrapperTask task = JavaServiceWrapper
				.uninstallServiceWrapper(wrapperExe, configurationFile);
