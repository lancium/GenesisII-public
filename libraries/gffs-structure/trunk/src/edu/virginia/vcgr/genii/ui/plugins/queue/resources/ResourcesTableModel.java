...
class ResourcesTableModel extends RowTableModel<QueueResourceInformation> {
	...
	private class RefreshCompletionListener implements
	...
	private class ResourceSlotsColumn extends
			AbstractRowTableColumnDefinition<QueueResourceInformation, Integer> {
		@Override
		protected void modifyImpl(QueueResourceInformation row, Integer column) {
			int answer = JOptionPane.showConfirmDialog(row.parent(), String
					.format("Change Max Slots for %s to %d?", row.name(),
							column), "Confirm Slot Change",
			...
			if (answer == JOptionPane.YES_OPTION) {
				_uiContext
						...
						.createMonitor(
								row.parent(),
								"Modifying Slot Count",
								String.format("Changing %s's slots to %d",
										row.name(), column), 100L,
								new SlotChangerTask(row, column),
								new RefreshCompletionListener(row.parent()))
						.start();
			}
		}
	...
	private RowTableColumnDefinition<?, ?>[] COLUMNS = {
			new ResourceNameColumn(), new ResourceTypeColumn(),
			new OperatingSystemTypeColumn(),
			new ProcessorArchitectureTypeColumn(),
			new AcceptingActivitiesColumn(), new AvailabilityColumn(),
			new LastUpdatedColumn(), new NextUpdateColumn(),
			new ResourceSlotsColumn() };

	private class QueueResourcesFetcherTask extends
			AbstractTask<Collection<QueueResourceInformation>> {
		@Override
		final public Collection<QueueResourceInformation> execute(
			...
			RNSIterable iterable = new RNSIterable(_rnsFuture.get()
					.lookup(null), _uiContext.uiContext().callingContext(),
					QueueConstants.PREFERRED_BATCH_SIZE);
			for (RNSEntryResponseType entry : iterable)
				resourceInfo
						.add(new QueueResourceInformation(_uiContext, entry));
	...
	private class QueueResourcesCompletionListener implements
			TaskCompletionListener<Collection<QueueResourceInformation>> {
		private Component _parentComponent;
		...
		public void taskCancelled(
				Task<Collection<QueueResourceInformation>> task) {
		...
		@Override
		public void taskExcepted(
				Task<Collection<QueueResourceInformation>> task, Throwable cause) {
			ErrorHandler.handleError(_uiContext.uiContext(),
					(JComponent) _parentComponent, cause);
		}
	}
	...
	private ArrayList<QueueResourceInformation> _contents = new ArrayList<QueueResourceInformation>();
	...
	void refresh(Component parentComponent) {
		_uiContext
				.uiContext()
				.progressMonitorFactory()
				.createMonitor(parentComponent, "Loading Queue Resources",
						"Fetching resources from queue", 1000L,
						new QueueResourcesFetcherTask(),
						new QueueResourcesCompletionListener(parentComponent))
				.start();
	}

	ResourcesTableModel(UIPluginContext uiContext) throws ResourceException,
			GenesisIISecurityException, RNSPathDoesNotExistException {
		_uiContext = uiContext;
		_rnsFuture = uiContext.uiContext().executor()
				.submit(new AsynchronousQueueResourcesExtractor(uiContext));
		_queue = ClientUtils.createProxy(QueuePortType.class, _uiContext
