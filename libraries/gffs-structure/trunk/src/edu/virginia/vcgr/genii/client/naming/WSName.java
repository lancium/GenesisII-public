...
public class WSName implements Comparable<WSName>, Serializable {
	...
	static public QName ENDPOINT_IDENTIFIER_QNAME = new QName(NAMING_NS,
			ENDPOINT_IDENTIFIER_LNAME);
	static public String REFERENCE_RESOLVER_LNAME = "ReferenceResolver";
	static public QName REFERENCE_RESOLVER_QNAME = new QName(NAMING_NS,
			REFERENCE_RESOLVER_LNAME);
	static public String ENDPOINT_IDENTIFIER_RESOLVER_LNAME = "EndpointIdentifierResolver";
	static public QName ENDPOINT_IDENTIFIER_RESOLVER_QNAME = new QName(
			NAMING_NS, ENDPOINT_IDENTIFIER_RESOLVER_LNAME);
			...
			for (MessageElement element : any) {
				if (element.getQName().equals(WSName.REFERENCE_RESOLVER_QNAME)) {
					try {
						...
						WSName.ENDPOINT_IDENTIFIER_RESOLVER_QNAME)) {
					try {
						EndpointReferenceType resolverEPR = (EndpointReferenceType) element
								.getObjectValue(EndpointReferenceType.class);
						if (resolverEPR != null) {
	...
	public void addReferenceResolver(EndpointReferenceType resolverEPR) {
		...
		ResolverDescription newResolverDesc = new ResolverDescription(
				_endpointIdentifier, resolverEPR,
				ResolverType.REFERENCE_RESOLVER);
		_epr = createEPRWithResolvers(_epr, resolverEPR,
				ResolverType.REFERENCE_RESOLVER, null);
		_resolvers.add(newResolverDesc);
	}

	public void addEndpointIdentifierReferenceResolver(
			EndpointReferenceType resolverEPR) {
		doExtraction();
		ResolverDescription newResolverDesc = new ResolverDescription(
				_endpointIdentifier, resolverEPR, ResolverType.EPI_RESOLVER);
		_epr = createEPRWithResolvers(_epr, resolverEPR,
				ResolverType.EPI_RESOLVER, null);
		_resolvers.add(newResolverDesc);
	}
	...
	private static EndpointReferenceType createEPRWithResolvers(
			EndpointReferenceType origEPR, EndpointReferenceType resolverEPR,
			ResolverType resolverType, List<ResolverDescription> resolvers) {
		AttributedURIType address = origEPR.getAddress();
		ReferenceParametersType referenceParameters = origEPR
		...
		// If we are adding a single new resolver, then include the original
		// resolvers.
		// If we are replacing the list of resolvers, then skip the original
		// resolvers.
		...
		if (resolverEPR != null) {
			newMetadataElements.add(makeResolverMessageElement(resolverEPR,
					resolverType));
		}
		// Add the new list of resolvers.
		if (resolvers != null) {
			for (ResolverDescription resolver : resolvers) {
				newMetadataElements.add(makeResolverMessageElement(
						resolver.getEPR(), resolver.getType()));
			}
		}

		MetadataType newMetadata = new MetadataType(
				newMetadataElements.toArray(new MessageElement[0]));
		return new EndpointReferenceType(address, referenceParameters,
				newMetadata, any);
	}

	private static boolean isResolverElement(MessageElement element) {
		return (element.getQName().equals(
				WSName.ENDPOINT_IDENTIFIER_RESOLVER_QNAME) || element
				.getQName().equals(WSName.REFERENCE_RESOLVER_QNAME));
	}

	private static MessageElement makeResolverMessageElement(
			EndpointReferenceType resolverEPR, ResolverType resolverType) {
		if (resolverType == ResolverType.REFERENCE_RESOLVER) {
			return new MessageElement(WSName.REFERENCE_RESOLVER_QNAME,
					resolverEPR);
		}
		if (resolverType == ResolverType.EPI_RESOLVER) {
			return new MessageElement(
					WSName.ENDPOINT_IDENTIFIER_RESOLVER_QNAME, resolverEPR);
		}
		return null;
	}
