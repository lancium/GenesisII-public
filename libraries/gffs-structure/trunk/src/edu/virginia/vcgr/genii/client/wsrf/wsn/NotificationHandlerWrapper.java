...
class NotificationHandlerWrapper<ContentsType extends NotificationMessageContents> {
	static private Log _logger = LogFactory
			.getLog(NotificationHandlerWrapper.class);
	...
	final String handleNotification(TopicPath topic,
		...
		try {
			return _handler.handleNotification(topic, producerReference,
					subscriptionReference,
					_handler.contentsType().cast(contents));
		} catch (Exception e) {
			_logger.warn("A notification handler threw an exception while "
