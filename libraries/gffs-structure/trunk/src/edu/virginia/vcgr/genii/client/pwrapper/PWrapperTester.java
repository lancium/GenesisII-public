...
public class PWrapperTester {
	static public void main(String[] args) throws Throwable {
		...
		File stdOutRedirect = new File(System.getProperty("user.home")
				+ "/pwd.out");

		File resourceUsageFile = new ResourceUsageDirectory(workingDirectory)
				.getNewResourceUsageFile();

		List<String> command = wrapper.formCommandLine(null, null,
				workingDirectory, null, stdOutRedirect, null,
				resourceUsageFile, null, "/bin/pwd");

		wrapper.execute(null, null, workingDirectory, null, resourceUsageFile,
				command);
	}
}