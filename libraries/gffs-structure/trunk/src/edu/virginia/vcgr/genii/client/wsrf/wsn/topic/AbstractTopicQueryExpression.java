...
abstract class AbstractTopicQueryExpression implements TopicQueryExpression,
		Serializable {
	...
	protected AbstractTopicQueryExpression(
			edu.virginia.vcgr.genii.client.wsrf.wsn.topic.TopicQueryDialects dialect) {
		_dialect = edu.virginia.vcgr.genii.client.wsrf.wsn.topic.TopicQueryDialects
				.convert(dialect);
	}
	...
	final public MessageElement toTopicExpressionElement(QName elementName,
		...
		for (Map.Entry<String, String> entry : factory._uri2PrefixMap
				.entrySet()) {
			ret.setAttributeNS(
					XMLConstants.XMLNS_ATTRIBUTE_NS_URI,
					String.format("%s:%s", XMLConstants.XMLNS_ATTRIBUTE,
