...
public class LookupCacheHandler {
	...
	public RNSEntryResponseType[] add(InvocationContext ctxt,
		...
		if (resp != null) {
			for (RNSEntryResponseType entry : resp) {
				...
				RNSNotificationHandler
						.updateLookupAndDirectoryCacheAfterEntryAddition(
								target, child, entryName);
			}
		}
		return resp;
	}
	...
	public CreateFileResponseType createFile(InvocationContext ctxt,
		...
		if (resp != null) {
			...
			RNSNotificationHandler
					.updateLookupAndDirectoryCacheAfterEntryAddition(target,
							child, entryName);
		}
		return resp;
	}
	...
	public RNSEntryResponseType[] remove(InvocationContext ctxt,
		...
		EndpointReferenceType target = ctxt.getTarget();
		if (resp != null) {
			RNSNotificationHandler.updateCacheAfterEntryRemoval(target,
					removeRequest);
		}
		return resp;
	}
	...
	@PipelineProcessor(portType = EnhancedRNSPortType.class)
	public RNSEntryResponseType[] rename(InvocationContext ctxt,
			NameMappingType[] renameRequest) throws Throwable {
		RNSEntryResponseType[] resp = (RNSEntryResponseType[]) ctxt.proceed();
		EndpointReferenceType target = ctxt.getTarget();
		for (NameMappingType mapping : renameRequest) {
			RNSNotificationHandler.updateCacheAfterEntryRename(target,
					mapping.getSourceName(), mapping.getTargetName());
		}
		return resp;
	}
}
