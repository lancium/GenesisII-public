...
public class MethodStatisticsReport implements Serializable {
	...
	void add(MethodDataPoint dp) {
		...
		MethodStatisticsReportPoint point = _classTotals.get(dp.serviceClass()
				.getName());
		if (point == null)
			_classTotals.put(dp.serviceClass().getName(),
					point = new MethodStatisticsReportPoint());
		point.add(dp);

		Map<String, MethodStatisticsReportPoint> map = _methodTotals.get(dp
				.serviceClass().getName());
		if (map == null)
			_methodTotals.put(dp.serviceClass().getName(),
					map = new HashMap<String, MethodStatisticsReportPoint>());
		point = map.get(dp.serviceMethod().getName());
		if (point == null)
			map.put(dp.serviceMethod().getName(),
					point = new MethodStatisticsReportPoint());
		point.add(dp);
	}
