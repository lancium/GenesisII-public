...
/**
 * Because there are so many flavors of ByteIO stream that can be created (and
 * so many ways to create them), I try and collect the most popular ways into
 * this one factory class. This class should generally be used for creating MOST
 * ByteIO streams.
 * 
 * @author mmm2a
 */
public class ByteIOStreamFactory {
	 ...
	 * 
	 * @param target
	 *            The target resource to read bytes from (can be streamable or
	 *            random).
	 * @param desiredTransferType
	 *            The desired transfer protocol (if any) to use. THis value CAN
	 *            be null.
	...
	static public InputStream createInputStream(EndpointReferenceType target,
		...
		if (tInfo.isRByteIO()) {
			RandomByteIOInputStream stream = new RandomByteIOInputStream(
					target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
		} else if (tInfo.isSByteIO() || tInfo.isSByteIOFactory()) {
			StreamableByteIOInputStream stream = new StreamableByteIOInputStream(
					target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
	 ...
	 */
	static public InputStream createInputStream(EndpointReferenceType target,
			boolean createBuffered) throws FileNotFoundException,
	 ...
	 *         bytes from the target ByteIO using the default transfer
	 *         mechanism.
	 * 
	 * @throws FileNotFoundException
	 * @throws RemoteException
	 ...
	 * 
	 * @return A newly created buffered Java InputStream to use for reading
	 *         bytes from the target ByteIO using the preferred transfer
	 *         mechanism.
	 * 
	 * @throws FileNotFoundException
	 * @throws RemoteException
	 ...
	 * 
	 * @param target
	 *            The target resource to write bytes to (can be streamable or
	 *            random).
	 * @param desiredTransferType
	 *            The desired transfer protocol (if any) to use. THis value CAN
	 *            be null.
	 ...
	 *            Whether or not the stream should be buffered.
	 * 
	 * @return A newly created Java OutputStream to use for writing bytes to the
	 *         target ByteIO.
	 * 
	 * @throws FileNotFoundException
	 * @throws RemoteException
	...
	static public OutputStream createOutputStream(EndpointReferenceType target,
		...
		TypeInformation tInfo = new TypeInformation(target);
		if (tInfo.isRByteIO()) {
			RandomByteIOOutputStream stream = new RandomByteIOOutputStream(
					target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
		} else if (tInfo.isSByteIO() || tInfo.isSByteIOFactory()) {
			StreamableByteIOOutputStream stream = new StreamableByteIOOutputStream(
					target, desiredTransferType);
			if (createBuffered)
				return stream.createPreferredBufferedStream();
			return stream;
	 ...
	 * 
	 * @return A newly created Java OutputStream to use for writing bytes to the
	 *         target ByteIO using the preferred transfer mechanism.
	 * 
	 * @throws FileNotFoundException
	 * @throws RemoteException
	 * @throws ConfigurationException
	 ...
	 *         bytes to the target ByteIO using the preferred transfer
	 *         mechanism.
	 * 
	 * @throws FileNotFoundException
	 * @throws RemoteException
