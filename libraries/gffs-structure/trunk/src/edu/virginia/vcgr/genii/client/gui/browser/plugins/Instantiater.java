...
public class Instantiater {
	/**
	 * Given a class and possible construction parameters, try and instantiate
	 * it into a plugin. In order to instantiate a plugin, the class must have a
	 * public constructor that takes an Element as a parameter (which will be
	 * the <construction-params/> element given with the plugin in the
	 * configuration files, or it must have a default empty constructor and NO
	 * construction-params given in the config file.
	 * 
	 * @param cl
	 *            The plugin class to instantiate.
	 * @param constructionParams
	 *            Any construction parameters given in the config file.
	...
	static public IPlugin instantiate(Class<IPlugin> cl,
		...
		try {
			try {
				 ...
				 * if the config file didn't indicate any construction
				 * parameters).
				 */
				 ...
				 * default constructor, so let's see if one exists and go ahead
			...
			throw new PluginException("Unable to instantiate plugin class \""
					+ cl.getName() + "\".", ia);
		} catch (IllegalAccessException iae) {
			throw new PluginException("Constructor for plugin class \""
					+ cl.getName() + "\" is not public.", iae);
		} catch (NoSuchMethodException nsme) {
			...
			throw new PluginException("Constructor for plugin class \""
					+ cl.getName() + "\" threw an exception.", ite.getCause());
		}
	}

	/**
	 * Given a classname and possible construction parameters, try and find the
	 * class and instantiate it into a plugin. In order to instantiate a plugin,
	 * the class must have a public constructor that takes an Element as a
	 * parameter (which will be the <construction-params/> element given with
