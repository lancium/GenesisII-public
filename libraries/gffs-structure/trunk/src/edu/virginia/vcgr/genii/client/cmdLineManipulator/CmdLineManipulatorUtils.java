...
public class CmdLineManipulatorUtils {
	static private Log _logger = LogFactory
			.getLog(CmdLineManipulatorUtils.class);
	...
	static public void addSPMDJobProperties(Map<String, Object> jobProperties,
		...
		if (variation != null) {
			jobProperties.put(CmdLineManipulatorConstants.SPMD_VARIATION,
					variation);
			jobProperties.put(CmdLineManipulatorConstants.NUMBER_OF_PROCESSES,
					numProcesses);
			jobProperties.put(
					CmdLineManipulatorConstants.NUMBER_OF_PROCESSES_PER_HOST,
					numProcessesPeHost);
		}
	}

	static public void addEnvProperties(Map<String, Object> jobProperties,
			File fuseMountPoint, Map<String, String> environment,
			File workingDirectory, File stdinRedirect, File stdoutRedirect,
			File stderrRedirect, File resourceUsagePath, File pathToWrapper) {
	...
	static public List<String> callCmdLineManipulators(
		...
		Set<VariationConfiguration> manipulatorVariations = cmdLineManipulatorConf
				.variationSet();
		...
		// get set of manipulators to be called as specified in construction
		// file
		...
		for (String manipulatorName : callChain) {
			...
			for (VariationConfiguration manipulatorVariation : manipulatorVariations) {
				...
				if (manipulatorVariation.variationName()
						.equals(manipulatorName)) {

					String manipulatorType = manipulatorVariation
							.variationType();
					...
					CmdLineManipulator<?> manipulator = CmdLineManipulators
							.getCmdLineManipulator(manipulatorType);

					if (_logger.isDebugEnabled())
						_logger.debug(String
								.format("  calling cmdLine manipulator \"%s\"'s transform",
										manipulatorName));
	...
	static public void addPwrapperManipulator(
		...
		Set<VariationConfiguration> variationConfigSet = manipulatorConf
				.variationSet();
		...
		pwrapperVariationConfig
				.variationType(pwrapperManipulator.MANIPULATOR_TYPE);
