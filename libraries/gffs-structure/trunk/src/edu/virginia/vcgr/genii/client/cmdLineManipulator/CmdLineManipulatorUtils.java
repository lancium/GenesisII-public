...
public class CmdLineManipulatorUtils
{
	...
	static public void addSPMDJobProperties(Map<String, Object> jobProperties, URI variation, Integer numProcesses,
		Integer numProcessesPeHost, Integer threadsPerProcess)
	{
		if (variation != null) {
		...
		File workingDirectory, File stdinRedirect, File stdoutRedirect, File stderrRedirect, File resourceUsagePath, File pathToWrapper)
	{
		...
		jobProperties.put(CmdLineManipulatorConstants.ENVIRONMENT, environment);
		jobProperties.put(CmdLineManipulatorConstants.WORKING_DIRECTORY, workingDirectory);
		jobProperties.put(CmdLineManipulatorConstants.STDIN_REDIRECT, stdinRedirect);
		jobProperties.put(CmdLineManipulatorConstants.STDOUT_REDIRECT, stdoutRedirect);
	...
	static public List<String> callCmdLineManipulators(Map<String, Object> jobProperties,
		CmdLineManipulatorConfiguration cmdLineManipulatorConf) throws CmdLineManipulatorException
	{
		// new manip conf should have been created on nativeq connection
		if (cmdLineManipulatorConf == null)
			throw new IllegalArgumentException("Null cmdLine manipulator construction params.");

		List<String> cmdLine = new ArrayList<String>();

		// get set of manipulator variations defined in construction file
		Set<VariationConfiguration> manipulatorVariations = cmdLineManipulatorConf.variationSet();

		// new var set should have been initialized on creation
		if (manipulatorVariations == null)
			throw new IllegalArgumentException("Null cmdLine manipulator variations in construction params.");
		...
		if (callChain == null)
			throw new IllegalArgumentException("Null manipulator call chain in construction params.");
		...
		for (String manipulatorName : callChain) {
			...
			for (VariationConfiguration manipulatorVariation : manipulatorVariations) {
				...
				if (manipulatorVariation.variationName().equals(manipulatorName)) {
					...
					CmdLineManipulator<?> manipulator = CmdLineManipulators.getCmdLineManipulator(manipulatorType);

					if (_logger.isDebugEnabled())
						_logger.debug(String.format("  calling cmdLine manipulator \"%s\"'s transform", manipulatorName));

					// call manipulator to transform cmdLine
					cmdLine = manipulator.transform(jobProperties, cmdLineManipulatorConf, manipulatorName);
					if (_logger.isDebugEnabled())
	...
	static public void addPwrapperManipulator(CmdLineManipulatorConfiguration manipulatorConf)
	{
		if (_logger.isDebugEnabled())
			_logger.debug("Adding default pwrapper cmdline manipulator.");

		Set<VariationConfiguration> variationConfigSet = manipulatorConf.variationSet();

		List<String> callChainConf = manipulatorConf.callChain();

		if (variationConfigSet == null)
			variationConfigSet = new HashSet<VariationConfiguration>();

		// add pwrapper manipulator
		VariationConfiguration pwrapperVariationConfig = new VariationConfiguration();
		String variationName = String.format("default_pwrapper");
		pwrapperVariationConfig.variationName(variationName);
		pwrapperVariationConfig.variationType(pwrapperManipulator.MANIPULATOR_TYPE);
