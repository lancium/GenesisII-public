...
class QueueManipulation {
	static private abstract class TypicalTask<Type> extends AbstractTask<Type> {
		...
		final protected QueuePortType queue() throws ResourceException,
				GenesisIISecurityException, RNSPathDoesNotExistException {
			return ClientUtils.createProxy(QueuePortType.class, _context
					.endpointRetriever().getTargetEndpoints().iterator().next()
	...
	static private class JobCompleterTask extends TypicalTask<Integer> {
		...
		final public Integer execute(TaskProgressListener progressListener)
				throws Exception {
			QueuePortType queue = queue();
			queue.completeJobs(_jobTickets.toArray(new String[_jobTickets
					.size()]));
			return 0;
		}
	}

	static private class OpenJobHistoryDumpTargetTask extends
			TypicalTask<OutputStream> {
	...
	static private class JobHistoryTask extends
			TypicalTask<Collection<HistoryEvent>> {
		...
		final public Collection<HistoryEvent> execute(
			...
			try {
				QueuePortType queue = queue();
				IterateHistoryEventsResponseType resp = queue
				...
				for (HistoryEventBundleType bundle : iterable) {
					ret.add((HistoryEvent) DBSerializer.deserialize(bundle
							.getData()));
					if (wasCancelled())
						break;
				}
	...
	static private abstract class TypicalTaskCompletionListener<Type>
		...
		protected TypicalTaskCompletionListener(Component ownerComponent,
	...
	static private class JobKillerCompletionListener extends
			TypicalTaskCompletionListener<Integer> {
	...
	static private class JobCompleterCompletionListener extends
			TypicalTaskCompletionListener<Integer> {
	...
	static private class JobHistoryCompletionListener extends
			TypicalTaskCompletionListener<Collection<HistoryEvent>> {
			...
			try {
				if (_dumpTarget != null) {
					...
					JobHistoryFrame frame = new JobHistoryFrame(
							_context.uiContext(), _context.endpointRetriever()
									.getTargetEndpoints().iterator().next(),
							_ticket, result);
					frame.pack();
	...
	static private class OpenJobHistoryDumpTargetCompletionListener extends
			TypicalTaskCompletionListener<OutputStream> {
		...
		public void taskCompleted(Task<OutputStream> task, OutputStream result) {
			_context.uiContext()
					...
					.createMonitor(
							_ownerComponent,
							"Getting Job History",
							"Getting job history events",
							1000L,
							new JobHistoryTask(_context, _jobTickets),
							new JobHistoryCompletionListener(_ownerComponent,
									_context, _jobTickets.iterator().next(),
									_model, result)).start();
		}
	...
	static void killJobs(UIPluginContext context, Component ownerComponent,
		...
		context.uiContext()
				...
				.createMonitor(
						ownerComponent,
						"Killing Jobs",
						"Asking queue to terminate jobs",
						1000L,
						new JobKillerTask(context, jobTickets),
						new JobKillerCompletionListener(ownerComponent,
								context, model)).start();
	}
	...
	static void completeJobs(UIPluginContext context, Component ownerComponent,
		...
		context.uiContext()
				...
				.createMonitor(
						ownerComponent,
						"Removing Jobs",
						"Asking queue to remove jobs",
						1000L,
						new JobCompleterTask(context, jobTickets),
						new JobCompleterCompletionListener(ownerComponent,
								context, model)).start();
	}
	...
	static void jobHistory(UIPluginContext context, Component ownerComponent,
		...
		context.uiContext()
				...
				.createMonitor(
						...
						new JobHistoryCompletionListener(ownerComponent,
								context, jobTickets.iterator().next(), model))
				.start();
	}
	...
	static void dumpJobHistory(UIPluginContext context,
		...
		context.uiContext()
				...
				.createMonitor(
						ownerComponent,
						"Opening Job History Dump Target",
						"Opening Target",
						1000L,
						new OpenJobHistoryDumpTargetTask(context, jobTickets,
								dumpPath),
						new OpenJobHistoryDumpTargetCompletionListener(
								ownerComponent, context, jobTickets, model))
				.start();
	}
}