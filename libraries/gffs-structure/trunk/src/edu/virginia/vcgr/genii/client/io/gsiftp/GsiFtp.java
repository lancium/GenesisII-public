...
public class GsiFtp {
	...
	private void executeShellScript() {
		...
		try {
			Process gsiftpExec = r.exec("sh "
					+ workingDirectory.getAbsolutePath() + "/gsiftp.sh");
			try {
				gsiftpExec.waitFor();
	...
	private void writeShellScriptToDisk() {
		try {
			...
			out.write("export X509_USER_PROXY="
					+ workingDirectory.getAbsolutePath()
					+ GenesisIIConstants.myproxyFilenameSuffix + " \n");
			out.write("chmod 600 " + workingDirectory.getAbsolutePath()
					+ GenesisIIConstants.myproxyFilenameSuffix + " \n");
