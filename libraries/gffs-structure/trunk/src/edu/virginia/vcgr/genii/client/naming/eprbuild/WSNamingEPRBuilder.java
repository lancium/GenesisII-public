...
public class WSNamingEPRBuilder extends GenericEPRBuilder {
	...
	final public void addEndpointIdentifierResolvers(
	...
	public Collection<Element> metadata() {
		...
		/* Add EPI */
		if (_epi != null)
			ret.add(new MessageElement(WSName.ENDPOINT_IDENTIFIER_QNAME, _epi
					.toString()));
		...
		if (!_epiResolvers.isEmpty()) {
			for (EndpointReferenceType epr : _epiResolvers) {
				try {
					ret.add(ObjectSerializer.toElement(epr,
