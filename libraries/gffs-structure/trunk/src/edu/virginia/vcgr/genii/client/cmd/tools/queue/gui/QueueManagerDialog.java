...
public class QueueManagerDialog extends JFrame {
	...
	QueueManagerDialog(EndpointReferenceType queue) throws RemoteException {
		...
		_table.getColumnModel().getColumn(4)
				.setCellRenderer(new StateRenderer());
		_table.getColumnModel().getColumn(1)
				.setCellRenderer(new TimestampRenderer());
		prepareColumns(_table);
		JScrollPane scroller = new JScrollPane(_table);
		scroller.setPreferredSize(new Dimension(1000, 500));
		content.add(scroller, new GridBagConstraints(0, 0, 5, 1, 1.0, 1.0,
		...
		content.add(new JButton(new DeleteAction(_table)),
				new GridBagConstraints(3, 1, 1, 1, 1.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.NONE,
						new Insets(5, 5, 5, 5), 5, 5));
		content.add(new JButton(new RefreshAction()), new GridBagConstraints(4,
	...
	private class CompleteAction extends AbstractAction implements
		...
		public void actionPerformed(ActionEvent event) {
			...
			int result = JOptionPane.showConfirmDialog(QueueManagerDialog.this,
					String.format("Are you sure you want to delete %d jobs?",
							indices.length), "Queue Delete Confirmation?",
	...
	private class ShowErrorsAction extends AbstractAction implements
			ListSelectionListener {
		static final long serialVersionUID = 0L;
		...
		public void actionPerformed(ActionEvent e) {
			...
			ErrorDisplay display = new ErrorDisplay(QueueManagerDialog.this,
					_queueEPR, new JobTicket(_table.getValueAt(index, 0)
							.toString()));
			display.pack();
	...
	private class ShowLogAction extends AbstractAction implements
			ListSelectionListener {
		static final long serialVersionUID = 0L;
