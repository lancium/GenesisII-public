...
public abstract class AbstractLoginHandler implements CallbackHandler {
	...
	protected AbstractLoginHandler(PrintWriter out, PrintWriter err,
			BufferedReader in) {
		_out = out;
		_err = err;
		_in = in;
	...
	protected Collection<CertEntry> retrieveCertEntries(InputStream storeInput,
			String storeType, String password) throws GeneralSecurityException,
			IOException {
			...
			for (int tryNumber = 0; tryNumber < _PASSWORD_TRIES; tryNumber++) {
				try {
					entry._privateKey = (PrivateKey) entry._keyStore.getKey(
							entry._alias, passwordChars.toCharArray());
					break;
					...
					if (_password != null) {
						...
						passwordChars = new String(getPassword("Key Password",
	...
	private void handle(PasswordCallback callback) {
		if (_password != null) {
			...
			_password = new String(getPassword("Password Entry",
					callback.getPrompt() + ": "));
			callback.setPassword(_password.toCharArray());
		}
	}
