...
public class WSResourceConfig {
	...
	/*
	 * These two properties indicate that the concerned resource will remain
	 * synchronized -- using container to client update notifications or by
	 * polling status -- with the container holding it for the specified period
	 * of time. This information is used to increase the lifetime of the related
	...
	/*
	 * This two fields are used to check whether the container has notified the
	 * client not to rely on cache for a specific period of time. The container
	 * does that when it receive too many notification to be delivered on behave
	...
	/*
	 * Represents in which GenesisII container the resource is located. For
	 * non-GenesisII resources this should be NULL.
	 */
	...
	public WSResourceConfig(WSName wsName) {
		...
		if (wsName.isValidWSName()) {
			...
			// It can happen that a subscribed resource's resource configuration
			// got evicted from
			// the
			// cache due to overload. In future, if we create another
			// configuration for the resource
			// as we start reusing it (coming back to a previously visited
			// directory, etc.) then we
			// should retrieve the subscription related information to ensure
			// that cache contents
			// related
			// to this resource are given appropriate lifetimes.
			String EPI = wsName.getEndpointIdentifier().toString();
			Date subscriptionTimeoutTime = SubscriptionDirectory
					.getSubscriptionTimeoutTime(EPI);
			if (subscriptionTimeoutTime != null
					&& subscriptionTimeoutTime.getTime() > System
							.currentTimeMillis()) {
	...
	public boolean isHasRegisteredCallback() {
		...
		// This is done to ensure correctness as otherwise we would need
		// something
		// like a cronjob to reset the flags of cached configurations.
		if ((callbackExpiryTime != null)
				&& callbackExpiryTime.before(new Date())) {
