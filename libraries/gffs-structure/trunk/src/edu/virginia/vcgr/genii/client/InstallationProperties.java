...
/**
 * This class supports queries for items normally found in the Security class
 * (via the KeystoreSecurityConstants). It also supports some crucial items that
 * were previously in ContainerProperties and WebContainerConstants. It should
 * be used as a first resource that provides a "more local" configuration than
 * older style (split configuration) installs. This new lookup process
 * implements the new configuration style we've called "unified configuration"
 * in the installer design document. Many configuration items can now be
 * overridden in the file called "installation.properties" that lives in the
 * state directory, which is managed by the new container configuration scripts
 * (see trunk/installer/scripts).
 * 
...
public class InstallationProperties extends Properties {
	...
	static private Log _logger = LogFactory
			.getLog(InstallationProperties.class);
	// our singular instance of this class.
	static private InstallationProperties _realInstallationProperties = null;

	/**
	 * for all normal run-time classes, the installation properties are accessed
	 * this way.
	 */
	...
	static private File getInstallationPropertiesFile() {
		File ret = new File(getUserDir(),
				InstallationConstants.INSTALLATION_PROPERTIES_FILENAME);
	...
	public HierarchicalDirectory getLocalCertsDirectory() {
		...
		return HierarchicalDirectory.openRootHierarchicalDirectory(new File(
				prop));
	}
	...
	public HierarchicalDirectory getDefaultOwnersDirectory() {
		...
		return HierarchicalDirectory.openRootHierarchicalDirectory(new File(
				prop + "/" + InstallationConstants.OWNER_CERTS_DIRECTORY_NAME));
	}
		...
		File[] found = dir.listFiles(new FileFilter() {
			public boolean accept(File f) {
				return f.getName().equals(
						InstallationConstants.OWNER_CERT_FILE_NAME);
			}
		});
	...
	static public String getUserDir() {
		String userDir = null;
		// see if we have a valid container properties and can retrieve the
		// value that way.
		ContainerProperties cProperties = ContainerProperties
				.getContainerProperties();
		if (cProperties != null)
			userDir = cProperties.getUserDirectoryProperty();
		// well, see if we can just get the state directory from the
		// environment.
		if (userDir == null)
			userDir = ApplicationBase.getUserDirFromEnvironment();
		// now, if we have something at all, try comparing it with our
		...
		// by now we'll have a state directory path, even if we have to use the
		// default.
		try {
			// load the state directory so we can get an absolute path and also
			// verify its health.
