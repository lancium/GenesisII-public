...
public class RunTool extends BaseGridTool {
	static private Pattern _STAGE_PATTERN = Pattern
			.compile("([^\\/]+)\\/(.+)$");

	static private final String _DESCRIPTION = "config/tooldocs/description/drun";
	static private final String _USAGE_RESOURCE = "config/tooldocs/usage/urun";
	static private final String _MANPAGE = "config/tooldocs/man/run";
	private BESActivityConstants aconsts = new BESActivityConstants();
	private BESConstants bconsts = new BESConstants();

	private Object _stateLock = new Object();
	private ActivityState _state = null;

	private class NotificationHandler extends
	...
	protected int runCommand() throws Throwable {
		...
		if (gPath.pathType() != GeniiPathType.Grid)
			throw new InvalidToolUsageException(
					"[async-name] must be a grid path. ");
		...
		if (gPath0.pathType() != GeniiPathType.Grid)
			if (_checkStatus)
				throw new InvalidToolUsageException(
						"<job-path> must be a grid path");
			else
				throw new InvalidToolUsageException(
						"<bes-container> | <scheduler> must be a grid path");
		...
		if (_checkStatus) {
			...
			if (asyncPath.equals("")) {
				...
				TopicQueryExpression topicFilter = BESActivityTopics.ACTIVITY_STATE_CHANGED_TOPIC
						.asConcreteQueryExpression();

				subscribeRequest = server.createSubscribeRequest(topicFilter,
						null, null);

				server.registerNotificationHandler(topicFilter,
			...
			if (_jsdl != null) {
				...
				activity = submitJob(_jsdl, besContainer, jobName,
						subscribeRequest);
			} else {
				besOrSchedPath = lookup(gPath0, RNSPathQueryFlags.MUST_EXIST);
				besContainer = getBESContainer(besOrSchedPath);
				String jobName = _name;
				JobDefinition_Type jobDef = createJobDefinition(jobName,
						getArguments().subList(1, getArguments().size()));
				ObjectSerializer.serialize(stdout, jobDef, new QName(
						"http://example.org", "job-definition"));
				stdout.flush();
	...
	protected void verify() throws ToolException {
		if (_checkStatus) {
			...
			if (numArguments() != 1)
				throw new InvalidToolUsageException(
						"No BES container specified.");
		} else if (_name != null) {
			if (numArguments() < 2)
				throw new InvalidToolUsageException(
						"No BES container or executable specified.");
		} else {
			throw new InvalidToolUsageException(
					"Missing required flags --check-status, --jsdl, or --name");
		}
	}

	public JobDefinition_Type createJobDefinition(String jobName,
			java.util.List<String> cLine) throws URI.MalformedURIException {
	...
	public ActivityState checkStatus(String jobPath) throws RemoteException,
			RNSException, IOException, InvalidToolUsageException {
	...
	public ActivityState checkStatus(EndpointReferenceType activity)
		...
		GetResourcePropertyResponse resp = common
				.getResourceProperty(aconsts.STATUS_ATTR);

		MessageElement elem = (MessageElement) (resp.get_any()[0]
				.getChildElements().next());
		return new ActivityState(elem);
	}

	static public ActivityState checkStatus(EndpointReferenceType besContainer,
			EndpointReferenceType activity) throws ResourceException,
			RemoteException, RNSPathDoesNotExistException {
		GeniiBESPortType bes = ClientUtils.createProxy(GeniiBESPortType.class,
				besContainer);
		GetActivityStatusesResponseType resp = bes
	...
	public EndpointReferenceType submitJob(String jsdlFileName,
			EndpointReferenceType besContainer, String optJobName,
			SubscribeRequest subscribeRequest) throws IOException,
			ResourceException, RNSException {
		...
		try {
			...
			JobDefinition_Type jobDef = (JobDefinition_Type) ObjectDeserializer
					.deserialize(new InputSource(fin), JobDefinition_Type.class);

			if (optJobName != null) {
				JobIdentification_Type ident = jobDef.getJobDescription()
	...
	public EndpointReferenceType submitJob(JobDefinition_Type jobDef,
			EndpointReferenceType besContainer,
			SubscribeRequest subscribeRequest) throws ResourceException,
		...
		CreateActivityType createActivityRequest = new CreateActivityType(adt,
				null);

		CreateActivityResponseType response = bes
	...
	static private QName _GENII_APP_PATH_ELEMENT = new QName(GENII_APP_NS,
			"ApplicationPath");
	static private QName _GENII_APP_ENDPOINT_ELEMENT = new QName(GENII_APP_NS,
			"ApplicationEndpoint");

	private JobDefinition_Type deployAndReify(GeniiBESPortType bes,
			JobDefinition_Type jobDef) throws RNSException, ResourceException,
			RemoteException {
		EndpointReferenceType applicationDescriptionEPR = null;

		Application_Type application = jobDef.getJobDescription()
				.getApplication();
		if (application != null) {
			...
			if (any != null) {
				for (MessageElement element : any) {
					...
					if (name.equals(_GENII_APP_PATH_ELEMENT)) {
						applicationDescriptionEPR = getApplicationDescriptionEndpoint(element
								.getAttribute("path"));
						break;
					} else if (name.equals(_GENII_APP_ENDPOINT_ELEMENT)) {
						applicationDescriptionEPR = ObjectDeserializer
								.toObject(element, EndpointReferenceType.class);
						break;
					}
				}
			}
		...
		if (applicationDescriptionEPR != null) {
			...
			EndpointReferenceType deploymentEPR = createDeployment(deployers
					.iterator().next(), applicationDescriptionEPR);
			if (deploymentEPR == null)
				return null;

			ApplicationDeployerPortType deployment = ClientUtils.createProxy(
					ApplicationDeployerPortType.class, deploymentEPR);
			jobDef = deployment.reifyJSDL(new ReifyJSDLRequestType(jobDef))
					.getReifiedDocument();
		}
	...
	static private EndpointReferenceType getApplicationDescriptionEndpoint(
		...
		RNSPath rPath = RNSPath.getCurrent().lookup(path,
				RNSPathQueryFlags.MUST_EXIST);

		return rPath.getEndpoint();
	}

	private Collection<EndpointReferenceType> getPossibleBESDeployers(
			GeniiBESPortType bes) throws ResourceUnknownFaultType,
			RemoteException {
		ArrayList<EndpointReferenceType> ret = new ArrayList<EndpointReferenceType>();

		GetResourcePropertyResponse response = bes
				.getResourceProperty(bconsts.DEPLOYER_EPR_ATTR);
		MessageElement[] any = response.get_any();
		if (any == null)
			return ret;

		for (MessageElement element : any) {
			ret.add(ObjectDeserializer.toObject(element,
	...
	static private EndpointReferenceType createDeployment(
		...
		ApplicationDeployerPortType deployer = ClientUtils.createProxy(
				ApplicationDeployerPortType.class, deployerService);

		EndpointReferenceType deploymentDescription = pickDeploymentDescription(
				deployer, applicationDescription);

		if (deploymentDescription == null)
			return null;

		return deployer.createDeployment(
				new CreateDeploymentRequestType(deploymentDescription))
				.getDeployment();
	}
	...
	static private EndpointReferenceType pickDeploymentDescription(
		...
		ApplicationDescriptionPortType application = ClientUtils.createProxy(
				ApplicationDescriptionPortType.class, applicationDescription);
		RNSIterable iterable = new RNSIterable(application.lookup(null), null,
		...
		for (RNSEntryResponseType entry : iterable) {
			...
			for (MessageElement element : mdt.get_any()) {
				SupportDocumentType doc = ObjectDeserializer.toObject(element,
						SupportDocumentType.class);
		...
		return _acceptableChoices.get(
				_generator.nextInt(_acceptableChoices.size())).getEndpoint();
	}
	...
	static private SupportDocumentType[] determineSupport(
		...
		for (int lcv = 0; lcv < any.length; lcv++) {
			ret[lcv] = ObjectDeserializer.toObject(any[lcv],
					SupportDocumentType.class);
		}
	...
	static private Throwable getError(EndpointReferenceType activity)
		...
		if (typeInfo.isBESActivity()) {
			BESActivityPortType act = ClientUtils.createProxy(
					BESActivityPortType.class, activity);
			...
			return new IOException(
					"Activity not a standard Genesis II activity -- Error information unavailable!");
		}
	}
}
