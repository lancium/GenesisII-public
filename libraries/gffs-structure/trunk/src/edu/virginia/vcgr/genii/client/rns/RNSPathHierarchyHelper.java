...
public class RNSPathHierarchyHelper implements TreeTraversalPathQuery<RNSPath> {
	static private Log _logger = LogFactory
			.getLog(RNSPathHierarchyHelper.class);
	...
	/**
	 * some basic tests on the path provided. true is returned if it looks like
	 * a normal file or directory at the location.
	 */
	public static PathOutcome checkPathIsNormal(RNSPath path,
			boolean bounceResourceForks) {
		if (path == null)
		...
		try {
			...
			if (!info.isRNS() && !info.isEnhancedRNS() && !info.isRByteIO()) {
				_logger.warn("RNS checkPath bouncing non-RNS & non-RByteIO type at: "
						+ path.pwd());
				bounce_it = true;
			}
			/*
			 * always bouncing on resource forks cannot be considered
			 * reasonable. exports are resource forks. jobs that we submit to
			 * queue submission points get considered to be resource forks
			 * afterwards. we need to be able to copy and delete into these
			 * unfettered, and just rely on the authorization to keep people
			 * from messing with more than they should.
			 */
			if (bounceResourceForks && !bounce_it && info.isResourceFork()) {
				_logger.warn("RNS checkPath bouncing resource fork at: "
						+ path.pwd());
				bounce_it = true;
			}
			if (bounce_it) {
			...
			if (_logger.isDebugEnabled())
				_logger.debug("caught exception in RNS checkPathIsNormal",
						cause);
	...
	@Override
	public PathOutcome checkPathSanity(RNSPath path,
			TreeTraversalActionAlert<RNSPath> bounce) {
		PathOutcome ret = checkPathIsNormal(path, _bounceResourceForks);
		if (ret.differs(PathOutcome.OUTCOME_BOUNCED)
		...
		// phase 2: check for whether we have seen this path before based on its
		// EPI.
		WSName epname;
		try {
			epname = new WSName(path.getEndpoint());
		} catch (RNSPathDoesNotExistException e) {
			_logger.error("RNS checkPath problem checking path: "
					+ path.toString());
			return PathOutcome.OUTCOME_NONEXISTENT;
		}
		if ((epname == null) || (epname.getEndpointIdentifier() == null)) {
			_logger.error("RNS checkPath problem getting EPI for: "
					+ path.toString());
	...
	public Collection<RNSPath> getContents(RNSPath path) {
		...
		try {
			...
			_logger.error(
					"failed to list path contents for " + path.toString(),
					cause);
			return new ArrayList<RNSPath>();
		}
	}
