...
public class ReplicateTool extends BaseGridTool {
	...
	protected int runCommand() throws Throwable {
		...
		if (endpointIdentifier == null) {
			stdout.println("replicate error: " + sourceRNS
					+ ": EndpointIdentifier not found");
			return (-1);
		}
		List<ResolverDescription> resolverList = ResolverUtils
				.getResolvers(sourceName);
		if ((resolverList == null) || (resolverList.size() == 0)) {
			stdout.println("replication error: " + sourceRNS
					+ ": Resource has no resolver element");
			return (-1);
		}
		TypeInformation type = new TypeInformation(sourceEPR);
		String serviceName = type.getBestMatchServiceName();
		if (serviceName == null) {
			stdout.println("replicate: " + sourceRNS
					+ ": Type does not support replication");
			return (-1);
		}
		String servicePath = containerPath + '/' + "Services" + '/'
				+ serviceName;
		...
		if (linkPath != null) {
			linkRNS = current
					.lookup(linkPath, RNSPathQueryFlags.MUST_NOT_EXIST);
		}
		...
		elementArr[0] = new MessageElement(
				...
				endpointIdentifier);
		elementArr[1] = new MessageElement(
				IResource.PRIMARY_EPR_CONSTRUCTION_PARAM, sourceEPR);
		...
		CacheManager.removeItemFromCache(sourceEPR,
				RNSConstants.ELEMENT_COUNT_QNAME, MessageElement.class);
		CacheManager.removeItemFromCache(newEPR,
				RNSConstants.ELEMENT_COUNT_QNAME, MessageElement.class);

		/*
		 * if (ADD_RESOURCES_TO_ACLS) { // Allow the new resource to modify the
		 * old resource, and vice versa, // even if the user did not delegate
		 * his identity to the resource. newSP.addResource(oldSP);
	...
	private void addPolicy(RNSPath currentRNS, Stack<RNSPath> stack)
		...
		GeniiCommon dirService = ClientUtils.createProxy(GeniiCommon.class,
				currentRNS.getEndpoint());
		MessageElement[] elementArr = new MessageElement[1];
		elementArr[0] = new MessageElement(
				GeniiDirPolicy.REPLICATION_POLICY_QNAME, "true");
		UpdateResourceProperties request = new UpdateResourceProperties(
				new UpdateType(elementArr));
	 ...
	 * 
	 * Be careful! If you specify a pathname that refers to an EPR with a
	 * resolver element, then this may failover and destroy the wrong replica.
	 * 
	...
	private int destroyReplica() throws Throwable {
		...
		RNSPath current = RNSPath.getCurrent();
		RNSPath replicaRNS = current.lookup(replicaPath,
				RNSPathQueryFlags.MUST_EXIST);
		EndpointReferenceType replicaEPR = replicaRNS.getEndpoint();

		MessageElement[] elementArr = new MessageElement[1];
		elementArr[0] = new MessageElement(SyncProperty.UNLINKED_REPLICA_QNAME,
				"true");
		...
		// In the ideal case, maybe the directory entry should be replaced with
		// an
		// equivalent entry with a different default address?
		// If the entry had no resolver element, or if this was the last
		// replica,
