...
class JobStateColumn
		extends
		AbstractRowTableColumnDefinition<JobInformation, Pair<QueueStates, String>> {
	...
	JobStateColumn() {
		super(
				"Job State",
				(Class<Pair<QueueStates, String>>) new Pair<QueueStates, String>(
						null, null).getClass(), 45);
	}

	@Override
	final public Pair<QueueStates, String> extract(JobInformation row) {
		return new Pair<QueueStates, String>(row.getJobState(),
				row.getScheduledOn());
	}
}