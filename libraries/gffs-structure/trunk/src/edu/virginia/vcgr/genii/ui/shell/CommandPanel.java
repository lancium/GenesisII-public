...
public class CommandPanel extends JPanel {
	...
	static final private Dimension OUTPUT_SIZE = new Dimension(500, 500);

	public CommandPanel(UIContext uiContext) {
		super(new GridBagLayout());

		CommandDisplay display = new CommandDisplay(uiContext);
		JLabel label = new JLabel(UserPreferences.preferences().shellPrompt()
				.toString());
		label.setMinimumSize(label.getMinimumSize());
		label.setPreferredSize(label.getPreferredSize());
		label.setMaximumSize(label.getMaximumSize());
		CommandField field = new CommandField(uiContext, label,
				display.display(), new GridExecutionContext(
						uiContext.callingContext()), 32);

		JScrollPane scroller = new JScrollPane(display);
		scroller.setMinimumSize(OUTPUT_SIZE);
		scroller.setPreferredSize(OUTPUT_SIZE);

		add(GUIUtils.addTitle("Output", scroller), new GridBagConstraints(0, 0,
				2, 1, 1.0, 1.0, GridBagConstraints.CENTER,
