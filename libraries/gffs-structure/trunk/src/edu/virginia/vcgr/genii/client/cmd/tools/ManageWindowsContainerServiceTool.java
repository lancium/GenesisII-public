...
public class ManageWindowsContainerServiceTool extends BaseGridTool {
	static private Log _logger = LogFactory
			.getLog(ManageWindowsContainerServiceTool.class);
	...
	public ManageWindowsContainerServiceTool() {
		super(new LoadFileResource(DESCRIPTION), new LoadFileResource(USAGE),
				true, ToolCategory.ADMINISTRATION);
	}
	...
	protected int runCommand() throws Throwable {
		...
		String portString = InstallationProperties.getInstallationProperties()
				.getContainerPort();
		if (portString == null)
			portString = webContainerProperties.getProperty(
		...
		if (_install)
			tasks = new ExecutionTask[] {
					...
					WindowsServices
							.createStartServiceTask("Genesis II Container"), };
		else
			tasks = new ExecutionTask[] {
					WindowsServices
							.createStopServiceTask("Genesis II Container"),
		...
		for (ExecutionTask eTask : tasks) {
			...
			for (int lcv = 0; lcv < 5; lcv++) {
				try {
					WindowsExecutionEngine.execute(null, System.out,
							System.err, eTask);
					success = true;
					break;
				} catch (Throwable cause) {
					_logger.error(
							String.format("Unable to execute task(%s).", eTask),
							cause);
					try {
