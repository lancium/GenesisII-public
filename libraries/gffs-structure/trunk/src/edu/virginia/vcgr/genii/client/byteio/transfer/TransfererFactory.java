...
abstract class TransfererFactory
{
	 ...
	 */
	protected TransfererFactory(Object clientStub) throws RemoteException, IOException
	{
	...
	private void fillInPreferences() throws RemoteException, IOException
	{
		...
		if (_supportedTransferTypes.size() == 0 || _preferredTransferType == null)
			throw new IOException("Unable to determine supported or preferred " + "transfer type for ByteIO.");
	}

	/**
	 * Create a transferer that uses the MTOM attachment specification for transferring bytes.
	 * 
	 * @param clientStub
	 *            The client stub to use for remote communication when doing transfers.
	 * 
	...
	/**
	 * Create a transferer that uses the DIME attachment specification for transferring bytes.
	 * 
	 * @param clientStub
	 *            The client stub to use for remote communication when doing transfers.
	 * 
	...
	/**
	 * Create a transferer that uses the Simple attachment specification for transferring bytes.
	 * 
	 * @param clientStub
	 *            The client stub to use for remote communication when doing transfers.
	 * 
	 ...
	 * @throws ConfigurationException
	 */
	protected Object createTransferer(String requestedTransferType) throws IOException
	{
		...
		if (!_supportedTransferTypes.contains(requestedTransferType))
			throw new IOException("Target ByteIO does not support the \"" + requestedTransferType + "\" transfer type.");
