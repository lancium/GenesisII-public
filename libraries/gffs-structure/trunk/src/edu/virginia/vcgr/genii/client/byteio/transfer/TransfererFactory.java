...
/**
 * This is an abstract base class for a convenience factory that can be used to
 * create transferer agents for both streamable and random byte io's.
 * 
 * @author mmm2a
 */
abstract class TransfererFactory {
	 ...
	 * time. They are sorted in order from most preferred, to least preferred in
	 * terms of efficiency.
	 */
	 ...
	 * transfer types (it depends on the target implementation). This variable
	 * keeps track of which ones are supported by the current target byteio.
	 */
	 ...
	 * 
	 * @param clientStub
	 *            THe target stub to use for remote communication. Note that
	 *            it's an object and type a parameterized type. This is because
	 *            it will either be a RandomByteIO client stub, or a streamable
	 *            byteio client stub. The two are auto generated classes and
	 *            don't have any thing in common with each other (code-wise) so
	 *            a parameterized type here wouldn't help.
	 * 
	 * @throws ConfigurationException
	 * @throws RemoteException
	 */
	protected TransfererFactory(Object clientStub) throws RemoteException,
			IOException {
	...
	/**
	 * Given the XML attributes of a given target ByteIO, parse those attributes
	 * and determine what transfer types the target claims it supports.
	 * 
	 * @param any
	...
	/**
	 * Given a target ByteIO, ask for it's attributes and figure out which
	 * transfer protocols it supports.
	 * 
	 * @throws ConfigurationException
	 * @throws RemoteException
	 */
	private void fillInPreferences() throws RemoteException, IOException {
		...
		/**
		 * Given all of the supported transfer types, figure out the "best" one.
		 */
		...
		if (_supportedTransferTypes.size() == 0
			...
			throw new IOException("Unable to determine supported or preferred "
	 ...
	 * 
	 * @param clientStub
	 *            The client stub to use for remote communication when doing
	 *            transfers.
	 * 
	 ...
	 * 
	 * @param requestedTransferType
	 *            The transfer type requested.
	 * 
	...
	protected Object createTransferer(String requestedTransferType)
		...
		if (!_supportedTransferTypes.contains(requestedTransferType))
			throw new IOException("Target ByteIO does not support the \""
		...
		else if (requestedTransferType
				.equals(ByteIOConstants.TRANSFER_TYPE_DIME))
			return createDimeTransferer(_clientStub);
		else if (requestedTransferType
				.equals(ByteIOConstants.TRANSFER_TYPE_SIMPLE))
	 ...
	 * @throws ConfigurationException
	 */
	protected Object createTransferer() throws IOException {
