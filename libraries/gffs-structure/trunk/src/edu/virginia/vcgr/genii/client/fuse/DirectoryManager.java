...
public class DirectoryManager {
	...
	public static FuseDirEnt[] getDir(String path) {
		UnixDirectory directory = (UnixDirectory) CacheManager
				.getItemFromCache(path, UnixDirectory.class);
		if (directory != null
				&& isCacheContainNecessaryDirectoryInfo(path, directory)) {
	...
	public static void addNewDirEntry(RNSPath path, GenesisIIFilesystem fs,
			boolean entryCreated) {
		String localPath = path.pwd();
		String parentPath = getParentPath(localPath);
		UnixDirectory directory = (UnixDirectory) CacheManager
				.getItemFromCache(parentPath, UnixDirectory.class);
		if (directory != null) {
			try {
				// In general cases the STAT call should not incur new RPC calls
				// because
				// the results of these calls are already cached when update
				// attribute
				// methods were invoked on related RP interfaces.
				FilesystemStatStructure structure = fs
						.stat(UnixFilesystemPathRepresentation.INSTANCE.parse(
								null, localPath));
				...
				// When creating a new directory we add a new item in the cache
				// to avoid the
				// subsequent, and highly probable getDirectory calls on the
				// created directory.
				if (entryCreated && entryType == FilesystemEntryType.DIRECTORY) {
					UnixDirectory newDirectory = new UnixDirectory(localPath,
							null);
	...
	public static String getParentPath(String path) {
		String[] pathComponents = UnixFilesystemPathRepresentation.INSTANCE
				.parse(null, path);
		int length = pathComponents.length;
		if (length > 1) {
			String[] parentsPathComponents = new String[length - 1];
			System.arraycopy(pathComponents, 0, parentsPathComponents, 0,
					length - 1);
			return UnixFilesystemPathRepresentation.INSTANCE
					.toString(parentsPathComponents);
		}
	...
	public static String getPathForDirectoryEntry(String parentPath,
			String entryName) {
	...
	/*
	 * Updating the resource configuration because of addition, removal, or
	 * movement of directory entries is not a required operation. This is done
	 * just to reflect the local updates in the resource configuration as soon
	 * as possible, which will subsequently help to reduce possible redundant
	 * lookup calls.
	 */
	private static void updateResourceConfigCache(RNSPath path,
			boolean pathAdded) {
		try {
			...
			if (pathAdded) {
				...
				if (!config.isMappedToRNSPaths()) {
					CacheManager.removeAllRelevantInfoFromCache(wsIdentifier,
	...
	/*
	 * This method is used to judge the effectiveness of using the local-cache
	 * for a directory lookup operation. As we prefetch attributes with lookup
	 * call, when attributes are absent in the cache but the directory entry is
	 * not, it is better to let the RPC to pass instead of returning result from
	 * the local-cache since at the end it will reduce the overall RPCs.
	 */
	private static boolean isCacheContainNecessaryDirectoryInfo(
			String directoryPath, UnixDirectory directory) {

		WSResourceConfig directoryConfig = (WSResourceConfig) CacheManager
				.getItemFromCache(directoryPath, WSResourceConfig.class);

		// We adhere to a pessimistic approach here. Additionally, since we
		// track last time usage of
		// per-container
		// resources by tracking down when an EPR or a ResourceConfiguration has
		// last been accessed,
		// it is safer to
		...
		// If the number of elements in the directory does not match with the
		// element-count property
		// of the RNS resource
		// then the cached directory is certainly stale. On the other hand, when
		// the element-count
		// property is not in
		// the cache it is safer to assume that the cached directory is invalid.
		// In both cases, an
		// RPC will be issued to
		// replenish the missing contents in the directory.
		MessageElement elementCountElement = (MessageElement) CacheManager
				.getItemFromCache(directoryConfig.getWsIdentifier(),
		...
		for (FuseDirEnt entry : directory.getEntries()) {
			String entryPath = getPathForDirectoryEntry(directoryPath,
					entry.name);
			WSResourceConfig entryConfig = (WSResourceConfig) CacheManager
					.getItemFromCache(entryPath, WSResourceConfig.class);
			if (entryConfig == null)
				continue;
			String entryContainerId = entryConfig.getContainerId();
			if (directoryContainerId.equals(entryContainerId)) {
				// Make a probabilistic judgment about the effectiveness of
				// returning result from
				// cache
				// based on the availability of permissions-string attribute.
				// This attribute is
				// chosen
	...
	public static String getEntryName(String entryPath) {
		String rootPath = UnixFilesystemPathRepresentation.INSTANCE
				.toString(null);
