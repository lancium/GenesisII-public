...
final public class WSIterable<Type> implements Iterable<Type>, Closeable {
	...
	private Type extractType(MessageElement[] any) {
		...
		try {
			if (_jaxbUnmarshaller != null)
				return _jaxbUnmarshaller.unmarshal(any[0], _elementType)
						.getValue();
			else
	...
	private WSIterable(Class<Type> elementType, Unmarshaller jaxbUnmarshaller,
			Type[] initialElementOverride,
			IteratorInitializationType initializer,
			ICallingContext callingContext, int blockSize)
			throws ResourceException, GenesisIISecurityException {
		_elementType = elementType;
		_jaxbUnmarshaller = jaxbUnmarshaller;
		WSIteratorPortType iterator;

		EndpointReferenceType iteratorEndpoint = initializer
				.getIteratorEndpoint();
		...
		if (initialElementOverride != null) {
			...
			IterableElementType[] initialElements = initializer
					.getBatchElement();
		...
		_blockSize = (blockSize <= 0) ? ((_initialBlock != null) ? _initialBlock.length
				: 100)
				: blockSize;
	}
		...
		private void prefetch(int nextStart) {
			...
			try {
				if (_initialBlock != null
						&& nextStart <= (_initialBlock.length - 1)) {
					_currentBlock = new Object[_initialBlock.length - nextStart];
					System.arraycopy(_initialBlock, nextStart, _currentBlock,
							0, _currentBlock.length);
				} else {
					if (_iterator == null) {
						StreamUtils.close(this);
						return;
					...
					int initialLength = (_initialBlock == null) ? 0
							: _initialBlock.length;

					IterateResponseType resp = _iterator.iterate(
							(nextStart - initialLength), _blockSize);
					if (resp == null) {
						StreamUtils.close(this);
						return;
					}
					...
					if (elements == null || elements.length == 0) {
						...
						for (int lcv = 0; lcv < _currentBlock.length; lcv++) {
							int index = elements[lcv].getIndex().intValue()
									- nextStart + initialLength;

							_currentBlock[index] = extractType(elements[lcv]
									.get_any());
						}
					}
				}
	...
	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType,
			...
			throws ResourceUnknownFaultType, ResourceNotDestroyedFaultType,
			ResourceUnavailableFaultType, RemoteException, JAXBException {
	...
	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType,
			...
			ICallingContext callContext, int blockSize) throws JAXBException,
	...
	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType,
			IteratorInitializationType iteratorInitializer,
			JAXBContext context, int blockSize) throws ResourceException,
			GenesisIISecurityException, JAXBException {
	...
	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType,
			IteratorInitializationType iteratorInitializer,
			JAXBContext context, ICallingContext callContext, int blockSize)
	...
	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType,
			IteratorInitializationType iteratorInitializer,
			Unmarshaller unmarshaller, int blockSize) throws ResourceException,
			GenesisIISecurityException {
		return jaxbIterable(elementType, iteratorInitializer, unmarshaller,
				null, blockSize);
	}

	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType,
			IteratorInitializationType iteratorInitializer,
			Unmarshaller unmarshaller, ICallingContext callContext,
			int blockSize) throws ResourceException, GenesisIISecurityException {
		return new WSIterable<Type>(elementType, unmarshaller, null,
				iteratorInitializer, callContext, blockSize);
	}
	...
	static public <Type> WSIterable<Type> axisIterable(Class<Type> elementType,
			...
			throws ResourceException, GenesisIISecurityException {
		return new WSIterable<Type>(elementType, null, null,
	...
	static public <Type> WSIterable<Type> axisIterable(Class<Type> elementType,
			...
			ICallingContext callContext, int blockSize)
			throws ResourceException, GenesisIISecurityException {
		return new WSIterable<Type>(elementType, null, initialElementOverride,
				iteratorInitializationType, callContext, blockSize);
	}
}