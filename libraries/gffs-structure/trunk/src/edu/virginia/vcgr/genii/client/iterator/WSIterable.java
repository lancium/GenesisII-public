...
{
	static private void failedConstructionCleanup(ICallingContext callContext, IteratorInitializationType initializer)
		throws ResourceUnknownFaultType, ResourceNotDestroyedFaultType, ResourceUnavailableFaultType, RemoteException
	{
		if (initializer != null) {
			...
			if (epr != null) {
				...
				if (callContext == null)
					...
					common = ClientUtils.createProxy(GeniiCommon.class, epr, callContext);
	...
	private Type extractType(MessageElement[] any)
	{
		if (any == null || any.length != 1)
			throw new IllegalArgumentException("Iterable element format not correct " + "(must have exactly 1 element in xsd:any)!");

		try {
			if (_jaxbUnmarshaller != null)
				return _jaxbUnmarshaller.unmarshal(any[0], _elementType).getValue();
			else
				return ObjectDeserializer.toObject(any[0], _elementType);
		} catch (ResourceException re) {
			throw new IllegalArgumentException("Error deserializing iterable element!", re);
		} catch (JAXBException e) {
			throw new IllegalArgumentException("Error deserializing iterable element!", e);
		}
	}

	private WSIterable(Class<Type> elementType, Unmarshaller jaxbUnmarshaller, Type[] initialElementOverride,
		IteratorInitializationType initializer, ICallingContext callingContext, int blockSize) throws ResourceException,
		GenesisIISecurityException
	{
		...
		if (iteratorEndpoint == null)
			...
			if (callingContext == null)
				iterator = ClientUtils.createProxy(WSIteratorPortType.class, iteratorEndpoint);
			else
				iterator = ClientUtils.createProxy(WSIteratorPortType.class, iteratorEndpoint, callingContext);
		}
	...
	{
		if (_iterator == null)
			throw new IllegalStateException("WS-Iterator already closed!");
	...
	final private class InternalWSIterator implements WSIterator<Type>
	{
		...
		private void prefetch(int nextStart)
		{
			...
			try {
				if (_initialBlock != null && nextStart <= (_initialBlock.length - 1)) {
					...
					System.arraycopy(_initialBlock, nextStart, _currentBlock, 0, _currentBlock.length);
				} else {
					if (_iterator == null) {
						StreamUtils.close(this);
						return;
					...
					if (elements == null || elements.length == 0) {
						...
						for (int lcv = 0; lcv < _currentBlock.length; lcv++) {
							int index = elements[lcv].getIndex().intValue() - nextStart + initialLength;
				...
				throw new RuntimeException("Unable to use iterator!", re);
			}
		}
		...
		{
			throw new UnsupportedOperationException("Remove not supported for WS-Iterators!");
		}
	...
	static public <Type> WSIterable<Type>
		jaxbIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializer, int blockSize) throws ResourceUnknownFaultType,
			ResourceNotDestroyedFaultType, ResourceUnavailableFaultType, RemoteException, JAXBException
	{
		return jaxbIterable(elementType, iteratorInitializer, (ICallingContext) null, blockSize);
	}

	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializer,
		ICallingContext callContext, int blockSize) throws JAXBException, ResourceUnknownFaultType, ResourceNotDestroyedFaultType,
		ResourceUnavailableFaultType, RemoteException
	{
		try {
			WSIterable<Type> ret =
				jaxbIterable(elementType, iteratorInitializer, JAXBContext.newInstance(elementType), callContext, blockSize);
	...
	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializer,
		JAXBContext context, int blockSize) throws ResourceException, GenesisIISecurityException, JAXBException
	{
		return jaxbIterable(elementType, iteratorInitializer, context, null, blockSize);
	}

	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializer,
		JAXBContext context, ICallingContext callContext, int blockSize) throws JAXBException, ResourceException, GenesisIISecurityException
	{
		return jaxbIterable(elementType, iteratorInitializer, context.createUnmarshaller(), callContext, blockSize);
	}

	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializer,
		Unmarshaller unmarshaller, int blockSize) throws ResourceException, GenesisIISecurityException
	{
		return jaxbIterable(elementType, iteratorInitializer, unmarshaller, null, blockSize);
	}

	static public <Type> WSIterable<Type> jaxbIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializer,
		Unmarshaller unmarshaller, ICallingContext callContext, int blockSize) throws ResourceException, GenesisIISecurityException
	{
		return new WSIterable<Type>(elementType, unmarshaller, null, iteratorInitializer, callContext, blockSize);
	}

	static public <Type> WSIterable<Type> axisIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializationType,
		int blockSize) throws ResourceException, GenesisIISecurityException
	{
		return axisIterable(elementType, iteratorInitializationType, null, blockSize);
	}

	static public <Type> WSIterable<Type> axisIterable(Class<Type> elementType, IteratorInitializationType iteratorInitializationType,
		ICallingContext callContext, int blockSize) throws ResourceException, GenesisIISecurityException
	{
		return new WSIterable<Type>(elementType, null, null, iteratorInitializationType, callContext, blockSize);
	}

	static public <Type> WSIterable<Type> axisIterable(Class<Type> elementType, Type[] initialElementOverride,
		IteratorInitializationType iteratorInitializationType, ICallingContext callContext, int blockSize) throws ResourceException,
		GenesisIISecurityException
	{
