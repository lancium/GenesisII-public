...
{
	static private Log _logger = LogFactory.getLog(FilesystemWatchCallback.class);
	...
	FilesystemWatchCallback(Integer callLimit, FilesystemWatchHandler handler)
	{
		...
		Class<? extends FilesystemWatchHandler> handlerClass, Collection<Element> configurationData) throws IOException
	{
		...
		try {
			try {
				...
				try {
					...
					if (configurationData.size() == 0)
						...
						_handler = cons.newInstance(configurationData.iterator().next());
			...
			throw new IOException(String.format("Unable to instantiate handler for class \"%s\".", handlerClass), ie);
		}
	}
	...
	final void performCallback(FilesystemManager manager, String filesystemName, Filesystem filesystem,
		FilesystemUsageInformation usageInformation, boolean matched)
	{
		...
		if ((matched || (_registerAntiCallback && _callsMade < 1)) && (_callLimit == null || _callsMade < _callLimit)) {
			try {
