...
class FilesystemWatchCallback {
	static private Log _logger = LogFactory
			.getLog(FilesystemWatchCallback.class);
			...
			try {
				...
				try {
					...
					if (configurationData.size() == 0)
						...
						_handler = cons.newInstance(configurationData
								.iterator().next());
		...
		if ((matched || (_registerAntiCallback && _callsMade < 1))
			...
			try {
				if (!_firstTime || matched)
					_handler.notifyFilesystemEvent(manager, filesystemName,
							filesystem, usageInformation, matched);
