...
{
	static final private String SCRIPT_NS = "http://vcgr.cs.virginia.edu/filesystems/filter-script";
	...
	static private BooleanExpression parseBinaryBoolean(BinaryBooleanOperators operator, Element[] children) throws FilterScriptException
	{
		...
		return BinaryBooleanExpression.createBinaryBooleanExpression(parseBooleanExpression(children[0]), operator,
			parseBooleanExpression(children[1]));
	}

	static private NumericValueExpression parseNumericValueExpression(Element node) throws FilterScriptException
	{
		...
		if (name.equals(VARIABLE_ELEMENT)) {
			...
			throw new FilterScriptException(String.format("%s element is not a value element.", name));
		}
	}

	static private BooleanExpression parseBinaryValueExpression(ComparisonOperators operator, Element[] children) throws FilterScriptException
	{
		if (children.length != 2)
			throw new FilterScriptException(String.format("%s binary operator requires exactly 2 child elements.", operator));

		NumericValueExpression left = parseNumericValueExpression(children[0]);
		NumericValueExpression right = parseNumericValueExpression(children[1]);

		return ComparisonExpression.createComparisonExpression(left, operator, right);
	}

	static private BooleanExpression parseBooleanExpression(Element node) throws FilterScriptException
	{
		QName name = XmlUtils.qname(node);

		if (name.equals(AND_ELEMENT)) {
			return parseBinaryBoolean(BinaryBooleanOperators.And, XmlUtils.getChildren(node));
		} else if (name.equals(OR_ELEMENT)) {
			return parseBinaryBoolean(BinaryBooleanOperators.Or, XmlUtils.getChildren(node));
		} else if (name.equals(XOR_ELEMENT)) {
			return parseBinaryBoolean(BinaryBooleanOperators.Xor, XmlUtils.getChildren(node));
		} else if (name.equals(NOT_ELEMENT)) {
			Element[] children = XmlUtils.getChildren(node);
			if (children.length != 1)
				throw new FilterScriptException("The Not operator requires exactly one child element.");
			return new NotBooleanExpression(parseBooleanExpression(children[0]));
			...
			return parseBinaryValueExpression(ComparisonOperators.LessThan, XmlUtils.getChildren(node));
		} else if (name.equals(LESS_EQUALS_ELEMENT)) {
			return parseBinaryValueExpression(ComparisonOperators.LessThanOrEquals, XmlUtils.getChildren(node));
		} else if (name.equals(NOT_EQUALS_ELEMENT)) {
			return parseBinaryValueExpression(ComparisonOperators.NotEquals, XmlUtils.getChildren(node));
		} else if (name.equals(EQUALS_ELEMENT)) {
			return parseBinaryValueExpression(ComparisonOperators.Equals, XmlUtils.getChildren(node));
		} else if (name.equals(GREATER_EQUALS_ELEMENT)) {
			return parseBinaryValueExpression(ComparisonOperators.GreaterThanOrEquals, XmlUtils.getChildren(node));
		} else if (name.equals(GREATER_ELEMENT)) {
			return parseBinaryValueExpression(ComparisonOperators.GreaterThan, XmlUtils.getChildren(node));
		} else
			throw new FilterScriptException(String.format("Unexpectd element \"%s\" encountered.", name));
	}

	static public FilesystemWatchFilter parseScript(Element filterNode) throws FilterScriptException
	{
