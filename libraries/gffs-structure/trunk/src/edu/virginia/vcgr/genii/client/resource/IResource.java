...
/**
 * A resource is the persistent state representation of a web service endpoint. This interface represents the minimum amount of functionallity
 * that a resource MUST implement, but it is expected that web service implementators will expand on this list to better suit the web services
 * that they write.
 * 
...
public interface IResource extends Closeable
{
	...
	static public QName ADDITIONAL_CNS_CONSTRUCTION_PARAM = new QName(GenesisIIConstants.GENESISII_NS, "additional-CNs-creation-spec");
	static public QName ADDITIONAL_ORGS_CONSTRUCTION_PARAM = new QName(GenesisIIConstants.GENESISII_NS, "additional-Os-creation-spec");
	static public QName ENDPOINT_IDENTIFIER_CONSTRUCTION_PARAM = new QName(GenesisIIConstants.GENESISII_NS, "endpoint-identifier");
	...
	/**
	 * Load a resources state from persistent storage (or do nothing if no loading needs to take place).
	 * 
	 * @param refParams
	 *            The refParams that identifies this resource. The type can be of any type as long as it matches the IResourceKeyTranslater
	 *            being used.
	 * @throws ResourceException
	 *             If anything goes wrong.
	 */
	...
	/**
	 * Retrieve the internal key that represents this resource. This method is generally to be used by the provider and ResourceKey and is not
	 * of interest to users in general.
	 * 
	...
	/**
	 * This method returns a key that can be used for locking the resource. It can be, but doesn't have to be the same as the resource key. As
	 * long as it is unique within a given virtual machine.
	 * 
