...
public class ResourcePathsWidget extends JComponent {
	...
	public ResourcePathsWidget(boolean getLocalPath, boolean getContainerPath,
			String ContainerPath, String TargetPath) {
		super();
		...
		if (getLocalPath) {
			...
			add(new JLabel(_LOCAL_LABEL), new GridBagConstraints(0, 0, 1, 1,
		...
		if (getContainerPath) {
			...
			add(new JLabel(_CONTAINER_LABEL), new GridBagConstraints(0, 2, 1,
			...
			add(new JButton(new BrowseRNSPathAction(null, _BUTTON_LABEL,
					_containerPath, _BROWSE_CONTAINER)),
