...
final class IDPLoginPanel extends LoginPanel {
	...
	NamespaceDefinitions nsd = Installation.getDeployment(new DeploymentName())
			.namespace();
	...
	IDPLoginPanel() {
		...
		add(_username, new GridBagConstraints(1, 0, 1, 1, 1.0, 0.0,
				GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 5, 5));
		add(new JLabel("Password"), new GridBagConstraints(0, 1, 1, 1, 0.0,
				0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5), 5, 5));
		add(_password, new GridBagConstraints(1, 1, 1, 1, 1.0, 0.0,
				GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 5, 5));
		add(new JLabel("Grid Path"), new GridBagConstraints(0, 2, 1, 1, 0.0,
				1.0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5), 5, 5));
		add(_rnsPath, new GridBagConstraints(1, 2, 1, 1, 1.0, 1.0,
				GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 5, 5));
	}

	@Override
	final public Collection<NuCredential> doLogin(UIContext uiContext)
			throws Throwable {
		Closeable token = null;

		try {
			ContextManager.temporarilyAssumeContext(uiContext.callingContext());
			RNSPath path = uiContext.callingContext().getCurrentPath()
					.lookup(_rnsPath.getText(), RNSPathQueryFlags.DONT_CARE);
			if (!path.exists()) {
				JOptionPane.showMessageDialog(this, "Grid path doesn't exist!",
						"Bad Grid Path", JOptionPane.ERROR_MESSAGE);
				return null;
			}

			UsernamePasswordIdentity upt = new UsernamePasswordIdentity(
					_username.getText(), new String(_password.getPassword()));
			...
			try {
				...
				KeyAndCertMaterial clientKeyMaterial = ClientUtils
						.checkAndRenewCredentials(uiContext.callingContext(),
	...
	private class InternalCaretListener implements CaretListener {
		...
		final public void caretUpdate(CaretEvent e) {
			_rnsPath.setText(nsd.getUsersDirectory() + "/"
					+ _username.getText());
		}
	}
}
