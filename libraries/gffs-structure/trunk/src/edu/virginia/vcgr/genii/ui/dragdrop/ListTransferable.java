package edu.virginia.vcgr.genii.ui.dragdrop;
...
public class ListTransferable extends Vector<Object> implements Transferable
{
	...
	public Object getTransferData(DataFlavor flavor) throws UnsupportedFlavorException, java.io.IOException
	{
		...
		if (size() == 0) {
			if (_logger.isDebugEnabled())
				_logger.debug("size was zero, so loading data just in time");
			boolean worked = loadDataJustInTime(flavor);
			if (!worked || (size() == 0)) {
				_logger.warn("failed to retrieve data just in time for getTransferData.");
				return null;
			}
		}
		...
		if (flavor.equals(DataFlavor.javaFileListFlavor)) {
			if (_logger.isDebugEnabled())
				_logger.debug("java file list flavor...");
			...
			if (_logger.isDebugEnabled())
				_logger.debug("returning URI string: " + data.toString());
			return data.toString();
		} else {
			if (_logger.isDebugEnabled())
				_logger.debug("getTransferData: didn't know how to handle the requested flavor.");
	...
	public boolean isDataFlavorSupported(DataFlavor flavor)
	{
		...
		if (_logger.isDebugEnabled())
			_logger.debug("failed to find flavor: " + flavor.toString());
		return false;
	}
	...
	static public List<Object> extractData(Transferable tran)
	{
		...
		_logger.error("extractData: Transferable did not support known data flavor.");
		return null;
	}
	...
	public static List<Object> textURIListToFileList(String data)
	{
		...
		for (StringTokenizer st = new StringTokenizer(data, "\r\n"); st.hasMoreTokens();) {
			...
			try {
				...
				_logger.error("Found a malformed URI of: " + data);
			} catch (IllegalArgumentException e) {
				// the URI is not a valid 'file:' URI
				_logger.error("Found invalid 'file:' URI of: " + data);
			}
		}
		return list;
	}
	...
	public static List<Object> processReaderFlavor(Transferable tran)
	{
		if (tran == null)
			return null;
		if (_logger.isDebugEnabled())
			_logger.debug("trying URI list flavor.");
		DataFlavor[] flavors = tran.getTransferDataFlavors();
		for (int i = 0; i < flavors.length; i++) {
			if (flavors[i].isRepresentationClassReader()) {
				// it looks like we can work with this flavor just fine.
				if (_logger.isDebugEnabled())
					_logger.debug("found a reader flavor.");
				try {
					...
					if (_logger.isDebugEnabled())
						_logger.debug("failed to scan reader for file list.");
				}
			}
		}
		return new ArrayList<Object>();
		...
		try {
			...
			while ((line = bReader.readLine()) != null) {
				try {
					...
					_logger.error("Error with " + line + ": " + ex.getMessage());
				}
			}

			return list;
		} catch (IOException ex) {
			_logger.error("IOException while working on file list");
		}
		return new ArrayList<Object>();
	}
