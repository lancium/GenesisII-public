...
public class FileLock implements Closeable {
	...
	/**
	 * Creates a file lock that can be used for both reading and writing (by
	 * creating a new file with the same name as the input file but .lock at the
	 * end) and locking it.
	 * 
	 * @param fileToLock
	 *            The file to create a lock for.
	 * @param maxAttempts
	 *            The maximum number of attempts to make at locking before
	 *            giving up
	 * @param pollInterval
	 *            The interval (in milliseconds) to wait between attempts to
	 *            lock the file before giving up. This value can be 0 indicating
	 *            infinitee).
