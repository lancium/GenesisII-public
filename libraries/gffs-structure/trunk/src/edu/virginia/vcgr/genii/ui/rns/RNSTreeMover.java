...
public class RNSTreeMover extends RNSTreeOperator {
	...
	static public RNSTreeOperator move(RNSTree sourceTree, RNSTree targetTree,
			TreePath targetPath, UIContext sourceContext,
			Collection<Pair<RNSTreeNode, RNSPath>> paths) {
		return new RNSTreeMover(sourceTree, targetTree, targetPath,
				sourceContext, paths);
	}

	private RNSTreeMover(RNSTree sourceTree, RNSTree targetTree,
			TreePath targetPath, UIContext sourceContext,
			Collection<Pair<RNSTreeNode, RNSPath>> paths) {
		super(sourceContext, targetTree, targetPath, new RNSTreeOperatorSource(
				sourceTree, paths));
	}
	...
	public boolean performOperation() {
		...
		_uiContext
				...
				.createMonitor(_targetTree, "Moving Endpoints",
						"Moving endpoints.", 1000L, new MoverTask(), null)
				.start();
		return true;
	}
		...
		public Integer execute(TaskProgressListener progressListener)
			...
			if (_sourceInformation.isRNSSource()) {
				...
				for (Pair<RNSTreeNode, RNSPath> path : source.sourcePaths()) {
					progressListener.updateSubTitle(String.format("Moving %s",
							path.second().getName()));
					...
					if (target != null) {
						...
						try {
							...
							new RefreshWorker(_targetTree, targetParentNode)
									.run();
							path.second().unlink();
							new RefreshWorker(source.sourceTree(), path.first())
									.run();
							...
							_logger.warn(
									String.format(
											"Unable to move endpoint for source \"%s\".",
