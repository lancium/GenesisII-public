...
class AttemptNumberBorder extends AbstractBorder
{
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height)
	{
		...
		JTree tree = (JTree) c;
		List<Pair<Integer, Rectangle>> boxes = new ArrayList<Pair<Integer, Rectangle>>();
			...
			if (number == null) {
				if (_currentAttempt != null) {
					...
					boxes.add(new Pair<Integer, Rectangle>(_currentAttempt,
						new Rectangle(_currentStart.x, _currentStart.y, insets.left, r.y - _currentStart.y - 1)));
		...
		if (_currentAttempt != null) {
			...
			boxes.add(new Pair<Integer, Rectangle>(_currentAttempt,
				new Rectangle(_currentStart.x, _currentStart.y, insets.left, r.y - _currentStart.y - 1 + r.height)));
		}
