...
public class BESManagerPanel extends JPanel {
	...
	private JPanel createActivityPoliciesPanel() {
		...
		ret.add(new JLabel("Screen Saver Inactive"), new GridBagConstraints(0,
				0, 1, 1, 0.0, 1.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		ret.add(_screenSaverInactiveCombo, new GridBagConstraints(1, 0, 1, 1,
				1.0, 1.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 5, 5));
		ret.add(new JLabel("User Logged In"), new GridBagConstraints(0, 1, 1,
				1, 0.0, 1.0, GridBagConstraints.WEST, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5), 5, 5));
		ret.add(_userLoggedInCombo, new GridBagConstraints(1, 1, 1, 1, 1.0,
				1.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 5, 5));
		ret.add(new JLabel("Activity Threshold"), new GridBagConstraints(0, 2,
				1, 1, 0.0, 1.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 5, 5));
		ret.add(_thresholdField, new GridBagConstraints(1, 2, 1, 1, 1.0, 1.0,
	...
	private void acquireState() {
		...
		monitor.addProgressNotifier(new DefaultProgressNotifier(this,
				"Query BES Container", "Getting resource properties from BES.",
				1000L), false);

		monitor.startTask(new BESManagerAcquisitionTask(_callingContext,
				_target));
	}
	...
	private void applyState() {
		...
		monitor.addProgressNotifier(new DefaultProgressNotifier(this,
				"Applying BES Configuration",
				"Applying new BES configuration.", 1000L), false);

		monitor.startTask(new BESManagerConfigurationTask(_callingContext,
				_target, new ManagementData(new BESPolicy(
						((BESPolicyActionWrapper) _userLoggedInCombo
								.getSelectedItem()).action(),
						((BESPolicyActionWrapper) _screenSaverInactiveCombo
								.getSelectedItem()).action()), threshold,
						_isAccepting.isSelected())));
	}
	...
	public BESManagerPanel(ICallingContext callingContext,
		...
		add(createActivityPoliciesPanel(), new GridBagConstraints(0, 0, 2, 1,
				1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(5, 5, 5, 5), 5, 5));
		add(_isAccepting, new GridBagConstraints(0, 1, 2, 1, 1.0, 0.0,
				GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(5,
						5, 5, 5), 5, 5));
		add(new JButton(_applyAction), new GridBagConstraints(0, 2, 1, 1, 1.0,
				0.0, GridBagConstraints.CENTER, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5), 5, 5));
		add(new JButton(_refreshAction), new GridBagConstraints(1, 2, 1, 1,
				1.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.NONE,
				new Insets(5, 5, 5, 5), 5, 5));
		add(_statusLabel, new GridBagConstraints(0, 3, 2, 1, 1.0, 0.0,
	...
	private class AcquireFinishProgressListener implements
			ProgressListener<ManagementData> {
		@Override
		...
		public void taskCompleted(ManagementData result) {
			...
			_userLoggedInCombo.setSelectedItem(BESPolicyActionWrapper
					.wrap(_data.policy().getUserLoggedInAction()));
			_thresholdField.setText(_data.threshold() == null ? "" : _data
					.threshold().toString());
	...
	private class ApplyFinishProgressListener implements
			ProgressListener<ManagementData> {
	...
	private class ChangeListener implements ActionListener, CaretListener {
		@Override
		public void actionPerformed(ActionEvent event) {
			_applyAction.setEnabled(true);
		}
