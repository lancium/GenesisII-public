...
public class TopicPath implements Serializable {
	...
	static final private Pattern QNAME_PATTERN = Pattern
			.compile("^\\{([^\\}]+)\\}(.+)$");
	 ...
	 * 
	 * @param pathElements
	 *            An arbitrary number of parameters that identify the topic
	 *            path. For the most part, any of these elements can either be a
	 *            QName, a String, or a NamespaceContext. The very first
	 *            non-namespace context element can also be a TopicPath
	 *            representing a parent for the new path. If an element is a
	 *            NamespaceContext, it then becomes the authoritative entity for
	 *            describing how to map prefixes to namespace URIs for elements
	 *            that follow it. If an element is a QName, then it represents
	 *            itself in the topic path, but also becomes the authoritative
	 *            namespace for elements in the path that follow that do not
	 *            have a namespace or prefix given. If an element is a string,
	 *            then it is a string of the form:
	 *            [ns-prefix:]topicName[/[ns-prefix:]topicName...].
	 * 
	 *            This list of path elements must have at least one QName, or
	 *            one string representation that resolves to a valid QName using
							...
							throw new IllegalArgumentException(
									String.format(
											"TopicPath element %s is invalid "
													+ "without preceeding topic definnitions.",
											path));

						current = new TopicPath(
								current,
								new QName(
										currentNamespaceURI,
										pathElement,
										(currentPrefix == null) ? XMLConstants.DEFAULT_NS_PREFIX
												: currentPrefix));
					} else {
						String prefix = pathElement.substring(0, index).trim();
						String localName = pathElement.substring(index + 1)
								.trim();
							...
							if (nsURI == null)
								throw new IllegalArgumentException(
										String.format(
												"TopicPath element %s is invalid.",
												path));
							current = new TopicPath(current, new QName(nsURI,
									localName, prefix));
	...
	static public void main(String[] args) {
		TopicPath path = new TopicPath(new QName("http://tempuri.org", "one",
				"ts1"));
