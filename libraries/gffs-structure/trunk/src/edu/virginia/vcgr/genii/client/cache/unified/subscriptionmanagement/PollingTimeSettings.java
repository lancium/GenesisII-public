...
public class PollingTimeSettings {
	...
	public void updateNextPollingTime() {
		...
		if (mostRecentPollingTime == null) {
			nextPollingTimeInMillis = System.currentTimeMillis()
					+ pollingRPCInterval;
		} else {
			nextPollingTimeInMillis = mostRecentPollingTime.getTime()
					+ pollingRPCInterval;
		}
		nextPollingTime = (nextPollingTimeInMillis < System.currentTimeMillis()) ? new Date()
				: new Date(nextPollingTimeInMillis);
	}

	public void notifyAboutPolling() {
		nextPollingTime = new Date(System.currentTimeMillis()
				+ pollingRPCInterval);
