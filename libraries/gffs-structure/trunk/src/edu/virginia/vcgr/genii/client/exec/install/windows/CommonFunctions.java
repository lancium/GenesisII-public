...
public class CommonFunctions {
	...
	static public void checkPath(File path, boolean canExecute)
		...
		if (!path.exists())
			throw new FileNotFoundException(String.format(
					"Unable to find file \"%s\".", path.getAbsolutePath()));
		...
		if (canExecute) {
			if (!path.canExecute())
				throw new IOException(String.format(
						"Cannot execute file \"%s\".", path.getAbsolutePath()));
		} else {
			if (!path.canRead())
				throw new IOException(String.format("Cannot read file \"%s\".",
						path.getAbsolutePath()));
		}
	}
	...
	static public String getAccount(String accountOverride) throws IOException {
		if (accountOverride == null)
			accountOverride = String.format("%s\\%s", getLocalUserDomain(),
