...
public class NotificationMessageContents implements Cloneable, Serializable {
	...
	/*
	 * This includes all the attributes that cannot be properly serialized or
	 * deserialized by JAXB. Information stored within this array do not travel
	 * as message content, rather these traveled as part of the extension of the
	 * notification message (in "any" field of Notify class). As the above
	 * suggests, this is not an ideal way of passing attributes. Hence, we
	 * should change how these attributes are propagated if we find any better
	 * alternative.
	 */
	transient private MessageElement[] _additionalAttributes;

	/*
	 * Publisher blocking is used to control the rate at which a resource can
	 * publish notifications. Upon receiving a notification with this flag set
	 * ON, the receiver should assume that it will not receive further
	 * notification from it for the time period specified in blockageTime field,
	...
	/*
	 * Indicate whether the notification messages are sent to the consumers
	 * holding subscription on the current resource only or consumers holding
	 * subscription in another resource can also receive notification. Such a
	 * scenario is desirable to pass ByteIO notifications through the parent RNS
	 * directories containing the publisher ByteIO resource.
	 */
	@XmlTransient
	protected boolean useIndirectPublishers;

	/*
	 * How to retrieve the resource keys of the resources that will be used as
	 * indirect publishers.
	 */
	@XmlTransient
	...
	public void setIndirectPublishersRetrieveQuery(
			String indirectPublishersRetrieveQuery) {
	...
	final public void additionalUserData(AdditionalUserData data)
		...
		_additionalUserData = (data == null) ? null : AdditionalUserData
				.toMessageElement(data);
	}
	...
	public Object clone() {
		try {
			...
			NotificationMessageContents copy = (NotificationMessageContents) ois
					.readObject();
	...
	/*
	 * Only used when the notification message is supposed to indirectly
	 * propagated through other resources. Subclasses using indirect
	...
	/*
	 * We have to clone the attributes explicitly as JAXB cannot process
	 * arbitrary attributes. Furthermore, to avoid JAXB serialization problem we
	 * have defined _additionalAttributes as transient. So explicit copying is
	 * the only option.
	 */
