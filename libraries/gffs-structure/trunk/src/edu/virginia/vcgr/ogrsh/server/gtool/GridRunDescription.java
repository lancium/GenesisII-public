...
public class GridRunDescription implements Closeable
{
	...
	public GridRunDescription(String gridToolClassName, String[] arguments)
		throws ClassNotFoundException, IllegalAccessException, InstantiationException, ToolException, IOException
	{
	...
	{
		if (_server == null)
			throw new IllegalStateException("Cannot get port after run description is closed.");
	...
	synchronized public GridRunManager prepare(Date timeoutTime) throws IOException
	{
		if (_server == null)
			throw new IllegalStateException("Cannot prepare grid run after run description is closed.");

		try {
			return new GridRunManager(_server, _secretKey, _gridTool, timeoutTime);
		} finally {
			StreamUtils.close(_server);
			_server = null;
		}
	...
	public GridRunManager prepare(long timeout) throws IOException
	{
