...
public class RandomByteIOFileDescriptor extends AbstractFileDescriptor
{
		...
		boolean isTruncate) throws OGRSHException
	{
		...
		_epr = epr;
		try {
			RandomByteIOPortType stub = ClientUtils.createProxy(RandomByteIOPortType.class, epr);
			_transferer = RandomByteIOTransfererFactory.createRandomByteIOTransferer(stub);
