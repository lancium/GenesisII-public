...
public class XScriptEngineFactory implements ScriptEngineFactory, XScriptConstants
{
	...
	public XScriptEngineFactory()
	{
		...
		_mimeTypes = new ArrayList<String>(MIME_TYPES.length);
		for (String mimeType : MIME_TYPES)
			_mimeTypes.add(mimeType);

		_shortNames = new ArrayList<String>(SHORT_NAMES.length);
		...
		_factoryParameters.put(ScriptEngine.LANGUAGE_VERSION, LANGUAGE_VERSION);
	}
