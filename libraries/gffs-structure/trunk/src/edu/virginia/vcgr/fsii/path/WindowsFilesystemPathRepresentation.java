...
public class WindowsFilesystemPathRepresentation extends
		AbstractFilesystemPathRepresentation {
	static private final Pattern WINDOWS_ROOT_PATTERN = Pattern
			.compile("^(?:[a-zA-Z]:)?\\\\+(.*)$");

	static public WindowsFilesystemPathRepresentation DRIVELESS_INSTANCE = new WindowsFilesystemPathRepresentation();

	private Character _driveLetter = null;

	/**
	 * Initializes the WindowsFilesystemPathRepresentation with a drive letter
	 * (used only in toString transformations).
	 * 
	 * @param driveLetter
	...
	protected String toStringImpl(String[] path) {
		...
		StringBuilder builder = new StringBuilder(_driveLetter == null ? ""
