...
/**
 * An SMBBuffer attempts to simplify working with the SMB protocol by keeping track of some important information.
 * For instance, this buffer keeps track of alignment, so that unicode strings are properly aligned.
 */
public class SMBBuffer {
	...
	public SMBBuffer getDataBuffer() throws SMBException {
		...
		if (type != 1) throw new SMBException(NTStatus.INVALID_SMB);
		return getBlock();
	}
	
	public String getOEMString() throws SMBException {
		byte type = buffer.get();
		if (type != 2) throw new SMBException(NTStatus.INVALID_SMB);
		return getString(false);
	}
	
	public String getPathname() throws SMBException {
		byte type = buffer.get();
		if (type != 3) throw new SMBException(NTStatus.INVALID_SMB);
		return getString(false);
	}
	
	public String getSMBString(boolean unicode) throws SMBException {
		byte type = buffer.get();
		if (type != 4) throw new SMBException(NTStatus.INVALID_SMB);
		return getString(unicode);
	}
	
	public SMBBuffer getVarBlock() throws SMBException {
		byte type = buffer.get();
		if (type != 5) throw new SMBException(NTStatus.INVALID_SMB);
		return getBlock();
	}
