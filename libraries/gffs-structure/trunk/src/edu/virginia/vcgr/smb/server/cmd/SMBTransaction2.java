...
public class SMBTransaction2 implements SMBCommand
{
		...
		SMBException
	{
		...
		if (paramCount < totalParamCount || dataCount < totalDataCount) {
			/* Need more data */
			SMBTransactionInfo trans =
				new SMBTransactionInfo(totalParamCount, totalDataCount, maxParamCount, maxDataCount, setup, maxSetupCount);
	...
	public static void reply(SMBConnection c, SMBHeader h, SMBTransactionInfo trans, SMBBuffer acc, SMBBuffer setup, SMBBuffer params,
		SMBBuffer data) throws IOException, SMBException
	{
