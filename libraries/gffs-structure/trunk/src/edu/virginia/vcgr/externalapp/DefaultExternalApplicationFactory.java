...
public class DefaultExternalApplicationFactory implements
	...
	static {
		try {
			CONTEXT = JAXBContext
					.newInstance(DefaultExternalApplicationConfiguration.class);
	...
	public ExternalApplication createApplication(Element configuration) {
		...
		try {
			...
			String description = MacroUtils.replaceMacros(variables,
					conf.description());
