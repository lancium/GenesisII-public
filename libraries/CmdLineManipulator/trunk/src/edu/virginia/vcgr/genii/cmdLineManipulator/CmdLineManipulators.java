...
public class CmdLineManipulators {
	...
	static private ServiceLoader<CmdLineManipulator> loader = ServiceLoader
			.load(CmdLineManipulator.class);

	synchronized static public CmdLineManipulator<?> getCmdLineManipulator(
			String manipulatorType) throws CmdLineManipulatorException {
		for (CmdLineManipulator<?> manipulator : loader) {
			if (manipulator.getManipulatorType().equals(manipulatorType)) {
				_logger.debug(String.format(
						"Loaded cmdline manipulator of type: %s",
						manipulatorType));
				return manipulator;
			}
		}
		throw new CmdLineManipulatorException(String.format(
				"Could not locate manipulator of type \"%s\" for loading.",
				manipulatorType));
	}
}