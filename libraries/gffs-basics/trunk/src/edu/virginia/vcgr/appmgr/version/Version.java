...
public class Version implements Comparable<Version> {
	static final private Pattern VERSION_PATTERN = Pattern
			.compile("^(\\d+)\\.(\\d+)\\.(\\d+)\\s+Build\\s+(\\d+)$");
	...
	public int hashCode() {
		return (_majorVersion << 3) ^ (_minorVersion << 2)
				^ (_subMinorVersion << 1) ^ _buildNumber;
	}

	@Override
	public String toString() {
		return String.format("%d.%d.%d Build %d", _majorVersion, _minorVersion,
				_subMinorVersion, _buildNumber);
	}
