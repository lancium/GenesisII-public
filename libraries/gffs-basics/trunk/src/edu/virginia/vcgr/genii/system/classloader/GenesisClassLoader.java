...
public class GenesisClassLoader extends ClassLoader {
	...
	/**
	 * the real entre to the class loaders. we use this to load all class
	 * loaders.
	 */
	synchronized public static GenesisClassLoader classLoaderFactory() {
		if (GenesisClassLoader._theRealLoaderList == null) {
			...
			_theRealLoaderList.addLoader(GenesisClassLoader
					.classLoaderFactory().getClass().getClassLoader());
		}
	...
	synchronized public boolean addLoader(ClassLoader newLoader) {
		if (!_loaders.contains(newLoader)) {
			...
			/*
			 * nope //temp design truncation to see if we are causing osgi
			 * problems. if (_loaders.size() > 0) { if
			...
			if (_logger.isTraceEnabled())
				_logger.trace("classloader was already loaded: "
						+ newLoader.toString());
			return false;
		}
	}
	...
	/**
	 * this is where we do all the work by consulting our table of known
	 * loaders.
	 */
	@Override
	synchronized protected Class<?> findClass(String name)
		...
		while (itty.hasNext()) {
			...
			try {
				...
				_logger.debug("did not find class " + name + " in loader: "
						+ curr.toString());
			}
		}
		_logger.error("failure to find class '" + name
				+ "' in any extant class loader.");
