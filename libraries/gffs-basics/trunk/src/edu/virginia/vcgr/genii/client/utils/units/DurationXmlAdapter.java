...
final public class DurationXmlAdapter extends XmlAdapter<javax.xml.datatype.Duration, Duration>
{
	...
	{
		return new Duration(String.format("%d years %d months %d days %d hours %d minutes %.3f seconds", v.getField(DatatypeConstants.YEARS),
			v.getField(DatatypeConstants.MONTHS), v.getField(DatatypeConstants.DAYS), v.getField(DatatypeConstants.HOURS),
			v.getField(DatatypeConstants.MINUTES), v.getField(DatatypeConstants.SECONDS)));
	}
}