...
public class GuaranteedDirectory extends File {
	...
	private boolean _ownerOnly = false; // true if only the owner should have
										// write and execute
										// permissions.
		...
		if (exists()) {
			...
			if (!mkdirs())
				throw new IOException("Unable to create directory \""
						+ getAbsolutePath() + "\".");
			if (_ownerOnly) {
				// if we were asked to give only the owner permission, we do it
				// here.

				// for now, since we need to keep supporting java 6, we're going
				// with the ugly but
				// working approach.
				...
				if (!((osType == OperatingSystemType.Windows_XP)
					...
					// we think we're good to try this; this doesn't seem to be
					// a windows variant.
					Runtime r = Runtime.getRuntime();
					String[] cmds = { "chmod", "u+rwx,g-rwx,o-rwx",
							getAbsolutePath() };
				...
				// a note about the above kludge: we cannot currently use
				// PosixFilePermission since
				// that is java 7 only.
				// we also have found that the setExecutable(true, true); style
				// methods have no
				// effect on group or user
				// permissions whatsoever. that's why we've gone to ground with
				// a process call. ugh.
			}
		}
	}
	...
	public GuaranteedDirectory(String path, boolean ownerOnly)
			throws IOException {
		super(path);
