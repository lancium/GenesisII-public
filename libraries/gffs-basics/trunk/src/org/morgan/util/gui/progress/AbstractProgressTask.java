...
public abstract class AbstractProgressTask<Type> implements ProgressTask<Type>
{
	...
	protected AbstractProgressTask(int minimumProgressValue, int maximumProgressValue)
	{
		_minimumValue = new Integer(Math.min(minimumProgressValue, maximumProgressValue));
		_maximumValue = new Integer(Math.max(minimumProgressValue, maximumProgressValue));
	}
	...
	public int getMaximumProgressValue()
	{
		if (_maximumValue == null)
			throw new IllegalStateException("This task has indeterminate progress.");
