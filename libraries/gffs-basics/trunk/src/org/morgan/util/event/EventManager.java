...
public class EventManager
{
	...
	public EventDescription registerEventDescription(String eventName) throws EventException
	{
		EventDescription desc;

		synchronized (_descriptions) {
			if (_descriptions.containsKey(eventName))
				throw new EventException("Event \"" + eventName + "\" already exists.");
	...
	public EventDescription registerEventDescription(String eventName, Class<? extends IEvent> requiredBase) throws EventException
	{
		EventDescription desc;
		...
		synchronized (_descriptions) {
			if (_descriptions.containsKey(eventName))
				throw new EventException("Event \"" + eventName + "\" already exists.");
			_descriptions.put(eventName, desc = new EventDescription(eventName, requiredBase));
		}

		return desc;
	}

	public EventDescription registerEventDescription(String eventName, Class<? extends IEvent> requiredBase, boolean stopOnException)
		throws EventException
	{
		...
		synchronized (_descriptions) {
			if (_descriptions.containsKey(eventName))
				throw new EventException("Event \"" + eventName + "\" already exists.");
			_descriptions.put(eventName, desc = new EventDescription(eventName, requiredBase, stopOnException));
		}
	...
	public IEventProgress raise(IEvent event, IEventDoneHandler doneHandler)
	{
		...
		EventRaiseWorker worker = new EventRaiseWorker(doneHandler, handlers, event);
		_eventExecutor.submit(worker);
		return worker;
	}

	public IEventHandlerToken registerHandler(EventDescription description, IEventHandler handler, int priority)
	{
		...
		synchronized (_handlers) {
			...
			if (bundles == null) {
				_handlers.put(description.getEventName(), bundles = new TreeSet<EventHandlerBundle>());
			}
		}
	...
	private class EventRaiseWorker implements Runnable, IEventProgress
	{
		...
		EventRaiseWorker(IEventDoneHandler doneHandler, EventHandlerBundle[] bundles, IEvent event)
		{
