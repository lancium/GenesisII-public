...
public class TimeBasedHistogramSet extends RollingTimeValueSet<Integer> {
	...
	public <DataRangeType> Histogram<DataRangeType> histogram(
		...
		TimeRange range = new TimeRange(now.getTimeInMillis() - interval,
				now.getTimeInMillis());
		...
		while (!values.isEmpty()) {
			...
			while (!values.isEmpty()
					&& range.during(values.peekFirst().first()))
				combo.add(values.removeFirst());

			histogramData.add(dataCombiner.combine(range.beginning(),
					range.end(), combo));
		...
		if (labelDelegate != null
			...
			(TimeAwareDataRangeLabelDelegate.class.cast(labelDelegate))
					.markTime(now);
