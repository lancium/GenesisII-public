...
public class ListEntry {
	static private final long _180Days = 1000 * 60 * 60 * 24 * 180;
	...
	public ListEntry(String name, Date lastMod, long size, String user,
			String group, FilePermissions permissions, int hardLinkCount,
			boolean isDirectory) {
	...
	public String toString() {
		...
		return String.format("%1$s%2$s %3$d %4$s %5$s %6$d %7$s %8$s",
				(_isDirectory ? "d" : "-"), _permissions, _hardLinkCount,
				_user, _group, _size, dateString, _name);
	}
