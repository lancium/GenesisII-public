...
/**
 * A simple java thread that hearkens back to the HOOPLE C++ ethread in
 * features.
 * 
...
public abstract class ethread implements Runnable {
	...
	/**
	 * this is the main function that derived classes must implement. it does
	 * the actual work that the thread is intended to perform. note that the
	 * derived version must not do anything to cause the thread to be ripped out
	 * while performActivity is still being invoked.
	 */
	// hmmm: should this stay void, or use a bool status to indicate if the
	// thread should die?
	...
	/**
	 * this is the override from Runnable that allows us to call our own
	 * performActivity method. implementors should not override this; they
	 * should override performActivity instead.
	 */
	public void run() {
		synchronized (c_lock) {
			if (false == threadRunning()) {
				return; // stopped before it ever started. how can this be? we
						// just got invoked.
			}
			performActivity();
		}
...
/*
 * hmmm: still in progress... missing the timed features of ethread. missing
 * cancel, exempt_stop, sleep_time, should_stop,
 */
