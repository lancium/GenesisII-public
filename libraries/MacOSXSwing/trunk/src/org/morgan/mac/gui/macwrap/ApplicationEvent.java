...
public class ApplicationEvent {
	...
	static {
		try {
			REAL_APPLICATION_EVENT_CLASS = Class
					.forName("com.apple.eawt.ApplicationEvent");

			GET_FILENAME_METHOD = REAL_APPLICATION_EVENT_CLASS
					.getDeclaredMethod("getFilename");
			IS_HANDLED_METHOD = REAL_APPLICATION_EVENT_CLASS
					.getDeclaredMethod("isHandled");
			SET_HANDLED_METHOD = REAL_APPLICATION_EVENT_CLASS
	...
	final public String getFilename() {
		try {
			...
			_logger.fatal(
					"Unable to call Mac getFilename method on ApplicationEvent.",
					e);
	...
	final public boolean isHandled() {
		try {
			...
			_logger.fatal(
					"Unable to call Mac isHandled method on ApplicationEvent.",
					e);
		} catch (IllegalAccessException e) {
			_logger.fatal(
					"Unable to call Mac isHandled method on ApplicationEvent.",
	...
	final public void setHandled(boolean state) {
		try {
			...
			_logger.fatal(
					"Unable to call Mac setHandled method on ApplicationEvent.",
					e);
		} catch (IllegalAccessException e) {
			_logger.fatal(
					"Unable to call Mac setHandled method on ApplicationEvent.",
