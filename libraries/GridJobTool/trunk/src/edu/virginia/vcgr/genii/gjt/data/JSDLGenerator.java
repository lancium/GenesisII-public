...
public class JSDLGenerator
{
		...
		Map<String, List<SweepParameter>> variables)
	{
		...
		Duple<String, List<VariableInformation>> results = VariableManager.findVariables(value);
	...
	static public Duple<String, FilesystemType> generate(FilesystemAssociatedString source, XPathBuilder builder,
		Map<String, List<SweepParameter>> variables)
	{
		if (source == null)
			return null;

		String stringReturn = generate((ParameterizableString) source, builder, variables);
	...
	static public List<Duple<String, FilesystemType>> generate(FilesystemAssociatedStringList list, XPathBuilder builder,
		Map<String, List<SweepParameter>> variables)
	{
		if (list == null)
			return null;
		...
		for (StringFilesystemPair item : list) {
			Duple<String, List<VariableInformation>> results = VariableManager.findVariables(item.get());

			ret.add(new Duple<String, FilesystemType>(results.first(), item.getFilesystemType()));
	...
	static public List<Triple<String, String, FilesystemType>> generate(EnvironmentList list, XPathBuilder builder,
		Map<String, List<SweepParameter>> variables)
	{
		if (list == null)
			return null;
		...
		for (StringStringFilesystemTriple item : list) {
			...
			Duple<String, List<VariableInformation>> keyResults = VariableManager.findVariables(item.getKey());
			...
			Duple<String, List<VariableInformation>> valueResults = VariableManager.findVariables(item.getValue());
