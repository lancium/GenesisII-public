...
public class DataStagePanel extends JPanel
{
	...
	public DataStagePanel(FilesystemMap filesystemMap, String label, StageList stageList, boolean isStageIn)
	{
		super(new GridBagLayout());

		DataStageTable table = new DataStageTable(filesystemMap, stageList, isStageIn);
		DataStageTableModel model = (DataStageTableModel) table.getModel();
		_table = table;
		_tableModel = model;

		add(new JLabel(label), new GridBagConstraints(0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE,
		...
		add(ButtonPanel.createHorizontalPanel(new SimpleIconButton(ShapeIcons.Plus, new PlusAction()), new SimpleIconButton(
			ShapeIcons.Minus, _minusAction)), new GridBagConstraints(0, 2, 1, 1, 1.0, 0.0, GridBagConstraints.WEST,
