	...
	private Configuration() throws IOException, JAXBException {
		...
		if (!confDirectory.exists())
			throw new RuntimeException(String.format(
					"Unable to find/create configuraiton directory %s.",
					confDirectory));
		...
		_operatingSystemNamesFilter = new Filter<OperatingSystemNames>(
				confDirectory, OperatingSystemNames.class);
		_proceesorArchitectureFilter = new Filter<ProcessorArchitecture>(
				confDirectory, ProcessorArchitecture.class);
		_spmdVariations = SPMDVariations.readVariations(confDirectory)
				.variations();
	}
