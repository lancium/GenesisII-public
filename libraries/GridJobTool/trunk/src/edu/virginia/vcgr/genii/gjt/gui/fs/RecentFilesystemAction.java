...
class RecentFilesystemAction extends AbstractAction {
	...
	RecentFilesystemAction(FilesystemMap filesystemMap,
			Duple<Calendar, Filesystem> recent) {
		super(recent.second().toString());
		_filesystemMap = filesystemMap;

		_recent = recent.second();

		putValue(Action.SMALL_ICON, Icons.createTextIcon(new Font(Font.DIALOG,
				Font.PLAIN, 10), Color.gray, new Dimension(150, 23), String
				.format("%1$tD %1$tr", recent.first())));
	}
