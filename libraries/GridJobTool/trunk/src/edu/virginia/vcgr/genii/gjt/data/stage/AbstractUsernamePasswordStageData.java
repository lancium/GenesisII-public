...
public abstract class AbstractUsernamePasswordStageData extends
		AbstractStageData {
	...
	public void analyze(String filename, Analysis analysis) {
		if (_username == null || _username.length() == 0)
			analysis.addWarning("No username entered for stage file \"%s\".",
					filename);

		if (_password == null || _password.length() == 0)
			analysis.addWarning("No password entered for stage file \"%s\".",
					filename);
	}
	...
	public void generateAdditionalJSDL(DataStaging jsdlStaging,
		...
		if (username.length() > 0 || _password.length() > 0)
			jsdlStaging.credentials(new Credential(new UsernameToken(username,
					password)));
	}
}