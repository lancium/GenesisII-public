...
public abstract class GridJobToolFrame extends JFrame
{
	...
	{
		GenerateJSDLAction generateJSDL = new GenerateJSDLAction(documentContext.applicationContext().willLaunch());

		ErrorPanel ePanel = new ErrorPanel(generateJSDL, documentContext.jobDocument());
	...
	protected JMenuBar createMenuBar(GenerateJSDLAction generateJSDLAction)
	{
		...
		addMenu(bar, new FilesystemMenu(_documentContext.jobDocument().filesystemMap()));
	...
	protected void addTabs(JobDocumentContext documentContext)
	{
		...
		_tabbedPane.add(vp);
		_tabbedPane.setEnabledAt(_tabbedPane.indexOfComponent(vp), vp.isEnabled());
	}
