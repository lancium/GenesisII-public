...
public class OperatingSystemComboModel extends AbstractListModel implements ComboBoxModel
{
	...
	{
		_elements.clear();
		boolean filter = ((Boolean) (preferences.preference(ToolPreference.LimitOperatingSystemChoices))).booleanValue();

		_elements.add(null);

		for (OperatingSystemNames name : OperatingSystemNames.values()) {
			if (!filter || Configuration.configuration.operatingSystemNamesFilter().contains(name))
				_elements.add(name);
		}
