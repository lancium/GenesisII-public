...
public class GridStageData extends AbstractStageData {
	...
	public String toString() {
		return String.format("grid://%s",
				((_path == null) || (_path.equals(""))) ? "<unknown>" : _path);
	}
	...
	public void analyze(String filename, Analysis analysis) {
		if (_path == null || _path.equals(""))
			analysis.addError(
					"Data stage for file \"%s\" does not specify a path.",
					filename);
		else if (!_path.startsWith("/"))
			analysis.addWarning("Data stage for file \"%s\" uses a relative"
					+ " path which is discouraged.", filename);
	}
