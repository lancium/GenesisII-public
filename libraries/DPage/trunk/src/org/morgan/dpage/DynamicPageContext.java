...
class DynamicPageContext extends AbstractHandler {
	...
	DynamicPageContext(ClassLoader loader, String resourceBase,
			ObjectInjectionHandlerFactory injectionHandlerFactory) {
		_loader = loader;
	...
	public void handle(String target, HttpServletRequest request,
		...
		try {
			...
			context = new InjectionContext(request,
					_injectionHandlerFactory.createHandler(String.format(
							"%s%s%s", targetDirectory,
			...
			response.sendError(HttpServletResponse.SC_NOT_FOUND,
					String.format(fnfe.getLocalizedMessage()));
