...
{
	/*
	 * for newer jetty. static public void addHandler(Server jettyServer, AbstractHandler
	 * newHandler) { ArrayList<Handler> handlerList = new ArrayList<Handler>();
	 * handlerList.addAll(Arrays.asList(jettyServer.getHandlers())); handlerList.add(0, newHandler);
	 * HandlerList newHandlers = new HandlerList();
	 * newHandlers.setHandlers((Handler[])handlerList.toArray());
	 ...
	 * handlerList.addAll(Arrays.asList(contexty.getHandlers())); handlerList.add(0, newHandler);
	 * HandlerList newHandlers = new HandlerList();
	 * newHandlers.setHandlers((Handler[])handlerList.toArray()); contexty.setHandler(newHandlers);
	 * }
	 */

	static public void addDynamicPages(Server jettyServer, ScratchSpaceManager scratchManager, File sourceDPagePackage)
		throws IOException
	{
		...
		for (String context : pageDescriptions.keySet()) {
			...
			cHandler.addHandler(new DynamicPageContext(dpagePackage.classLoader(), pageDescription.resourceBase(),
