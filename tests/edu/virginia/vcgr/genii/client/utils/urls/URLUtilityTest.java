...
public class URLUtilityTest {
	...
	public void testFormURL() throws MalformedURLException {
		...
		URL testValue = URLUtilities.formURL(
				"http://www.tempuri.org/some-path.txt", false);
		Assert.assertEquals(expected, testValue);

		expected = new URL("ftp://www.tempuri.org/some-path.txt");
		testValue = URLUtilities.formURL("ftp://www.tempuri.org/some-path.txt",
				false);
		Assert.assertEquals(expected, testValue);
	...
	/*
	 * We can't actually run these tests because they behavior is incorrect for
	 * paths that occur on platforms other than the one that the path is for --
	 * in otherwords, a windows path only works on a windows machine and
	 * vice-versa
	 */
	...
	public void testFormURLFromWindowsPath() throws MalformedURLException {
		...
		testValue = URLUtilities.formURL("C:\\home\\morgan\\some-path.txt",
				true);
		Assert.assertEquals(expected, testValue);
	}
}