...
public class TestMacros {
	...
	public void testSimpleSubstitution() {
		...
		Assert.assertEquals("One Mark Morgan Two",
				macros.toString("One ${user.name} Two"));
		Assert.assertEquals("Mark Morgan was here",
				macros.toString("${user.name} was here"));
		Assert.assertEquals("Hello, Mark Morgan",
				macros.toString("Hello, ${user.name}"));
	}

	// TODO: this is bad; this is supposed to work, and it looks like the test
	// is correct, but the
	// results are not.
	// @Test
	public void testEscapes() {
		Assert.assertEquals("Mark Morgan has $0", MacroUtils.replace(
				"${user.name} has \\$0", _propertiesResolver));
	}

	@Test
	public void testMultiples() {
		Assert.assertEquals(
				"Mark Morgan is 35 years old.  Email him at right@you.bet.",
				MacroUtils
						.replace(
								"${user.name} is ${user.age} years old.  Email him at ${user.email}.",
								_propertiesResolver));
	}
}